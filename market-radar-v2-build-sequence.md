...
- [ ] **M1.6-P0R Capacity + Recovery Remediation**：P0R-D0 本地无扩容机器证明已通过：8 周期/11,552 Fact 的稳态/峰值根盘预计 59%/67%，满足固定 60%/70% 双门槛；fresh P0 组合准入本地实现并继承全部非容量 blocker。腾讯 COS Object Lock=`COMPLIANCE` 31 天已回读，真实 age 身份仅在 macOS Keychain，历史 `bed938...` staging 保持 `REJECTED_SUPERSEDED_SECURITY_SOURCE`。`bd20bd5b73ef0beb41c331aa43c58051ef01d37a` 的四门、fresh production read-only rebind、生产零漂移和 13-member v3 plan/bundle staging 均保留为历史 PASS，但旧裸 `tee`、raw response 落盘、manual compile、AX reconstruction 与 Compose env 重插值已被两次真实 STS 失败永久证伪。第一枚 STS 已过期且禁用；第二枚虽进入 receiver，却因超过签发后 5 分钟被真实时钟编译器正确 BLOCKED。其 exact expiry `2026-07-28T20:57:29Z` 已由本机 UTC `2026-07-28T20:57:48Z` 与腾讯 STS HTTPS Date `2026-07-28T20:57:56Z` 双重证明超过，现永久禁止复用。raw path 删除后仍发现 PID `1175383` 持有 unlink 文件；核对身份并终止后，生产 P0R 文件数和进程数均复核为 0。没有数据库读取、COS 对象、backup、retrieval、restore 或业务 mutation。本地替代实现 `m1-production-storage-p0r-session.sh` 通过 TTY no-echo、有界内存接收、即时编译、raw 零落盘、exclusive mode-600 `/dev/shm` secret、12 键 binding 白名单、exact Compose label 定位、两项 secret 到齐后自动 Runner 和失败全清理；credential ingress CLI 不接受 caller-supplied clock override。Runner 内部数据库描述和 canary 只进入 owner-bound mode-700 私有 `/dev/shm` 目录，以 no-clobber mode-600 regular file 生成，内部 `tee` 禁止，成功前验证整目录删除。fresh rebind schema v2 现把历史 transport v1 三文件替代比较与当前 transport v2 七文件运行资格分开绑定，新的 session helper 不得游离于 exact-source 证据。P0R 81/81、Go helper、recurrence 10/10、dispatch 24/24 与完整 `ci:production` PASS。剩余顺序固定为 `最终权威文档同步与复核 -> clean exact commit -> GitHub 四门 -> fresh production read-only rebind -> 新 run-id/plan/14-member bundle staging 验证 -> 新 exact STS 通过 no-echo memory ingress 即时编译 -> 第二 fresh session 注入临时 age identity -> helper 自动 backup / exact retrieval / isolated PG16 restore -> secret/container/volume/runtime cleanup -> production zero-drift -> fresh health/topology -> exact-release calibration -> fresh P0`。两个 OrcaTerm 入口仍须单一职责、`<=200` UTF-8 字节、clear/settle/exact-preview；凭证可见期间禁止截图、AX/OCR 和日志输出。当前状态：`BD20_REMOTE_GATES_REBIND_AND_BUNDLE_HISTORICAL_PASS_SUPERSEDED / SECOND_STS_DELAYED_COMPILE_BLOCKED_EXPIRED_FORBIDDEN_REUSE_DUAL_CLOCK_VERIFIED / PROD_P0R_FILES_AND_PROCESSES_ZERO / LOCAL_ATOMIC_SESSION_AND_FULL_CI_PASS / REPLACEMENT_REMOTE_REBIND_BUNDLE_AND_REAL_RECOVERY_PENDING / PRODUCTION_P0_BLOCKED`。
...
Current execution entry: V2-M1.6-P0R-B3-NOECHO-MEMORY-INGRESS-ATOMIC-RECOVERY-SESSION; bd20 gates/rebind/bundle remain historical PASS but are superseded for execution, second STS delayed compile was correctly blocked, production P0R files and processes are zero, replacement atomic session is local PASS only, remote qualification/rebind/new bundle/real recovery remain pending
