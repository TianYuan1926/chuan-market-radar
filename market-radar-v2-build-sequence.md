...
Current execution entry: V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY health-envelope remediation and requalification. Source dd67d819 passed all four GitHub gates and received an exact approval, but signed dispatch a52aa6a failed closed before gateway or Caddy mutation because the runner and fixture read data.health while the real production API returns top-level health. Fresh read-only evidence proves health, scan and persistence ready, the exact 11-container baseline unchanged, the exact route still 404, gateway/outbound/staging/global lease absent, and all non-target production surfaces unchanged. The expired approval and claimed failed dispatch are not reusable. Current remediation evidence is gateway 8/8 plus exact full CI PASS; a clean commit, four gates for the new bytes, a fresh production baseline, a new deterministic package and a new exact approval remain pending. The immutable sequence is clean commit/push -> same-commit four gates -> fresh baseline -> new package -> current approval -> current-image isolated validation -> Caddy-only deploy or automatic rollback -> autonomous evidence verification -> fresh rebind -> P0R recovery.
