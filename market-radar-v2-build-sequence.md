...
- [ ] **M1.6-P0R Capacity + Recovery Remediation**：P0R-D0 本地无扩容模型、Object Lock `COMPLIANCE` 31 天、macOS Keychain age 身份、atomic remote session、private `/dev/shm` runner boundary 和 fresh rebind v2 七文件合同均已形成；真实 backup/retrieval/restore 仍未执行。source `e3626387ee8d57ef8e4f9c11c2e098b781ac6fbe` 的 GitHub 四门、fresh rebind 和新 staged transport-v2 曾真实 PASS，但第三枚 STS response 在 post-response browser-state recovery 中进入工具输出，因此该 credential 与绑定 run/plan/object key/bundle/staging 永久失去执行权。该 STS 从未进入服务器、从未编译或访问 COS；生产独立复核得到 P0R files/processes/containers/volumes 均为 0，业务服务和 authority 零漂移。新 `m1-production-storage-p0r-local-tty-bridge.exp` 在签发前固定 SSH/host-key/proxy/run-derived command 和 Node `v22.23.1`，等待 echo-disabled READY，只消费 API Explorer 原生 Copy，先清 clipboard 再 handoff，并从固定 Keychain 内部注入 age identity；response 后 browser state/AX/OCR/screenshot/computer-use/OrcaTerm secret entry 全部退役。当前伪 TTY 10/10、P0R 87/87、Go helper、recurrence 10/10、dispatch 24/24 与完整 `ci:production` 均已 PASS；新 clean commit、GitHub 四门、fresh rebind、新 run/plan/bundle 和真实恢复仍待完成；第三枚 STS 双时钟过期证明已 PASS。剩余顺序固定为 `新 clean exact commit -> GitHub 四门 -> fresh rebind -> 全新 run/object key/plan/14-member bundle staging -> 预先启动 bridge 并取得 READY -> 用户 MFA + API 原生 Copy -> 即时编译 + Keychain age handoff -> backup / exact retrieval / isolated PG16 restore -> secret/container/volume/runtime cleanup -> production zero-drift -> fresh health/topology -> calibration -> fresh P0`。当前状态：`E362_REMOTE_GATES_REBIND_STAGING_HISTORICAL_PASS_RUN_INVALIDATED / THIRD_STS_NEVER_USED_EXPIRED_FORBIDDEN_REUSE_DUAL_CLOCK_PROOF_PASS / PROD_P0R_FILES_PROCESSES_CONTAINERS_VOLUMES_ZERO / LOCAL_FIXED_TTY_BRIDGE_FULL_LOCAL_QUALIFICATION_PASS_NEW_EXACT_SOURCE_REMOTE_AND_REAL_RECOVERY_PENDING / PRODUCTION_P0_BLOCKED`。
...
Current execution entry: V2-M1.6-P0R-B4-PREARMED-LOCAL-TTY-BRIDGE-RECOVERY; e362 gates/rebind/staging remain historical PASS but its run is invalidated, third STS was never used and is expired forbidden reuse, production P0R files/processes/containers/volumes are zero, 38b49 remote qualification is blocked at 3/4 because Full Quality lacked Expect, the exact Expect runtime root fix has full local CI PASS, and new clean exact source/four remote gates/fresh rebind/new execution identity/real recovery remain pending
