...
- [ ] **M1.1B0 Tencent Isolated Live Source Conformance**：首次无密钥派发在业务 artifact 前阻断且永久不计 live PASS；R1 形成 0/15 共同 `TRANSPORT_FAILURE_UNAVAILABLE` 证据，并定位固定 Node `--jitless` 的 Web Fetch/WebAssembly 冲突。R2 精确提交 `d557c666e2e27b67842354b869a64271c91ceae1` 与派发 `m1b0-r2-live-source-20260723t165411z` 已真实取得 14/15：Identity 与 CoinGlass Gate PASS，Listing Gate 因唯一 `BINANCE_SPOT_CATALOG` 失败而 BLOCKED，生产身份和 secret 边界未变。现场测得默认 Binance 现货目录约 17,407,074 bytes，超过固定 8 MiB；R3 仅使用官方 `showPermissionSets=false` 有界查询，响应 6,629,806 bytes，保留所需 schema 字段，不提高上限、不删探针。R3 当前 package 24/24、fixed dispatch 21/21、V2 Ops 125/125 和独立正确分支完整 CI PASS；精确提交和腾讯重派发待完成。Bybit 最新两页继续为 `BOUNDED_COMPLETE`，完整历史归 M1.4B；Bitget Venue、Listing Lifecycle、股票 Asset Domain 仍分别核算。状态：`ATTEMPT_1_BLOCKED_NOT_COUNTED / R1_LIVE_0_OF_15_COMMON_RUNTIME_TRANSPORT_BLOCKED / R2_LIVE_14_OF_15_LISTING_GATE_BLOCKED / R3_BINANCE_SPOT_BOUNDED_QUERY_DIRECTED_AND_FULL_CI_PASS_COMMIT_REDISPATCH_PENDING / PRODUCTION_UNCHANGED`。
...
- [ ] **M1.4B Endpoint Batching + Runtime Adapter Profiles + Listing History Runtime**：只为 M1.1B0 实际 live PASS 的 source-capability 冻结 endpoint 批量语义、请求 token 成本、并发、分页、WebSocket/REST 组合、断线恢复和 runtime Adapter；失败 capability 保持 absent。Bybit listing 完整历史必须在这里执行一次性 bootstrap backfill、持久 checkpoint、缺口检测和增量页，不能拿 B0 两页窗口冒充。Bitget Venue、Listing Lifecycle 与股票 Asset Domain 分别核算。必须在同 release/no-authority Shadow 证明实际请求率、配额和完整分母，M1.4A 的每意图 1 token 仅为保守上界。
...
- [ ] **M3.4-M3.6 Feasibility + Risk + Runtime**：先重审当前未提交的 V1 M3.4 草稿并加入 scopeEpoch；再完成真实成本、执行可行性、Personal/Portfolio Risk、Trigger 与 Runtime Gate。只有 Portfolio Risk 可以汇合加密和股票。状态：`M3.4_PARTIAL_UNCOMMITTED_REQUIRES_SCOPE_REBASE / M3.5-M3.6_NOT_STARTED`。
...
Last completed local package: V2-M1.1B0-R3-BINANCE-SPOT-BOUNDED-QUERY-DIRECTED-PASS
Last Scope V2 production attempt: V2-M1.1B0-R2-TENCENT-ISOLATED-LIVE-SOURCE-CONFORMANCE = BLOCKED_14_OF_15_LISTING_GATE
Next Scope V2 evidence package: V2-M1.1B0-R3-TENCENT-ISOLATED-LIVE-SOURCE-CONFORMANCE
