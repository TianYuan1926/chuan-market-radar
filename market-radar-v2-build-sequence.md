...
- [ ] **M1.6-P0R Capacity + Recovery Remediation**：P0R-D0 本地无扩容模型、Object Lock `COMPLIANCE` 31 天、macOS Keychain age 身份、atomic remote session 和 private `/dev/shm` runner boundary 均已形成；真实 backup/retrieval/restore 仍未执行。B7 clean source `be87cf040472559979f4b6a602350970d9bf2c32` 已取得 GitHub 四门、fresh read-only rebind `p0r-rebind-preflight-20260729t220958z-0bc442e3` 和全新 run `p0r-20260729t221859z-48eee3208ed0c519e49c2519f05e665e` 的 16-member transport v3；包大小 9,176,819 bytes、SHA-256=`44f5e34fdd2bbdbf94dbc642a3a45b39b6271f9d14e6dc5cb7173d7bddf2aa9b`。OrcaTerm 文件管理器随后三次受控运输失败，目标文件和 run staging 均 absent，生产零变更；该通道永久退出生产包运输。B8 fixed-dispatch transport-stage 本地 `10/10`、recurrence+transport `21/21`、真实主机权限下 P0R `110/110`、Go helper 与完整 `ci:production` PASS，只允许无 secret 的 exact 包原子落盘，禁止 credential、数据库和 recovery launch。剩余顺序固定为 `clean exact commit -> GitHub 四门 -> signed fixed-dispatch staging -> exact 16-member/mode/owner/hash/outer-cleanup/zero-drift 验收 -> 临时 8022 bootstrap gate -> bridge READY -> 用户 MFA + API 原生 Copy -> 即时编译 + Keychain age handoff -> backup / exact retrieval / isolated PG16 restore -> secret/container/volume/runtime cleanup -> 删除 8022 listener 与云规则并证明不存在 -> production zero-drift -> fresh health/topology -> calibration -> fresh P0`。当前状态：`B7_SOURCE_REMOTE_GATES_REBIND_AND_INNER_PACKAGE_PASS / ORCATERM_TRANSPORT_RETIRED / B8_LOCAL_TRANSPORT_P0R_GATES_AND_FULL_CI_PASS / CLEAN_SOURCE_REMOTE_GATES_SIGNED_TARGET_ACCEPTANCE_AND_REAL_RECOVERY_PENDING / PRODUCTION_P0_BLOCKED`。
  ...
  - **2026-07-31 Fixed-dispatch transport staging 根治**：B7 source `be87cf...` 的四门、fresh rebind 和 16-member inner package 均已通过，故旧“clean source/远端资格/新包待完成”结论失效。OrcaTerm 文件管理器对 exact 9.18 MB 包的 disconnect、`0B` 和短路径 A/B 复发已登记为 open incident；禁止 reload/retry 或直接上传。B8 外包固定五成员和 90 秒运行上限，只将 exact inner bytes atomic no-clobber 解包到 run-bound staging，验 owner/mode/hash 后清理自身外层 staging；不请求 STS、不访问数据库、不调用 session/Runner。本地定向、P0R 与完整 CI 均 PASS，real-target signed dispatch acceptance 未发生。
...
Current execution entry: V2-M1.6-P0R-B8-FIXED-DISPATCH-TRANSPORT-STAGING-ROOT-REMEDIATION; be87 exact source passed four remote gates, fresh production read-only rebind and the new 16-member transport v3 package, OrcaTerm file-manager delivery recurred across three controlled attempts with no target file and zero production drift, the old upload operation is retired, and the new signed fixed-dispatch delivery-only stage passes 10/10 plus complete P0R 110/110 and full CI locally while clean B8 source, four remote gates, signed target staging acceptance and real recovery remain pending
