...
- [ ] **M1.6-P0R Capacity + Recovery Remediation**：P0R-D0 本地无扩容机器证明已通过：clean commit `15746813245744af4f4ba73f61a976b722ad9a21` 在隔离 PG16 完成 8 周期/11,552 Fact，最大周期 33,660 ms；按 1,805 Facts/分钟、30h retention、6h partition、1h sweep、1.5 倍字节成本和固定 reserve，稳态/峰值根盘预计 59%/67%，低于 70%。这不声称 production capacity PASS。腾讯 COS 专用空桶仍为白名单待开通，真实 age/STS/backup/retrieval/restore 未执行；旧 topology 过期且旧远端 bundle 摘要不合格。固定剩余顺序为 `提交并通过 Object Lock 白名单 -> 独立确认 COMPLIANCE 31d -> 真实离机 age 身份与运行级 STS -> 同快照加密 backup / exact retrieval / 独立 PG16 restore -> fresh boot/filesystem/Docker/Postgres/Redis/app health + topology -> fresh P0`。当前状态：`NO_COST_CAPACITY_LOCAL_MACHINE_PROOF_PASS / OBJECT_LOCK_WHITELIST_REQUIRED / RECOVERY_PENDING / PRODUCTION_P0_BLOCKED`。
