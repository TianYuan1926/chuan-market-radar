...
- [ ] **M1.4B Endpoint Batching + Runtime Adapter Profiles + Listing History Runtime**：本地核心已实现并通过 M1.1B 26/26、M1.4A 28/28、M1.4B 23/23、V2 Foundation 445/451（6 explicit skip）、V2 Ops 125/125、M0、Next production build、Golden 16/16 与 security；现场 no-authority runtime 仍待出口。Profile 绑定 exact live artifact、registry/probe digest、HTTPS endpoint、分页、credential 与恢复语义；snapshot batching 和 listing-history checkpoint 使用两本请求预算，所有 ready intent 精确一次核算。Bybit 已有 provider-available history 的 bootstrap/resume/gap/incremental 状态机，Bitget 只声明官方一个月窗口完整。Bitget Venue、Listing Lifecycle、股票 Asset Domain 与 Source Capability 分别核算，股票当前仅 catalog accounting、tradable Fact 为 0。R3 15/15 endpoint conformance 中当前只有 14/15 route eligible；`BINANCE_SPOT_CATALOG` 因 registry 仍为 `UNAVAILABLE` 明确阻断，必须先修订 registry 并绑定新 digest 重跑 live conformance。状态：`LOCAL_ENGINEERING_AND_FULL_CI_PASS / LIVE_NO_AUTHORITY_RUNTIME_UNPROVEN / PRODUCTION_UNCHANGED`。只有同 release 腾讯隔离运行、真实 listing checkpoint、请求率/配额/完整分母和恢复证据通过后，本项才能勾选完成。
