...
- [ ] **M3.3E Strategy Archetype Labeling + Outcome Attribution（本地核心合同通过，完整 runtime/评估/UI/Shadow 待完成）**：`StrategyDraft v3` 现在必须恰好一个内容寻址的后端 canonical 主标签和有界 context tags；Strategy Construction input v2 显式消费 Thesis，并按 pattern、Analysis structure、方向和结构位确定性分类，无法证明时 no-draft abstain。`StrategyDecision v2`、`DecisionSnapshot v2`、`AlertEvent v2`、`OutcomeRecord v2` 原样传播标签和 Action State 派生状态，跨对象合同可拒绝重新计算 hash 后的事后改名。M3 核心回归 88/88、多资产 28/28、Scope Rebase 12/12；当前仍是 `TEST_ONLY_UNBOUND_SCOPE / NO_RUNTIME_OR_PRODUCTION_AUTHORITY`，Venue/domain/lifecycle/liquidity 明确未绑定。真实 Read Model/Alert/Outcome builder、按标签评估、UI、Scope V2、cohort/holdout/Shadow 和独立审计未完成，因此本项仍不勾选、不计完整 M3 完成。
