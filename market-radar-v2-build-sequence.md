...
前三条是独立业务范围，第四条是横跨所有来源的能力开放策略；它们是新增内容的唯一责任映射，不再另建重叠路线。代码包可以为效率合并交付，但测试、状态、证据、回滚和完成判定必须按 `Venue / Listing Lifecycle / Asset Domain / Source Capability` 分开记账。
...
- [ ] **M1.4B Endpoint Batching + Runtime Adapter Profiles + Listing History Runtime**：本地核心已实现并通过 M1.1B 26/26、M1.4A 28/28、M1.4B 23/23 与 ESLint；完整 CI、提交推送和现场 no-authority runtime 仍待出口。Profile 绑定 exact live artifact、registry/probe digest、HTTPS endpoint、分页、credential 与恢复语义；snapshot batching 和 listing-history checkpoint 使用两本请求预算，所有 ready intent 精确一次核算。Bybit 已有 provider-available history 的 bootstrap/resume/gap/incremental 状态机，Bitget 只声明官方一个月窗口完整。Bitget Venue、Listing Lifecycle、股票 Asset Domain 与 Source Capability 分别核算，股票当前仅 catalog accounting、tradable Fact 为 0。R3 15/15 endpoint conformance 中当前只有 14/15 route eligible；`BINANCE_SPOT_CATALOG` 因 registry 仍为 `UNAVAILABLE` 明确阻断，必须先修订 registry 并绑定新 digest 重跑 live conformance。状态：`LOCAL_ENGINEERING_AND_DIRECTED_REGRESSION_PASS / FULL_CI_PENDING / LIVE_NO_AUTHORITY_RUNTIME_UNPROVEN / PRODUCTION_UNCHANGED`。只有同 release 腾讯隔离运行、真实 listing checkpoint、请求率/配额/完整分母和恢复证据通过后，本项才能勾选完成。
...
Last completed local package: V2-M1.1B0-R3-BINANCE-SPOT-BOUNDED-QUERY-DIRECTED-PASS; M1.4B final CI is pending
Last Scope V2 production attempt: V2-M1.1B0-R3-TENCENT-ISOLATED-LIVE-SOURCE-CONFORMANCE = PASS_15_OF_15_ALL_GATES
Next Scope V2 evidence package: V2-M1.4B-ENDPOINT-BATCHING-RUNTIME-ADAPTER-AND-LISTING-HISTORY
Current conditional local package: V2-M1.4B-ENDPOINT-BATCHING-AND-RUNTIME-ADAPTER-PROFILES = LOCAL_ENGINEERING_AND_DIRECTED_REGRESSION_PASS_FULL_CI_PENDING_LIVE_RUNTIME_UNPROVEN
