...
- [ ] **M1.6-P0R Capacity + Recovery Remediation**：P0R-A recovery 工具链与 P0R-B 云资源前置安全合同已本地通过；当前腾讯 COS inventory=0，尚未创建 bucket。固定剩余顺序为 `P0R-B1 单 AZ 私有 COS + versioning + SSE-COS + 永久 Object Lock COMPLIANCE 31d -> P0R-B2 离机 age 身份与运行级 STS -> P0R-C 同快照加密 backup / 远端精确 version retrieval / 独立 PG16 restore -> P0R-D 用户确认 180 GiB 付费关机升级 -> P0R-E boot/filesystem/Docker/Postgres/Redis/app health -> fresh P0`。P0R 只允许重跑 P0，不能直接授权 P1。当前状态：`LOCAL_RECOVERY_AND_CLOUD_PREREQUISITE_ENGINEERING_PASS / EXTERNAL_COS_RECOVERY_AND_CAPACITY_ACTION_PENDING`。
...
Current execution entry: V2-M1.6-P0R-B1-COS-KEY-STS-EXTERNAL-PROVISIONING
...
Current status: M1.5-B1_COMPLETE / B1-B1_EXECUTION_INVALID_NOT_COUNTED / B1-B3_PASS / M1.6-P0_EXECUTED_BLOCKED_CAPACITY_AND_RECOVERY / M1.6-P0R_LOCAL_RECOVERY_AND_CLOUD_PREREQUISITE_ENGINEERING_PASS_EXTERNAL_ACTION_PENDING / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED / PRODUCTION_SERVICES_DATA_AND_AUTHORITY_UNCHANGED
```
