...
- [ ] **M1.2 Point-in-Time Feature + Context 纯函数纵切**：先实现三 Venue 同一 underlying 的价格离散度、FeatureQuality、在线/回放同源和最小非方向性 Market Context；不能访问 cutoff 后数据，不能输出 Candidate 或方向。
...
Current package: V2-M1.1 Three-Venue Identity and Fact Slice
Current package status: LOCAL_PASS_FROZEN_PROVIDER_CONTRACT / LIVE_CONNECTIVITY_UNPROVEN
Next package: V2-M1.2 Point-in-Time Feature and Context Slice
```
