...
所有标的必须同时绑定 Venue、asset domain、instrument class 和 listing lifecycle。Venue 不决定资产域：加密交易所上的股票永续仍进入 Equity 通道；没有合格合约的新币只进入 `WATCH_ONLY`，不能产生合约交易计划。
...
| 最新成果 | 数据与运行 | 发现与研究 | 决策与展示 | 完成边界 |
| --- | --- | --- | --- | --- |
| Market Mechanics Cube | M1.4C 建立价格结构、参与杠杆、流动性响应三轴 Feature 和 lineage | M2.1A 只以 research hypothesis 使用，M2.4A 真实分层验证 | M3 只解释已验证 Evidence，M4 按三轴展示 | 不能合成一个总分，不能用个案或截图证明有效 |
| 双向前兆图谱 | M1.5D 积累上涨、下跌和未爆发的前向事实 | M2.1A 建立八类 long/short/unknown DRAFT；M2.4A matched control + holdout | M5 做漏报、误报、消融和晋级治理 | 未经真实 cohort/holdout/Shadow 不发 Candidate |
| 订单墙生命周期与压力响应 | M1.4C 定义 Trade/Book Fact、LiquidityWallEpisode、吸收、撤单/补单/迁移、spoof uncertainty；M1.5D 实采 | 历史 L2 不足时固定 forward-only，不用 Kline 伪造 | M4 只显示 Evidence Overlay，不显示未校准“控盘分” | 单快照、固定美元阈值或单 Venue 均不能给方向 |
| 板块传播 | M1 Feature/Context 建 point-in-time AssetRelationshipSnapshot | M2.1A/M2.4A 验证先涨/先跌扩散、leader failure 和未跟随 control | M4 展示传播依据与冲突，M5 评价独立增量 | 禁止事后分类和共同 BTC beta 冒充传播 |
| CoinGecko/DefiLlama/链上/新闻社交 | 先扩展 SourceCapabilityRegistry；通过权利、live、quota、checkpoint、容量后分层接入 | 只在消融证明增量后进入研究 | 低权威来源只能补充 EventContext/反证 | CoinGecko 免费 key 不进 Git；不新增付费服务 |
| 分层缓存与专业图层 | L1/Redis/PostgreSQL/COS 四层，stale 明确、后端单一入口 | 缓存不参与标签或未来补值 | 灵敏度只改变提醒/展示；AI/自定义指标无 authority | 前端不得直连 Provider 或二次生成计划 |
...
- [x] **Long-Term Governance Gate 长期目标治理门禁**：固定“提前发现真实机会并形成可信、可执行、可复盘决策链”的核心、事实真值、安全和最终验收底线；允许施工顺序、并行方式和实现技术基于当前证据受控调整。机器矩阵与 M0 出口已强制每次调整核对核心正向性、真实依赖、当前瓶颈、影响、验证、回滚、权威文档和旧结论失效，并将模型、规则、阈值、特征、币种、Venue、时间、regime、方向和事后解释纳入反过拟合门禁。状态：`LONG_TERM_ENGINEERING_GOVERNANCE_PASS / NO_ROUTE_DRIFT / NO_PRODUCTION_AUTHORITY`。
- [ ] **A0 Engineering Foundation 总门禁**：材料、供应链和 exact-runtime 已完成精确 Node/npm/直接依赖声明、安全补丁、零高危依赖审计、许可证、CycloneDX SBOM、固定 Action/runner/base-image、双重 lint、Next build、原生烟测、纯 Node USTAR 和完整 Git 祖先治理；Signed Dispatch `30199692352` 与 Full Quality `30200077285` 均真实 PASS。独立安全源 `4f501b0fb8b917ce87e0687eab8480b5c9595f27` 的 Security `30209898205` 已取得 Gitleaks 完整历史 finding=0、CodeQL blocking=0、collector image Trivy HIGH=0/CRITICAL=0；credential-shaped identity 修复又由 Security `30212437973` 和 Full Quality `30212437974` 复验通过。最新 source parts `9ef63b85d1a76f3ad7ac + 815e081506c5dbc074a5` 已由 A0 run `30217335595` 取得两次独立 RootFS、确定性应用胶囊、精确镜像配置、fail-closed runtime smoke、三个隔离回滚场景和冻结 1,440 instrument 性能资源基线全部 PASS；同源 Full Quality `30217335543` 与 Security `30217335622` 也全部 PASS。RootFS ownership/路径、证据 TOCTOU 和 event-loop p99 超限均已根因治理，未减少样本、分母、工作量或门槛。Legacy 历史制品与生产均未改。状态：`ENGINEERING_MATERIALS_SUPPLY_CHAIN_INDEPENDENT_SECURITY_REPRODUCIBLE_RELEASE_AND_RESOURCE_BASELINE_PASS / TOTAL_GATE_INCOMPLETE_P0R_PENDING / PRODUCTION_UNCHANGED`。剩余唯一控制是 P0R 真实加密备份、精确版本取回和独立 PostgreSQL 16 恢复；总门禁通过前禁止启动 M1.5C/M1.5D。
...
- [x] **M1.4B Endpoint Batching + Runtime Adapter Profiles + Listing History Runtime**：本地核心与无 secret 固定 Bundle/Runner/Entrypoint 已通过 23/23、9/9 和正式实施分支完整 CI。精确 source `3c21a75009aeb4f4f7d9fd8954245238c38d9636` 的腾讯 bootstrap `m1-4b-runtime-live-20260723t232457z` 与 checkpoint-bound resume `m1-4b-runtime-live-20260723t233213z` 均真实取得 14/14 route PASS、0 failed、1 registry blocked；两轮 request token budget=203、attempts=80、listing gap=0、checkpoint committed=2。第二轮严格绑定第一轮 Bitget/Bybit checkpoint、原 `PASS` result 路径和 SHA-256 后生成新 checkpoint。Bitget Venue、Listing Lifecycle、Equity Asset Domain 与 Data Maximization 四轴均独立 PASS；`BINANCE_SPOT_CATALOG` 继续保持零请求。生产 HEAD、clean worktree、11 容器集合、listener、timer、health 前后不变，staging 均已清理。状态：`LOCAL_ENGINEERING_AND_FULL_CI_PASS / TENCENT_BOOTSTRAP_AND_CHECKPOINT_BOUND_RESUME_PASS / NO_RUNTIME_AUTHORITY / PRODUCTION_APPLICATION_UNCHANGED`。本项关闭不代表 M1、Shadow、容量、股票 tradable Fact、Detector、Strategy 或 READY 完成。
- [x] **M1.4C Microstructure Fact + Market Mechanics Feature + Cache Contract**：六类 Trade/Book/Liquidation/Mark-Index Fact、LiquidityWallEpisode 生命周期、十三项 Market Mechanics Feature、exact lineage/cutoff/freshness 和 ONLINE/双 REPLAY parity 已落地；L1/Redis/PostgreSQL/COS 的 12 行 artifact policy 明确缓存、审计真值和回放真值，missing 不变 0、stale 不冒充 fresh。固定美元大单阈值不能成为 Detector 权威。定向 22/22 与完整 `ci:production` PASS。状态：`LOCAL_CONTRACT_PASS / NO_RUNTIME_FACT_CANDIDATE_OR_DECISION_AUTHORITY / PRODUCTION_UNCHANGED`。
- [x] **M1.4D Multi-Asset Identity + Catalog + Base Fact Runtime**：四 Venue catalog、listing watch、确定性 identity snapshot、T0 lifecycle 与 T1 wide-market 已形成同 cutoff Base Fact Snapshot；Provider URL/transport 只位于 Adapter，Core 只保存 URL hash 和有界请求合同。future catalog/fact/lifecycle、分页不完整、身份冲突、URL hash 漂移和 unavailable 假 0 全部 fail closed。状态：`LOCAL_RUNTIME_AND_POINT_IN_TIME_CONTRACT_PASS / LIVE_FACT_AUTHORITY_FALSE / PRODUCTION_UNCHANGED`。
...
- [ ] **M1.5C Four-Venue Multi-Asset Shadow**：本地 31 周期/60 秒 runtime、四责任轴逐周期核算、内容寻址 evidence、独立 verifier 与 exact no-secret package 已通过；原 B1-B3 仍只属于 V1。V2 真实出口必须在 A0/P0R 关闭后，以 fresh same-commit conformance + M1.4B upstream 和同一 exact release/config/scopeEpoch 对四 Venue、加密/股票/指数和 listing state 运行。中断不可拼接，失败窗口不计数。状态：`LOCAL_RUNTIME_EVIDENCE_AND_PACKAGE_PASS / LIVE_ZERO_OF_31 / INDEPENDENT_ACCEPTANCE_PENDING / NO_AUTHORITY`。
- [ ] **M1.5D Adaptive Microstructure Forward Capture + Quality Shadow**：M1.4C 本地出口、point-in-time 确定性 research/control 选择、31 周期 Worker、隔离 PostgreSQL、独立 verifier 和与 M1.5C 共用的 exact package 已通过。A0/P0R 关闭后才可绑定 fresh same-commit upstream 真实运行，并独立记录 trades/book/mark-index coverage、event latency、gap、matched-control parity、隔离存储成本和宿主恢复。M2 runtime 未开放前只允许冻结 research trigger、确定性轮转样本和 matched control，不能冒充 Candidate。状态：`LOCAL_RUNTIME_EVIDENCE_AND_PACKAGE_PASS / LIVE_ZERO_OF_31 / A0_AND_FRESH_EXACT_UPSTREAM_PENDING / NO_CANDIDATE_AUTHORITY`。
...
- [ ] **M1.6-P0R Capacity + Recovery Remediation**：P0R-D0 本地无扩容模型、Object Lock `COMPLIANCE` 31 天、macOS Keychain age 身份和 private `/dev/shm` runner boundary 均已形成；真实 backup/retrieval/restore 仍未执行。B8 exact target、B9 COS CAM action、bridge/session v5 和旧 route evidence v1/v2 资格只保留历史证据。request-v4 signer source `4d1042618eff24d39f6ecceb93d5715e2e798b5a` 已提交推送，但 GitHub 只通过 Signed Dispatch `30755615525`、A0 `30755615545`、Independent Security `30755615533`；Full Quality `30755615546` / job `91517146516` 因 bridge v5 clipboard wait 将 credential/age 会话重标为 generic secret session 而 FAIL。bridge v6 贯穿 session-specific failure code，分别注入 immediate credential、clipboard-wait credential 与 clipboard-wait age disconnect；directed `13/13`、P0R `142/142`、Go helper 和最终字节完整 CI 本地 PASS，V2 Ops `283/283`。正确剩余顺序调整为 `clean commit/push -> 同源四门从零 PASS -> fresh signed read-only rebind -> fresh production baseline -> deterministic double build -> 新 exact approval -> signer-bound gateway execution/自动回滚 -> 自动取回/验签 -> 新 route/lease/run/plan/transport -> 双 TTY PREARMED -> 一次 fresh STS native Copy -> COS preflight -> read-only encrypted backup / exact retrieval / isolated PG16 restore -> lease-bound cleanup -> production zero-drift -> fresh health/topology -> calibration -> fresh P0`。当前禁止复用旧批准、旧 dispatch、bridge v5 当前资格、STS、8022 和旧 package。当前状态：`REQUEST_V4_SIGNER_COMMIT_PUSH_PASS / REMOTE_3_OF_4_FULL_QUALITY_BRIDGE_RACE_FAIL / BRIDGE_V6_LOCAL_P0R_142_AND_FINAL_BYTES_FULL_CI_PASS / CLEAN_COMMIT_REMOTE_FOUR_GATES_FRESH_REBIND_AND_REAL_RECOVERY_PENDING / PRODUCTION_BASELINE_ROLLBACK_PASS / PRODUCTION_P0_BLOCKED`。
  - **2026-07-27 最新覆盖事实**：source `94118d3b8270b6ac58c449380911ea77b8abeace` 已完成 GitHub 四门和腾讯 fresh read-only rebind `PASS_P0R_READ_ONLY_REBIND_PREFLIGHT`，生产零漂移；因此上段的 `FRESH_REDISPATCH_REQUIRED` 只属于更早时点。随后 v2 exact plan/bundle 的 STS 请求被腾讯以 `MissingParameter.Region` 拒绝，没有 credential、数据库读取、backup 或 COS 对象，失效 staging 已清理。根因修复将 plan 升级 v3 并把 `ap-hongkong` 绑定 request、plan digest、credential request digest 和 Go helper；P0R 72/72、Go helper、V2 Ops 194/194 与完整本地 CI PASS。当前固定顺序为 `新提交四条 exact-source 门禁 -> fresh production read-only rebind -> v3 deterministic plan/bundle -> fresh STS + age /dev/shm -> backup / exact retrieval / isolated restore / cleanup -> fresh health + topology -> calibration -> fresh P0`；禁止给旧 v2 请求手工补 Region。
  - **第四门触发合同整改**：Region 修复 source `b33661d6d503df89ab8f1aa1a3e2a3e962e0c7ae` 的 A0、Full Quality 和 Independent Security 已 PASS，但旧路径过滤没有为 P0R 生产脚本触发 Signed Production Dispatch Quality，因此仍不是四门通过。工作流现统一监听 `scripts/v2/production/**`，并以回归测试防止缩回 `fixed-channel` 子目录；必须由包含该修复的新 exact source 自动取得四门 PASS，才继续 fresh rebind。
  - **2026-07-29 当前覆盖事实**：上段 `bd20...` 四门/rebind/bundle 仍为历史证据，但“按双会话 exact tee 重签”的执行结论已失效。第二枚 STS 的真实延迟编译和 unlink 后残留 `tee` 证明该路线仍会丢失即时编译窗口并残留 secret-bearing process。替代 no-echo atomic session 已本地通过专项门禁与完整 `ci:production`，但尚无 clean commit、远端资格、fresh rebind、新 plan/bundle 或真实恢复证据；必须先完成这些前置，再以新 exact source 进行一次原子化生产恢复。
  - **2026-07-29 最新覆盖事实**：`e362...` 已完成上一条要求的 clean source、GitHub 四门、fresh rebind 和新 staging，但第三枚 STS 在 browser-state recovery 中披露，证明“response 后再恢复浏览器/OrcaTerm 上下文”仍不是安全路径。该 STS 未进入生产且四类残留为 0；新 pre-armed local TTY bridge 已通过本地故障注入，必须取得新的完整资格和全新 execution identity 后才能再请求 credential。
  - **2026-07-29 Full Quality 运行材料根治**：replacement source `38b49f43e0dc15514d1da9c1169d2fed233d8f5a` 的 A0、Signed Dispatch 和 Independent Security PASS，但 Full Quality `30448234790` 因 Ubuntu 24.04 缺 `/usr/bin/expect` 而 FAIL，故只能记 3/4 门通过，禁止 rebind、STS 和执行。当前根治精确安装 Noble `expect/tcl-expect 5.45.4-3`，由 A0 材料门禁与防回退测试锁定且不允许 skip；18 项定向测试和完整本地 `ci:production` PASS。下一动作仍是新 clean exact source 的四门全量复验，不得沿用 `38b49...` 的三门证据拼接资格。
  - **2026-07-30 Cross-layer dispatch binding 根治**：Expect 根治后的 `0e035...` 四门全部 PASS；fresh rebind `c85ec1a4` 在生产前因 outer runtime 5400 秒与 package 90 秒不一致而 BLOCKED，零 mutation 且 staging 已清理。禁止只改参数复发式重发。request v3 现把 90 秒纳入权威合同，并由 fixed channel 与 runner 四层交叉验证；唯一高层 release 入口从 request 派生重复绑定并拒绝 operator-supplied binding 选项。定向 29/29、P0R 88/88、最小 PATH dispatch 回归和完整 `ci:production` PASS；新 source、四门和 real-target rebind 待完成。
  - **2026-07-30 Runtime namespace 根治**：bridge-v2 后续 source `e83c1f...` 的真实 run 已到达 Runner，但在 backup 前因宿主看不到 Web 容器 mount namespace 内的 `node_modules` 而 fail closed。现场 A/B、零 mutation 和完整清理均已证明；transport v3 以 source-bound runtime capsule 取消生产 `node_modules` 前置，并拒绝额外包、嵌套依赖、symlink、native module、`.bin`、非规范 USTAR、路径逃逸、摘要/版本/权限漂移。P0R 100/100、两次真实隔离 npm 构建的字节一致性与完整本地 CI PASS；clean exact source 和远端/生产资格待完成，当前不得请求 STS 或开放 8022。
  - **2026-07-31 Fixed-dispatch transport staging 根治**：B7 source `be87cf...` 的四门、fresh rebind 和 16-member inner package 均已通过，故旧“clean source/远端资格/新包待完成”结论失效。OrcaTerm 文件管理器对 exact 9.18 MB 包的 disconnect、`0B` 和短路径 A/B 复发后永久退役。B8 outer source `15d7cb...` 的四门 PASS；首次 dispatch `9fac599e...` 在祖先 `0755` 上 fail closed，精确 `0700` 修复后 fresh dispatch `d5ea6e...` 已验收 exact 16 members、owner/mode/hash、outer cleanup 和生产零漂移。事故已 `CLOSED_VERIFIED`。新增祖先目录防复发测试要求不安全 ancestor 在 child 创建前被拒绝且不得 chmod；release 只接受短 branch 名。B8 不请求 STS、不访问数据库、不调用 session/Runner，真实 P0R 仍未执行。
  - **2026-08-01 COS Object Lock CAM 根治**：B9 的 STS 与 age 交接和 Runner 启动已真实通过，但 COS preflight 在数据库读取前 BLOCKED。腾讯官方合同区分 REST 操作 `GET Bucket ObjectLockConfiguration` 与 CAM action `cos:GetBucketObjectLock`；旧 v3 plan 使用前者构造了无效权限。plan/credential/bridge 现升为 v4/v3/v4，旧 authority 自动拒绝，helper 只输出 allowlisted 脱敏阶段码。P0R 113/113、Go helper 与完整 CI PASS；clean source、四门、fresh rebind、新 run/package/staging 和 corrected recovery 待完成。失败后的 8022 listener/规则、secret 和临时 runtime 已清零，生产零漂移。
  - **2026-08-02 外部事务复发根治**：两次 clipboard window 超时后，后到 response 作废；代理 egress 从旧 `/32` 漂移到 `156.248.15.36`，严格 8022 失去可达性。当前只证明本机 Bridge 为 0 和 clipboard 已覆盖，云端 listener/规则、服务器 residue 与生产零漂移待重新核验。bridge/session v5 现签发前双 TTY prearm、1200 秒心跳等待且无 post-issuance reconnect；transaction v1 强制 exact lease、120 秒 route/cleanup freshness、唯一终态状态机、65,536-byte/64-state 总上限、稳定 no-follow 控制文件读取、内部 READY 授权和 preflight clipboard cleanup。fresh rebind v4/v3 又把 TCP 8022 count=0 与 exact transient unit=`not-found/inactive` 纳入显式零残留门禁；rebind 13/13、P0R 125/125、Go helper 与最终字节完整 CI PASS。clean source 和外部验收待完成。
  - **2026-08-02 Route authority producer 与 identity race 根治**：route evidence v1 只有 validator、没有权威 producer 的缺口由 `157e9a79...` 关闭；首轮远端安全门随后真实发现 public identity reader 的 pre-open lstat 竞争窗口。`8059edf...` 只增加有界脱敏诊断，保留 run `30725896115` FAIL；`913bae3a...` 改为先 `O_NOFOLLOW` open、descriptor-authoritative identity 和读取后 path/realpath 复核，新增 open-before-lstat 回归且无 suppression。transport v4=17 members、current runtime=9 files；定向 `24/24`、P0R `139/139`、launcher `3/3`、V2 Ops `266/266`、完整本地 CI 和 Signed Dispatch `30726178696`、Full Quality `30726178679`、A0 `30726178659`、Independent Security `30726178672` 均 PASS。历史 branch-identity 与 CodeQL 红灯不被覆盖。最终资料 HEAD、fresh rebind 与真实目标验收仍待完成。
  - **2026-08-02 Autonomous evidence return 安全资格根治**：gateway source `d67c0e87...` 的 Signed Dispatch `30733232254`、Full Quality `30733232256`、A0 `30733232263` PASS，但 Independent Security `30733232255` FAIL，故资格仍为 3/4。CodeQL 私钥 no-clobber 测试的两处 path-read race 已统一改为 descriptor-first `O_NOFOLLOW`、regular-file/mode/size/dev/ino/mtime 稳定读取且无 suppression；Gitleaks 的唯一 finding 已证明为固定 X25519 recipient 公钥指纹，历史只忽略 exact fingerprint，活动源码使用两段表示并由结构化 `PUBLIC_KEY_FINGERPRINT` 审查锁定。官方 Gitleaks `8.30.1` 完整历史 finding=`0`，定向 `33/33` 与完整本地 `ci:production` PASS。下一步必须形成新的 clean exact source 并从零取得四门，前三门不得借用；随后才构建确定性 Caddy-only package并申请唯一一次精确生产 mutation 批准。
  - **2026-08-02 Gateway health-envelope 与生成制品公钥指纹根治**：exact source `dd67d819...` 已从零取得四门和旧 exact 批准；signed dispatch `a52aa6a...` 在任何 gateway/Caddy mutation 前因 runner/fixture 错读真实顶层 `health` 而 `FAIL_DISPATCH_NOT_REUSABLE`。fresh read-only 复核证明 exact 11-container baseline、404 route 与所有非目标生产面零漂移，旧批准和 dispatch 不可复用。真实-envelope 绿例与 obsolete nested-envelope 零 mutation 红例已形成 clean commit `51886b0...`；authority-sync source parts `d1c3d987cf55105e94e8 + 15992dbb989ea1045629` 随后取得 Signed Dispatch `30739919674`、Full Quality `30739919649`、A0 `30739919664` PASS，但 Independent Security `30739919662` FAIL。唯一 finding 是历史 `approval-request.json:1` 中固定 X25519 公钥指纹被命名为 `evidenceRecipientKeySha256`，不是 credential，但属于 scanner-shaped 生成制品。永久修复使用 `evidenceRecipientFingerprintSha256`、拒绝旧字段，fixed dispatch 在发布前阻断歧义高熵 `*KeySha256`；历史只登记 exact finding。clean implementation commit `8742828086041da1db51a5abeeef01ba7656e5fd` 已形成，recurrence `11/11`、dispatch `38/38`、gateway `8/8` 合计 `57/57`，最终 authority-sync 字节完整 exact-toolchain CI 已 PASS；authority-sync commit/push、replacement exact source 同源四门、fresh baseline、新 package 和新批准仍待完成，生产未改变。
  - **2026-08-02 Gateway production-runtime preflight 复发根治**：replacement source `d9a1e5fedf481f00e3cc88705c7e2b884add3ebe` 的 Signed Dispatch `30743899298`、Full Quality `30743899329`、A0 `30743899304`、Independent Security `30743899295` 同源四门全部 PASS；第二个 exact dispatch `39732e2e...` 仍在任何 Caddy mutation 前 `FAIL_DISPATCH_NOT_REUSABLE`。生产等价 A/B 直接复现 current Caddy image 在 `cap-drop ALL` 下 RC 255，以及绕过 root-owned Compose identity wrapper 后缺 `POSTGRES_USER` 的 RC 1；保留相同 isolation 且仅加 `NET_BIND_SERVICE` 后 Caddy validate PASS，exact wrapper 带 gateway override 后 config PASS。生产 HEAD、clean worktree、11-container identity、health ready/fresh、404 route 和全部非目标面零漂移，唯一空 outbound residue 已按批准清理。该失败与首次 health-envelope 偏差共同触发 occurrence 2 STOP-THE-LINE。local remediation commit `bb9abe5cf0404ca7ce9af55f6ed99a6e3f646d76` 绑定 wrapper/override exact identity、最小 capability、稳定 command fingerprint、owned-state cleanup，并把 checksum-bound recurrence registry 同时接入构包和生产 pre-mutation gate。gateway `12/12`、recurrence `11/11`、dispatch `38/38` 与完整 exact-toolchain CI PASS；完整 CI 为 market `965/969`（4 explicit skips）、workers `23/23`、historical `4/4`、Foundation `638/644`（6 explicit skips）、Ops `279/279`、M0 `12/12`、Next build、Golden `16/16` 和 security。final authority commit/push、新 source 四门、fresh baseline、新 deterministic package 和新批准仍待完成。
  - **2026-08-03 Gateway signer identity 与 bridge-v6 资格根治**：第三次 source `ad9aeb9842f6b57d934e7729e5e8a5cf2680a3b7` 真实部署 Caddy、完成健康/mount/route probe 后在 evidence seal 因 request v3 未绑定 signing key path 失败；自动 rollback 恢复 baseline，十个非 Caddy IDs、生产 HEAD、clean worktree 和非目标面零漂移。request v4 source `4d1042618eff24d39f6ecceb93d5715e2e798b5a` 已提交推送并通过 Signed Dispatch、A0、Independent Security，但 Full Quality `30755615546` / job `91517146516` 暴露 bridge v5 clipboard wait 的 session-classification race，因此严格为 `3/4`。bridge v6 使用显式 `{id,failure_code}` 保留 credential/age 分类，三种断线故障注入 `13/13`、P0R `142/142`、Go helper 与最终字节完整 CI 本地 PASS，完整 CI 包含 Ops `283/283`。当前只允许形成新 clean commit/push 和四门全量复验；旧批准、旧 dispatch、旧三门和 bridge v5 资格不得拼接。
...
- [x] **M2.1A Bidirectional Precursor Atlas + Market Mechanics + Sector Propagation Research**：八族各自登记 LONG/SHORT/UNKNOWN 共 24 个 DRAFT/UNCALIBRATED 假设，禁止用同一 Feature 的符号翻转冒充双向机制；三 Outcome、point-in-time 板块关系、逐 family-direction 的四 Venue/三 regime/三 liquidity segment、matched control、消融、sealed holdout、forward Shadow、rights 和独立审计均进入 fail-closed Gate。定向 13/13 与完整 `ci:production` PASS。历史 L2 缺失时保持 `FORWARD_ONLY_RESEARCH`。状态：`LOCAL_RESEARCH_CONTRACT_PASS / REAL_COHORT_MISSING / NO_CANDIDATE_EMISSION / PRODUCTION_UNCHANGED`。
...
- [x] **M2.2-B0.2-C1 Egress-Capable Forward Capture Start**：历史 release `4139cc631d3d760876c3e39404c494462541a910` 的两轮三 Venue COMPLETE 起点保持不可覆盖；每家 2/2 完整、跨度约 368.5 秒、raw 全链复核、active gap/unresolved/conflict/blocker=0。最新 evidence release parts `a02aecbbf8b289e409fd + e33b150fb55cb62ef3f0` 又取得四轮 COMPLETE，并由 verifier release parts `be5c0c9682cd8f503679 + 7d52d426783eb0f6615f` 全链只读复核：4 条 journal、28 个 artifact 引用、12 个 raw 引用、37 个精确保留文件全部通过，三家跨度约 998.5-998.7 秒、gap=0，均为 `FORWARD_ONLY_READY`。Scope V2 正式 normalizer 重放最新 raw 后，Binance/OKX/Bybit 均诚实保持 `PARTIAL`；观察到加密、单股、指数/ETF、广义 RWA 与 unresolved 分域，证明旧 `CANONICAL_TARGET` 仅是合约形状标签而非资产域结论。状态：`OPERATIONAL_CAPTURE_START_PASS / LATEST_FOUR_CYCLE_INTEGRITY_AND_DOMAIN_ISOLATION_PASS / FORWARD_ONLY / NO_AUTHORITY / PRODUCTION_UNCHANGED`；永久禁止冒充历史回填、24h SLO、官方 mapping 完整、Fact、Detector、Candidate、Strategy 或 READY。
...
- [ ] **M3 唯一决策纵向切片**：完成 family-specific Analysis、Evidence/Setup 双评级、StrategyDraft、canonical Strategy Archetype Label、Execution Feasibility 唯一终审、Personal/Portfolio Risk。验证：只有 Strategy Construction 能生成后端主策略标签，只有 Final Decision 能产生 READY；false READY、无标签完整草案、前端生成标签、事后改写标签均为 0，结构与净 RR 均不低于 3，所有关键缺失 fail closed。
...
- [ ] **M3.3E Strategy Archetype Labeling + Outcome Attribution（已完成设计权威，实施待开始）**：每份完整 `StrategyDraft` 必须恰好一个后端 canonical 主标签，例如突破回踩确认做多、跌破反抽确认做空、支撑反弹做多或压力受阻做空；同时携带 taxonomy version、reason codes、有界 context tags 和 exact Analysis/Qualification/Evidence/level/release lineage。Candidate/Analysis 只能给 `setupHypothesis`，Final Decision 只校验和冻结，DecisionSnapshot/Alert/Outcome 原样传播，前端只本地化、展示和筛选。标签不得修改评级、结构止损、净 RR、Action State 或 READY；新增标签必须通过真实 cohort、matched control、sealed holdout、前向 Shadow 和独立审计。当前仅 `DESIGN_AUTHORITY_ADDED / IMPLEMENTATION_NOT_STARTED / NO_RUNTIME_OR_PRODUCTION_AUTHORITY`，不计入 M3 完成。
...
- [x] **M3.4-R0 Scope Rebase Review Gate（治理前置，不计 M3.4 完成）**：R0 当时只读审计旧 V1 草稿为 typecheck FAIL 3、lint 1 warning、测试 0，且缺 scopeEpoch、Bitget、assetDomain、listing lifecycle 与股票执行事实；机器 gate 随后强制四 Venue、资产域、生命周期、release、14 项通用、3 项加密、7 项股票与 warm-up 独立证据逐项绑定，Bitget/Listing/Equity/Data Maximization 四轴不得互借 PASS。定向 12/12 和完整 CI 已通过。当前已把该草稿的 import/schema/fixture 兼容阻断修到全仓 typecheck/lint/CI PASS，但草稿自身定向测试仍为 0、Scope V2 前置证据仍缺，因此继续不计 M3.4 出口。状态：`LOCAL_GOVERNANCE_CONTRACT_PASS / R1_PARTIAL_TEST_ONLY_DRAFT_COMPILE_COMPATIBLE_NO_DIRECTED_EXIT / NO_FEASIBILITY_OR_READY_AUTHORITY / PRODUCTION_UNCHANGED`。
- [ ] **M3.4-R1-M3.6 Feasibility + Risk + Runtime**：M3.4-R0 已把 scope rebase 前置条件机器化，M1.4B runtime 已通过；真实实现仍必须等待 M1.5C、M1.6-D1、M2.3A/B、M2.4A/B 和 M3.1A-M3.3D。随后按成熟加密、listing warm-up、单股、指数/ETF 分域完成真实成本与 Execution Feasibility，再接 Personal/Portfolio Risk、Trigger 与 Runtime Gate；只有 Portfolio Risk 可以汇合加密和股票。状态：`M3.4_R0_GOVERNANCE_PASS_IMPLEMENTATION_BLOCKED_BY_REMAINING_SCOPE_V2_PREREQUISITES / M3.5-M3.6_NOT_STARTED`。
- [ ] **M4 单一读模型与专业工作台**：先建立 DecisionSnapshot、EvidenceOverlaySnapshot、canonical Strategy Archetype Label 和站内 Alert，再重建 Inbox、Token/Equity Workbench、Review、System。加密、股票、指数和 warm-up 先分域展示，不得混成总分；证据按 event time 锚定，盘口/主动成交/OI/Funding/Liquidation/新闻/结构/计划分层开关，默认不遮挡 K 线。高灵敏/标准/低噪声只改变提醒与展示；AI 和自定义指标无 authority。未形成 StrategyDraft 时只显示“策略类型待确认”，不得由页面猜测。验证：页面零 provider/decision/label 推导，同一 snapshot 在 Inbox、Workbench、Alert 和 Review 中标签一致，E2E、a11y、visual、performance 和注意力预算通过。
- [ ] **M5 结果与研究治理**：从 M2 首个 Episode 起并行采集 Outcome，但只有冻结数据成熟后才评估；Research 与 Evaluation 物理分离。Outcome 必须冻结决策时的原始策略标签，并按标签、方向、regime、Venue、流动性、资产域和 lifecycle 分层报告样本量、precision/recall、误报/漏报、提前率、净 R、费用、滑点与 abstention。验证：future leak=0、事后标签改写=0、单币种专属 archetype=0、Missed Movers/对照组完整、全部试验登记、Challenger 不能自批或自动晋级。
...
-> M1.4D multi-asset identity/catalog/base-fact runtime [LOCAL PASS]
-> A0 Engineering Foundation total Gate [PARTIAL]
-> M1.5C four-venue multi-asset shadow [LOCAL RUNTIME/PACKAGE PASS, LIVE PENDING]
+ M1.5D adaptive microstructure forward shadow [LOCAL RUNTIME/PACKAGE PASS, same exact release, independent acceptance]
-> M1.6-D1 expanded capacity proof
-> M2.1A bidirectional precursor/mechanics research [LOCAL PASS] + M2.3A/B + M2.4A/B domain-specific detection and holdout
-> M3.1A-M3.3D domain-specific analysis/qualification/strategy
-> M3.3E backend strategy archetype labeling + immutable outcome attribution
-> M3.4-R1-M3.6 domain-specific feasibility and risk
-> M4-M5 domain-separated Evidence Overlay workbench and learning
...
-> refresh same-commit source conformance + M1.4B upstream evidence
-> M1.5C four-venue multi-asset shadow
 + M1.5D adaptive microstructure forward shadow [shared release, independent acceptance]
...
+ M1.4C microstructure fact/feature/cache contract [LOCAL PASS]
+ M1.4D multi-asset identity/catalog/base-fact runtime [LOCAL PASS]
+ M1.5C/M1.5D exact no-secret runtime/evidence/package [LOCAL PASS, LIVE BLOCKED]
+ M2.1A bidirectional precursor atlas and market-mechanics research contract [LOCAL PASS]
...
Last completed scope design package: V2-M0.4-EXPANDED-MARKET-SCOPE-AMENDMENT
...
Current engineering package: V2-A0-ENGINEERING-FOUNDATION-AND-MATERIAL-QUALIFICATION = MATERIALS_SUPPLY_CHAIN_INDEPENDENT_SECURITY_REPRODUCIBLE_RELEASE_AND_RESOURCE_BASELINE_PASS_TOTAL_GATE_INCOMPLETE_P0R_PENDING
Last completed parallel local package: V2-M1.4D-M1.5C-M1.5D-LOCAL-RUNTIME-AND-EXACT-PACKAGE = LOCAL_RUNTIME_EVIDENCE_AND_PACKAGE_PASS_23_PLUS_70_PLUS_12_TESTS_NO_AUTHORITY
Next forward evidence companion after A0: V2-M1.5D-ADAPTIVE-MICROSTRUCTURE-FORWARD-SHADOW = LOCAL_RUNTIME_AND_PACKAGE_PASS_LIVE_ZERO_OF_31_PENDING_FRESH_SAME_COMMIT_UPSTREAM
...
Last completed forward evidence package: V2-M2.2-B0.2-C1-FORWARD-EVIDENCE-REFRESH-AND-DOMAIN-ISOLATION = LOCAL_AND_EXACT_SOURCE_REMOTE_QUALIFICATION_PASS_FOUR_COMPLETE_CYCLES_FULL_HISTORY_INTEGRITY_AND_SCOPE_V2_DOMAIN_REPLAY_PASS_NO_AUTHORITY
Paused local draft: V2-M3.4-EXECUTION-FEASIBILITY-CONTRACT = PARTIAL_TEST_ONLY_DRAFT_COMPILE_COMPATIBLE_NO_DIRECTED_EXIT_SCOPE_V2_PREREQUISITES_PENDING
Last production gate execution: V2-M1.6-P0-PRODUCTION-STORAGE-READ-ONLY-PREFLIGHT = BLOCKED
Current execution entry: V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY signer-identity root remediation and bridge-v6 qualification. The first health-envelope, second Caddy-capability/Compose-identity and third signer-key-path dispatches are claimed and permanently non-reusable. Third source ad9aeb9842f6b57d934e7729e5e8a5cf2680a3b7 reached a real Caddy-only deploy, health, mount and route probe before evidence sealing failed with evidence_signing_key_path_invalid; automatic baseline rollback restored Caddy and preserved every non-target identity. Request-v4 signer remediation source 4d1042618eff24d39f6ecceb93d5715e2e798b5a, tree c28fa60f1236e8b80a7185de4bcfdc4c084d34e0, is committed and pushed. Signed Dispatch 30755615525, A0 30755615545 and Independent Security 30755615533 pass; Full Quality 30755615546, job 91517146516, fails because bridge v5 overwrites credential/age-specific classifications with generic prearmed_secret_session during clipboard wait. Remote qualification is therefore 3/4. Bridge v6 preserves explicit per-session failure codes and passes three independent disconnect timings 13/13, the complete P0R suite 142/142, Go helper and final-byte exact-toolchain CI locally; V2 Ops is 283/283. A new clean commit/push and all four new same-source gates remain mandatory before fresh baseline, deterministic double build, new approval or real-target acceptance. No previous PASS gate or approval may be combined with the bridge-v6 bytes.
...
Current status: SCOPE_EPOCH_V1_EVIDENCE_PRESERVED / SCOPE_EPOCH_V2_M0.5_MARKET_MECHANICS_MICROSTRUCTURE_PRECURSOR_ATLAS_DESIGN_PASS / A0_SUBGATES_PASS_TOTAL_GATE_INCOMPLETE_P0R_PENDING / M1.1B0_R3_LIVE_15_OF_15_ALL_GATES_PASS / M1.4A_M1.4D_LOCAL_CONTRACTS_PASS_NO_RUNTIME_AUTHORITY / M1.5C_M1.5D_LOCAL_RUNTIME_EVIDENCE_AND_EXACT_PACKAGE_PASS_LIVE_ZERO_OF_31_EACH / M2.1A_LOCAL_RESEARCH_CONTRACT_PASS_REAL_COHORT_MISSING_NO_CANDIDATE_EMISSION / M2.2-C1_FORWARD_ONLY_NO_AUTHORITY / M1.5-B1_COMPLETE_V1_ONLY / M1.6-P0_EXECUTED_BLOCKED_CAPACITY_AND_RECOVERY / P0R_ROUTE_AUTHORITY_UPSTREAM_QUALIFIED / PRODUCTION_EVIDENCE_GATEWAY_THREE_ATTEMPTS_NOT_REUSABLE / THIRD_ATTEMPT_CADDY_DEPLOY_THEN_AUTOMATIC_BASELINE_ROLLBACK_PASS / SIGNER_KEY_PATH_DIRECT_ROOT_CAUSE / RECURRENCE_OCCURRENCE_3_STOP_THE_LINE / REQUEST_V4_SIGNER_COMMIT_4D104261_PUSHED_REMOTE_3_OF_4_FULL_QUALITY_BRIDGE_RACE_FAIL / BRIDGE_V6_LOCAL_DIRECTED_13_OF_13_P0R_142_OF_142_AND_FINAL_BYTES_FULL_CI_PASS / NEW_CLEAN_COMMIT_AND_SAME_SOURCE_REMOTE_FOUR_GATES_PENDING / M3.0-M3.3_LOCAL_CONTRACT_PASS_TEST_ONLY_UNCALIBRATED_NO_READY_AUTHORITY / M3.3E_STRATEGY_ARCHETYPE_LABELING_DESIGN_ONLY / M3.4_R0_SCOPE_REBASE_GOVERNANCE_PASS_R1_PARTIAL_DRAFT_NO_DIRECTED_EXIT / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED / P0R_BACKUP_RETRIEVAL_RESTORE_NOT_EXECUTED
```
...
- [ ] Market Mechanics、订单墙生命周期、压力-价格响应和板块传播均使用上涨/下跌/未爆发分母、matched control、消融和 untouched holdout，固定金额阈值或个案截图没有被冒充为规律。
- [ ] 加密、股票和上市 warm-up 分别使用独立 cohort、calibration 和 untouched holdout；工作台不混成总分。
- [ ] 每个 READY 都有后端完整计划、执行可行性、结构来源、成本后 RR 和风险视图。
- [ ] Workbench 只读 DecisionSnapshot/EvidenceOverlaySnapshot；灵敏度、AI 和自定义指标没有改变后台真值或 authority。
