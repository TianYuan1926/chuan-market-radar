...
- [ ] **A0 Engineering Foundation 总门禁**：第一批材料资格已完成精确 Node/npm/直接依赖声明、Next/PostCSS/Sharp 安全补丁、零高危依赖审计、许可证、CycloneDX SBOM、固定 GitHub Action/runner/Node/V2 base-image、ESLint + Biome、Next build、Sharp/PostCSS 原生烟测和 exact Node 22.23.1/npm 10.9.8 本地完整 CI；该 CI 发现并根治 timeout `unref` 导致 Promise 可悬空的问题。首次 GitHub Ubuntu 门禁在 host tar `--uid=0` 方言处失败，修复提交 `29ab47dec0b9fbbbe66e1cfe7ce90aa2e1e4c25d` 已用纯 Node USTAR 根治四个活跃 V2 Bundle 并完成本地全门禁，Legacy 历史制品未改。状态：`ENGINEERING_MATERIALS_AND_SUPPLY_CHAIN_LOCAL_PASS / PORTABLE_USTAR_LOCAL_REMEDIATION_PASS / REMOTE_LINUX_REVERIFY_PENDING / TOTAL_GATE_INCOMPLETE / PRODUCTION_UNCHANGED`。剩余必须补齐 GitHub 远端 exact-runtime、独立 secret/SAST/容器镜像扫描、完整制品 provenance、性能/资源基线、回滚演练和 P0R 真实恢复；总门禁通过前禁止启动 M1.5C/M1.5D。
