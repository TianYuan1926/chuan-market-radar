...
- [ ] **M2.3A Listing/Venue Event Detection**：R0 已完成 point-in-time 事件研究真值纵切：四 Venue、十四类事件、M1 lifecycle ledger v2 publication/effective/knowledge 三时间、exact event denominator、identity epoch 和 content hash 均由 strict schema/builder 验证；未关联公告不猜 symbol，首次目录观察只算 baseline，目录消失不推断 delist，无支持合约新币只 WATCH_ONLY，股票事件 handoff M2.3B，partial identity 显式 BLOCKED。R1 source `777af03d18bb8c677854bdd3579c19d003f71864` 关闭 caller-supplied coverage 缺口：Coverage、ledger 和 research Bundle 只能从 exact upstream、registry、catalog/identity 与 listing refresh 推导。R2 source `d5d70bc50484db570748a7744319e2b35e0e8426` 又关闭真实运行证据落盘断点：M1.5C 每轮保存完整 refresh batch，31 周期独立 verifier 绑定 280 个文件、initial/prior checkpoint 连续性和首尾 batch hash；精确包从 independently verified cycle 30/31 source 生成并回读重建 M2 五件套，refresh evidence v2 与原 source batch hash 交叉闭环。R2 相邻 `148/148`、完整 candidate CI、GitHub Signed Dispatch `30706385082`、A0 `30706385111`、Independent Security `30706385064`、Full Quality `30706385069` 全部 PASS。当前仍为 `R2_LOCAL_RUNTIME_EVIDENCE_CHAIN_PASS / LIVE_ZERO_OF_31 / NO_CANDIDATE_DIRECTION_PROBABILITY_SIGNAL_STRATEGY_READY_OR_PRODUCTION_AUTHORITY`；真实 same-release M1.5C/M1.5D、M1.6-D1、M2.4A cohort/matched control/untouched holdout/calibration、Shadow 和独立审计未完成，因此不勾选、不计完整 Detector。
...
Last completed listing event engineering package: V2-M2.3A-R2-LIVE-EVIDENCE-PERSISTENCE-AND-RUNTIME-JOIN = LOCAL_RUNTIME_EVIDENCE_CHAIN_PASS_ADJACENT_148_OF_148_FULL_CANDIDATE_CI_REMOTE_FOUR_GATES_PASS_LIVE_ZERO_OF_31_NO_CANDIDATE_OR_PRODUCTION_AUTHORITY
