...
- [ ] **M1.6-P0R Capacity + Recovery Remediation**：P0R-D0 本地无扩容模型、Object Lock `COMPLIANCE` 31 天、macOS Keychain age 身份和 private `/dev/shm` runner boundary 均已形成；真实 backup/retrieval/restore 仍未执行。production evidence gateway 已由 bridge-v6 source `df85b485b6d2c0ede8cdd2b656978ba2fdc372ab` 的同源四门、deterministic package、signer-bound Caddy-only real-target release、本机自动取回、独立解密验签与非目标零漂移验收关闭；三个失败 dispatch 保留且不可复用，gateway recurrence count=3、状态 `CLOSED_VERIFIED`。正确剩余顺序为 `权威同步 source 资格 -> fresh P0R read-only rebind/零残留基线 -> 新 route/lease/run/object key/v4 plan/transport -> fixed-dispatch no-secret target acceptance -> 双 TTY PREARMED + 腾讯权威 /32 route -> 一次 fresh STS native Copy + age handoff -> COS preflight -> read-only encrypted backup -> exact COS version retrieval -> isolated PG16 restore/integrity -> lease-bound cleanup -> production zero-drift -> fresh health/topology/capacity -> calibration -> fresh P0`。当前禁止复用 gateway approval/dispatch、旧 STS、8022、route、lease、run、plan 和 package。当前状态：`PRODUCTION_EVIDENCE_GATEWAY_REAL_TARGET_ACCEPTANCE_AND_VERIFICATION_PASS / P0R_FRESH_REBIND_BACKUP_EXACT_RETRIEVAL_ISOLATED_RESTORE_AND_FRESH_P0_PENDING / PRODUCTION_P0_BLOCKED`。
  ...
  - **2026-08-03 Gateway signer identity、bridge-v6 与真实目标验收**：第三次 source `ad9aeb...` 的 signer-key-path 失败和自动 rollback 保留为历史。bridge-v6 source `df85b485b6d2c0ede8cdd2b656978ba2fdc372ab` 已独立通过 Signed Dispatch `30758244706`、Full Quality `30758244710`、Independent Security `30758244707`、A0 `30758244715`；dispatch `3cca64f...` 完成 signer-bound Caddy-only release，fixed retriever 对 `22ebec...mre` 完成 pinned-host 解密验签。独立生产复核证明 HEAD、clean worktree、十个非 Caddy IDs、ready/fresh health、staging/lease absence 与 expiry timer；非目标面零漂移。gateway recurrence 已 `CLOSED_VERIFIED`；下一入口是 fresh P0R rebind/recovery，不能重复发布 gateway 或把其 PASS 计为 P0R。
...
Current execution entry: V2-M1.6-P0R-FRESH-REBIND-AND-RECOVERY. Production evidence gateway source df85b485b6d2c0ede8cdd2b656978ba2fdc372ab, tree f32cc5aa70f7f1c46e772b3b7ba2abbbd7aaf2a5, passed all four same-source gates and exact signer-bound Caddy-only real-target acceptance. Signed dispatch 3cca64f63ea160e0b772c513c9c6624bc76a28ec returned PASS_PRODUCTION_EVIDENCE_GATEWAY_CADDY_ONLY; the fixed local retriever returned PASS_PRODUCTION_EVIDENCE_DECRYPTED_AND_VERIFIED for object 22ebecfad6dd9d83b8e40203889f2f36d7c22fa7237fcf3bf63142acafe14acf.mre. Independent production checks proved non-target zero drift, clean bound HEAD, ready/fresh health, exact staging and lease absence, and active expiry cleanup. Three historical failures remain immutable and non-reusable; recurrence count 3 is CLOSED_VERIFIED. Next: qualify the authority-sync source, fresh P0R read-only rebind, new route/lease/run/plan/transport, one fresh STS handoff, encrypted backup, exact COS retrieval, isolated PG16 restore, cleanup and fresh P0.
