...
- [ ] **M1.6-P0R Capacity + Recovery Remediation**：P0R-D0 本地无扩容模型、Object Lock `COMPLIANCE` 31 天、macOS Keychain age 身份和 private `/dev/shm` runner boundary 均已形成；真实 backup/retrieval/restore 仍未执行。B8 exact target 和 B9 COS CAM action 根治只保留历史证据。2026-08-02 两个 bridge 窗口均只预建 credential TTY，随后在 540 秒 clipboard deadline 超时；后到 native Copy 已作废且从未进入 Bridge 或服务器。两端点随后同时测得代理出口 `156.248.15.36`，严格 8022 已不可达，证明签发后的第二次 age SSH 不能依赖 dynamic egress + 精确 `/32`。本地已升级 bridge/session v5：primary 在 secret ingress 前写入 PID/start-token/source binding，credential 和 age 两条严格 no-echo TTY 均在 operator READY 前建立，1200 秒等待期间验证两条会话并输出无 secret 心跳，签发后不再建立网络连接。transaction v1 以 7200 秒 exact lease 绑定全资源；execute 强制消费 120 秒内 mode-600 的精确 route evidence，detached authorize CLI 已删除。Bridge 只允许 `PREARMED -> READY -> HEARTBEAT* -> CREDENTIAL -> AGE -> PASS`，BLOCKED/PASS 为终态，两个输出流合计最多 65,536 bytes、状态最多 64 个；plan/lease/route/cleanup 使用 canonical current-user mode-600、`O_NOFOLLOW`、1 MiB 上限和读取前后稳定 identity。fresh rebind v4/v3 又明确要求 TCP 8022 listener=0 与 exact unit=`not-found/inactive`。rebind `13/13`、P0R `125/125`、Go helper 与当前最终字节完整 CI PASS；完整 CI 包含 recurrence `11/11`、dispatch `25/25`、V2 Foundation `631 PASS / 6 explicit skips`、V2 Ops `247/247`、Next build、Golden `16/16` 和 security。正确剩余顺序调整为 `clean exact commit + GitHub 四门可先行 -> fresh signed read-only rebind 证明服务器 8022/unit/P0R residue 为零 -> 本人确认敏感页关闭 -> 自动清理旧云规则并完成外部零漂移 -> 新 lease/run/object key/v4 plan/transport -> fixed-dispatch staging 与独立 target acceptance -> API Explorer 预备 -> 双端点 egress -> 临时 8022 + exact /32 + fresh route evidence -> 两条 TTY 全部 PREARMED -> 一次 fresh STS native Copy + Keychain age handoff -> COS preflight -> read-only encrypted backup / exact retrieval / isolated PG16 restore -> fresh close evidence + lease-bound 全量 cleanup -> production zero-drift -> fresh health/topology -> calibration -> fresh P0`。外部全清场前仍禁止新 route、STS、数据库和 COS 动作。当前状态：`STOP_THE_LINE_EXTERNAL_TRANSACTION_RECURRENCE / LATE_RESPONSE_INVALIDATED / LOCAL_BRIDGE_ZERO_AND_CLIPBOARD_CLEARED / CLOUD_CLEANUP_AND_PRODUCTION_ZERO_DRIFT_UNVERIFIED / LOCAL_TRANSACTION_AND_REBIND_EXPLICIT_8022_ZERO_RESIDUE_P0R_125_OF_125_AND_FULL_CI_PASS / PRODUCTION_P0_BLOCKED`。
...
Current execution entry: V2-M1.6-P0R-B9-R2-LEASED-DUAL-TTY-TRANSACTION-ROOT-REMEDIATION; every 2026-08-02 response, route, run, plan and staging identity is invalidated; local bridge/session v5 and exact transaction v1 enforce a unique terminal state machine, bounded aggregate output/statuses and stable no-follow control-file reads, while rebind v4/v3 explicitly rejects any TCP 8022 listener or exact transient unit residue; P0R 125/125, the Go helper and final-byte production CI pass, while sensitive-page closure, external listener/firewall/server residue cleanup, production zero-drift, clean exact source, four remote gates, fresh read-only rebind, a new lease/run/package/staging and corrected production recovery remain pending
