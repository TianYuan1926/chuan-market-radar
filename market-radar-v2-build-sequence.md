...
- [ ] **M1.1B0 Tencent Isolated Live Source Conformance**：无密钥固定派发包已实现；Bybit `type=new_crypto` 与 Bitget `annType=coin_listings` 完整分页、同源串行/跨来源并行、12 秒与 8 MiB 边界均绑定 `probePlanDigest`，定向 20/20、固定通道 rehearsal 和独立干净克隆完整 `ci:production` PASS。剩余硬动作是 package-only exact commit/push、fresh 生产身份绑定和腾讯 `LIVE_READ_ONLY` 15/15；套餐、地区、schema、分页、时钟、空目录和 CoinGlass Hobbyist 实证通过前不减数，失败 capability 不得进入 M1.4B。状态：`LOCAL_PACKAGE_AND_FULL_CI_PASS / LIVE_B0_PENDING / PRODUCTION_UNCHANGED`。
