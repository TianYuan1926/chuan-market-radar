...
- [ ] **M1.6-P0R Capacity + Recovery Remediation**：P0R-D0 本地无扩容模型、Object Lock `COMPLIANCE` 31 天、macOS Keychain age 身份、atomic remote session、private `/dev/shm` runner boundary 和 fresh rebind 七文件合同均已形成；真实 backup/retrieval/restore 仍未执行。历史 `e362...` 的第三枚 STS 从未进入服务器、已由双时钟证明过期并永久禁用；其 run/plan/object key/bundle/staging 均无执行权。pre-armed local TTY bridge 固定 SSH/host-key/proxy/run-derived command 和 Node `v22.23.1`，只在 echo-disabled READY 后接受 API 原生 Copy；response 后 browser state/AX/OCR/screenshot/computer-use/OrcaTerm secret entry 永久退役。Expect 运行材料根治 source `0e035784988261926eb9656d25050180c87b0d1e` 已取得 GitHub 四门全 PASS，但 rebind `p0r-rebind-preflight-20260729t150004z-c85ec1a4` 因外层 5400 秒与包级 90 秒不一致，在任何生产动作前被 `PACKAGE_BINDING` 阻断，staging 自动清理；fresh 复核确认 production HEAD、clean worktree、11 containers 和 P0R files/containers/volumes 零漂移。当前 request schema v3 显式绑定 90 秒，固定通道签名前/outbox/生产代理和 runner 四层校验；唯一高层 release 入口从 canonical request 派生全部重复字段并拒绝 operator-supplied binding 选项。定向 29/29、P0R 88/88、最小 PATH dispatch 回归和显式 Node/npm/Go 的完整 `ci:production` 均 PASS；新 clean commit、四门和新 rebind 仍待完成。剩余顺序固定为 `新 clean exact commit -> GitHub 四门 -> fresh rebind -> 全新 run/object key/plan/14-member bundle staging -> 预先启动 bridge 并取得 READY -> 用户 MFA + API 原生 Copy -> 即时编译 + Keychain age handoff -> backup / exact retrieval / isolated PG16 restore -> secret/container/volume/runtime cleanup -> production zero-drift -> fresh health/topology -> calibration -> fresh P0`。当前状态：`SOURCE_0E035_REMOTE_FOUR_GATES_PASS / REBIND_C85EC1A4_BLOCKED_PACKAGE_BINDING_ZERO_MUTATION / REQUEST_V3_CROSS_LAYER_ROOT_FIX_FULL_LOCAL_CI_PASS_NEW_SOURCE_PENDING / PRODUCTION_P0_BLOCKED`。
...
Current execution entry: V2-M1.6-P0R-B5-CROSS-LAYER-DISPATCH-BINDING-ROOT-REMEDIATION; source 0e035 remote four gates passed, rebind c85ec1a4 was blocked before mutation because outer runtime 5400 exceeded package 90, staging was cleaned and production identity reverified unchanged, request v3 four-layer binding targeted tests pass, and full local CI/new exact source/four remote gates/fresh rebind/new execution identity/real recovery remain pending
