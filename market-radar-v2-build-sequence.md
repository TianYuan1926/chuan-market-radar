...
- [ ] **M1.4B Endpoint Batching + Runtime Adapter Profiles + Listing History Runtime**：本地核心已实现；新增无 secret 固定 Bundle、Runner 与 Entrypoint，定向包 9/9 PASS，正式实施分支完整 CI 待重验。Profile 绑定 exact live artifact、registry/probe digest、HTTPS endpoint、分页、credential 与恢复语义；现场包绑定 R3 artifact、源提交、生产 HEAD、容器、listener、timer 与 health。15 个 live-conformant Profile 保持完整分母，只执行 14 个 route-eligible Profile，`BINANCE_SPOT_CATALOG` 请求数固定为 0；snapshot 和 listing-history 使用两本预算，同源并发 1。Bybit provider-available history 与 Bitget 官方一个月窗口均以最多 64 页的 bootstrap/resume/gap/incremental 状态机推进；blocked segment 不写 checkpoint，历史 checkpoint 必须绑定原精确 `PASS` result 与 SHA-256。Bitget Venue、Listing Lifecycle、Equity Asset Domain 与 Data Maximization 四轴分别核算；股票当前仅 catalog accounting、tradable Fact 为 0。状态：`LOCAL_CORE_AND_EXACT_DISPATCH_PACKAGE_PASS / FULL_CI_REVERIFY_PENDING / LIVE_NO_AUTHORITY_RUNTIME_UNPROVEN / PRODUCTION_UNCHANGED`。只有正式分支完整 CI、GitHub 同步、同 release 腾讯隔离运行、真实 checkpoint、请求率/配额/完整分母和恢复证据通过后，本项才能勾选完成。
