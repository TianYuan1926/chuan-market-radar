...
- [ ] **M1.6-P0R Capacity + Recovery Remediation**：P0R-D0 本地无扩容机器证明已通过：clean commit `15746813245744af4f4ba73f61a976b722ad9a21` 在隔离 PG16 完成 8 周期/11,552 Fact，最大周期 33,660 ms；按 1,805 Facts/分钟、30h retention、6h partition、1h sweep、1.5 倍字节成本和固定 reserve，稳态/峰值根盘预计 59%/67%，满足固定 60%/70% 双门槛。fresh P0 组合准入也已本地实现，继承全部非容量 blocker。腾讯 COS 已回读 Object Lock=`COMPLIANCE` 31 天，真实 age 身份已在 macOS Keychain 读回验证。历史 `bed938...` staging 的完整性仍保留，但其 backup/credential/recovery 运行文件早于当前 `O_NOFOLLOW` 单句柄和独占输出安全修复，状态已改为 `REJECTED_SUPERSEDED_SECURITY_SOURCE`，禁止执行或绑定新 STS。source parts `408803e0bdc21051124a + 79e307db8e9eb39c793c` 已完成无 secret 只读重绑定包，本地 package `9/9`、P0R `70/70`、V2 Ops `179/179` 与完整 CI PASS；exact-source Full Quality `30219999104`、A0 Release Qualification `30219999094` 和 Independent Security `30219999063` 也全部 PASS，Gitleaks/CodeQL/Trivy 阻断计数均为 0。signed read-only rebind dispatch `p0r-rebind-preflight-20260726t213258z-e77631a3` 已确认未执行：旧代理的 Git child 被 systemd 180 秒超时终止后遗留空锁，导致 4,526 次重复阻断；fixed channel source parts `2b4fccc9f3affe613d4f + 0da0f97295d97b0c6452` 已以 90 秒 Git 硬上限和 owner-aware stale-lock recovery 完成生产根治。旧 dispatch commit parts `34b1f137e92be1649d64 + c6507c3e36204ce0b31e` 现为 `FAIL_DISPATCH_NOT_REUSABLE / dispatch_not_current`，无 claim、解包或业务 Runner，原 `UNKNOWN` 状态失效。尚无 STS、生产对象、backup/retrieval/restore 或业务生产 mutation；旧 topology 仍过期。固定剩余顺序已推进为 `生成并派发 fresh exact signed read-only rebind -> 目标零漂移验收 -> current-source plan + deterministic bundle -> 受限无 secret 上传 -> /dev/shm fresh 7200 秒 exact-plan STS + age identity -> 同快照加密 backup / exact retrieval / 独立 PG16 restore / cleanup -> fresh health + topology -> exact-release calibration -> fresh P0 composition admission`。当前状态：`LOCAL_AND_EXACT_SOURCE_REMOTE_QUALIFICATION_PASS / PREVIOUS_DISPATCH_CONFIRMED_NOT_EXECUTED_EXPIRED_NOT_CLAIMED / FIXED_DISPATCH_TIMEOUT_LOCK_RECOVERY_PASS / FRESH_REDISPATCH_REQUIRED / LEGACY_BED938_STAGING_REJECTED / STS_AND_RECOVERY_PENDING / PRODUCTION_P0_BLOCKED`。
...
- [ ] **M4 单一读模型与专业工作台**：先建立 DecisionSnapshot、EvidenceOverlaySnapshot 和站内 Alert，再重建 Inbox、Token/Equity Workbench、Review、System。加密、股票、指数和 warm-up 先分域展示，不得混成总分；证据按 event time 锚定，盘口/主动成交/OI/Funding/Liquidation/新闻/结构/计划分层开关，默认不遮挡 K 线。高灵敏/标准/低噪声只改变提醒与展示；AI 和自定义指标无 authority。验证：页面零 provider/decision 调用，同一 snapshot 在所有视图一致，E2E、a11y、visual、performance 和注意力预算通过。
...
Current execution entry: V2-M1.6-P0R-B2-STS-REGION-CONTRACT-REMEDIATION; source 94118 production read-only rebind passed with zero drift, the subsequent v2 exact-plan STS request was rejected for missing required Region without generating credentials, plan schema v3 root remediation passed local P0R 72/72 and V2 Ops 194/194, and fresh exact-source qualification plus production rebind are required
