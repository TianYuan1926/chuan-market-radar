...
- 2026-07-17 `WP-G0.2-LEGACY-PENDING-DRAIN-PRODUCTION` 已完成本地生产执行包准备，但尚未生成当前 commit-bound request、未上传或修改生产。Microsoft Edge/OrcaTerm 动态只读预检确认生产仍是 clean detached `cec0b657...`/tree `eb217a7...`、Web image `sha256:cd3652c1...`、Candidate worker absent、production lease absent、scan ready；同时真实发现初版请求校验器错误要求不存在的 `/var/lib/market-radar-ops/identity/`，而三个 root-owned 文件实际同处 `/var/lib/market-radar-ops/wp-g0-2-identity-runner-20260711T034847Z`。最小修复现在只接受同一时间戳 identity-runner 根下的精确 wrapper、override 和 admin env 文件名，跨根、伪 wrapper 与不存在别名均拒绝；定向生产包 17/17 PASS，runner artifact 已重新冻结为 `19aed3028f3a70b5b632820ea42df9d81f4ce28e6d2b9bdbf4e47276a5e2e405`，提交前完整自治门禁 12/12 PASS、market 1027/0/7、build 与 Golden 16/16 均 PASS，提交后 commit/tree-bound 门禁与新 Bundle 仍待执行。目标代码继续以 `CANDIDATE_EPISODE_DRAIN_ONLY=true` 硬阻断 source enqueue，同时保留受控 Candidate consumer；执行器固定为 scanner 先停且 Redis `scan:lock:*` absent -> 精确 DB preflight -> 临时 Web/worker -> control epoch 4->5 -> 只消费既有 pending -> worker stop -> epoch 5->6 refreeze -> 原 env/Git/Web/scanner 镜像和 scanner 新一次 completedAt/ready/fresh 全恢复。生产仍是 migration 10、legacy/frozen epoch4、outbox 5,914、pending/unresolved 2,957、Candidate worker absent，G0 仍剩 8 个主包，系统仍只能写 `R1 / 可运行但不完整 / 不能支撑实战`。
