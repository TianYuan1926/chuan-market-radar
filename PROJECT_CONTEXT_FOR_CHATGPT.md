...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.85。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.90。
...
M1.6P0R_B8_EXACT_TARGET_ACCEPTANCE_HISTORICAL_PASS / M1.6P0R_B9_R1_COS_CAM_ACTION_ROOT_REMEDIATION_HISTORICAL_PASS / M1.6P0R_B9_R2_TWO_CLIPBOARD_TIMEOUTS_LATE_RESPONSE_INVALIDATED_DYNAMIC_EGRESS_DRIFT / LOCAL_BRIDGE_ZERO_AND_CLIPBOARD_CLEARED / CLOUD_LISTENER_FIREWALL_SERVER_RESIDUE_AND_PRODUCTION_ZERO_DRIFT_UNVERIFIED / ROUTE_AUTHORITY_PRODUCER_TRANSPORT_V4_P0R_138_OF_138_AND_EXACT_MAIN_BRANCH_FULL_CI_PASS / REMOTE_QUALIFICATION_REBIND_AND_REAL_RECOVERY_PENDING
...
- 已建立爆发行情/提前发现评价合同、数据许可/成本/回放基线、Capability 级 Legacy Atlas 和第一条 M1 test-only fixture；M0.5 冻结 Market Mechanics 三轴、双向前兆图谱、LiquidityWallEpisode、压力-价格响应、板块传播、补充来源、缓存读路径和 Evidence Overlay。M1.4C 与 M2.1A 本地合同已实现并通过完整 CI，但仍没有真实微观结构数据、cohort 或运行 authority。
...
- M1.4 已建立 21 observed / 15 eligible 的 V1 三 Venue fixture、reconciliation、quota/backpressure、故障恢复与 PG16 原子持久化。M1.4A 已把 Bitget、listing watch、股票独立资产域和 CoinGlass Hobbyist 纳入 Scope V2 T0-T3 有界调度合同并通过 28/28。M1.4B 精确 source `3c21a75009aeb4f4f7d9fd8954245238c38d9636` 的腾讯 bootstrap 与 checkpoint-bound resume 均取得 14/14 route PASS、0 failed、1 registry blocked、request budget/attempts=203/80、listing gap=0 和两个持久 checkpoint；`BINANCE_SPOT_CATALOG` 请求数固定为 0。M1.4C 已实现六类 Microstructure Fact、LiquidityWallEpisode、十三项 Market Mechanics Feature、ONLINE/双 REPLAY parity 和四层缓存真值边界，定向 22/22 PASS。M1.4D 又完成四 Venue catalog/listing watch、identity v2/snapshot v3、T0 lifecycle、T1 wide-market 与同 cutoff Base Fact Snapshot；Provider URL/transport 已移入 Adapter，Core 只保留哈希和请求边界。
...
- M1.6 v1 日分区迁移 checksum 保持不可变；additive v2 六小时分区、无扩容 59%/67% 模型、Object Lock `COMPLIANCE` 31 天、Keychain age 身份和 private `/dev/shm` runner boundary 均已形成。B8 target、B9 COS CAM action 和 route evidence v1 资格只保留历史证据。2026-08-02 两个 bridge window 都在第一条 TTY READY 后 540 秒超时，后到 response 作废且未进入 Bridge 或服务器；随后双端点 egress 一致变为 `156.248.15.36`，strict 8022 不可达。当前只证明本机 Bridge=0 与 clipboard 已覆盖；云 listener、旧 `/32`、服务器 residue 和生产零漂移未复核。bridge/session v5、transaction v1 和 rebind v4/v3 已关闭双 TTY、事务状态机、稳定控制文件读取和显式 8022/unit absence 缺口。继续审计证明旧 route evidence 只有 validator、没有 producer，测试曾以任意 final hash 冒充 firewall identity；implementation commit `157e9a79a635d8f31857481fbd6a51d10df51160` 已在唯一 V2 实施分支新增 route target v1、Tencent firewall capture v1、strict listener observer、authoritative producer 与 route evidence v2。route target、lease、firewall capture 和 listener observation 必须 canonical，既有 provisioning plan 继续由稳定安全读取及精确 SHA-256 绑定；transaction 重算 provider firewall identity 和 listener digest。101 条两页分页、官方 ICMP 空 `Port`、协议端口边界、RequestId 唯一性和 SSH identity no-follow/观察前后漂移均已进入回归。transport v4=17 members，current runtime=9 files。定向 `23/23`、P0R `138/138`、exact-toolchain launcher `3/3`、全 V2 Ops `263/263`、Go helper、ESLint 与 Biome PASS；错误默认工具链运行不计资格。M0 exact CI binding `6/6` 锁定外层 launcher、14 项有序生产门禁和 self-building verifier。隔离候选 Foundation `643 total / 636 pass / 6 explicit skip / 1 expected branch-identity blocker` 保留为历史失败；精确合入后官方分支完整 `ci:production` 已从头 PASS，Foundation=`643 total / 637 PASS / 6 explicit skip`，M0=`12/12`。GitHub 四门、fresh rebind、外部清场和 corrected real-target recovery 仍待完成；GitHub 与生产未改变。
- fresh rebind request v4/result v3 已把历史 transport v1 三文件替代比较与当前 transport v4 九文件运行资格分离，并增加精确 8022 listener/unit absence 合同；九文件集包含 runner、atomic session helper、runtime capsule helper 和 route listener observer，缺任一摘要 fail closed。旧 request v3/result v2、transport v2/v3、七/八文件合同和所有 16-member package 只保留历史证据；当前实现已取得本地官方分支资格，但尚无远端或生产执行权。
...
- 当前有持续生产运行证明的来源仍是 Binance Futures、OKX Swap、Bybit Linear Perpetual 的公开目录、mark-price、Kline 和 WebSocket。Bitget、四 Venue listing lifecycle 与股票/RWA 合约已进入 Scope V2 能力登记、本地 Adapter、身份和生命周期合同；M1.1B0 R3 已取得腾讯 exact `LIVE_READ_ONLY` 15/15 来源一致性证明，M1.4B 又取得两次 14/14 route-eligible no-authority 有界运行与真实 Bitget/Bybit checkpoint，但尚无 Scope V2 持续 Collector Shadow、容量或生产运行 authority。Binance 股票产品已由官方资料确认。CoinGecko 免费 API、DefiLlama/链上和新闻社交现仅为待登记、权利/live/增量价值审查的补充来源，不具有运行或决策 authority。
...
npm run test:v2-m1-collector && npm run test:v2-m1-adaptive-collector
...
不把服务器密码、SSH 私钥或 secret 交给 Codex。数据库 migration、清库、volume 删除和 production authority 切换必须有独立任务与明确边界。P0R 腾讯 STS/MFA 仍是 `/dev/shm` 短期 secret 例外，不能通过 Git 通道运输；它只能经签发前已握手的 fixed local TTY bridge 进入远端 no-echo session。

腾讯 MFA 仍在 Microsoft Edge 的腾讯 API Explorer 由用户本人完成。P0R response 出现后只允许页面原生 Copy，不得读取 browser state、截图、OCR、AX、computer-use 或进入 OrcaTerm secret session。OrcaTerm 只保留给不含 secret 的紧急救援或只读诊断，并且不替代机器证据和独立生产身份校验。
...
工程描述：可运行但不完整
实战描述：不能支撑实战
V2：M0.5 已把 Market Mechanics、Microstructure、双向前兆、板块传播、补充来源、缓存和 Evidence Overlay 融入权威设计；M1.4C、M1.4D、M1.5C/M1.5D 本地包与 M2.1A 均通过各自本地出口，但真实 live、cohort/holdout、校准和 authority 仍未完成。P0R inner source `be87cf...` 的 GitHub 四门、fresh rebind 和 exact 16-member transport v3 已 PASS；OrcaTerm package transport 永久退役。B8 outer source `15d7cb...` 四门通过，首次 signed dispatch 在父目录 `0755` 上安全阻断，收紧为 `0700` 后 fresh dispatch `d5ea6e...` 已通过 exact target、outer cleanup 和生产零漂移验收。B8 只关闭 package staging；真实 STS、backup/retrieval/restore 与 fresh P0 仍待完成。M1 未完成，Detector=DRAFT、Candidate 禁发，无 Signal Grade/Strategy/Feasibility/持续 runtime/READY authority
A0：精确版本、零高危依赖、许可证、SBOM、固定 CI/Action/runner/base-image、双重 lint、Next build、原生材料烟测与 exact Node 22.23.1/npm 10.9.8 本地/远程完整 CI 已 PASS。host tar 方言、浅 Git 历史、Gitleaks 重复误报和既有 CodeQL 治理均已根因收口。最新 source parts `9ef63b85d1a76f3ad7ac + 815e081506c5dbc074a5` 的 A0 `30217335595` 已取得双独立 RootFS、确定性应用胶囊、精确镜像配置、fail-closed runtime smoke、三个隔离回滚场景和冻结 1,440 instrument 性能资源基线 PASS；event-loop p99=`64.750 ms`，固定门槛=`200 ms`。同源 Full Quality `30217335543` 与 Security `30217335622` 全部 PASS：Gitleaks finding=0、CodeQL untriaged=0、Trivy HIGH/CRITICAL=0。RootFS ownership/路径、证据 TOCTOU 和 event-loop 阻塞均已根因治理，没有减少样本、分母、工作量或门槛。A0 现在只缺 P0R 真实加密备份、精确版本取回和独立 PostgreSQL 16 恢复，因此总门禁仍未完成；允许无 authority 本地工程，不允许 M1.5C/M1.5D 真实 live 启动
本轮 B8 fixed-dispatch transport staging 已完成本地、远端门、生产 fail-closed 权限验证、0700 根治和 fresh exact target acceptance。其唯一生产变更是受权收紧 `p0r` 父目录权限并创建 exact run staging；业务服务、数据库、Redis、Worker、env、migration 与业务 authority 变更为 0。Runtime Control 保持 timer enabled/active，COS Object Lock COMPLIANCE 31 天保持启用
当前生产存储门禁：P0_BLOCKED_CAPACITY_AND_RECOVERY；B8 与 B9-R1 只保留历史证据；B9-R2 route authority producer 定向 `23/23`、P0R `138/138`、Go helper、ESLint、Biome 和唯一 V2 实施分支最终字节完整 CI 均 PASS，GitHub 四门和远端资格待完成；敏感页关闭、云 listener/规则/服务器残留清理及生产零漂移尚未复核；没有可用 STS、backup、restore 或 fresh P0
固定生产执行通道：PRODUCTION_FIXED_DISPATCH_TIMEOUT_LOCK_RECOVERY_PASS / B8_EXACT_TARGET_ACCEPTANCE_HISTORICAL_PASS。当前外部事务必须使用 lease-bound transaction v1、120 秒精确 route evidence、签发前双 TTY prearm 和 1200 秒心跳窗口；本轮 exact source 的远端资格仍待完成。旧 raw/tee/manual-compile、browser-state recovery、OrcaTerm secret entry、OrcaTerm file-manager package transport、default SSH port 22、单 TTY prearm、签发后第二次 SSH 和超长复合命令均禁止复用
```
...
- replacement source `be87cf...` 已通过 GitHub 四门、fresh production read-only rebind，并生成 exact 16-member transport v3；包 SHA-256=`44f5e34f...`，大小 9,176,819 bytes，不含 secret。
- OrcaTerm 文件管理器三次受控上传均失败，包含短路径同 SHA 对照；805 秒后服务器目标文件和 run staging 均不存在，生产应用、数据库、Redis、容器、env、migration、Feature Flag 和 authority 零变更。
- 该运输操作已在 recurrence registry 中永久退役。B8 outer source `15d7cb...` 四门通过；首次 signed dispatch 在祖先目录 `0755` 上 fail closed，精确收紧为 `0700` 后 fresh commit `d5ea6e...` 已通过 exact 16-member target、mode/owner/hash、outer cleanup、服务健康与生产零漂移验收。B8 已完成，真实 recovery 未开始，当前转入 B9。

### 2026-07-30 / P0R Runtime Namespace Root Remediation
- source `e83c1f...` 的 exact run 完成 STS 与 age identity 交接后，在 backup 前 fail closed；现场 A/B 证明容器内 `pg@8.16.3` 存在，而宿主跨 mount namespace 路径不可见。该 run 没有 backup、COS object、retrieval、restore 或业务 mutation。
- 临时 staging/evidence、secret、P0R process/container/volume、8022 listener 和腾讯 `/32` 规则均已清理并验证为 0；生产数据库、服务、仓库、env、migration 和 authority 未变。
- transport v3 现内置 checksum-bound Node/npm/pg runtime capsule并把 current-source rebind 扩为八文件；Runner plan v6、session v4、P0R 100/100、两次独立字节一致的真实隔离 npm 构建和完整 `ci:production` PASS。clean commit、GitHub 四门、fresh rebind、新 package 与真实恢复仍待完成。

### 2026-07-29 / P0R Fixed Local TTY Bridge After Post-Response Disclosure
- `e362...` 的本地 CI、GitHub 四门、fresh rebind 和新 staging 曾真实 PASS，但第三枚 STS response 在 browser-state recovery 中进入工具输出；credential 与该 run/plan/object key/bundle/staging 永久作废。
- credential 未进入服务器或 COS；生产 P0R files/processes/containers/volumes 均复核为 0，业务服务与 authority 零漂移。exact expiry=`2026-07-29T06:09:17Z`，必须独立证明到期且永不复用。
- 新 pre-armed local TTY bridge 固定 SSH identity/host-key/proxy/两个远端命令和 Node `v22.23.1`，只在 echo-disabled READY 后接受 API 原生 Copy，先清 clipboard 再 handoff，并从固定 Keychain 内部取 age identity。Expect 运行材料根治后的 source `0e035784988261926eb9656d25050180c87b0d1e` 已取得 A0、Signed Dispatch、Full Quality 和 Security 四门全 PASS。
- 该 source 的 fresh rebind `p0r-rebind-preflight-20260729t150004z-c85ec1a4` 因外层 runtime 5400 秒与包级 90 秒不一致，在 `PACKAGE_BINDING` 阶段、任何生产动作前被拒绝；staging 自动清理。fresh 只读复核确认 production HEAD/clean worktree/11 containers 及 P0R files/containers/volumes 零漂移。request schema v3 现显式绑定 90 秒，固定通道签名前/outbox/生产代理和 runner 四层校验；唯一高层 release 入口从 canonical request 派生重复字段并拒绝 operator-supplied binding 选项。定向 29/29、P0R 88/88、最小 PATH dispatch 回归和显式 Node/npm/Go 的完整 `ci:production` 已 PASS；新 clean commit 和远端重验仍待完成。

## 15. 当前风险

### P0

- M1.6-P0 存储准入仍为 BLOCKED；Object Lock、age、atomic session、qualified inner transport 和已验收的 B8 target staging 都不是恢复 PASS。B8 exact target acceptance 已 PASS；当前仍缺 B9 usable STS、加密备份、exact retrieval、独立 PG16 restore、cleanup、fresh topology 和 exact-release calibration，P1 严禁启动。
...
- C1 历史两轮捕获起点与最新四轮 evidence refresh 都已通过完整性门禁；最新三家连续跨度仍只有约 16.6 分钟，且 Scope V2 资产域重放仍为 `PARTIAL`。它不能替代 Bitget、持续采集、24h SLO、历史 instrument source、历史权利、官方 mapping 完整性、真实 cohort 或 Detector，旧未绑定 release 的诊断根不得并入正式链。
- M3.0-M3.3 只验证 V1 三 Venue加密 fixture；M3.1A-D-R0 只验证 Scope V2 四轨 research contract 能拒绝串轨、缺证、未来读和伪成本，不能证明真实 Detector、cohort/holdout、校准或 Strategy 有效。M0.5 也只冻结设计，订单墙、主动流、板块传播和 CoinGecko 等补充来源尚无真实前向证据或消融，任何“主力/控盘分”、固定美元阈值或个案规律都无权进入 Signal。M3.4-R0 也只证明前置门禁，不证明 Feasibility。持续 Adapter、listing checkpoint、Deep Validation、真实执行成本、风险和 runtime 仍缺失，任何 V2 Signal Grade、Strategy authority 或 READY 声明均为 P1 风险。
...
4. Candidate/Evidence/Setup/Action/User Fit 是否越层；静态订单墙、固定美元大单阈值、单 Venue 现象、AI 或自定义指标是否越权。
...
7. 前端是否只读 Decision Snapshot/Evidence Overlay；灵敏度是否只改变提醒和展示。
...
2026-08-01 的 COS CAM action 修复只保留 R1 历史证据。2026-08-02 两个新窗口均只预建 credential TTY 并在 540 秒 clipboard deadline 超时；后到 native Copy 已作废且从未进入 Bridge、`/dev/shm` 或服务器。随后双端点 egress 一致漂移为 `156.248.15.36`，旧 exact `/32` 下的 8022 已不可达。当前仅证明本机 Bridge=0、clipboard 已覆盖；云 listener、旧规则、服务器 residue 和生产零漂移仍是 `UNVERIFIED`，不得沿用旧清理 PASS。

本地整改保留 plan v4、credential v3 和官方 CAM action，并把 bridge/session 升为 v5：两条 strict no-echo TTY 在 operator READY 前全部建立，1200 秒窗口持续检查双会话，签发后不再联网。transaction v1 以 7200 秒 exact lease 绑定全资源；Bridge child 按唯一状态机终结，两个输出流累计最多 65,536 bytes、状态最多 64 个；控制文件采用 current-user/mode-600、`O_NOFOLLOW`、1 MiB 有界稳定读取。fresh rebind v4/v3 明确要求 TCP 8022 listener=0 和 exact transient unit=`not-found/inactive`。继续审计新增 route target v1、Tencent firewall capture v1、strict listener observer、authoritative producer 和 route evidence v2；route target、lease、firewall capture 与 listener observation 必须 canonical，provisioning plan 保持 exact SHA-256 绑定，transaction 从完整 provider/server 观察重算 identity/digest。transport v4=17 members、current runtime=9 files；定向 `23/23`、P0R `138/138`、Go helper、ESLint、Biome 和官方主分支最终字节完整 CI 均 PASS。候选分支身份门失败保留为历史审计证据。当前正向顺序是 `权威证据同步并在干净最终提交上复跑 full CI -> GitHub 四门 -> fresh signed read-only rebind -> 本人仅确认旧敏感页关闭 -> 代理自动删除旧云规则并完成 external zero drift -> 新 route target/lease/run/plan/transport-v4/staging -> API Explorer 预备 -> listener + exact /32 -> 自动保存完整 firewall capture + listener observation -> route evidence v2 -> 双 TTY PREARMED -> 一次 fresh STS native Copy -> backup/exact retrieval/isolated restore -> fresh close evidence -> lease-bound cleanup/zero drift -> fresh topology/calibration/P0`。外部全清场前不得创建新 route、STS、数据库或 COS 动作；P0R 与 fresh P0 未 PASS 前，P1、M2 runtime、页面交易计划和任何 READY authority 继续关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
