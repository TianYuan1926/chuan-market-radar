...
- 2026-07-17 `WP-G0.2-LEGACY-PENDING-DRAIN-PRODUCTION` 已完成本地生产执行包准备，但尚未生成 commit-bound request、未上传、未连接或修改生产。目标代码新增 `CANDIDATE_EPISODE_DRAIN_ONLY=true` 硬阻断 source enqueue，同时保留受控 Candidate consumer；执行器固定为 scanner 先停且 Redis `scan:lock:*` absent -> 精确 DB preflight -> 临时 Web/worker -> control epoch 4->5 -> 只消费既有 pending -> worker stop -> epoch 5->6 refreeze -> 原 env/Git/Web/scanner 镜像和 scanner 新一次 completedAt/ready/fresh 全恢复。任何 retry_wait/quarantine/resolution/outbox 总数变化/partial/timeout 都自动冻结和恢复基线，mutation 前失败单独写 `SAFE_STOP_PRE_MUTATION`，mutation 后回滚只能写 `ROLLBACK_PASS`。本地治理/执行器 16/16、旧 drain 合同 12/12、drain-only composition 7/7、隔离 PostgreSQL 16 成功排空和失败保留 pending 两条路径均 PASS，runner artifact=`b3f91b6278c3a84bba023e9c3b6493faeb275040d1d80880f0ce735d32b6419b`；typecheck、零 warning lint、完整 market 1027/0/7、workers 23/23、historical 4/4、build、Golden 16/16、三项安全门禁和自治 31/31 均 PASS。首次并行 typecheck 曾因 `next build` 同时重建 `.next/types` 出现 TS6053，已在 build 完成后串行复核 PASS，不包装并行竞争为代码通过。生产仍是 migration 10、legacy/frozen epoch4、outbox 5,914、pending/unresolved 2,957、Candidate worker absent，G0 仍剩 8 个主包，系统仍只能写 `R1 / 可运行但不完整 / 不能支撑实战`。
