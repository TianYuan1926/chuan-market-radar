...
- M1.6 v1 日分区迁移 checksum 保持不可变；additive v2 六小时分区、无扩容 59%/67% 模型、Object Lock `COMPLIANCE` 31 天、Keychain age 身份和 private `/dev/shm` runner boundary 均已形成。B8 target 与 B9 COS CAM action 修复只保留历史证据。2026-08-02 两个 bridge window 都在第一条 TTY READY 后 540 秒超时，后到 response 作废且未进入 Bridge 或服务器；随后双端点 egress 一致变为 `156.248.15.36`，strict 8022 不可达。当前只证明本机 Bridge=0 与 clipboard 已覆盖；云 listener、旧 `/32`、服务器 residue 和生产零漂移未复核。bridge/session v5 已在签发前双 TTY prearm，1200 秒窗口检查双会话并输出 no-secret heartbeat，签发后无 reconnect；transaction v1 以 7200 秒 exact lease 绑定全资源，execute 消费 120 秒内精确 route evidence，detached authorize CLI 已删除。Bridge 现强制唯一终态顺序、65,536-byte/64-state 总上限；plan/lease/route/cleanup 采用 canonical mode-600、`O_NOFOLLOW`、1 MiB 上限和读取前后稳定 identity。fresh rebind v4/v3 进一步明确要求 TCP 8022 listener=0、`market-radar-p0r-8022.service`=`not-found/inactive`，不再用全量 listener 哈希冒充零残留。P0R `125/125`、Go helper 与当前最终字节完整 CI PASS；CI 包含 recurrence `11/11`、dispatch `25/25`、Foundation `631 PASS / 6 explicit skips`、V2 Ops `247/247`、Next build、Golden `16/16` 和 security。外部清场、clean commit、GitHub 四门、fresh rebind、新 lease/run/v4 plan/transport/staging 和 corrected real-target recovery 仍待完成。
- fresh rebind request v4/result v3 已把历史 transport v1 三文件替代比较与当前 transport v3 八文件运行资格分离，并增加精确 8022 listener/unit absence 合同；八文件集包含 runner、atomic session helper 和 runtime capsule helper，缺任一摘要 fail closed。旧 request v3/result v2、七文件和 transport-v2 合同只保留历史证据；当前本地修复尚无远端或生产执行权。
...
当前生产存储门禁：P0_BLOCKED_CAPACITY_AND_RECOVERY；B8 与 B9-R1 只保留历史证据；B9-R2 本地 rebind `13/13`、P0R `125/125`、Go helper 与当前最终字节完整 CI PASS；敏感页关闭、云 listener/规则/服务器残留清理及生产零漂移尚未复核；没有可用 STS、backup、restore 或 fresh P0
...
本地整改保留 plan v4、credential v3 和官方 CAM action，并把 bridge/session 升为 v5：两条 strict no-echo TTY 在 operator READY 前全部建立，1200 秒窗口持续检查双会话，签发后不再联网。transaction v1 以 7200 秒 exact lease 绑定全资源，execute 消费 120 秒内的 mode-600 精确 route evidence；detached authorize CLI 已删除。Bridge child 必须按唯一状态机终结，两个输出流累计最多 65,536 bytes、状态最多 64 个；控制文件必须 canonical/current-user/mode-600，`O_NOFOLLOW` 有界读取最多 1 MiB，并在读取前后保持同一 identity。fresh rebind request v4/result v3 明确要求 TCP 8022 listener=0 和 exact transient unit=`not-found/inactive`。rebind `13/13`、P0R `125/125`、Go helper 与当前最终字节完整 CI PASS。当前正向提速顺序是 `clean exact commit + GitHub 四门可在不触碰敏感 Edge 页时先行 -> fresh signed read-only rebind 证明服务器 8022/unit/P0R residue 为零 -> 本人仅确认旧敏感页关闭 -> 自动删除旧云规则并完成外部零漂移 -> 新 lease/run/plan/package/staging -> API Explorer 预备 -> listener + exact /32 + route evidence -> 双 TTY PREARMED -> 一次 fresh STS native Copy -> backup/exact retrieval/isolated restore -> fresh close evidence -> lease-bound cleanup/zero drift -> fresh topology/calibration/P0`。外部全清场前不得创建新 route、STS、数据库或 COS 动作；P0R 与 fresh P0 未 PASS 前，P1、M2 runtime、页面交易计划和任何 READY authority 继续关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
