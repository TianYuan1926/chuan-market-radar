...
- M1.6 v1 日分区迁移 checksum 保持不可变；additive v2 六小时分区、无扩容 59%/67% 模型、Object Lock `COMPLIANCE` 31 天、Keychain age 身份和 private `/dev/shm` runner boundary 均已形成。B8 target、B9 COS CAM action 和 route evidence v1 资格只保留历史证据。2026-08-02 两个 bridge window 都在第一条 TTY READY 后 540 秒超时，后到 response 作废且未进入 Bridge 或服务器；随后双端点 egress 一致变为 `156.248.15.36`，strict 8022 不可达。当前只证明本机 Bridge=0 与 clipboard 已覆盖；云 listener、旧 `/32`、服务器 residue 和生产零漂移未复核。bridge/session v5、transaction v1 和 rebind v4/v3 已关闭双 TTY、事务状态机、稳定控制文件读取和显式 8022/unit absence 缺口。继续审计证明旧 route evidence 只有 validator、没有 producer，测试曾以任意 final hash 冒充 firewall identity；隔离分支现新增 route target v1、Tencent firewall capture v1、strict listener observer、authoritative producer 与 route evidence v2。route target、lease、firewall capture 和 listener observation 必须 canonical，既有 provisioning plan 继续由稳定安全读取及精确 SHA-256 绑定；transaction 重算 provider firewall identity 和 listener digest。101 条两页分页、官方 ICMP 空 `Port`、协议端口边界、RequestId 唯一性和 SSH identity no-follow/观察前后漂移均已进入回归。transport v4=17 members，current runtime=9 files。定向 `23/23`、P0R `138/138`、exact-toolchain launcher `3/3`、全 V2 Ops `263/263`、Go helper、ESLint 与 Biome PASS；错误默认工具链运行不计资格。M0 exact CI binding `6/6` 锁定外层 launcher、14 项有序生产门禁和 self-building verifier；隔离候选 CI 为 Foundation `643 total / 636 pass / 6 explicit skip / 1 expected branch-identity blocker`，必须合入主线后重跑最终字节完整 CI。clean commit、GitHub 四门、fresh rebind、外部清场和 corrected real-target recovery 仍待完成。已资格化主线 `6c7f382...` 与生产未改变。
- fresh rebind request v4/result v3 已把历史 transport v1 三文件替代比较与当前 transport v4 九文件运行资格分离，并增加精确 8022 listener/unit absence 合同；九文件集包含 runner、atomic session helper、runtime capsule helper 和 route listener observer，缺任一摘要 fail closed。旧 request v3/result v2、transport v2/v3、七/八文件合同和所有 16-member package 只保留历史证据；当前隔离修复尚无远端或生产执行权。
...
当前生产存储门禁：P0_BLOCKED_CAPACITY_AND_RECOVERY；B8 与 B9-R1 只保留历史证据；B9-R2 route authority producer 隔离分支定向 `23/23`、P0R `138/138`、Go helper、ESLint 与 Biome PASS，候选 CI 在唯一 V2 实施分支身份门安全阻断，主线最终字节 CI 和远端资格待完成；敏感页关闭、云 listener/规则/服务器残留清理及生产零漂移尚未复核；没有可用 STS、backup、restore 或 fresh P0
固定生产执行通道：PRODUCTION_FIXED_DISPATCH_TIMEOUT_LOCK_RECOVERY_PASS / B8_EXACT_TARGET_ACCEPTANCE_HISTORICAL_PASS。当前外部事务必须使用 lease-bound transaction v1、120 秒精确 route evidence、签发前双 TTY prearm 和 1200 秒心跳窗口；clean exact source 和远端资格仍待完成。旧 raw/tee/manual-compile、browser-state recovery、OrcaTerm secret entry、OrcaTerm file-manager package transport、default SSH port 22、单 TTY prearm、签发后第二次 SSH 和超长复合命令均禁止复用
```
