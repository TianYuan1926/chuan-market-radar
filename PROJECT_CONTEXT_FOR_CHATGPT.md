...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.82。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.87。
...
M2.1_DRAFT_REPLAY_KERNELS_LOCAL_PASS / M2.3A_R2_LIVE_EVIDENCE_PERSISTENCE_RUNTIME_JOIN_LOCAL_AND_REMOTE_QUALITY_PASS_LIVE_ZERO_OF_31_NO_CANDIDATE_OR_PRODUCTION_AUTHORITY_COHORT_HOLDOUT_PENDING
...
- M1.6 v1 日分区迁移 checksum 保持不可变；additive v2 六小时分区、无扩容 59%/67% 模型、Object Lock `COMPLIANCE` 31 天、Keychain age 身份、atomic remote session 和 private `/dev/shm` runner boundary 均已形成。B8 accepted target 上的 B9 曾完成 STS/age handoff 并启动 Runner，但因旧 policy 错把 REST 操作名当成 CAM action，在数据库读取前被 COS Object Lock preflight 阻断；没有 backup、COS object、retrieval 或 restore。plan/credential/bridge 已升至 v4/v3/v4并统一官方 `cos:GetBucketObjectLock`，旧 authority 自动失效，诊断仅允许脱敏 `p0r_cos_*` 阶段码。随后 B9-R1 已在冻结 source `6a70b8d3a964dd109d5051ba731813c4bccda62d` 上完成 GitHub 四门、fresh signed read-only rebind、新 v4 run/plan/transport、无凭证 fixed-dispatch staging 和独立 target acceptance；这只恢复下一次执行资格，真实 corrected recovery 仍未执行。8022 listener/云规则、secret、process、container、volume 已清零，生产 HEAD/worktree、11 个容器和 Web/PostgreSQL/Redis 保持零漂移。
- B9-R1 fresh signed read-only rebind 与独立 target acceptance 已绑定冻结 source、完整 current-source 文件集和 v4 plan/transport；缺任一摘要、目标文件、owner/mode 或 no-secret 边界均 fail closed。旧七文件、transport-v2/v3 和失败 run 只保留历史证据；当前资格不等于 recovery 已执行，也不授予数据库写入、migration、服务或业务 authority。
...
- M2.1 已建立三个 Pre-Move 与两个 Breakout/Retest 独立 DRAFT 纯回放内核。M2.1A 又建立八族各自 LONG/SHORT/UNKNOWN 共 24 个 DRAFT/UNCALIBRATED 假设、三 Outcome、point-in-time 板块传播、逐 family-direction 的 Venue/regime/liquidity 分层，以及 matched control、消融、sealed holdout、forward Shadow、rights 和独立审计 Gate；定向 13/13 PASS。M2.3A R0/R1 已建立四 Venue十四类事件研究 Bundle、三时间与 exact upstream coverage 推导；R2 source `d5d70bc50484db570748a7744319e2b35e0e8426` 进一步让 M1.5C 每轮保存完整 listing refresh batch，由独立 verifier 对 31 周期 280 个文件、initial/prior checkpoint 连续性及首尾 batch hash 验真，并让 exact runtime package 从 cycle 30/31 source 生成、回读和重建 M2 五件套。相邻 148/148、完整 candidate CI 与 GitHub 四门 PASS。所有 Candidate、方向、概率、Grade、Strategy、READY、production runtime 权限仍为 false；真实运行仍为 0/31，cohort/holdout/calibration、Shadow 与独立审计缺失，所以 M2.3A 主步骤仍未完成。
