...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.94。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.99。
...
- M1.6 v1 日分区迁移 checksum 保持不可变；additive v2 六小时分区、无扩容 59%/67% 模型、Object Lock `COMPLIANCE` 31 天、Keychain age 身份和 private `/dev/shm` runner boundary 均已形成。`V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY` source `dd67d819...` 的旧四门、过期批准、pre-mutation health-envelope 失败和 failed-attempt-only 零漂移只保留历史证据；真实顶层 `health` 修复已形成 clean commit `51886b038b6dfa5e1bf0169abc518810de7eb2d5`。authority-sync source parts `d1c3d987cf55105e94e8 + 15992dbb989ea1045629` 远端只有 Signed Dispatch `30739919674`、Full Quality `30739919649`、A0 `30739919664` PASS，Independent Security `30739919662` 因历史 `approval-request.json:1` 的固定 X25519 公钥指纹使用 secret-shaped 字段名而 FAIL；它不是 credential，但不能绕过 scanner。永久修复使用 `evidenceRecipientFingerprintSha256`、拒绝旧字段，并在 signed dispatch 发布前阻断歧义高熵非 public `*KeySha256`；历史只登记 exact finding。clean implementation commit parts `8742828086041da1db51 + a5abeeef01ba7656e5fd` 已形成，recurrence `11/11`、dispatch `38/38`、gateway `8/8` 合计 `57/57` PASS；最终 authority-sync 字节完整 exact-toolchain CI 也已通过 Foundation `638 PASS / 6 skip`、Ops `275/275`、M0 `12/12`、Next build、Golden `16/16` 与 security。authority-sync commit/push、replacement exact source 同源四门、新 package、新批准、bootstrap、自动取回、fresh rebind 和真实 P0R recovery 尚未完成；生产未改变。
...
两个 gateway exact dispatch 均已 claim、失败且永久不可复用。首次 source `dd67d819...` 因 mock 与 runner 错读生产顶层 `health` 在 preflight 失败；第二次 source `d9a1e5fedf481f00e3cc88705c7e2b884add3ebe` 虽已通过 Signed Dispatch `30743899298`、Full Quality `30743899329`、A0 `30743899304`、Independent Security `30743899295` 同源四门，仍在任何 Caddy mutation 前失败。生产等价 A/B 直接复现 current Caddy image 在 `cap-drop ALL` 下 RC 255，以及 runner 绕过 root-owned Compose identity wrapper 后缺 `POSTGRES_USER` 的 RC 1；相同 isolation 仅加 `NET_BIND_SERVICE` 后 Caddy validate PASS，exact wrapper 带 gateway override 后 config PASS。两次 fresh read-only 复核都证明 production HEAD `cec0b657...`、clean worktree、11-container identity、ready/fresh health、404 route 与所有非目标面零漂移；第二次唯一空 outbound residue 已按批准清理。两个旧 approval 均已过期。
该问题以 occurrence 2 登记为 `REC-2026-08-02-PRODUCTION-GATEWAY-PREFLIGHT-EQUIVALENCE / REMEDIATION_IN_PROGRESS`，generic Caddy release STOP-THE-LINE。local remediation commit `bb9abe5cf0404ca7ce9af55f6ed99a6e3f646d76` 使用 request v3 绑定 root-owned wrapper/override exact hash、mode 和 owner；Caddy validation 保留 network-none/read-only/no-new-privileges/cap-drop-all 并只恢复 `NET_BIND_SERVICE`；Compose config/deploy/rollback 全部使用 exact wrapper；command failures 形成稳定脱敏 digest；runner 只清理自己创建的 gateway/empty-outbox state。checksum-bound recurrence registry 和 gate 同时进入 deterministic bundle，并在构包及生产 gateway state 创建前各验证一次。gateway `12/12`、recurrence `11/11`、dispatch `38/38`、lint、tracked-source secret scan 和 diff check 已 PASS；完整 exact-toolchain CI 尚未运行，因此当前只到本地定向修复检查点。本机无 Docker/Caddy CLI，但生产 current-image 和 exact wrapper 的无 mutation component validation 已 PASS，仍不能替代完整新包 real-target acceptance。固定顺序为 `权威同步 -> 完整 CI -> final exact source commit/push -> 同源四门从零 PASS -> fresh production baseline -> 双构建 deterministic package -> 新 exact approval -> registered remediation execution/自动 rollback -> 自动取回/解密/验签 -> fresh rebind -> 新 P0R identity/package -> backup/exact retrieval/isolated restore/cleanup -> fresh health/topology/calibration/P0`。生产未改变；gateway real-target acceptance、P0R 与 fresh P0 未 PASS 前，P1、M2 runtime、页面交易计划和任何 READY authority 继续关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
