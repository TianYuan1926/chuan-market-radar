...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.68。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.74。
...
M1.6P0R_E362_REMOTE_GATES_REBIND_AND_STAGING_HISTORICAL_PASS_RUN_INVALIDATED / THIRD_STS_POST_RESPONSE_DISCLOSED_NEVER_USED_EXPIRED_FORBIDDEN_REUSE_DUAL_CLOCK_PROOF_PASS / SOURCE_0E035_REMOTE_FOUR_GATES_PASS / REBIND_C85EC1A4_BLOCKED_PACKAGE_BINDING_BEFORE_MUTATION_STAGING_CLEANED / PROD_HEAD_WORKTREE_11_CONTAINERS_AND_P0R_FILES_CONTAINERS_VOLUMES_ZERO_DRIFT / REQUEST_V3_CROSS_LAYER_RUNTIME_BINDING_ROOT_FIX_FULL_LOCAL_CI_PASS_NEW_EXACT_SOURCE_PENDING / LEGACY_BED938_STAGING_REJECTED
...
- M1.6 v1 日分区迁移 checksum 保持不可变；additive v2 六小时分区、无扩容 59%/67% 模型、Object Lock `COMPLIANCE` 31 天、Keychain age 身份、atomic remote session、private `/dev/shm` runner boundary 和 fresh rebind request v3 当前七文件合同均已形成。source `e3626387ee8d57ef8e4f9c11c2e098b781ac6fbe` 的本地 CI、GitHub 四门、fresh rebind 和新 staging 曾真实 PASS，但第三枚 STS response 在 post-response browser-state recovery 中进入工具输出，因此该 credential 和绑定 run/plan/object key/bundle/staging 全部永久失去执行权。它从未进入服务器、从未编译或访问 COS；生产四条独立只读检查证明 P0R files/processes/containers/volumes 均为 0，应用、数据库、Redis、仓库、env、migration、authority 和 11 个既有容器未改变。第三枚 exact expiry=`2026-07-29T06:09:17Z`；本机 UTC `2026-07-29T10:32:53Z` 与腾讯 STS HTTPS Date `2026-07-29T10:35:42Z` 已双重证明过期，状态为 `EXPIRED_FORBIDDEN_REUSE`。新 `m1-production-storage-p0r-local-tty-bridge.exp` 在签发前固定 SSH/host-key/proxy/run-derived command，等待远端 echo-disabled READY，只消费 API Explorer 原生 Copy，先清 clipboard 再 handoff，并从固定 Keychain 内部注入 age identity；response 后 screenshot、OCR、AX、browser state、computer-use 和 OrcaTerm secret entry 全部禁止。桥与 session 伪 TTY 10/10、当前 P0R 88/88、Go helper、recurrence 10/10、dispatch 24/24、Node `v22.23.1` / npm `10.9.8` 完整 `ci:production`、Next build、Golden 16/16 和 security 均已 PASS。replacement source `38b49f43e0dc15514d1da9c1169d2fed233d8f5a` 的 A0、Signed Dispatch 和 Independent Security PASS，但 Full Quality `30448234790` 因 Ubuntu runner 缺少 `/usr/bin/expect` 而 FAIL，因此只能记 3/4 门通过且无 rebind/STS/生产执行权。当前根治已把 Noble `expect/tcl-expect 5.45.4-3` 精确安装纳入 Full Quality，并由材料门禁和防回退测试锁定；18 项定向测试与完整本地 `ci:production` PASS，新 clean source 的四门、fresh rebind、新 run/plan/bundle 和真实 recovery 仍待完成。数据库 capture、backup、COS object、exact retrieval、isolated PG16 restore、fresh topology 和 fresh P0 均未发生，P1 保持关闭。
- fresh rebind request/result schema v2 已在本地把历史 transport v1 三文件替代比较与当前 transport v2 七文件运行资格分离；七文件集包含 runner 和 atomic session helper，缺任一摘要 fail closed。该合同尚未形成 clean commit、远端四门或生产 rebind 证据。
...
工程描述：可运行但不完整
实战描述：不能支撑实战
V2：M0.5 已把 Market Mechanics、Microstructure、双向前兆、板块传播、补充来源、缓存和 Evidence Overlay 融入权威设计；M1.4C、M1.4D、M1.5C/M1.5D 本地包与 M2.1A 均通过各自本地出口，但真实 live、cohort/holdout、校准和 authority 仍未完成。P0R source `e362...` 曾完成本地 CI、GitHub 四门、fresh rebind 和新 staging，但第三枚 STS response 在 browser-state recovery 中进入工具输出，故 credential 与 run/plan/object key/bundle/staging 永久失去执行权。该 STS 未进入服务器或 COS；生产 P0R files/processes/containers/volumes 均复核为 0，双时钟已证明其过期且永久禁用。新 pre-armed local TTY bridge、echo-before-ready session 和 request v3 高层 release 根治已通过伪 TTY 10/10、P0R 88/88、recurrence 10/10、dispatch 24/24 与完整 `ci:production`；新 clean exact source、四门、fresh rebind、新 execution identity 和真实恢复仍待完成。M1 未完成，Detector=DRAFT、Candidate 禁发，无 Signal Grade/Strategy/Feasibility/持续 runtime/READY authority
...
本轮 fixed dispatch 仅更新生产控制面 agent、Git SSH wrapper 与 README；业务服务、数据库、Redis、Worker、env、migration 与业务 authority 变更为 0。旧 P0R dispatch 已确认为 `FAIL_DISPATCH_NOT_REUSABLE / dispatch_not_current`，无 claim、解包或 Runner；Runtime Control timer enabled/active，轮询 IDLE；COS Object Lock COMPLIANCE 31 天保持启用
当前生产存储门禁：P0_BLOCKED_CAPACITY_AND_RECOVERY；P0R_OBJECT_LOCK_31D_AGE_VAULT_PASS_E362_GATES_REBIND_STAGING_HISTORICAL_PASS_RUN_INVALIDATED_THIRD_STS_NEVER_USED_EXPIRED_FORBIDDEN_REUSE_DUAL_CLOCK_PROOF_PASS_PROD_FILES_PROCESSES_CONTAINERS_VOLUMES_ZERO_LOCAL_FIXED_TTY_BRIDGE_FULL_LOCAL_QUALIFICATION_PASS_NEW_EXACT_SOURCE_REMOTE_AND_REAL_RECOVERY_PENDING；没有可用 STS、backup、restore 或 fresh P0
固定生产执行通道：PRODUCTION_FIXED_DISPATCH_TIMEOUT_LOCK_RECOVERY_PASS；机器复发门禁 4 项 CLOSED、1 项 P0R secret transport REMEDIATION_IN_PROGRESS，旧 raw/tee/manual-compile、browser-state recovery、OrcaTerm secret entry、approved_orcaterm_bundle_upload 和超长复合命令均禁止复用
```
...
- 第二枚 exact STS 最终进入真实 receiver，但因手工步骤超过签发后 5 分钟被真实时钟编译器正确 BLOCKED；未使用 `--now`，未调用 COS、数据库或 Runner。
- raw path 删除后仍发现 PID `1175383` 的 `tee` 持有 unlink 文件；核对为本 run 后已精确终止，生产 P0R 文件与进程均复核为 0。
- 裸 `tee`、raw response 落盘、manual compile、caller-supplied clock override、AX response reconstruction 与 Compose env 重插值永久退役；本地 no-echo atomic session 已通过 P0R 81/81、Go helper、recurrence 10/10 和 dispatch 24/24，fresh rebind request v3 另行绑定当前七文件运行集。
- 当前完整本地 CI 已 PASS；仍缺新 clean commit、GitHub 四门、fresh production read-only rebind、新 plan/bundle、真实 target acceptance 和 recovery；旧 `bd20...` 只保留历史证据。
...
- 旧代理的无界 Git child 在 systemd 超时后遗留空锁并触发 4,526 次重复阻断；90 秒 Git 硬上限、owner identity 与 stale-lock recovery 已通过 24/24、本地完整 CI、腾讯 Linux smoke 和生产验收。
- 生产应用 HEAD/clean worktree、11 容器、数据库、Redis、六 Worker 与 health 零漂移；下一动作是生成并执行 fresh signed read-only P0R rebind。

## 15. 当前风险

### P0

- M1.6-P0 存储准入仍为 BLOCKED；Object Lock、age 和本地 atomic session 不是恢复 PASS，仍缺 replacement exact source 资格、fresh rebind、new plan/bundle、usable STS、加密备份、exact retrieval、独立 PG16 restore、cleanup、fresh topology 和 exact-release calibration，P1 严禁启动。
...
## 18. 当前执行入口与关键外部门
```text
V2-M1.6-P0R-B4-PREARMED-LOCAL-TTY-BRIDGE-RECOVERY
```
B1-B3 已关闭 V1 M1.5-B1；P0 已执行并因容量与 recovery evidence BLOCKED。Object Lock 31 天和 age Keychain 身份仍有效。`e362...` 的四门、fresh rebind 和 staging 只保留历史资格；第三枚 STS disclosure 已使其 run/plan/object key/bundle/staging 失去执行权。该 credential 未进入生产，四类 P0R 残留均为 0；exact expiry `2026-07-29T06:09:17Z` 尚须双时钟证明超过。当前先完成 fixed local TTY bridge 的完整测试、文档/追踪同步、clean exact commit、GitHub 四门、fresh rebind 和全新 execution identity。只有 bridge 在签发前取得 echo-disabled READY，用户才可完成 MFA 和 API Explorer 原生 Copy；之后 bridge 自动完成即时编译、Keychain age handoff和 Runner。response 后禁止 browser state/AX/OCR/screenshot/computer-use/OrcaTerm secret entry。只有 backup、exact retrieval、isolated PG16 restore、cleanup、零漂移、fresh topology、校准和 fresh P0 全部 PASS 才能继续 P1。禁止修改 exact policy、Region、duration、source IP、resource 或 action，禁止签名通道运输 P0R secret。Scope V2 的 C1、M1.4B/M1.4C、M2.1A 和 A0 子门证据仍有效但无 runtime authority；A0 总门禁唯一剩余控制仍是 P0R 真实恢复。外部门 B0.2-B 未解决前 historical bulk、Detector lifecycle 和 runtime 一律关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
