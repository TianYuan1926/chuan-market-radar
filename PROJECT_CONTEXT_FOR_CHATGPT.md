...
- M1.6 v1 日分区迁移 checksum 保持不可变；additive v2 六小时分区、无扩容 59%/67% 模型、Object Lock `COMPLIANCE` 31 天、Keychain age 身份和 private `/dev/shm` runner boundary 均已形成。`V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY` source `dd67d819...` 已真实取得四个 GitHub PASS 门和 exact 生产批准；signed dispatch `a52aa6a...` 在初始 health preflight 因 runner/fixture 错把生产顶层 `{ok,health}` 合同读取为 `data.health` 而 `FAIL_DISPATCH_NOT_REUSABLE`。该尝试在 gateway/Caddy mutation 前停止；fresh read-only 证据证明 health、scan、persistence 均 ready，exact 11-container baseline 不变，目标 route 仍 404，gateway/outbound/staging/global lease 均 absent，仓库、数据库、Redis、Worker、env、migration、Feature Flag 和流量零漂移。旧批准已过期且旧 dispatch 不可复用。当前 parser、真实 fixture 和旧 nested-envelope 零 mutation 红例已修复并形成 clean commit `51886b038b6dfa5e1bf0169abc518810de7eb2d5`；authority-sync 最终字节的 gateway `8/8`、recurrence `11/11` 与完整 `ci:production` PASS。推送、authority-sync exact source 四门、新 package、新批准、bootstrap、自动取回、fresh rebind 和真实 P0R recovery 尚未完成。
...
source `dd67d819...` 已从零取得 Signed Dispatch `30734390429`、Full Quality `30734390438`、A0 `30734390439`、Independent Security `30734390634` 四门，并在旧 exact 批准窗口内发布；目标以 stderr digest `sha256:d41cd1e0993f4f20b1024ac7e1c9ba17ea70cd9a1f0f5ec61fd96523bde9701f` 唯一解码为 `evidence_gateway_health_not_ready`。真实 API 是顶层 `health`，旧 parser 与 fixture 同时错成 `data.health`，测试因此没有覆盖真实生产合同。真实 envelope 绿例与 obsolete nested envelope pre-mutation 红例已进入 clean remediation `51886b0...`；authority-sync 最终字节的 gateway `8/8`、recurrence `11/11`、dispatch `37/37`、Foundation `638 PASS / 6 skip`、Ops `275/275`、M0 `12/12`、Next build、Golden `16/16` 与 security PASS。本机没有 Docker/Caddy CLI，真实 syntax/runtime validation 仍必须在 production mutation 前使用当前生产 Caddy image、network-none/read-only 隔离执行。固定顺序调整为 `push -> authority-sync exact source 四门 -> fresh production baseline -> 新 deterministic package -> 新 exact approval -> current-image 隔离 validate -> Caddy-only deploy/自动 baseline rollback -> 自动取回/解密/验签 -> fresh rebind -> 新 P0R identity/package -> backup/exact retrieval/isolated restore/cleanup -> fresh health/topology/calibration/P0`。P0R 与 fresh P0 未 PASS 前，P1、M2 runtime、页面交易计划和任何 READY authority 继续关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
