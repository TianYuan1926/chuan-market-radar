...
- 2026-07-17 `WP-G0.2-CANONICAL-ROLLBACK-SAFETY-PRODUCTION-ADD-SCHEMA` 已达到本地 `ready_for_gate`，生产未连接、未执行。新执行包只接受精确 migration 001-009 applied ledger 和唯一待执行 010；使用非特权 `market_radar_migration_login` 显式切换到 `candidate_migration_role`，在 advisory lock、`REPEATABLE READ`、5 秒 lock timeout、30 秒 statement timeout 的单事务内执行并写 ledger。合同/边界/确定性 Bundle 单测 10/10、隔离 PostgreSQL 16 精确 1-9/故障事务回滚/只应用 010 到 10/least privilege/业务数据不变、Autonomy 31/31、typecheck、零警告 lint、market 1026/0/7、workers 23/23、historical 4/4、build、Golden 16/16、三项安全门禁和提交前自治总门禁 12/12 全部 PASS，`worktreeUnchanged=true`。首次 PG16 因 NOINHERIT 登录不能隐式访问 schema 而失败，最小修复为显式 SET ROLE，没有放宽登录权限。当前仍只能写 `R1 / 可运行但不完整 / 不能支撑实战`；确定性提交绑定 Bundle、推送和独立生产执行尚未完成，Canonical Cutover 继续阻断。
- 2026-07-17 `WP-G0.2-CANONICAL-ROLLBACK-STATE-MACHINE-REMEDIATION-LOCAL-SUPERPACKAGE` 已完成本地实现并达到 `ready_for_gate`。审计确认 migration 009 的 phase graph 允许 `canonical_compat -> canonical`，但没有 `canonical -> legacy` 受控恢复边；在该缺口关闭前直接 Cutover 会成为只能 fail closed 503、不能恢复旧读取的单向门。新增 migration 010 只增加 `candidate_authority.rollback_canonical_migration_control_v1`：仅 `candidate_migration_role` 可把 active canonical 的精确 epoch 推进一代并回到 `legacy/frozen`，PUBLIC/应用角色、stale epoch、非 canonical、重复回退和坏 approval digest 全部拒绝，Candidate 业务数据不变；001-009 内容/checksum 未改，历史 009 runner 继续冻结到九文件 fixture 并会拒绝含 010 的当前仓库。治理和历史 runner 定向 57/57、隔离 PostgreSQL 16 migrations 1-10、`canonical epoch 9 -> legacy/frozen epoch 10`、权限与数据保留、Autonomy 31/31、typecheck、零警告 lint、market 1026/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁均 PASS；formal 未运行。生产仍只有 migrations 1-9，010 未部署/未执行，Canonical Cutover 继续阻断；当前系统仍是 `R1 / 可运行但不完整 / 不能支撑实战`。
