...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.71。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.75。
...
M1.6P0R_E83_EXACT_ATTEMPT_STS_AND_AGE_HANDOFF_PASS_RUNNER_BLOCKED_PRE_BACKUP / HOST_NAMESPACE_PRODUCTION_NODE_MODULES_ASSUMPTION_INVALID / PRODUCTION_ZERO_DRIFT_AND_TEMPORARY_ROUTE_SECRET_STAGING_EVIDENCE_RESIDUE_ZERO / TRANSPORT_V3_SOURCE_BOUND_NODE_RUNTIME_CAPSULE_LOCAL_P0R_100_OF_100_AND_FULL_CI_PASS / CLEAN_COMMIT_REMOTE_GATES_FRESH_REBIND_NEW_EXECUTION_AND_REAL_RECOVERY_PENDING / LEGACY_RUNS_AND_PACKAGES_REJECTED
...
- M1.6 v1 日分区迁移 checksum 保持不可变；additive v2 六小时分区、无扩容 59%/67% 模型、Object Lock `COMPLIANCE` 31 天、Keychain age 身份、atomic remote session 和 private `/dev/shm` runner boundary 均已形成。source `e83c1f238b19a3495d17f791d0ca5b65a9447734` 的 exact run `p0r-20260729t193336z-0ec1106cf1a2ee7402b0309cddfa34b0` 已真实完成 STS 与 age handoff，但在第一份 backup evidence 前 BLOCKED：Web 容器内 `pg@8.16.3` 正常，宿主 `/proc/<web-pid>/root/app/node_modules` 不可见，证明旧 Runner 错误跨越 mount namespace。该 run 没有读取生产业务行、没有 backup/COS object/retrieval/restore 或业务 mutation；staging/evidence、`/dev/shm`、P0R process/container/volume、8022 listener 和腾讯 `/32` 规则均已清理为 0。transport v3 现携带 source-bound Node `22.23.1` / npm `10.9.8` / pg `8.16.3` runtime capsule，Runner plan v6、session v4、P0R 100/100 与完整 `ci:production` 已本地通过；两次独立真实 `npm ci` 构建的胶囊摘要一致。clean commit、GitHub 四门、fresh rebind、全新 16-member package 和真实恢复仍待完成。
...
工程描述：可运行但不完整
实战描述：不能支撑实战
V2：M0.5 已把 Market Mechanics、Microstructure、双向前兆、板块传播、补充来源、缓存和 Evidence Overlay 融入权威设计；M1.4C、M1.4D、M1.5C/M1.5D 本地包与 M2.1A 均通过各自本地出口，但真实 live、cohort/holdout、校准和 authority 仍未完成。P0R source `e83c1f...` 的真实运行完成 STS/age handoff后，在 backup 前暴露宿主读取容器 `node_modules` 的错误假设并 fail closed；生产业务数据、服务和 authority 零变更，临时 8022 路线、staging、evidence、secret、process/container/volume 全部清理为 0。transport v3 自包含 runtime capsule、P0R 100/100 与完整本地生产 CI 已 PASS；clean commit、GitHub 四门、fresh rebind、新 execution identity、16-member package 和真实 backup/retrieval/restore 仍待完成。M1 未完成，Detector=DRAFT、Candidate 禁发，无 Signal Grade/Strategy/Feasibility/持续 runtime/READY authority
...
当前生产存储门禁：P0_BLOCKED_CAPACITY_AND_RECOVERY；P0R_E83_STS_AGE_HANDOFF_PASS_RUNNER_BLOCKED_PRE_BACKUP_HOST_NAMESPACE_ASSUMPTION_INVALID_PRODUCTION_ZERO_DRIFT_ROUTE_AND_SECRET_RESIDUE_ZERO_TRANSPORT_V3_LOCAL_REMEDIATION_PASS_REMOTE_AND_REAL_RECOVERY_PENDING；没有可用 STS、backup、restore 或 fresh P0
固定生产执行通道：PRODUCTION_FIXED_DISPATCH_TIMEOUT_LOCK_RECOVERY_PASS；旧 raw/tee/manual-compile、browser-state recovery、OrcaTerm secret entry、default SSH port 22、宿主读取容器 node_modules、approved_orcaterm_bundle_upload 和超长复合命令均禁止复用
```
...
- M1.6-P0 存储准入仍为 BLOCKED；Object Lock、age、atomic session、transport-v3 runtime capsule 和完整本地 CI 都不是恢复 PASS，仍缺 replacement clean exact source 资格、远端四门、fresh rebind、new 16-member plan/bundle、usable STS、加密备份、exact retrieval、独立 PG16 restore、cleanup、fresh topology 和 exact-release calibration，P1 严禁启动。
...
```
B1-B3 已关闭 V1 M1.5-B1；P0 已执行并因容量与 recovery evidence BLOCKED。Object Lock 31 天和 age Keychain 身份仍有效。source `e83c1f...` 的真实 P0R 运行已证明 secret handoff 可达，但旧 Runner 在 backup 前错误依赖宿主可见的容器 `node_modules`；该 run、plan、object key 和 package 均禁止复用。当前生产临时 8022 listener、云规则、staging、evidence、secret 和 P0R runtime 残留均为 0。transport v3 八文件 source-bound runtime capsule、Runner plan v6、session v4、P0R 100/100 和完整本地生产 CI 已 PASS；下一步是 clean exact commit、GitHub 四门、fresh rebind 和全新 16-member execution package。只有新 source 下的 bridge 在重新建立受限 8022 bootstrap gate 后取得 echo-disabled READY，用户才需完成 MFA 和 API Explorer 原生 Copy；当前无需用户操作。response 后禁止 browser state/AX/OCR/screenshot/computer-use/OrcaTerm secret entry。只有 backup、exact retrieval、isolated PG16 restore、cleanup、零漂移、临时路线清理、fresh topology、校准和 fresh P0 全部 PASS 才能继续 P1。禁止修改 exact policy、Region、duration、source IP、resource 或 action，禁止签名通道运输 P0R secret。Scope V2 的 C1、M1.4B/M1.4C、M2.1A 和 A0 子门证据仍有效但无 runtime authority；A0 总门禁唯一剩余控制仍是 P0R 真实恢复。外部门 B0.2-B 未解决前 historical bulk、Detector lifecycle 和 runtime 一律关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
