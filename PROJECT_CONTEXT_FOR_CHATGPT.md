...
当前本地证据为 dispatch `37/37`、gateway `7/7`、P0R rebind `14/14`、V2 Ops `274/274` 与故障注入 PASS。本机没有 Docker/Caddy CLI，真实 syntax/runtime validation 必须在生产 mutation 前使用当前生产 Caddy image、network-none/read-only 隔离执行。第一次最新字节完整 CI 已真实在 recurrence authority 10/11 处失败，暴露三份活跃资料仍把旧 route authority 写成当前入口；现已同步修复并加入“新入口唯一、旧入口仅历史上游”的防复发断言，最终字节完整 CI 尚待重跑。之后固定顺序是 `最终字节完整本地 CI -> clean exact commit -> GitHub 四门 -> 确定性 Caddy-only package -> 精确生产 mutation 批准 -> current-image 隔离 validate -> 只重建 Caddy或自动 baseline rollback -> 本机自动取回/解密/验签 -> fresh read-only rebind 与零漂移 -> 新 P0R identity/package -> backup/exact retrieval/isolated restore/cleanup -> fresh health/topology/calibration/P0`。P0R 与 fresh P0 未 PASS 前，P1、M2 runtime、页面交易计划和任何 READY authority 继续关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
