...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.1。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.1。
...
- `/api/frontend/candidate-lifecycle`
...
因此现在不能声称 observer 仍在运行、Cycle-7 PASS/FAIL、G0 完成或生产当前健康/异常。任何下一生产动作前必须重新只读验证 release identity、Compose、health、Candidate runtime、Postgres、Redis 和 observer/终证据。
...
### 2026-07-20 / V2 Controlled Replacement Initial Design

- 用户重新确认核心使命：首要提前抓爆发前机会，同时全面捕捉其他优质结构机会。
- 决定提取已验证地基，重建错误的领域职责，逐 authority Shadow/cutover，不做 Big Bang。
- 当前系统继续为 R1，设计不等于能力落地。
...
- V2 所有能力目前都只是设计，尚无实现、回放、Shadow、SLO 或实战证据。

### P2

- 仓库保留大量 Legacy 治理脚本、历史报告和旧蓝图，当前只作参考；没有 Capability Atlas 前不得批量删除。
...
目标是冻结 18 个 Module 的 schema、五维状态、依赖规则、Legacy Capability Atlas 和第一条 M1 纵向 slice。生产保持零变更；没有独立批准不得 migration、接入页面、删除 Legacy 运行代码或切换 authority。
