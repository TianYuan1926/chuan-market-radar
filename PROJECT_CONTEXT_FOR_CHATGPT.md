...
- 2026-07-14 `WP-G0.2-SCAN-SUSTAINED-HEALTH-PRODUCTION-RELEASE` 已完成 standing-authorization、仓库外全局 lease、一次性消费、递增 fencing 和真实 Bundle CLI 回归，代码提交 `039eb09...`、修复提交 `6b33a78...` 均已推送 GitHub main，腾讯生产尚未部署或改变。不可变 package authorization 与 runtime execution lease 已分离；runner 在 rollback retention、checkout、build、两次 recreate、首次扫描等待、每次 1800 秒观察采样和成功/回滚收口前重验，旧 runner、重放、撤销、过期、路径穿越和 manifest tree 漂移均 fail closed。自治 29/29、release/execute/CLI 15/15、typecheck、lint、test:market 960/0/4 explicit skip、worker 23/23、historical 4/4、build、Golden 16/16 与三项安全门禁全部 PASS；`6b33a78...` 提交后自治门禁 10/10 PASS。可复现 Bundle 两次逐字节一致，SHA-256=`d9f170a80faf5ebbcac227ed67467cee7e4170ae478073e63910b98af2a99737`，但因生产 Gate 状态转换，该 Bundle 只作可复现性证据，执行前必须从最终生产 Gate commit 重建。Microsoft Edge 动态只读预检于 `2026-07-13T20:53Z` 证明生产仍为 clean `main@0599f802...`、target=`70722ea...`、Compose/env/identity 指纹一致、Web healthy、Scanner heartbeat healthy、Candidate absent、Postgres/Redis ready、无活动生产 lease；基线 `scan.completedAt=null` 如实保留，正是待修 P1。当前包已进入 `production / ready_for_gate`，外部 `0600` exact approval、最终 gate evidence、final bundle、Web+scanner-worker 发布及 1800 秒/至少两次 completedAt 推进尚未完成；系统继续是 `R1 / 可运行但不完整 / 不能支撑实战`，G0 未关闭。
