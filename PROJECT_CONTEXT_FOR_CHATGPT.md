...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.77。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.82。
...
M3.3E_LOCAL_RUNTIME_BUILDERS_CONTENT_ADDRESSED_LINEAGE_AND_DESCRIPTIVE_ATTRIBUTION_PASS_TEST_ONLY_UNBOUND_SCOPE_REAL_EVALUATION_UI_SHADOW_PENDING_NO_PRODUCTION_AUTHORITY
...
- M3.3E 本地运行构建链已落地：`StrategyDraft v3` 必须恰好一个内容寻址主标签和有界 context tags，input v2 显式消费 Thesis，无法从 pattern、Analysis structure、方向与结构位证明时 no-draft abstain；`StrategyDecision v2`、`DecisionSnapshot v3`、`AlertEvent v2`、`OutcomeRecord v3` 原样传播。Read Model 强制 READY risk/freshness/release 门并冻结 firstDetectedAt，Alert 只派生 READY/WAIT/DEGRADED，Outcome 绑定 measurement facts、objective event、checkpoint 并客观计算 lead time；描述性归因拒绝重复记录并明确无概率 authority。M3 核心 93/93、runtime/schema 48/48、多资产 28/28、Scope Rebase 12/12 PASS；当前 scope 仍是 `M3_TEST_ONLY_UNBOUND_SCOPE_EPOCH`，真实 Scope V2 接线/存储、canonical Risk builder、真实 cohort/holdout 评估、UI、Shadow、独立审计和生产 authority 均未完成。
