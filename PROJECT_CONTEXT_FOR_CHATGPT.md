...
- 2026-07-14 `WP-G0.2-SCAN-SUSTAINED-HEALTH-PRODUCTION-RELEASE` 已形成本地生产执行包，生产未连接、未部署、未改变。持续健康代码修复已提交推送为 `b1a0eddf973129375292b139a81b326b924a57fa`；由于当前 GitHub main 还包含尚未获准进入生产的 Candidate/Dormant 代码，不能直接 pull main 重建 Web，因此从精确生产基线 `0599f802f261fe8e3c1982a07106f362bd62ac13` 另建了单父、16文件最小发布提交 `70722ea71b33268b688be5d42af9908d40f49859`，diff SHA-256=`80bab7d7e3cdd5a9811dc0815c5df10205bce54e3f87c14d1791c94bcd3f6f58`，专用远端分支已推送；当前 main=`9c9df7de...` 通过 tree 不变的 ancestry merge 包含该发布提交，未改变 `b1a0eddf` 的代码树。新执行包只允许 fetch 精确专用分支、从 clean `main@0599` 切换为 detached `70722ea`、build/force-recreate Web 与 scanner-worker；绑定两个当前镜像、Compose/env/identity wrapper/override、artifact/contract/bundle 和不超过90分钟窗口。失败自动恢复两个旧镜像和 `main@0599`；成功必须从第一次新完成后连续观察1800秒、至少两个后续 `completedAt` 推进、全程 fresh/scanner heartbeat healthy、最终 health ready，且日志为 fixed-rate、updated-only、零假 `task-ok`/零 task failure。可复现 transport artifact=`5dc432045b3e0ebdf9bd83b90dd3b720a024544da2c46872dc6ef4898892c7c5`；成功和观察失败双镜像回滚的隔离假生产演练已通过，定向12/12，基础/安全门禁全绿，自治总门禁11/11 PASS、`worktreeUnchanged=true`、`canAutoCommit=true`、`canAutoDeploy=false`。提交、final bundle、Microsoft Edge 动态只读预检和 exact approval 尚未完成，所以当前只能写 `LOCAL_RELEASE_PACKET_GATES_PASS / PRODUCTION_UNCHANGED / P1_OPEN`。
...
- 当前只允许完成 Web Identity Recovery closeout，再为 Dormant Runtime Deploy 生成新的 clean-commit exact approval。Codex 不得把全自动目标解释成自动生产审批；Dormant final PASS 前不能配置 Runtime Identity，之后也不能提前启用 writer、启动 control lifecycle、backfill、read cutover、G1、R4 或实盘。
...
- 最小生产发布包：基于生产 `0599f802...` 的16文件单父 release=`70722ea...` 已推专用分支并纳入 main ancestry；Web+scanner-worker 双镜像回滚和1800秒/两个完成推进执行包定向12/12，基础总门禁、提交、final bundle、Edge动态预检和exact approval仍待完成。
...
1. 问题：scanner 持续健康修复尚未部署和跨周期观察。
   ...
   - 当前状态：修复提交和16文件最小 release 已推送；生产执行包定向12/12，基础总门禁和 final approval 材料仍未完成，生产 P1 未关闭。
   - 下一步：完成本包基础/安全/自治门禁、提交和可复现 final bundle；Microsoft Edge 动态只读绑定两个当前镜像与 env 指纹后申请 exact approval，部署并连续观察1800秒与至少两个后续完成推进。
