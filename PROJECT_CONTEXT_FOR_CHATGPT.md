...
- 2026-07-13 `WP-G0.2-PRODUCTION-WEB-IDENTITY-RECOVERY-RUNNER`：针对当前 Web 持久化认证降级建立了独立恢复 runner，本地生产 mutation=false。审批锁定生产 HEAD=`0599f802...`、identity override SHA=`1b7f8ba4...`、root-owned wrapper SHA=`fb473dc3...`、生产 Compose SHA=`2749a24d...`、Recovery artifact=`7ffa5a84...`，并要求实时绑定 `.env`、`.env.production`、当前 Web image 以及 runner 两文件 checksum。唯一变更命令是 no-build/no-deps force-recreate `web`；禁止 source sync、镜像 build、其它服务、env write、DB/Redis mutation、migration、Feature Flag、Candidate worker 和 Dormant 新 release。成功必须证明身份指纹匹配、health ready/scan fresh、持久化认证错误消失、三份合同/Postgres/Redis/Candidate dormant 通过且其它容器 ID 完全不变；失败以 base Compose 恢复执行前 Web 基线，但不得把 degraded 基线写成健康。宿主机无 Node 时使用当前 Web 容器 Node 验证同一 base64 request/contract。Recovery 7/7、deploy safety 5/5、typecheck/lint、test:market 952/0/4 skip、worker 18/18、historical 4/4、build、Golden 16/16、安全门禁和自治总门禁 13/13 PASS；`canAutoCommit=true`、`canAutoDeploy=false`。生产未恢复、未部署，仍需 exact approval。
