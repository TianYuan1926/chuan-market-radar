...
- 2026-07-15 `WP-G0.2-RUNTIME-IDENTITY-PRODUCTION-PREFLIGHT-REMEDIATION` 已完成本地最小修复与生产只读兼容证明，生产未发生 mutation。首次绑定 commit=`1ba960fff5d29dda42898f7cb797e5e974efe728` 的运输包因两个真实预检缺口立即作废，禁止上传或执行：旧 runner 假定 Dormant summary 存在并不存在的字段；通用 verifier 虽使用 root-owned identity wrapper，却没有向 wrapper 传入锁定的 `.env` 与 `.env.production`，导致 Compose 在 `POSTGRES_USER` 插值阶段 fail closed。当前 runner 改为校验真实 19 字段 Dormant 脱敏摘要，并精确绑定 package/baseline/target/Web image、24 小时 freshness、1800 秒/57 样本、continuous ready/fresh、Candidate dormant/worker absent、rollback retained 与四项 mutation=false；summary exact SHA-256=`2ced16ca970c61e889eb966d5c32e8276f88d2f61d093ae9ab01c58f1330fc0c` 已进入合同。verifier 通过已批准 wrapper 显式传入两份 env-file，不读取、不打印、不复制 secret；Microsoft Edge/OrcaTerm 对当前生产执行只读 `config --services` 已证明该调用方式兼容，生产仍为 clean detached `cec0b6572bb09ae91ff9e013f8bb160f73c045e2`、Web image=`sha256:cd3652c1e72c8aabea87cee11233fb662a9209187435c14107f3da6426ba9efd`、Candidate flags/URLs/runtime LOGIN=`0/0/0`、worker absent、schema ledger/control=`9|0`、health ready/fresh、Postgres/Redis 正常。修复后 Runner 12/12、Packet 9/9、Identity 14/14、Deploy Safety 6/6、隔离 PostgreSQL 16、typecheck、lint、market 960/0/4 explicit skip、worker 23/23、build、Golden 16/16 与三项安全门禁均 PASS；historical 隔离测试 4/4 PASS，额外联网回放因本机到 Binance/Bybit 443 同时超时而标记 `等待外部条件`，未冒充 PASS。当前 8 文件 runner artifact=`8335ea43de4ac12dce8506af234ef20f66fb1c87f68e938725aa895b5f8be872`，11 文件 production packet artifact=`864a10af242465df6c340b770b91f8c244b42a7dfde187977461e3437b5a3b3a`。这仍只证明修复后的本地生产包和只读兼容性，不是 Runtime Identity 生产 PASS；Candidate activation 继续禁止，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
