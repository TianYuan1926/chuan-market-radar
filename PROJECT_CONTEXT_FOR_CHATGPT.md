...
- 2026-07-17 `WP-G0.2-FRESH-VERIFICATION-CYCLE-LINEAGE-CAPTURE-PRODUCTION-PACKET` 已达到本地 `ready_for_gate`。新增确定性脱敏运输包、最长 90 分钟一次性请求、仓库外租约、fencing token、`Restart=no` transient systemd runner、三组共 9 个私有原始证据文件校验、生产 Git/Web image/Compose/env/容器身份前后不变性证明和精确 stage/secure/ops 清理。生产采集只允许 `REPEATABLE READ READ ONLY + candidate_audit_role`，输出仅保留 Lineage、9 个来源文件哈希、数据库只读身份、租约与运行身份，不保存数据库业务行；Reconciliation、phase、Feature Flag、服务和 authority 均不自动推进。生产包 10/10、原 Lineage 6/6、Reconciliation 12/12、Autonomy 31/31、隔离 PostgreSQL 16 两 release 10005+15=10020 的端到端 production runner 演练、typecheck、零警告 lint、market 1025/0/7、workers 23/23、historical 4/4、build、Golden 16/16、三项安全门禁和自治总门禁 15/15 均 PASS，`worktreeUnchanged=true`；生产未连接、未上传、未执行。真实执行仍被 Activation、累计 10,000 和新鲜相邻周期三项外部前置阻断。
