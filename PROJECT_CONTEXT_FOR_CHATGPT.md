...
- 2026-07-18 `WP-G0.2-CYCLE-2-FRESH-ACTIVATION-AND-ACCUMULATION-REMEDIATION` 已完成本地实现与完整测试，生产未上传、未执行、未改变。生产只读审计证明旧 Activation 唯一留存的是 197 个样本、约 16.5 小时，closeout=`ROLLBACK`，且不存在 `observation-final.json`；此前“289 样本/24 小时 PASS”属于错误过期信息，旧 source-lane refresh 包 `b034484` 已停止使用。新包已删除请求、Bundle validator 和生产入口对旧 Activation final 的全部依赖；Cycle-2 必须从当前 `candidate-episode-v1 / legacy / frozen / epoch4 / active cycle=0 / Candidate Worker absent` 严格相邻启动，并由同一只读 observer 同时证明至少 289 个连续样本、至少 24 小时健康，以及 `legacy_scan_candidate` completed 至少 10,000、至少 1,800 秒、至少 7 样本和两次真实推进。任一单项达标都不能 PASS；每个样本同时绑定 health、Candidate runtime/monitor、migration/release/epoch、数据库锁等待和长事务。Observer 6/6、production packet 26/26、core 28/28、governance 2/2、Autonomy 31/31、PostgreSQL 16、typecheck、零报错 lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁均 PASS；formal 未运行。commit-bound 自治门禁、commit/push、Bundle 和生产执行仍待完成。当前生产仍保持最近只读基线，G0 仍为 7，不能减为 6；系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
