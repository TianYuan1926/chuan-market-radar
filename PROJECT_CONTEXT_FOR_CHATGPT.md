...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.87。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.92。
...
M1.6P0R_ROUTE_AUTHORITY_UPSTREAM_QUALIFIED / PRODUCTION_EVIDENCE_GATEWAY_CADDY_ONLY_LOCAL_IMPLEMENTATION / SIGNED_ENCRYPTED_RESULT_CHANNEL_DISPATCH_37_OF_37_GATEWAY_7_OF_7_P0R_REBIND_14_OF_14_V2_OPS_274_OF_274_PASS / LATEST_FULL_CI_CLEAN_COMMIT_REMOTE_FOUR_GATES_EXACT_PRODUCTION_APPROVAL_REAL_CADDY_VALIDATION_BOOTSTRAP_RETRIEVAL_FRESH_REBIND_AND_REAL_RECOVERY_PENDING / PRODUCTION_INTENTIONAL_MUTATION_NONE / FRESH_PRODUCTION_ZERO_DRIFT_UNVERIFIED
...
- M1.6 v1 日分区迁移 checksum 保持不可变；additive v2 六小时分区、无扩容 59%/67% 模型、Object Lock `COMPLIANCE` 31 天、Keychain age 身份和 private `/dev/shm` runner boundary 均已形成。B8 target、B9 COS CAM action、route evidence v1 及所有旧 response/run/plan/staging 只保留历史证据。route authority 的 implementation `157e9a79...`、真实 CodeQL 失败 `8059edf...` 和 descriptor-first 修复 `913bae3a...` 保留为已资格化上游。当前根因已进一步收紧为 fixed dispatch 只有送达、没有自主结果回传，重复依赖 Edge/API response、AX/browser-state、人工复制或 OrcaTerm 取回会继续复发。新 `V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY` 使用生产 SSH Ed25519 host key namespace 签名、X25519/HKDF-SHA256/AES-256-GCM 加密、既有 Caddy 高熵精确 ciphertext 路由、本机 SOCKS poller、pinned host identity、mode-600 no-clobber receipt 和两小时 exact systemd TTL 删除；只允许重建 Caddy，失败自动恢复 baseline Caddy，其他容器、仓库、数据库、Redis、Worker、env、migration 和 Feature Flag 不得改变。implementation `1ddc0fdc...`、authority-sync remediation `35d7b63a...` 和资料同步 `d67c0e87...` 已形成 clean commits。`d67c0e87...` 远端资格为 3/4：Signed Dispatch、Full Quality、A0 PASS，Independent Security FAIL，不能拼接；CodeQL 私钥测试 path-read race 已改为 descriptor-first `O_NOFOLLOW` 稳定读取，Gitleaks 唯一 X25519 公钥指纹误报以历史 exact fingerprint 加活动源码两段表示根治。官方 Gitleaks `8.30.1` 完整历史 finding=0，当前 recurrence `11/11`、dispatch `37/37`、gateway `7/7`、P0R rebind `14/14`、V2 Foundation `638 PASS / 6 explicit skips`、V2 Ops `274/274`、M0 `12/12`、Next build、Golden `16/16` 和 security 全部 PASS。本机无 Docker/Caddy CLI，真实 current-image Caddy validation 必须在生产 mutation 前的隔离 preflight 完成。新的 clean remediation commit、GitHub 四门、精确生产批准、bootstrap、自动取回、fresh rebind 和真实 P0R recovery 尚未完成；生产没有本轮有意 mutation，fresh zero-drift 尚未证明。
...
当前生产存储门禁：P0_BLOCKED_CAPACITY_AND_RECOVERY；B8 与 B9-R1 只保留历史证据；B9-R2 route authority producer 定向 `24/24`、P0R `139/139`、V2 Ops `266/266`、完整本地 CI 和修复 source GitHub 四门均 PASS；最终资料 HEAD 资格、fresh rebind、敏感页关闭、云 listener/规则/服务器残留清理及生产零漂移尚未复核；没有可用 STS、backup、restore 或 fresh P0
固定生产执行通道：PRODUCTION_FIXED_DISPATCH_TIMEOUT_LOCK_RECOVERY_PASS / B8_EXACT_TARGET_ACCEPTANCE_HISTORICAL_PASS。当前外部事务必须使用 lease-bound transaction v1、120 秒精确 route evidence、签发前双 TTY prearm 和 1200 秒心跳窗口；执行前必须完成最终 HEAD 资格和 fresh rebind。旧 raw/tee/manual-compile、browser-state recovery、OrcaTerm secret entry、OrcaTerm file-manager package transport、default SSH port 22、单 TTY prearm、签发后第二次 SSH 和超长复合命令均禁止复用
```
...
当前根因是 fixed dispatch 只能把签名包送达生产，却不能自主取回严格脱敏结果。替代网关由生产 SSH Ed25519 host key 对 canonical evidence 作 namespace 签名，以固定本机 X25519 recipient 经 HKDF-SHA256 + AES-256-GCM 加密，只在既有 Caddy HTTP 入口暴露高熵 exact ciphertext 路径；本机固定 SOCKS poller 校验 pinned host identity、dispatch/schema/time/hash，以 mode-600、no-clobber 方式保存 payload、sealed object 和 receipt。每个生产 ciphertext 由 exact transient systemd timer 在两小时租约后删除。生产包只允许复用当前 Caddy image 并重建 Caddy；失败必须自动恢复 baseline Caddy，生产仓库、数据库、Redis、Worker、env、migration、Feature Flag 和非目标容器 identity 全部禁止改变。

第一次最新字节完整 CI 已真实在 recurrence authority `10/11` 处失败，暴露三份活跃资料仍把旧 route authority 写成当前入口；同步修复已加入“新入口唯一、旧入口仅历史上游”的防复发断言。Caddyfile 变更触发 Legacy consumer map 与 reviewed commit fail closed，重新审查后 22 capabilities、539 source files、273 runtime edges全部不变，仅受影响 capability digest 更新；M0 入口校验改为里程碑合同加精确 production-control 白名单并拒绝任意 ID、Legacy ID 和换行注入。随后 source `d67c0e87...` 的远端资格为 3/4，Independent Security 因测试私钥 path-read race 和一个 X25519 公钥指纹误报保持 FAIL。当前两项根因均已本地永久修复：私钥测试统一 descriptor-first `O_NOFOLLOW` 稳定读取，活动指纹改为两段表示且历史只允许 exact fingerprint；官方 Gitleaks `8.30.1` 完整历史 finding=0，完整 `ci:production` 从头 PASS。新 clean commit 与四门重验仍待完成。本机没有 Docker/Caddy CLI，真实 syntax/runtime validation 必须在 production mutation 前使用当前生产 Caddy image、network-none/read-only 隔离执行。之后固定顺序是 `新 clean source GitHub 四门 -> 确定性 Caddy-only package -> 精确 production mutation 批准 -> current-image 隔离 validate -> 只重建 Caddy或自动 baseline rollback -> 本机自动取回/解密/验签 -> fresh read-only rebind 与零漂移 -> 新 P0R identity/package -> backup/exact retrieval/isolated restore/cleanup -> fresh health/topology/calibration/P0`。P0R 与 fresh P0 未 PASS 前，P1、M2 runtime、页面交易计划和任何 READY authority 继续关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
