...
- 2026-07-12 `WP-G0.2-POST-DORMANT-CHAIN-CLEAN-GATE-INTEGRATION` 在隔离 worktree 重新审计 Dormant 之后的 Runtime Identity、Activation、Reconciliation、Canonical、Trusted Context 与 Review Truth 链。先物理删除 `.tmp/market-tests` 和 TypeScript 增量缓存重建，真实基线仍为 995 pass/0 fail/7 explicit DB skip；随后把每次构建清理、`incremental=false`、核心测试数必须大于 0 和 7 个代表性 future-chain 编译测试写入防回退门禁。Activation 12/12、Reconciliation 8/8、Canonical 14/14、Oracle 23/23、Route 12/12、Trusted Context 19/19、Review 42/42 与全部 validator PASS；五组隔离 PostgreSQL 16 证明 control start/rollback、10,000 compared writes/0 difference/只读事务、Reader 最小权限、Raw Oracle 同快照和 audit trusted context，全部 `productionConnected=false`。typecheck/lint/build、golden 16/16 和三项安全门禁 PASS。未修改 `src/**`、生产 runner、migration、Compose、DB、Redis、worker 或 secret；该 future-chain 分支在 Dormant production deploy PASS 前禁止合入当前 `main`。
- 2026-07-12 当前发布正本已更新为 GitHub `main=a8dd51954c35cad6c4b14efd3adf6bf97127a342`，Dormant 14 文件 artifact=`78f1e3fa045615fd46dc38739adce0ed14a267e3665a3a1c99501f0520478449`，release diff 为 149 个 A/M 路径且 path-set SHA-256=`f39c8a26ddf5ed8047a081a79bbbcaeed2ebfcc9540466d6e806adad8ce91f37`。Microsoft Edge/OrcaTerm 新鲜只读预检确认生产仍是 rollback=`0599f802f261fe8e3c1982a07106f362bd62ac13`、worktree clean、11 个既有容器运行、Candidate Worker 不存在、三条 Candidate URL 与五个 Flag/release/worker expectation 均未配置，health ready/fresh、Postgres/Redis ready。生产未部署或改变，仍等待 exact commit + artifact + diff + web-only + 90 分钟独立审批。
- 2026-07-12 `WP-G0.2-REVIEW-NULL-DIRECTION-TRUTH-REMEDIATION` 已关闭旧 Review 的 P1 真值污染：未知方向不再默认多头/上涨，缺失 entry/stop/target/验证窗口/MFE/MAE 保持 null，只有明确 `expired` 才显示超时，pending 与 unknown 分开；统计样本必须同时有真实 MFE 与 MAE，空样本平均值为 null；页面无指标时显示“暂无有效指标/幅度待记录”。Review 定向 42/42、Autonomy unit 16/16、typecheck/lint/build、test:market 995 pass/0 fail/7 explicit DB skip、worker 18/18、historical 4/4、golden 16/16 和三项安全门禁均 PASS。开发中 TypeScript 曾因漏判方向联合类型推宽和组件空值收窄真实 FAIL，均以更严格类型修复并从头重跑，没有补默认值或放宽合同。未修改 scan/analysis/strategy/Risk Gate/backtest/API route/Candidate DB/Redis/worker/deploy/secret，未连接或改变生产；当前系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`，下一生产包仍只能是获得独立审批的 Dormant Runtime Deploy。
- 2026-07-12 `WP-G0.2-TRUSTED-READ-CONTEXT-LOCAL-PREPARATION` 已达到本地 PASS：审计发现上一 Route Adapter 将 policy/control 分开读取，未来接数据库时可能跨快照且无法强制 release/evidence 同源，属于 canonical cutover P1，已立即修复。新增 `candidate-trusted-read-context.v1`，在同一 `SERIALIZABLE READ ONLY DEFERRABLE` 事务读取 control 与数据库时钟；policy release 只取 `approved_release_id`、cohort 从 `started_at` 开始、API review horizon 固定 24h。固定路径 authority manifest 的原始字节 SHA-256 必须等于数据库 `approval_digest`，release/epoch/phase 必须一致；phase 禁止自行推断 evidence PASS，PASS 必须有独立 SHA-256，missing hash 必须为 null；运行 release/显式 read flags 必须同时匹配 manifest/control/phase。Route 只接受单一 context provider，并在数据读取前后复核 authority fingerprint；漂移会丢弃结果并返回 503。Trusted Context/Route 治理 19/19 与 12/12、Context PG16 1/1、Resource 9/9、Oracle 23/23 + PG16 1/1、Canonical 14/14、Candidate 156 pass/0 fail/7 explicit DB skip、Autonomy 16/16、typecheck/lint/build、test:market 993 pass/0 fail/7 explicit DB skip、worker 18/18、historical 4/4、golden 16/16 和安全门禁均 PASS。新增 PG 测试在普通 Candidate 门禁明确 skip，但由独立 PG16 Gate 实跑。现有 API、前端、migration、Compose、生产 DB/manifest/flags/control 均未接入或改变；代码 canonical read 授权仍为 false，下一生产包仍只能是 Dormant Runtime Deploy。
- 2026-07-12 `WP-G0.2-CANONICAL-API-ROUTE-ADAPTER-LOCAL-PREPARATION` 已达到本地 PASS：新增纯服务 `candidate-canonical-api-route.v1`，公共 query 只允许 limit 与成对 cursor；release/asOf/cohort/phase/evidence/code authorization 只能来自无公共参数的可信 provider，并在任何数据读取前验证。adapter 直接读取固定为 false 的 `CANDIDATE_PRODUCTION_CANONICAL_READ_ALLOWED`，即使可信 control 声称 canonical，当前仍只读取 bounded Legacy diagnostic，Candidate read/Oracle compare 调用均为 0。非法 query=400；control/policy/依赖/资源 unavailable=503；所有响应 `no-store` 并带 contract/status/source/authority headers。控制 deadline=2s、数据 deadline=15s，全部 provider 收到 AbortSignal；挂起 control 已实测约 2s 中止且不触碰数据。Adapter/治理 11/11、Resource 9/9、Oracle 23/23 + PG16 1/1、Canonical 14/14、Candidate 149 pass/0 fail/6 explicit DB skip、Autonomy 16/16、typecheck/lint/build、test:market 986 pass/0 fail/6 explicit DB skip、worker 18/18、historical 4/4、golden 16/16 和安全门禁均 PASS。开发中治理验证器因 provider 签名升级正确 FAIL，已同步更严格 AbortSignal 签名后重跑通过。现有 API、前端、数据库和生产均未接入或改变，下一生产包仍只能是 Dormant Runtime Deploy。
- 2026-07-12 `WP-G0.2-CANONICAL-API-RESOURCE-CONTRACT-LOCAL-PREPARATION` 已达到本地 PASS：新增纯后端 `candidate-canonical-api-resource.v1`，显式分离 route mode、read source、authority、status、policy、parity、Candidate/Legacy data、blockers 和稳定 content hash。Legacy empty/diagnostic 永远非 Candidate authority；dual-read 即使 parity PASS 仍返回 Legacy diagnostic；canonical compat 只有 Candidate ready + 当前请求 0 difference + 合法 SHA-256 proof 才能返回 Candidate，否则只能显式 `legacy_fallback`；canonical authority 的 partial/unavailable 原样保留且禁止 silent fallback。运行时会重新验证 authority、allowedUse、禁止能力、null 形状与 content hash，非法组合、伪造 parity hash 或 Legacy 权威抬高均返回 `resource_contract_unavailable`。资源/治理 9/9、Oracle 23/23 + PG16 1/1、Canonical 14/14、Candidate 141 pass/0 fail/6 explicit DB skip、Autonomy 16/16、typecheck/lint/build、test:market 978 pass/0 fail/6 explicit DB skip、worker 18/18、historical 4/4、golden 16/16 和安全门禁均 PASS。现有 API、前端、数据库和生产均未接入或改变；canonical read 授权仍为 false，下一生产包仍只能是 Dormant Runtime Deploy。
- 2026-07-12 `WP-G0.2-CANONICAL-READ-COMPATIBILITY-ORACLE-LOCAL-PREPARATION` 已达到本地 PASS：审计确认 Legacy journal/event 投影缺少 Episode identity、release/scan lineage、Checkpoint/Outcome 唯一性、evidence version 和权威 Review 分母，禁止再把 Legacy 填充成完整 Canonical 结构参与零差异。新增 Legacy diagnostic adapter，固定 `canProveCanonicalParity=false`、`canAuthorizeCutover=false`，不输出 outcomeMetrics/entry/MFE/MAE；新增 Candidate Raw-table Oracle，在同一个 `SERIALIZABLE READ ONLY DEFERRABLE` 快照内独立重算 policy、毫秒 cohort、分页、Episode/Checkpoint/Outcome 分母、8 位 metric、excluded reason、不变量、重复 identity 和 lineage，再与主聚合 Read Model 比较。parity 升级为 v2/referenceStatus，policy 纳入差异，路由强制同快照 pair；不同快照、partial/unavailable 或任一差异均 fail closed。Oracle/治理 23/23、Oracle PG16 1/1、上一 Canonical 14/14 + PG16 1/1、Candidate 135 pass/0 fail/6 explicit DB skip、Reconciliation 8/8、Activation 12/12、Runtime Identity Runner 8/8、Composition 28/28、Autonomy 16/16、typecheck/lint/build、test:market 972 pass/0 fail/6 explicit DB skip、worker 18/18、historical 4/4、golden 16/16 和安全门禁均 PASS；自治执行 24/24 Gate PASS。生产 Reader/API/前端/真实双观察均未实施；旧 Review 的 missing direction 偏 long 和 null MFE/MAE 补 0 仍是后续切换 blocker。生产未连接或变更，下一生产包仍只能是 Dormant Runtime Deploy。
- 2026-07-12 `WP-G0.2-CANONICAL-COMPAT-READ-MODEL-LOCAL-PREPARATION` 已达到本地 PASS：新增 Candidate Episode/Checkpoint/Outcome/Review 统一只读模型，每次查询必须固定 `production_radar`、当前 asOf（数据库时钟差不超过 600 秒）、exact release、Episode observation cohort、Checkpoint due cohort、单一 1h/4h/24h checkpoint kind 和 `eg.v1`。开发审计发现原始实现无条件聚合全历史数据会造成 P1 分母漂移，已立即修复；数据库不变量失败或 stale as-of 返回 `partial`，数据库失败返回 `unavailable`，unknown/neutral 和 observation/MFE/MAE null 均不转方向或补 0。路由锁定当前代码授权=false，shadow_verify 为 Legacy 权威双读，canonical_compat 只有逐请求 parity PASS 才返回 Candidate，否则显式 legacy fallback；canonical 阶段禁止静默回退。Dual Read 与 Canonical Compat 各自需要独立 24 小时/289 样本/最大 600 秒间隔/0 差异窗口且不自动切 phase。Canonical/治理 13/13、隔离 PG16 Reader 最小权限 1/1、Candidate 125 pass/0 fail/5 explicit DB skip、Reconciliation 8/8、Activation 12/12、Runtime Identity Runner 8/8、Composition 28/28、Autonomy 16/16、typecheck/lint/build、test:market 962 pass/0 fail/5 explicit DB skip、worker 18/18、historical 4/4、golden 16/16 和三项安全门禁均 PASS。生产 Reader LOGIN/URL、Legacy 归一化 adapter、API/前端接线和真实双读均未实施；生产未连接或变更，下一生产包仍只能是 Dormant Runtime Deploy。
- 2026-07-12 `WP-G0.2-SHADOW-VERIFY-RECONCILIATION-PREPARATION` 已达到本地 PASS：新增只读分页对账器，只有不可变 `legacy_scan_candidate` Source Outbox 的 payload hash、source identity、唯一 Candidate Event、完整 `open_or_refresh_episode_v1` command hash、release/epoch/control 时间窗与 Episode identity 全部一致时才计为一条 compared write。门槛固定为真实 Activation/Observation 24 小时/289 样本、至少 10,000 writes、0 difference、0 pending/claimed/retry_wait/unresolved；resolved quarantine 只显式报告且不计入。纯函数 10,000 条与隔离 PG16 10,000 条均 0 difference，PG16 拒绝只读事务写入且 phase 保持 `shadow_capture`；Reconciliation 8/8、Activation 12/12、Composition 28/28、Autonomy 16/16、typecheck/lint/build、test:market 952 pass/0 fail/4 explicit DB skip、worker 18/18、historical 4/4、golden 16/16 和三项安全门禁全部 PASS。工具不读取 future outcome/MFE/MAE/qualityHit/回测/排序输入，不自动切 phase；未连接生产，真实生产 observation/compared writes 尚未发生，下一生产包仍只能是 Dormant Runtime Deploy。
- 2026-07-12 `WP-G0.2-SHADOW-CAPTURE-ACTIVATION-OBSERVATION-RUNNER-PREPARATION` 已达到本地 PASS：当前代码授权继续固定为 false，future activation release 必须另行把授权改为 true，并同时匹配 exact commit、16 文件 artifact 和 SHA-256；环境变量不能单独授权。Runner 只允许 `web` 与 profile 隔离的 `candidate-shadow-worker`，只开启 Shadow write/release/worker expected，三条 Candidate URL 和角色不变，canonical/dual/review 继续关闭。即时失败、镜像构建失败或 24 小时观察任一样本失败会停 worker、恢复 env/旧 Web/Git，并把 control 回退到 `legacy`、epoch 单调增加、`write_frozen=true`，不删除 Candidate 证据。观察固定 5 分钟、至少 289 样本/24 小时、最大间隔 10 分钟；10,000 compared writes 仍保留给下一 reconciliation Gate，不自动 phase advance。定向 12/12、隔离 shell 激活/构建失败/运行失败回滚/observer、真实 PG16 control start/rollback、完整 Shadow PG16、Identity Runner 8/8、Composition 28/28、typecheck/lint/build、test:market 952 pass/0 fail/4 explicit DB skip、worker 18/18、historical 4/4、golden 16/16 和安全门禁均 PASS。生产未连接或变更，下一生产包仍只能是 Dormant Runtime Deploy。
