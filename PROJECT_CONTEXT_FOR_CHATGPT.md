...
- 2026-07-18 `WP-G0.2-VALIDATION-CYCLE-CONTINUATION-PRODUCTION-REFRESH-AFTER-SOURCE-LANE-CLASSIFICATION` 已完成本地 source-lane-aware 刷新，生产未连接、未执行。旧包的 `shadow_capture / epoch3 / Candidate Worker running / global unresolved=0` 前提已全部作废；新包只接受生产当前 `candidate-episode-v1 / legacy / frozen / epoch4 / active cycle=0 / Candidate Worker absent`。续接门只检查 `legacy_scan_candidate` completed=2,957、unresolved=0，同时强制 2,957 条 `candidate_episode_event` 保持 pending、non-pending=0、orphan=0、contract mismatch=0。Activation 证据必须使用原 commit/release/migration/epoch3 从 289 个原始样本重算，不能用当前 epoch4 冒充。运行时修复了 staged ESM `pg` 解析、生产 env 精确只读挂载、Worker-absent 基线、目标镜像 DB runner、回滚完整性和“Worker 已消失仍必须回滚”缺口。第一轮 PostgreSQL 16 演练真实暴露 SQL alias 大小写错误并失败，修复为显式驼峰 alias 后第二轮 PASS，证明旧 deadline、Candidate 全部计数和事件通道不变，cycle-2 严格相邻启动、失败回滚到全部 Legacy。定向包 24/24、core 28/28、governance 2/2、Autonomy 31/31、PG16、typecheck、零警告 lint、market、build、Golden 16/16 和三项安全检查均 PASS；两轮完整自治总门禁均 15/15、`worktreeUnchanged=true`。最终动态 evidence 不回写 tracked 文档，提交资格只以无后续修改的 `autonomy:verify` 为准。G0 仍为 7，不能减为 6；系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 `WP-G0.2-LEGACY-PENDING-DRAIN-PRODUCTION` 第六次执行已经**安全回滚并揭示原路线的 source-lane 分类错误**。执行绑定 commit=`1c42a4166c097dc16274984a7c30bd90cdaca445`、tree=`8fa789eeee7bd1dee102eaab1c43eac472178876`、Bundle=`8727f05f767d351c3a35401bc5b216e05d90f48c26886a5e764e3538eb7d8da9`、request=`858251c29431bba10b007c14813f7fa73f4859f24383ea556ce71cd9ae15367b`。目标镜像、scanner pause、真实 DB preflight、精确 env renderer、临时 Web/worker 和 `legacy/frozen epoch4 -> shadow_capture epoch5` 均通过，但 Candidate consumer 持续 claimed=0；生产 aggregate-only 核查证明 `legacy_scan_candidate` 已 2,957/2,957 completed、unresolved=0，全局 2,957 pending 全部是 `candidate_episode_event`。Shadow Capture 合同明确规定 consumer 不得消费 Candidate 自己产生的 event outbox。随后主动 TERM transient unit，runner 完成 `ROLLBACK_PASS`，fencing token 19 已释放、global lease/staging/secure absent，生产恢复 `legacy/frozen epoch4`、Candidate absent、Web/scanner ready/fresh。进一步 `REPEATABLE READ READ ONLY` 聚合证明 2,957 条 source 对应投影事件缺失=0，event outbox 孤儿=0、合同字段不匹配=0、非 pending=0，checkpoints/outcomes=0。旧 drain packet 已标记 `SUPERSEDED_SOURCE_LANE_CLASSIFICATION`，新 source-lane 门会以 `legacy_pending_work_missing` 在 control open 前拒绝当前形态；PostgreSQL 16 已证明 Candidate event pending 原样保留时，相邻 validation cycle 可在 legacy source unresolved=0 下合法续接。总门禁 16/16 PASS、market 1,027/0/7、workers 23/23、historical 4/4、Golden 16/16、Autonomy 31/31，formal 未运行。原“生产 pending drain”主步骤按正确 source lane 已闭环，G0 剩余主步骤由 8 变为 7；下一步是刷新并执行相邻 cycle continuation，不是第七次 drain。系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
