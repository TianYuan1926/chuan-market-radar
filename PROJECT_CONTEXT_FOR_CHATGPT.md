...
- 2026-07-14 `WP-G0.2-SCAN-SUSTAINED-HEALTH-PRODUCTION-RELEASE` 已完成本地 standing-authorization 集成并进入 `ready_for_gate`，生产未连接、未部署、未改变。审计发现旧 runner 仍只认识历史逐包 approval，且把 runtime lease ID/fencing token 预填进授权会形成“授权写入提交后使 commit/tree/gate 绑定自行失效”的循环；现已把不可变 package authorization 与运行时 execution lease 分离，当前 standing approval 只能是仓库外 `0600` 普通文件。生产 runner 在固定 trust root `/home/ubuntu/.local/state/market-radar-autonomy` 原子取得全局 lease 和递增 fencing token，在第一次 rollback tag 前一次性消费 approval，并在两次 retention tag、checkout、build、Web/scanner recreate、首次扫描等待、每次 1800 秒观察采样和成功/回滚收口前重验。旧 runner 恢复、重放、撤销、过期、路径穿越、manifest source tree 漂移均 fail closed；revoked/expired lease 只可做当前 fencing 身份的安全回滚。定向自治 29/29、release/execute/CLI 14/14，typecheck、lint、test:market 960/0/4 explicit skip、worker 23/23、historical 4/4、build、Golden 16/16 与三项安全门禁均 PASS；提交前 autonomy 总门禁 10/10 PASS 且 `worktreeUnchanged=true`、`canAutoCommit=true`、`canAutoDeploy=false`。六文件 artifact SHA-256=`5937a72025173ffd703bbaa2034159ae0e89326b635423e3685be53bec013cd8`。报告回填后的最终冻结门禁、clean commit、当前 gate evidence、动态生产指纹和 final bundle 尚未完成，所以 `canAutoDeploy` 仍不得写 true。系统继续是 `R1 / 可运行但不完整 / 不能支撑实战`，G0 Scanner P1 未关闭。
...
- Sustained-health 发布恢复加固：原发布因 OrcaTerm 会话断开未 PASS，生产已恢复 `main@0599`，scanner 为 baseline 重建且摘要不相同；会话独立 runner、双 rollback image retention、定向12/12、自治11/11、clean Git 收口和可复现 final bundle 已完成。Edge 动态预检、新 exact approval、重新发布与1800秒观察仍未完成。
