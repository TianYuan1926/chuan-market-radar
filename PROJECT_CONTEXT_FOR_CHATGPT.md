...
- 2026-07-19 `WP-G0.2-CYCLE-7-LINEAGE-RECONCILIATION-BINDING-FIX` 已完成本地修复，**未连接、上传或修改生产**。审计发现 Current-Cycle Lineage/Reconciliation 的 v4 合同已是 Cycle-7/7-window，但部分 governance validator 与 production shell 仍按 Cycle-6/6-window 检查；现已统一为 `candidate-episode-v1-cycle-7`、`sourceReleaseWindowsExact=7`、`controlLineageExactCount=7`，生产 shell 输出门槛要求 sourceReleaseCount/validationCycle/sourceReleaseWindows length 均为 7。Lineage local 7/7 PASS、Lineage production packet 10/10 PASS、Reconciliation local 16/16 PASS、Reconciliation production packet 11/11 PASS，四个 validator 均 PASS。该修复只关闭当前周期只读前置执行会被旧窗口数阻断的风险，不代表生产 Lineage/Reconciliation 已执行；G0 主步骤仍为 7。
- 2026-07-19 `WP-G0.2-CYCLE-7-SHADOW-VERIFY-PHASE-SHELL-IDENTITY-FIX` 已完成本地修复，**未连接、上传或修改生产**。审计发现 `scripts/production/candidate-shadow-verify-phase/production-runner.sh` 仍硬编码旧 production commit/tree=`3315b54.../cccd577...`，会导致后续 Cycle-7 Shadow Verify phase 现场执行被 shell 身份校验阻断；现已改为 Cycle-7 commit=`47741f3222247562843932b01607a1ec3abb534e`、tree=`bff1d1b3f27a0608004c379189bd1adc038477ec`，并新增 boundary test 拒绝旧 `3315b54`/`72ee289` 残留。Shadow Verify Phase 22/22 PASS，`candidate:shadow-verify-phase:validate` PASS，Shadow Verify Handoff 12/12 PASS，`candidate:shadow-verify-handoff:validate` PASS。productionAuthorization=false、productionExecuted=false、phaseTransitionExecuted=false、observationExecuted=false；G0 主步骤仍为 7。
- 2026-07-19 `WP-G0.2-CYCLE-7-CANONICAL-COMPAT-HANDOFF-REFRESH` 已在本地完成，**未连接、上传或修改生产**。本轮把 Canonical Compat code-presence、phase transition 和 automatic handoff 从旧 Cycle-5/6 身份刷新到当前 Cycle-7：production/reference commit=`47741f3222247562843932b01607a1ec3abb534e`，tree=`bff1d1b3f27a0608004c379189bd1adc038477ec`，migration=`candidate-episode-v1-cycle-7`，release=`candidate-shadow-cycle-7-47741f3`，sourceReleaseWindowsExact=`7`。8 个 Canonical read 关键 blob 已确认在 47741f 中与既有 reference 一致，因此本轮是身份绑定和交接准备刷新，不是功能重写。Code Presence 11/11、Canonical Compat Phase 21/21、Canonical Compat Handoff 18/18 及三组 validator 均 PASS；生产仍必须等待 Cycle-7 final、Lineage/Reconciliation 和 Shadow Verify 289 样本/24 小时 PASS 后才能执行 Canonical Compat。G0 主步骤仍为 7。
- 2026-07-19 `WP-G0.3-G0.5-SECURITY-RELEASE-INCIDENT-LOCAL-REFRESH` 已在 Cycle-7 生产观察期间完成本地刷新。首次复核真实失败于 `artifact_checksum_mismatch`，说明 7 月 17 日旧本地包已随仓库变化过期；现已只更新 `docs/governance/wp-g0-3-g0-5-security-release-incident-local-superpackage.v1.json` 的 artifact SHA-256 为当前真实值 `9888cd21d31906eee08dfd7905a5cdb70767416021b2508ba004b2549b23fa39`。同时修复 `scripts/ci/check-forbidden-files.sh` 的规则冲突：允许 `reports/**/*.md` 脱敏交付报告被追踪，仍禁止 reports 下压缩包、日志、jsonl、raw/evidence 和其它高风险文件。`npm run test:g0-security-closeout-superpackage` PASS（13/13，auth 9/9），`npm run g0:closeout:validate` PASS，`typecheck`、`lint`、`test:market`、`build`、`backtest:golden`、`ci:forbidden-files`、`ci:secret-patterns`、`security:check` 均 PASS，且仍明确 `productionMutationAllowed=false`、`g0Completed=false`、violations=[]。本地准备 PASS 不等于 G0.3/G0.4/G0.5 生产 PASS；HTTPS/private session 仍需独立生产证据和 7 天 burn-in。
- 2026-07-19 `WP-G0.2-VALIDATION-CYCLE-CONTINUATION-PRODUCTION` 已在 Cycle-6 drain PASS 后基于 fresh read-only preflight 启动 Cycle-7。preflight evidence path=`/home/ubuntu/.cache/market-radar-ops/evidence/wp-g0-2-cycle-continuation-preflight-cycle7-47741f3-20260719t091258z/preflight.json`，SHA-256=`5ba159767688c000ae207a9cf6d0be2ba225dd89a51e3409ae165f4c3acb2a6f`，production health ready/fresh，current commit=`cec0b6572bb09ae91ff9e013f8bb160f73c045e2`，current Web image=`sha256:cd3652c1e72c8aabea87cee11233fb662a9209187435c14107f3da6426ba9efd`，current phase=`legacy/frozen/epoch4`，events=`5,266`，outbox=`10,532`，Legacy unresolved=`0`，Candidate event pending=`5,266`。Cycle-7 approvalRef=`MR-G0-CYCLE/47741f322224/1959d0a2`，request SHA-256=`814d87d369f88eca4322f099ac9f41714702859a40134428657d18db35f3a0ad`，transport bundle SHA-256=`e480a06d8a3201b96b8acc225d7705ea634563a934358d9a437a17da46462056`，runner unit=`market-radar-cycle-continuation-47741f3-1959d0a2.service` 已 `Result=success`，observer unit=`market-radar-cycle-observer-47741f3-1959d0a2.service` 已启动。即时结果为 `PASS_IMMEDIATE_CYCLE_CONTINUATION_AWAITING_FRESH_ACTIVATION_AND_REAL_WRITE_ACCUMULATION`；截至 2026-07-19 17:32 CST，observer 至少完成 sample 3，状态仍为 `IN_PROGRESS_FRESH_ACTIVATION_AND_ACCUMULATION`，health ready/fresh。该状态只表示 Cycle-7 已正确进入后台观察并持续采样，不能冒充 24 小时/289 样本/10,000 writes PASS，不能减少 G0 主步骤；G0 仍为 7。
- 2026-07-19 `WP-G0.2-CYCLE-6-LEGACY-PENDING-DRAIN-PRODUCTION` 已通过 Microsoft Edge / OrcaTerm 在腾讯云生产执行并 PASS。执行绑定 approvalRef=`MR-G0-PENDING-DRAIN/47741f322224/591e5e9b`、request SHA-256=`14443ee3e8fd8f804b82028b6258f25f436889f5af10a119eef61764b61a1ea3`、transport bundle SHA-256=`ffb016f807a4efc935cafc958211481752edaf246fd4e1b6628aee897b533455`、systemd unit=`market-radar-pending-drain-47741f3-591e5e9b.service`。最终 evidence：`PASS_LEGACY_PENDING_DRAINED_AND_REFROZEN`，drained=`48`，completed/events=`5,266`，Legacy pending/unresolved=`0`，outbox=`10,532`，global pending/unresolved=`5,266`，final phase=`legacy`，final epoch=`4`，writeFrozen=`true`，Candidate Worker absent，scanner ready/fresh，nextCycleStarted=`false`，secretsPrinted=`false`；unit `Result=success`，lease outcome=`PASS`，生产 `/api/health` 为 ready/fresh，frontend/backend/business 三个合同端点 OK。失败的 41 条 Cycle-6 观察样本仍只能作为失败历史；本 drain PASS 只形成干净 baseline，不能冒充 Cycle-7、Shadow Verify、Canonical 或 G0 PASS。G0 主步骤仍为 7；下一步是基于 fresh production preflight 生成并启动 Cycle-7 request。
- 2026-07-19 Cycle-6 observer 已在第 42 条采样 **FAIL 并自动回滚 PASS**；此前“observer active/running”的条目均为失败前历史。41 条已接受样本、12,166 秒和 5,218 次真实写入保留为失败证据，禁止复用或重标 PASS；失败原因是相邻样本之间短暂出现 `observation_unresolved_outbox`，不是 24 小时/289 样本/10,000 writes 出口通过。回滚后生产保持 clean detached rollback baseline commit=`cec0b6572bb09ae91ff9e013f8bb160f73c045e2`、tree=`eb217a7fbaad5b464279a08d4441a8249fc266e3`；失败观察 target commit=`72ee289388eea922d0aee58fd4ec7a3f18a91007`，两者不得混写。Cycle-6=`legacy/frozen/epoch2`、Candidate Worker absent；600 episodes、5,218 events、10,484 outbox，Legacy lane completed=5,218/pending=48/unresolved=48，Candidate event mirror pending=5,218/non-pending=0/orphan=0/mismatch=0。`WP-G0.2-CYCLE-6-RECOVERY-AND-CYCLE-7-CONTINUATION-SUPERPACKAGE` 已完成本地实现、42/42 continuation production tests、Drain 14/14、production drain 24/24、三套 PostgreSQL 16 成功/失败/相邻周期演练、Autonomy 31/31、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁；formal 未运行且禁止，生产 mutation 尚未执行。2026-07-19 fresh read-only preflight 再次证明 rollback baseline、数据库计数、Candidate absent 和全局 lease absent；旧 Bundle/request 曾上传并完成 staging，但上线前验证发现宿主机缺少 Node 且 Compose 无法解析 `POSTGRES_USER`，会让通用 `production-check.sh` 误判失败。runner 因此没有启动，数据库、服务、Git、env 均未变更，远端 staging/lease/secure root 和本地旧制品已精确清理。修复后的 Drain 与 Cycle-7 runner 都直接验证四个现有 API、Postgres `pg_isready` 和 Redis `PONG`，分别保留 1,200 秒恢复等待和 600 秒启动等待；Drain 8 文件 runner artifact=`3bcd4b1552d5cdaa1785e833fbdca00bc7ded4db070ebd09afb79bacb83d2dd4`，Cycle-7 6 文件 runner artifact=`ca0896ab1562b49bbda2409e22d0a321b1586aaa209e567bcd770635c11fef3d`。要求先精确 drain 48 条 Legacy pending，形成 events=5,266/outbox=10,532、Legacy unresolved=0、Candidate event pending=5,266、Cycle-6=`legacy/frozen/epoch4` 的干净基线，再以全新 request 启动 Cycle-7。G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-19 `MARKET_RADAR_ACCELERATED_DELIVERY_PLAN_V3` 已生效。它把工程组织为本地工程线、生产单写入线和后台观察线：同一恢复列车一次构建/上传，内部子 Gate 仍保留独立 preflight、request、lease、rollback 和 evidence；长观察期间继续准备无冲突后续包。Candidate 24 小时/289 样本/10,000 writes、HTTPS 7 天、Production WIP=1、完整基础/安全门禁和所有交易红线均未缩短或放宽。
- 2026-07-19 `WP-G0.2-CYCLE-6-SHADOW-AND-CANONICAL-CLOSURE-SUPERPACKAGE` 已完成本地实现与提交前门禁，**未连接、上传或修改生产**。审计发现原顺序把 Cycle-6 24 小时、Shadow Verify 24 小时和 Canonical Compat 24 小时写观察塞进不可重置的 72 小时 deadline，且 Canonical 前置还要求额外 24 小时余量，数学上无法闭合；修复没有延长 deadline 或缩短观察，而是把 Canonical Compat 收口为 `write_frozen=true` 的只读阶段：shadow/canonical write=false、Candidate Worker absent，只有该精确 phase/freeze 组合允许 deadline 后继续读，其他过期 phase 仍 fail closed。生产关闭列车拆为四个独立授权子包自动串接：`Cycle-6 final + 72ee289 下只读 Lineage/Reconciliation -> Web-only 72ee289 到 3315b54d 发布 -> 3315b54d Code Presence -> Shadow Verify R2`；Shadow 24 小时 PASS 后再执行 Canonical Code Presence 与冻结 Canonical R2，两个阶段仍各为精确 289 样本、至少 24 小时、最大 600 秒间隔、完整游标链和零差异。Web release 7/7、Shadow Code Presence 10/10、Shadow Phase 21/21、Trusted Context 22/22、Canonical Code Presence 11/11、Canonical Phase 21/21、两套 PostgreSQL 16 transition/rollback、Autonomy 31/31、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁均 PASS；formal 未运行。Cycle-6 final、生产只读重算、Web 发布、两个 phase transition 和两个 24 小时观察仍未执行，本地 PASS 不能冒充生产或 G0 PASS；G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-19 `WP-G0.2-CYCLE-6-SHADOW-VERIFY-AUTHORIZATION-PHASE-HANDOFF-REFRESH` 已完成本地实现与全量门禁，**未连接、上传或修改生产**。Code Authorization v4 只接受 Cycle-6 六窗口和零变更 Code Presence，不再复用历史 Web release；Shadow Verify Phase v5 精确绑定 production commit=`72ee289388eea922d0aee58fd4ec7a3f18a91007`、tree=`bb1492d5a3c79a75c79dfa392dd9a7c2d185f70d`、migration=`candidate-episode-v1-cycle-6`、release=`candidate-shadow-cycle-6-72ee2893`，保持 Legacy 响应权威和 24 小时/289 样本/最大 600 秒间隔；Automatic Handoff v2 严格执行 R0 Readonly Superwindow 通过后动态签发独立 R2 Phase request。差异审计修复了 `94b6d415`/`eb48827b` 冲突身份、Cycle-5 硬编码、只信摘要 path/SHA、Phase immediate 缺少完整生产身份和未从原始 289 samples 重算 Cycle final 五类风险；Handoff 现在逐字节复核 R0/Phase 证据并复用冻结 evaluator 重算全部 Cycle 出口门槛。Code Authorization 37/37、Phase 21/21、Handoff 12/12、PostgreSQL 16 transition/rollback、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁均 PASS；formal 未运行。Cycle-6 final、生产 R0 三子包、Phase transition 和 Shadow Verify 24 小时观察仍未执行，本地 PASS 不能冒充生产或 G0 PASS；G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-19 `WP-G0.2-CYCLE-6-CODE-PRESENCE-AND-READONLY-SUPERWINDOW-REFRESH` 已形成 commit=`6e6fc9c16e40ec8ded69fa6ccd4609b8ce9f49d4` 并推送当前工程分支，**未连接、上传或修改生产**。Code Presence v2 精确绑定 production commit=`72ee289388eea922d0aee58fd4ec7a3f18a91007`、tree=`bb1492d5a3c79a75c79dfa392dd9a7c2d185f70d`、真实 Cycle-6 build record、当前 Web image 字段和三个精确 Git blob；Readonly Superwindow v2 只接受 Cycle-6 final 与六窗口 Lineage/Reconciliation，并按 `Code Presence -> Lineage -> Reconciliation` 严格串行。攻击性复核发现旧最终摘要只校验声明 SHA 格式而未回读真实文件字节的 P1，现已增加 13 个审计文件快照、逐字节 SHA-256 复算、私有普通文件约束、独立 approval ID、Lineage/Reconciliation 已释放 PASS 租约链及篡改/重复授权回归测试。Code Presence 10/10、Superwindow 8/8、Autonomy 31/31、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁均 PASS；formal 未运行。Cycle-6 最终生产出口仍未证明，本地 PASS 不能冒充生产三子包、Shadow Verify 或 G0 PASS；G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-19 `WP-G0.2-CYCLE-6-LINEAGE-RECONCILIATION-DEPENDENCY-REFRESH` 已形成实现 commit=`f9b55684aac6d36bafc53901977730c52151f6f0` 并推送当前工程分支，**未连接或修改生产**。四份 v4 外围合同把 Current-Cycle Lineage/Reconciliation 精确绑定到 Cycle-6 六窗口；Cycle-1 至 Cycle-5 必须为连续 `legacy/frozen/even epoch`，Cycle-6 必须为唯一 `shadow_capture/unfrozen/odd epoch`，旧 Cycle-5 v3 证据不得冒充 Cycle-6 PASS。动态证据格式 `candidate-multi-cycle-lineage-evidence.v3` 与 `candidate-multi-cycle-reconciliation-evidence.v3` 保持不变。Lineage 7/7、生产包 10/10、Reconciliation 16/16、生产包 11/11、两套 PostgreSQL 16 六窗口/10,020 写入演练、逐行 difference=0、Autonomy 31/31、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁均 PASS；formal 未运行。差异审阅真实发现 Lineage production shell 同时残留 Cycle-5 与 Cycle-6 条件，已删除旧条件并增加回归断言。并行只读审计确认旧 Cycle-5 Readonly Superwindow、Code Presence、Shadow Verify authorization/release/phase/handoff 和 Canonical 依赖仍不可用于 Cycle-6；G0.3-G0.5 总关闭合同的声明 artifact 也已与当前文件漂移。这些旧本地 PASS 现只保留为历史，必须刷新后才可执行。Cycle-6 最终生产出口仍未证明，G0 主步骤仍为 7。
- 2026-07-19 Cycle-6 已在生产真实启动，但**尚未通过最终观察出口**。生产为 clean detached commit=`72ee289388eea922d0aee58fd4ec7a3f18a91007`、tree=`bb1492d5a3c79a75c79dfa392dd9a7c2d185f70d`；确定性 Bundle SHA-256=`5d3a312ba0bb2b10ff5a3207c7d008e138e5022ab71c8a70c2c44a9a57ebf562`，fresh machine preflight SHA-256=`381a26389824353d347f1fb8ec3c96e7e3713e728c8c1653d82a28f2fa5d30bf`，一次性 request SHA-256=`21f6003248342cc5865b9839d5e33cc1a7460fc81fc23675a9672e2331827ac9`。即时启动结果为 `PASS_IMMEDIATE_CYCLE_CONTINUATION_AWAITING_FRESH_ACTIVATION_AND_REAL_WRITE_ACCUMULATION`：Cycle-5 保持 `legacy/frozen/epoch2`，Cycle-6 为唯一 `shadow_capture/unfrozen/epoch1` 活动窗口，release=`candidate-shadow-cycle-6-72ee2893`，started=`2026-07-18T23:58:46.887Z`，deadline=`2026-07-21T23:58:46.887Z`。Web、Candidate Worker、Postgres、Redis 和既有 workers 健康；首条 sample v3 为 ready/fresh、completed=4,602、unresolved=0、locks=0、long transactions=0。observer=`market-radar-cycle-observer-72ee289-2b13c6e6` 正在后台运行。首条样本和 immediate PASS 只证明正确启动，不能代替 24 小时、289 样本、10,000 次真实写入、1,800 秒、7 样本、2 次推进和最大 600 秒间隔，故 G0 主步骤仍为 7。
- 2026-07-19 `MARKET_RADAR_ACCELERATED_DELIVERY_PLAN_V2` 已从方案进入生产执行。它保持 Gate 顺序、Production WIP=1、24 小时/289 样本/10,000 writes、HTTPS 7 天和后续长期窗口不变；提速点限定为一次上传的 G0.2 关闭列车、session-independent 自动交接、发布候选冻结、构建一次按 digest 晋级、生产前并发预演、内容寻址门禁复用、观察期间 Lane B 准备和机器证据收口。G0 主步骤仍为 7，只有对应生产出口 Gate PASS 才减数；Cycle-6 已成功启动后的 G0 理想硬关键路径约 10 至 11 天，真实缺陷会增加时间，不能用本地完成或即时启动冒充生产完成。
- 2026-07-19 Cycle-5 observer 已于 05:12 CST **FAIL 并自动回滚 PASS**，此前“observer active/running”的条目均为失败前历史快照，不再代表当前生产。observer 共捕获 57 条样本，其中前 56 条有效，最后有效数据库 completed=`4,556`；第 57 条在 API Candidate monitor=`4,578` 后顺序读取数据库 completed=`4,602`，期间 Worker 又完成 24 条，旧 validator 因强制两次非原子读取完全相等而报 `sample_monitor_completed_mismatch`。该样本当时 health=`ready/fresh`、Candidate claimed/unresolved=`24/24`、oldest age=`22.116749s`、数据库 unresolved=`0`，因此根因是采样时序竞态，不是积压、数据损坏或健康失败。自动回滚已恢复 clean detached production baseline commit=`cec0b6572bb09ae91ff9e013f8bb160f73c045e2`、tree=`eb217a7fbaad5b464279a08d4441a8249fc266e3`、Web image=`sha256:cd3652c1e72c8aabea87cee11233fb662a9209187435c14107f3da6426ba9efd`，Candidate Worker absent，Cycle-5=`legacy/frozen/epoch2`，596 episodes、4,602 events、0 checkpoints/outcomes、9,204 outbox、legacy completed=4,602、所有 unresolved/orphan/contract mismatch=0；即时 health ready/fresh、Postgres ready、Redis healthy。Cycle-5 的 57 条捕获和 56 条有效样本保留为失败历史，禁止复用或重标 PASS。
- 2026-07-19 `WP-G0.2-CYCLE-5-OBSERVATION-SNAPSHOT-COHERENCE-REMEDIATION` 的 v4 sample bracket 已由 Cycle-6 生产首样本验证，历史 commit=`abae0b5009d3cb40903d2c61b636252fa32f6679` 和 Bundle=`f769a64fe02612be67df527e92dc6eb52f8f048009f5633f13cadf75be18948f` 只作为实现来源证据，不再表示“尚未启动”。两次生产启动尝试分别因旧 env fingerprint 和人工转录 SHA 字符错误在 mutation 前 fail-closed；第三次使用服务器动态生成的 fresh machine preflight 后才进入真实启动。该修复保持 sample v3 顺序 `strict fresh health -> database before -> Candidate monitor -> database after -> isolated validation`，数据库括号最多 60 秒；monitor completed 必须位于前后 completed 闭区间，旧 v2、越界、超时或身份漂移仍全部拒绝。失败尝试不得重标 PASS，也不消耗 Cycle-6 正式观察门槛。
- 2026-07-19 `WP-G0.2-CYCLE-5-TO-SHADOW-VERIFY-AUTOMATIC-HANDOFF-SUPERWINDOW` 已完成本地实现并进入 `ready_for_gate`，**未上传、未启动、未修改生产**。它把已冻结的 Cycle-5 只读验证超级窗口与既有 Shadow Verify phase/24h dual-read Packet 封装为一次上传、一次外层 transient unit；R0 与 R2 仍保留独立 manifest、授权、租约、回滚和 evidence。执行顺序固定为 `Cycle-5 final PASS -> Code Presence -> Lineage -> Reconciliation -> 由本次三份精确 path/SHA 动态生成独立 R2 request -> Web-only phase transition -> immediate full snapshot -> 独立 289 样本/24 小时 observer`，任一步失败都阻断后续。外层 runtime 采用精确字段白名单，R0 只接收自己的 13 个字段；生产 launcher 在启动前拒绝仍 active 的 Cycle-5 observer 和任何重叠 handoff/phase/observer unit。组合包 12/12、R0 8/8、Shadow Verify 21/21、Autonomy 31/31、PG16 phase/rollback、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁均 PASS；formal 未运行。Edge OrcaTerm 于 04:29 CST 只读刷新：Cycle-5 observer `active/running`、NRestarts=0、48/289 samples、14,247 秒、4,413/10,000 writes、15 advances、unresolved=0、两个 readiness=false、G0=false；因此生产 handoff 继续硬阻断，G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-19 Cycle-5 Read-Only Verification Superwindow 已形成 commit=`628e49e018361e082292b7089d05e8fa710c6d42`、tree=`f8499b2e8ac6704a80051e4a9fd6552da59a1f4f` 并推送当前 GitHub 工程分支；提交绑定自治总门禁 14/14 PASS、`worktreeUnchanged=true`、gate evidence SHA-256=`f568895796820783143f49f67ca6f57207d770a7f9dc2539cd2aae450467dc47`，两次独立 Bundle 逐字节一致，SHA-256=`e059f47a134070aa971df56a9637b6784601d775ba728f597afc45e712f39f80`、106,298 bytes。**没有上传或连接生产**。新路径把 `Code Presence -> Current-Cycle Lineage -> Current-Cycle Reconciliation` 压缩为一次上传、一次 transient unit，但三个子 Packet 仍保留独立 manifest、request、一次性授权/租约和 evidence；顺序严格 fail-closed，Reconciliation request 只能在 Lineage PASS 后由刚生成的 `lineage-final.json` path/SHA/五窗口动态派生。超级窗口自身 8/8、Code Presence 8/8、Lineage 10/10、Reconciliation 11/11、Autonomy 31/31、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁 PASS；第一次 lint 的 4 error/1 warning 已最小修复并重跑 PASS，formal 未运行。最近一次生产只读快照仍为 observer active、33/289 samples、9,699 秒、4,173/10,000 writes、10 advances、unresolved=0、两个 readiness=false；因此超级窗口继续硬阻断，G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-19 Shadow Verify production code-presence identity remediation v1 已完成本地实现与提交前自治总门禁 12/12 PASS，**没有上传或执行生产 Packet、发布 Web、切换 Git、重建容器、修改数据库/Redis/Worker/env/Compose/Feature Flag/manifest/migration，且没有切换 Candidate phase**。历史 release target `eb48827b...` 不是当前生产 `94b6d415...` 的祖先，但三个 Shadow Verify 应用文件在参考 commit、生产 commit 与当前工程中的 Git blob 逐个完全一致；因此新路径要求参考 blob、生产 clean detached Git/tree、Cycle-5 target-images-record、运行 Web 容器/image、Candidate read manifest absent 和 Candidate read endpoint fail-closed 六项只读身份闭环，任一漂移都强制 `requiresWebRelease=true`，不能用本地比较或硬编码 authority 冒充生产 PASS。code-presence 定向 8/8、phase 21/21、Autonomy 31/31、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16、三项安全门禁和完整隔离执行均 PASS；runner artifact=`d4f25083...`、contract=`8df2d658...`、phase v4 runner artifact=`08261d8e...`、contract=`9052112e...`。formal 未运行。生产 verify-only 尚未执行，Cycle-5 observer 双门禁尚未 PASS，Lineage/Reconciliation/phase transition 继续阻塞；G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-19 Current-Cycle Lineage / Reconciliation / Shadow Verify dependency refresh v3 本地超级包已完成实现和提交前门禁，**没有连接生产、执行生产对账、发布 Web、切换 phase 或启动 dual-read 观察**。Lineage 和 Reconciliation 的 Cycle-3 硬编码已移除：当前 migration cycle 必须推导出精确五窗口，Cycle-1 至 Cycle-4 必须为连续 `legacy/frozen/even epoch` 历史窗口，Cycle-5 必须为唯一 `shadow_capture/unfrozen/odd epoch` 当前窗口。Reconciliation 只接受同一 Cycle 的 Lineage v3，并要求至少 10,000 条逐行比较、difference/duplicate/outside lineage/unresolved 全为 0；Shadow Verify 仍保持 Legacy response authority。联动定向 71/71、Lineage PG16 5 controls/10,020 writes、Reconciliation PG16 10,020 comparisons/difference=0、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16、三项安全门禁和 Autonomy 31/31 均 PASS；formal 未运行。历史 v2 合同全部保留。旧 Shadow Verify release `54837d03... -> eb48827b...` 与当前生产 `94b6d415...` 漂移，明确禁止执行，未来必须重新绑定。Cycle-5 observer 仍是唯一生产 WIP；本地 Packet 不代表生产 Lineage/Reconciliation PASS，G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 Cycle-4 已真实启动但 **observer FAIL 后自动回滚 PASS**，不是观察通过。生产绑定 target commit=`57f02f2d2228c5e838992e4ccb242a8f44f2cd83`、Bundle SHA-256=`64fcdef876f879cfcefec1d1c816d4d8c896869c1e8b9a47630cf93bc36e732f` 和 fresh preflight SHA-256=`4af3b9a4d7fab9519517de68f777855c06140563b59403241b27a60130d69467`；第一个样本为 ready/fresh，第二个样本 completed=3,705 且数据库 ready、Redis healthy、Candidate Worker 与全部预期 worker healthy，但全局 health 因 scan freshness=`aging` 为 `degraded`，严格 validator 以 `sample_health_not_ready` 失败并触发回滚。当前生产已恢复 clean detached baseline commit=`cec0b6572bb09ae91ff9e013f8bb160f73c045e2`、tree=`eb217a7fbaad5b464279a08d4441a8249fc266e3`、旧 Web image=`sha256:cd3652c1e72c8aabea87cee11233fb662a9209187435c14107f3da6426ba9efd`，Candidate Worker absent，Cycle-4=`legacy/frozen/epoch2`、release=`candidate-shadow-cycle-4-57f02f2`、deadline=`2026-07-21T14:41:44.429Z`，577 episodes、3,705 events、0 checkpoints/outcomes 和 7,410 outbox 全部保留。当前本地 Cycle-5 v3 修复不接受 aging 样本；仅当 HTTP/数据库/Redis/Candidate Worker/全部预期 worker 均健康时，允许最多 180 秒、每 15 秒等待 fresh，等待期间不调用 Candidate 写入口、不追加样本，超时或其它健康状态仍自动回滚。定向健康/观察 10/10、Production Packet 37/37、治理 2/2、Autonomy 31/31、PostgreSQL 16 Cycle-4 frozen -> Cycle-5 adjacent、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁均 PASS；formal 未运行。Cycle-4 两个样本与 3,705 writes 禁止复用，Cycle-5 仍须从零完成 10,000 writes、1,800 秒/7 样本/2 次推进和 24 小时/289 样本双门禁。G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 Cycle-3 Shadow Verify v2 依赖刷新本地超级包已完成实现并等待提交绑定总门禁，**没有连接生产、执行 Web 发布、phase transition 或 dual-read 观察**。Web-only release 和 phase transition 两个入口现只接受 `candidate-multi-cycle-lineage-evidence.v2 / PASS_CYCLE3_UNIFIED_LINEAGE_READY_FOR_RECONCILIATION_REFRESH` 与 `candidate-cycle3-reconciliation-evidence.v2 / PASS_CYCLE3_UNIFIED_RECONCILIATION_ELIGIBLE_FOR_SEPARATE_SHADOW_VERIFY_APPROVAL`；必须精确三窗口、当前 `candidate-episode-v1-cycle-3`、至少 10,000 条、零差异/零重复/零 unresolved，并把 Reconciliation 绑定到 Lineage 文件 SHA-256 与三个语义哈希。旧 v1、两窗口和身份脱钩证据均由攻击性测试拒绝。Code authorization `37/37`、Web release `7/7`、phase `20/20`、Autonomy `31/31`、PostgreSQL 16 隔离 phase/rollback、typecheck、lint、market `1,027/0/7`、workers `23/23`、historical `4/4`、build、Golden `16/16` 与三项安全门禁均 PASS；formal 未运行。历史 Web release 的 baseline/target 身份仍是 `54837d0 -> eb48827`，与当前生产 `b098238` 不同；合同虽强制 `releaseTargetRefreshOnBaselineDrift=true`，但本地 v2 PASS 绝不能让该历史 release 直接执行，未来生产前必须按届时 HEAD 另做身份刷新。生产 observer 截至 18:53 CST 仍 active，40/289 样本、11,825 秒、13 次推进、completed=3,563、unresolved=0，两个 readiness 均为 false；因此生产 Lineage、Reconciliation、Web release 与 Shadow Verify 均不得执行，G0 仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 Cycle-3 统一 Reconciliation v2 本地超级包已形成 commit=`a4fbaf39c2393e7afc01925c3efe95258409b7f5` 并推送，提交绑定自治门禁 `14/14 PASS`，但**没有连接生产数据库、执行生产对账或推进 Shadow Verify**。旧 Reconciliation 对历史 Activation final/samples/closeout、Lineage v1、双窗口和人工 epoch 假设的依赖已删除；唯一上游输入固定为私有普通单链接 `candidate-multi-cycle-lineage-evidence.v2`，文件 SHA-256、三个语义来源哈希、Cycle-1/2/3 三个 release window、当前 Cycle-3 身份和数据库 control 必须逐字段一致。正式查询只能使用 `REPEATABLE READ READ ONLY + candidate_audit_role`，至少 10,000 条 `legacy_scan_candidate` 必须逐行核对 source payload hash、projection command hash、Candidate event 与 Episode identity；difference、duplicate、outside lineage 和 unresolved 必须全为 0。定向 `26/26`、自治 `31/31`、PostgreSQL 16 隔离演练 10,020 writes=`[2957,0,7063]`、typecheck、lint、market `1,027/0/7`、workers `23/23`、historical `4/4`、build、Golden `16/16` 与三项安全门禁均 PASS；local runner artifact=`f89675114c9ac7cbb9492db9ee1c33a9ed06f1a69af5f31d9fce565a43cf57b8`，9 文件 production execution artifact=`e8a9b18566635e5843da63433f5ce9f3f72924f79521693215402bc8e0b8174f`。生产双门禁仍未成熟，因此生产 Lineage 与 Reconciliation 仍被门禁。
- 2026-07-18 Cycle-3 统一 Lineage v2 本地超级包已达到 `ready_for_gate`，但**没有执行生产 Lineage 采集**。旧 Activation/Accumulation/Fresh 三目录合同和 sample v1 假设已由单一 Cycle-3 `cycle-observation-final.json`、`cycle-observation-samples.jsonl`、`cycle-observation-closeout.json` 取代；final 必须由 289 个 v2 原始样本重新计算并同时证明 24 小时、completed writes>=10,000、至少 1,800 秒/7 样本/2 次推进、最大间隔 600 秒和 unresolved=0。数据库血缘必须在 `REPEATABLE READ READ ONLY` 与 `candidate_audit_role` 下精确覆盖 Cycle-1/2/3 三条 control：历史两轮为 Legacy/frozen/even epoch，Cycle-3 为唯一 active shadow_capture/odd epoch。旧 197 样本回滚和 Cycle-2 零样本失败禁止重标为 PASS。定向总测试 16/16、生产包 10/10、自治 31/31、PostgreSQL 16 隔离演练 10,020 writes=`[2957,0,7063]`、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 与三项安全门禁均 PASS；local runner artifact=`27c283986c20e2f5db6f8d9372df022b2f86163ed0e43bb87cac7a84aa932775`，9 文件 production execution artifact=`125b21128b521f46fa7caf7010f6e206efa2a700063a0918de625e5faf19d90b`。截至 17:42 CST，生产 observer 仍 active，26/289 样本、7,579 秒、8 次推进、completed=3,333、unresolved=0，两个 readiness 均为 false；因此 Lineage 生产包仍被门禁，G0 仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 Cycle-3 已进入真实生产统一观察，但**尚未通过双门禁**。生产目标 commit=`b098238b5d86ae6dd168c509ac1dce68e3a7adba`、tree=`494a9e6583562bc32cd41a1da82c5d4c91f4938e` 已推送；commit-bound 自治总门禁 15/15、`worktreeUnchanged=true`，gate evidence=`05c849fb96cc3a0c2ecc0a3f49e7a4236aaa64226492b75517d98970c208859c`。两次独立 Bundle 逐字节一致，SHA-256=`4e438503d100a67b6c4e4744ebfc793a70134245dd9302d9616e2c353d077496`、37,692 bytes；远端 SHA/size/`gzip -t`/`tar -tzf` 与隔离合同验证 PASS，preflight=`decb44d34b5dc84c6c9fe4532eed19782c8167c4c557e264e51dbe78d463979a` PASS。第一次一次性请求在 lease、Git、DB、env 和 service mutation 前因 staging 目录为 0755 被 `staged_packet_boundary_invalid` 正确拒绝；未复用该请求。新 0700 staging 与新请求 `e4e6798c0d017840914ec4f9411b0f81401c42a015ce992e9ab2f83710772ea5` 经隔离 validator PASS 后启动 `market-radar-cycle-continuation-b098238-196d9054`，即时事务 `Result=success / ExecMainStatus=0`，生产 HEAD 精确为 `b098238...` 且 clean，Web 与 Candidate Worker 已运行，绑定版 `production-check.sh` 返回 `production check ok`，Web/Postgres/Redis/既有 Worker healthy。观察单元 `market-radar-cycle-observer-b098238-196d9054` 为 active/running；截至 15:44 CST 的固定快照为 3/289 样本、608 秒、completed writes=3,004/10,000、completion advances=1、unresolved=0、`accumulationReady=false`、`freshActivationReady=false`。脱敏启动证据包 SHA-256=`0b91a24ec574d02994936384f2d4ee14019f721f33d5d4a929727b7f11b7a5b6`，`unzip -t` PASS，不含 env、secret、原始业务行或原始日志。因此 Cycle-3/G0 均未完成，G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 observer/rollback 修复已形成 commit=`a54811c69c901645b3fb47069f8bcd6379d0ba0c`、tree=`700217fbba9e89419bbb85a0ca5f1b8d1404647e` 并推送；commit-bound 自治总门禁 15/15、`worktreeUnchanged=true`，gate evidence=`d9926bb61d9f1e9c0b0c72fca7d0242b0c24ee9b436e266f277a6e2aae21ce76`。两次独立 Bundle 逐字节一致，SHA-256=`5d077211403be9cbadb3181679c73878eabeed7082f0f97fe06de9fa725d924d`、37,518 bytes。第四次执行前的现场只读绑定确认生产仍为 clean detached `cec0b657...`、tree `eb217a7...`、旧 Web `sha256:cd3652c1...` running/healthy、Candidate Worker 0、health ready/fresh、Postgres ready、Redis healthy、前后端 radar contract HTTP 200/true，Candidate 数据仍为 543/2,957/0/0/5,914，`legacy_scan_candidate completed=2,957`、`candidate_episode_event pending=2,957`、orphan=0；但控制表真实包含 Cycle-1=`legacy/frozen epoch6` 和最新 Cycle-2=`legacy/frozen epoch2 / candidate-shadow-cycle-2-4ce18da`。旧 Packet 仍把当前源写死为 Cycle-1/epoch6，故在 request、租约、Git/DB/env/service mutation 前被主动作废，上传 Bundle 的 38 个分片和暂存目录已精确清理，未生成授权或 preflight 文件。路线按真值调整为从最新冻结 Cycle-2/epoch2 只启动严格相邻 Cycle-3；红灯已证明旧 runner 会拒绝该形态，最小修复只允许缺失的 disabled migration 字段绑定到数据库锁定的当前冻结周期，显式 Cycle-1/Cycle-3 冒充当前周期继续拒绝；request/治理合同同时精确绑定 Cycle-2 migration/release/epoch。当前 runner artifact=`69a817a78255d5e7c8f04423069a2329a1d071c254a89adba6146922b1a4b13e`、implementation artifact=`6770ccd26867a598bea857a31a659fe1ff097d1d5f885ca567d57e7f0eb02207`，定向 15/15、治理 4/4、Production Packet 32/32、PostgreSQL 16、typecheck、lint、test:market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁均 PASS。clean commit、commit-bound 自治总门禁、新 Bundle/request/preflight 和生产启动仍待执行；双观察门禁尚未开始，G0 仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 Cycle-2 第三次生产尝试绑定 commit=`4ce18da680e8500275e7889848613aa34a4808cd`、tree=`d20846c0861b24a95e3042e03e05bba40b3e8e62`、Bundle=`4a39df3e9fe363ff341b61e5f3ca783d075ef407969c3d1efcb20544c69865e7`、request=`9c6fc842f6c7289977b0c0a88ef2bc0bb34afe9478ff82189f5cadc6026798a4`。38 个传输分片逐片校验、远端重组双哈希、`gzip -t`、`tar -tzf` 和隔离 request validator 全部 PASS；生产 runner 完成目标构建、Cycle-2 激活、Web/Candidate Worker 切换和即时核验，输出 `PASS_IMMEDIATE_CYCLE_CONTINUATION_AWAITING_FRESH_ACTIVATION_AND_REAL_WRITE_ACCUMULATION`。随后 observer 因隔离 Node 容器缺少 `docker run -i`，heredoc 未进入 `node -`，没有生成 `combined-sample.json` 而立即失败；自动回滚又被共享 precheck 误要求“目标 commit 的 Compose 必须等于基线 Compose”而自锁，真实结果为 `ROLLBACK_FAILED`。已在既有自动回滚授权边界内执行最小人工恢复：使用受限 migration role 将 Cycle-2 冻结回 `legacy / epoch2`，移除 Candidate Worker，恢复 `.env.production`、基线 Web 镜像与 detached baseline Git=`cec0b6572bb09ae91ff9e013f8bb160f73c045e2`，只重建 Web；独立核验 Web Docker healthy，容器内 `/api/health`、frontend/backend radar contract 均 HTTP 200/`ok=true`，Postgres accepting、Redis PONG、既有 Workers healthy，Cycle-1=`legacy/frozen epoch6`、Cycle-2=`legacy/frozen epoch2`，租约以 `ROLLBACK_PASS` 释放。远端保留脱敏恢复证据 SHA-256=`c4f4cb19cd542d12398d348494d8a7933dfc112aceeb4033b4e8e136dd0e59371`，本地脱敏证据包 SHA-256=`07e02e99b8d8041cd06cbbee44a7926f5b6b23cc3f439f6804b4fa9f01c8f13d`；其余 staging/secure/ops/upload、失败 unit、目标 Web 镜像和 rollback tag 均按精确身份清理并输出 `EXACT_CLEANUP_PASS`。本地最小修复为 observer Node 容器增加 stdin，并把 baseline Compose checksum 约束限定在 continue 前置分支，使 rollback 可从干净且精确批准的目标 commit 恢复；边界回归 14/14、Production Packet 31/31、PostgreSQL 16、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 均 PASS，runner artifact=`4d0e7c6f1e67b0597fb960acb8778612b4c11146dac2955c9f1d5d6bb618f0da`，formal 未运行。修复尚待 clean commit、commit-bound 总门禁、新 Bundle/preflight/request 和第四次启动；Cycle-2 双观察门禁仍未开始累计，G0 仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 Cycle-2 第二次生产尝试绑定 commit=`096fad5ab069b8e6d7680e7bb89b5d60bba0af04`、Bundle=`78d0ee9c89fee653bc4fe6bef27a0ba3346575e1311de3c00f1b8034382d511e`、request=`ae6d23cf2c3e6bd713f828e777b78efce5d5f818fee9acc133912759c239d84e`、preflight=`18bcef5e90b265203cd1f4077bfcb278b94096a6ff691a079cc8e2aa2ccf44ff`。远端双哈希、隔离 request validator 和授权/租约 schema 均 PASS；runner 取得 fencing token 20、消费一次性授权、精确 checkout 目标并构建 Web/Candidate Worker 镜像，随后在 DB control、env 切换和 Candidate Worker 启动前以 `candidate_environment_source_mismatch:CANDIDATE_EPISODE_CANONICAL_WRITE` fail closed。现场证明 `.env` 与 `.env.production` 均未保存 Candidate override，而运行中 Web 依据 `docker-compose.yml` 的精确默认值实际为所有 Candidate flag=false、cycle=`candidate-episode-v1`、release=`disabled`；旧 renderer 错把安全缺省值当成污染。自动回滚输出 `ROLLBACK_PASS`，lease token 20 已释放；独立核验生产为 clean detached `cec0b657...`、旧 Web `sha256:cd3652c1...`、Candidate Worker 0、唯一 control `candidate-episode-v1 / legacy / frozen / epoch6 / candidate-shadow-e5eb90026d8b`，使用本事务绑定版 `production-check.sh` 再验为 ready/fresh、Postgres/Redis/既有 Worker healthy。最小修复只允许 `legacy` 阶段按 Compose 明确定义的 disabled/cycle1 默认值补全；任何显式 true、错误 cycle/release、重复字段或 active 缺字段仍拒绝。第二次尝试的脱敏证据包 SHA-256=`3532b8b385f71aa75d9c267e79600aa1117e3c78059c3ce285d236ebfa96c068`，敏感信息扫描正常完成且零命中；历史 UI 截图句柄已失效，证据清单明确标记未恢复，未伪造截图。进一步红测揭示观察器启动前失败只回滚业务状态、未清 staging/secure/ops/rollback tag/目标镜像；现已补成仅在每项 Docker/Git 查询自身成功、Legacy 基线、Web、Candidate absent、health 和镜像无人使用全部复核通过后精确清理，查询报错与“查询成功但为空”严格区分，脱敏 evidence 永远保留，runner artifact=`06fb4d202544cda98cc1038910efb3125a5f5e7ea4b600dc5e2dec37d545975f`。Production Packet 29/29、Core 29/29、Governance 2/2、Autonomy 31/31、PostgreSQL 16、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16、安全门禁和最终自治总门禁 15/15 全部 PASS，`worktreeUnchanged=true`；formal 未运行。旧远端残留仍待按精确清单清理，clean commit/Bundle/request/preflight 和第三次生产重试尚未执行；Cycle-2 与双观察门禁未 PASS，G0 仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 `WP-G0.2-CYCLE-2-FRESH-ACTIVATION-AND-ACCUMULATION-REMEDIATION` 的 epoch 6 第一份生产事务绑定 commit=`3c432e511f38786dd7585443bb7994212af53163`、Bundle=`4a63e3b9e9f45819c2ba948eadcbcb2638804e82832d4b22a3036d99dfa360c7`、request=`4bd1989425902604e7ce250065d2a85c5ae5e96b9b7f6f6ebb84ae067e8576e3` 和只读 preflight=`55a0c024b7238817c052fbe36d944b0b98a40e67830221cd7e63558fcc90a1a0`。Bundle/request 上传双哈希和隔离容器 request validator 均 PASS，但 transient runner 在取得全局生产租约前被通用 lease CLI 以 `autonomy_authorization_schema_invalid` 拒绝；根因是 Cycle-2 validator/生成器漏掉租约强制的 `market-radar-package-authorization.v1`。本次没有取得 lease、没有切 Git、没有修改 DB control/env、没有构建或重建 Web/Worker；失败后独立核验生产仍为 clean detached `cec0b657...`、旧 Web `sha256:cd3652c1...`、Web/Postgres/Redis healthy、Candidate Worker absent、唯一 control `candidate-episode-v1 / legacy / frozen / epoch6 / candidate-shadow-e5eb90026d8b`、global lease absent，并精确清理 staging/secure/ops/evidence/upload 与临时 rollback tag。当前最小修复先红后绿，缺失或错误 schema 都会在本地请求生成阶段拒绝；runner artifact 已刷新为 `c76e5c8ec38ae88fc679994293cb8a041d3857ced3401c53a8e928262bd6205e`，Production Packet 26/26、Core 28/28、Governance 2/2、Autonomy 31/31、PostgreSQL 16、typecheck、零 warning lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和安全门禁均 PASS。新 clean commit、提交绑定门禁、Bundle/request、新鲜 preflight 和生产重试仍待完成；Cycle-2 仍必须全新证明 289 连续样本/24 小时与至少 10,000 completed writes/1,800 秒/7 样本/两次真实推进双门禁。G0 仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 `WP-G0.2-LEGACY-PENDING-DRAIN-PRODUCTION` 第六次执行已经**安全回滚并揭示原路线的 source-lane 分类错误**。执行绑定 commit=`1c42a4166c097dc16274984a7c30bd90cdaca445`、tree=`8fa789eeee7bd1dee102eaab1c43eac472178876`、Bundle=`8727f05f767d351c3a35401bc5b216e05d90f48c26886a5e764e3538eb7d8da9`、request=`858251c29431bba10b007c14813f7fa73f4859f24383ea556ce71cd9ae15367b`。目标镜像、scanner pause、真实 DB preflight、精确 env renderer、临时 Web/worker 和 `legacy/frozen epoch4 -> shadow_capture epoch5` 均通过，但 Candidate consumer 持续 claimed=0；生产 aggregate-only 核查证明 `legacy_scan_candidate` 已 2,957/2,957 completed、unresolved=0，全局 2,957 pending 全部是 `candidate_episode_event`。Shadow Capture 合同明确规定 consumer 不得消费 Candidate 自己产生的 event outbox。随后主动 TERM transient unit，runner 完成 `ROLLBACK_PASS`，fencing token 19 已释放、global lease/staging/secure absent；最新控制行核查证明该回滚的真实结果是 `legacy/frozen epoch6`，而不是此前报告中的 epoch4，Candidate absent、Web/scanner ready/fresh。进一步 `REPEATABLE READ READ ONLY` 聚合证明 2,957 条 source 对应投影事件缺失=0，event outbox 孤儿=0、合同字段不匹配=0、非 pending=0，checkpoints/outcomes=0。旧 drain packet 已标记 `SUPERSEDED_SOURCE_LANE_CLASSIFICATION`，新 source-lane 门会以 `legacy_pending_work_missing` 在 control open 前拒绝当前形态；PostgreSQL 16 已证明 Candidate event pending 原样保留时，相邻 validation cycle 可在 legacy source unresolved=0 下合法续接。总门禁 16/16 PASS、market 1,027/0/7、workers 23/23、historical 4/4、Golden 16/16、Autonomy 31/31，formal 未运行。原“生产 pending drain”主步骤按正确 source lane 已闭环，G0 剩余主步骤由 8 变为 7；下一步是刷新并执行相邻 cycle continuation，不是第七次 drain。系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 `WP-G0.2-LEGACY-PENDING-DRAIN-PRODUCTION` 第五次真实执行仍为 **FAIL，但 control open 前自动回滚完整**，不能写成 drain PASS。执行绑定 commit=`c0cce68da8e58d6f3b314505561779cfdb84b1f4`、tree=`19ee3919167a7994ffc4f09f5f8f3a27e70bb9b2`、提交后 gate evidence=`099ae55eeef3d7675acbc4cd61845bbb73f41e689f52f77673396630cec18597`、deterministic Bundle=`f0676ee6ebe06b1041983f2b3a2d3d7aa0cad06f3efbcd7f84a83a887c23a951`、单次 request=`f58bef9a0e138d873850413bec95baffd39958e9f696070e7360586b6ed5aa2c`。OrcaTerm 双哈希、request 容器校验、目标镜像构建、scanner pause、目标镜像 `/app` 模块解析、三道单行 `jq` 合同和真实 DB preflight 均通过；随后 `drain-only-environment` 阶段的隔离 renderer 因没有挂载宿主 `.env.production`，以 `ENOENT` 安全失败，未打开 epoch、未启动 Candidate worker、未推进 pending。fencing token 18 已以 `ROLLBACK_PASS` 释放，staging/secure/global lease absent；独立复核生产 Git/tree、env 指纹、Web/scanner、Candidate absent、数据库 migration 10、`legacy/frozen epoch4`、outbox=5,914、completed=2,957、pending/unresolved=2,957 均恢复，health=`ready/fresh`。本地最小修复新增专用 renderer，只把生产 env 精确单文件只读挂载为 `/runtime/env.production`，输出只写本轮临时 OPS，通用 lease runner 仍拿不到 env；合同和真实 Docker 参数回归共同锁定该边界，runner artifact=`f90f202489f8c763612c976fe795cd33e1cbc1b807d92ce68ec07ebbe06343f5`。生产包 23/23、旧 drain 12/12、PostgreSQL 16 成功/失败再冻结、typecheck、lint、market、build、Golden 16/16、安全和 Autonomy 31/31 均 PASS；事实回填后的最终冻结门禁 12/12 PASS，result=`e64ffb07-2a61-484f-b5b9-b90f2b4a949d`、evidence SHA-256=`a2be4ebee43cacbd291dcbecb4c209c8156dd035a14c6ffb7eb79358f914b212`、`worktreeUnchanged=true`。clean commit、提交后门禁、新 Bundle/request 和第六次生产执行仍待完成。G0 主步骤仍为 8，系统仍只能写 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 `WP-G0.2-LEGACY-PENDING-DRAIN-PRODUCTION` 第四次真实执行仍为 **FAIL，但数据库写入前安全停止且自动回滚完整**，不能写成 drain PASS。执行绑定 commit=`2e2d088abfcc0f758f829eb5256f129f02e4ef11`、tree=`3947f8e15499a195cf43ad490d9ddf2c538076fa`、提交后 gate evidence=`7df90957f503e617acdeb11f20f27d4e1d76f49fcd4ef10786f1597c339ba5de`、deterministic Bundle=`6e817f25a3aa266f81ae9756ab596ffff0f0d1510a1655c0e8cc4f52fc0d760f`、单次 request=`797aba1e10776833784b64bcb5a52b83cc955067ee223706d89dc789c75457f9`。远端运输双哈希、request 容器校验、目标生产镜像内 `/packet` DB runner 从 `/app/package.json` 加载 `pg` 均 PASS；真实 DB preflight 也输出了精确成功 JSON，但 production runner 的单引号 `jq` filter 内含反斜杠续行，导致合法结果被本地语法错误拒绝。fencing token 17 已以 `ROLLBACK_PASS` 释放，staging/secure temp/全局 lease 均 absent；独立复核生产仍为 migration 10、`legacy/frozen epoch4`、outbox=5,914、completed=2,957、pending/unresolved=2,957，Web/scanner/Git/env 已恢复并为 ready/fresh。本地最小修复把 preflight、control open、final verify 三道 `jq` 门冻结成可独立编译的单行只读过滤器，并由真实 `jq` fixture 和治理弱化测试锁定，runner artifact=`1ab33385dd3750f1be137d3c90247dfcb6e57c18ebad4ea8ad6a6e6f5e762459`；生产包 22/22、旧 drain 12/12、PostgreSQL 16 成功/失败再冻结双路径、完整基础/安全/自治总门禁 12/12 均 PASS，market 1027/0/7 explicit skip、Golden 16/16、`worktreeUnchanged=true`，`sourceWritesAdded=0`、`productionConnected=false`。clean commit、提交后门禁、新 Bundle/request 和第五次生产执行仍待完成。G0 主步骤仍为 8，系统仍只能写 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-18 `WP-G0.2-LEGACY-PENDING-DRAIN-PRODUCTION` 第三次真实执行仍为 **FAIL，但数据库前安全停止且自动回滚完整**，不能写成 drain PASS。执行绑定 commit=`d3c17b517849af4b42c6734a8f917b8212f75188`、tree=`802b93d9457404963cab8e5bef654465fa685f2b`、提交后 gate evidence=`1798323e630d5eee85c0cd99c70ecaeb28c96fa0633b0e7e24e7e5acb5f322a5`、deterministic Bundle=`4fd0210dd163b0b07c5c4c120869e64dc11a367537e692d8f069052882ba959c`、单次 request=`7ef5b22db3ee4e9293141aff5875dc722204e5bdf4b2144fad88d92a6dd2621b`。远端运输双哈希、容器内 request 校验和 transient unit 启动均 PASS；fencing token 16 在 `database-preflight` 报 `ERR_MODULE_NOT_FOUND: pg`。目标 Web 镜像实际包含 `pg`，但 bind-mounted `/packet/.../db-runner.mjs` 的静态 ESM import 只沿 `/packet` 模块树解析，不能到达镜像 `/app/node_modules`。执行未打开 epoch、未启动 Candidate worker、未推进 pending；回滚为单值 `ROLLBACK_PASS`，lease 已释放且全局 lease absent。独立复核数据库仍为 migration 10、`legacy/frozen epoch4`、outbox=5,914、completed=2,957、pending/unresolved=2,957，生产 Git/env/Web/scanner 已恢复并为 ready/fresh。当前本地最小修复改为优先使用 `/app/package.json` 的 `createRequire` 加载 `pg`，仅为本地测试保留模块 URL fallback；治理合同显式禁止从 `/packet` 解析依赖，runner artifact 已重新冻结为 `073d83c63c7fb72a5ac1bec0ab6992a811a340fb591285d674a44cad00a323d2`。生产包 21/21、旧 drain 12/12、PostgreSQL 16 成功/失败再冻结双路径均 PASS，`sourceWritesAdded=0`、`productionConnected=false`；本机无 Docker CLI，因此未伪称完成本地 bind-mount 容器演练。提交前基础/安全/自治总门禁已 12/12 PASS、market 1027/0/7 explicit skip、Golden 16/16、`worktreeUnchanged=true`；clean commit、提交后绑定门禁、新 Bundle/request 和第四次生产执行仍待完成。G0 主步骤仍为 8，系统仍只能写 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-17 `WP-G0.2-LEGACY-PENDING-DRAIN-PRODUCTION` 第二次真实执行仍为 **FAIL，但自动回滚已完整收口**，不能写成 drain PASS。执行绑定 commit=`1856990852a373faae625618a80ec5fdee3026bd`、tree=`2ecad546c1c1e0f6a36f5702da77cab0512851c7`、提交后 gate evidence=`a2ed5d3379c5838eaa679475ed79fc485b8ba1f5f7a8c90a5155a2e0a962f2b3`、deterministic Bundle=`ceaf387f7d10e1ce4e27022296e31b40646f5a27b44d6ceb62fb2b0495caa29d`、request=`768402ec9ddcc871a248685d989e5b71e3bfb26392e131646099aa5af468d078`。远端 staging 双哈希和 request 容器验证均 PASS，fencing token 15 自动归档过期 token 14；runner 停止 scanner 后在 `database-preflight` 发现旧基线 Web 镜像不能加载新 `db-runner.mjs` 所需 `pg`，因此未打开 epoch、未启动 Candidate worker、未推进 pending。回滚结果是单值 `ROLLBACK_PASS`、`leaseReleased=true`、`leaseRetained=false`，全局 lease absent；独立只读复核数据库仍为 migration 10、`legacy/frozen epoch4`、outbox=5,914、completed=2,957、pending/unresolved=2,957，生产 Git/env/Web/scanner 已恢复基线。另一次 OrcaTerm 错误粘贴曾误装 `mailcap`，已精确 purge 并复核 absent，未 autoremove 其它包。当前最小修复要求先在 scanner 仍在线时构建目标 Web/Candidate 镜像，再停止 scanner，并只用含 `pg` 的目标 Web 镜像执行全部 DB runner；生产包 20/20、旧 drain 12/12、PostgreSQL 16 成功/失败双路径均 PASS，runner artifact=`e081261e5c0aa04b24145e41a9d1aa71733c415c6cd48d184c3eb356a2b245db`。完整基础/安全/提交绑定门禁、clean commit、新 Bundle/request 和第三次生产执行仍待完成；G0 主步骤仍为 8，系统仍只能写 `R1 / 可运行但不完整 / 不能支撑实战`。
- 2026-07-17 `WP-G0.2-LEGACY-PENDING-DRAIN-PRODUCTION` 历史首次真实执行结果为 **FAIL / P0 rollback closeout incomplete**，不能写成 drain PASS。执行绑定 local commit=`381cd7be9a7c9f793b55953aa11eb976da2d3ae7`、Bundle SHA-256=`270ccaad8661a84bffcccd23141e9fad453aa7ca0e9dcbd511454cc778bb4826`，生产取得 fencing token 14 后停止 scanner；因仍存在 `scan:lock:*`，在数据库 preflight、epoch open、临时 Web/worker 之前安全失败。独立只读核查证明数据库始终是 migration 10、`candidate-episode-v1 / legacy / frozen / epoch4`、outbox=5,914、completed=2,957、pending/unresolved=2,957，Candidate worker absent；production Git/tree=`cec0b657...`/`eb217a7...`、env/Compose 指纹、Web image=`sha256:cd3652c1...`、scanner image=`sha256:b11c0cec...` 均恢复基线，Redis scan lock=0，扫描后续推进到 `completedAt=2026-07-17T14:10:25.013Z`，health=`ready/fresh`。真实缺陷是 rollback health 只等 600 秒，短于 scanner 900 秒 cadence；随后 runner 又用租约系统不接受的 `ROLLBACK_FAIL` outcome 释放，导致 active lease 保留，并把证据错误拼成 `ROLLBACK_FAIL\nROLLBACK_PASS`。本地最小修复现已固定 Redis 锁只读等待 660 秒、基线新扫描/ready/fresh 等待 1,200 秒、回滚不完整时保留租约并只写 `ROLLBACK_INCOMPLETE_LEASE_RETAINED`、回滚状态单值化；定向 20/20、隔离 PostgreSQL 16、typecheck、零 warning lint、market 1027/0/7、build、Golden 16/16、三项安全门禁和 Autonomy 31/31 均 PASS，runner artifact=`597abb94ac3192b8bdd9fe64f4a5f8841fe036b47eeb5ddf316766144d32a357`，formal 未运行。该历史状态已被上方第二次执行及其完整回滚证据取代。
...
- 当前最大短板：
  1. Cycle-3 Shadow Capture 已激活并通过即时健康验证，但 24 小时/289 样本与 10,000 真实 writes 统一观察仍在进行；双门禁最终证据前不能写观察 PASS，任何样本失败必须自动回滚。
  2. Reconciliation v2 本地 10,020 条三轮工具和只读生产 Packet 已准备，但生产 Lineage、生产只读对账、shadow_verify 独立审批、canonical compat/read cutover 均未执行；Candidate 新链仍无 canonical authority。
...
- 真实观察：截至 18:20 已有 33/289 样本、9,701 秒、11 次真实推进、3,471/10,000 completed writes、unresolved=0；observer active，两个 readiness 仍为 false，严格保持“进行中”。
- Lineage v2：本地三轮统一证据与只读生产采集 Packet 已完成 commit=`854295b...` 并推送，生产 Lineage 未执行，不能替代双门禁。
- Reconciliation v2：旧 Activation 文件、Lineage v1、双窗口和人工 epoch 假设已清除；定向、PG16、基础、安全和自治门禁全部 PASS，但生产对账、Shadow Verify 和 G0 均未完成。
