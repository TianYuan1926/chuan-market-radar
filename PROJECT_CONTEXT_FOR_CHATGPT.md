...
- 2026-07-16 `WP-G0.2-SHADOW-CAPTURE-ACTIVATE-AND-OBSERVE` 首次精确生产事务失败，不能写 Activation PASS。执行绑定 source=`d07bfe37c711b4d56be2a7277ec382e44dfc436e`、Bundle=`0aa008b974bf09b241ec62de928ba42bcd9ec62d360f47a3a7081a82a9016fe8`、request=`3634e1f59754ff0c8324be37c7991f35e3c1a63d9f6e81ba7ff18682737880da`；入口与 systemd 启动成功，但数据库 control preflight 的隔离容器继承 Web 镜像用户，无法穿透 root-owned `0700` staging bind mount，Node 返回 `MODULE_NOT_FOUND`。失败发生在 lease acquire、Git checkout、control start、env switch 和 service recreate 前；清理器删除 staging/secure/ops，独立复核确认生产仍为 clean detached `cec0b657...`、既有容器运行且 Candidate worker absent。旧 Bundle/request 已消费，禁止复用。最小修复只给 database control 容器增加 staging owner UID/GID，继续保留 read-only、cap-drop、no-new-privileges 和精确挂载，并新增真实权限回归；current runner artifact=`b8f04110...`、activation artifact=`2b4ae59c...`、contract=`f23431b8...`。修复尚待完整门禁、clean commit、main 推送和全新精确 Bundle/request；Candidate 仍 dormant，WP-G0.2/G0 未完成。
