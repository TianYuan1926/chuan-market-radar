...
- 2026-07-17 `WP-G0.2-RECONCILIATION-MULTI-CYCLE-LINEAGE-REMEDIATION-LOCAL-SUPERPACKAGE` 正在本地收口。审计发现旧 Reconciliation 固定 `candidate-episode-v1` 并只过滤当前 release，因而无法对多个 72 小时周期累计的 10,000 条写入做真实核对；该 P1 会让 G0 路线在累计达标后进入死路。当前修复分离 Activation 首周期身份和当前新鲜验证周期身份，请求绑定从 cycle1 开始的完整相邻 release 窗口；逐行按自身 release 窗口验证 source/event/episode/command hash；同一 `REPEATABLE READ READ ONLY + candidate_audit_role` 事务读取全部 control 血缘，历史周期必须 Legacy/frozen、当前周期必须唯一 shadow_capture；任何血缘外写入或未列 control 都失败。定向 runner 13/13、production packet 10/10、隔离 PostgreSQL 16 两周期各 5,000 条/合计 10,000 条零差异、只读拒写、未批准第三周期拒绝均 PASS。生产未连接、未执行；基础和安全总门禁尚在收口，因此当前仍不能写完成。
