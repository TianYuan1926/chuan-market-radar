...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.25。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.29。
...
M0_ENGINEERING_EXIT_LOCAL_PASS / M0.4_EXPANDED_SCOPE_DESIGN_PASS_IMPLEMENTATION_NOT_STARTED / SCOPE_EPOCH_V1_EVIDENCE_PRESERVED / SCOPE_EPOCH_V2_MULTI_ASSET_4V_UNPROVEN
...
- M1.1 已建立 `SCOPE_EPOCH_V1_CRYPTO_3V` 的独立 GET-only/HTTPS allowlist Transport、Binance/OKX/Bybit catalog 与 mark-price Adapter、100% observed accounting、稳定 canonical identity、Point-in-Time `MARK_PRICE / MARK_PRICE_SNAPSHOT`、FactQuality 和 duplicate/out-of-order/gap/stale/recovery 门禁。价格数值不变但 Provider 快照序列前进仍是新事实；同序列才是 duplicate。该证据不能证明 Bitget、股票或上新生命周期。M0.4 已冻结 `SCOPE_EPOCH_V2_MULTI_ASSET_4V`、五个资产域和 T0-T3 采集层；实现下一入口为 M1.1A。
...
- 当前已实现来源仍是 Binance Futures、OKX Swap、Bybit Linear Perpetual 的公开目录、mark-price、Kline 和 WebSocket；Bitget、四 Venue listing lifecycle、OKX/Bybit/Bitget 股票永续仅进入设计目标，尚无 Adapter/运行证明，Binance 股票域保持 `UNVERIFIED_UNAVAILABLE`。
...
- 本包只完成设计权威，生产零变更；下一本地包为 M1.1A 四 Venue Capability Registry，当前 M3.4 草稿暂停等待 scope rebase。
