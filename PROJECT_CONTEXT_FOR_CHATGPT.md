...
- 2026-07-14 `WP-AUTO-02-G0-G8-STANDING-AUTHORITY-AND-EVIDENCE-HARDENING` 已进入最终门禁候选，生产未连接、未部署、未改变。用户已把全自动范围扩展为 G0-G8；直接长期授权合同为 `docs/governance/G0_G8_STANDING_AUTONOMY_AUTHORIZATION_V1.json`，只取消常规逐包等待，不取消 Gate 顺序、最长 90 分钟 mutation 窗口、逐包 commit/tree/diff/artifact/environment/identity/preflight/rollback/observation 绑定、仓库外生产租约、递增 fencing token、一次性消费、固定基础/安全门禁和真实观察时间。审核 Agent 只能只读找问题，不能授权。新执行蓝图为 `docs/blueprints/MARKET_RADAR_G0_G8_AUTONOMOUS_EXECUTION_BLUEPRINT_V1.md`，唯一核心和七段核心链路已进入合同及工作包校验；G9、自动交易、自动调权、未批准交易规则、RR/Risk Gate 放宽、future outcome 生产回写、formal 自动运行、观察窗口缩短、破坏性数据操作和未知批量清理继续禁止。代码审查额外关闭了生产授权缺失仍可能误判 `canAutoDeploy` 的 P0 控制缺口；现在授权必须绑定当前 commit/tree/policy/gate evidence，缺失时只允许只读门禁。攻击性测试 26/26 PASS；首轮完整门禁为自治 9/9、typecheck、lint、test:market 960 pass/0 fail/4 explicit skip、worker 23/23、historical 4/4、build、Golden 16/16 与三项安全检查全部 PASS，`worktreeUnchanged=true`。状态已切到 `ready_for_gate`，报告/状态冻结后的最终完整门禁仍必须再次 PASS 才能提交。当前生产事实和 G0 Scanner P1 均未因本控制包改变；控制包通过并提交后，唯一下一生产动作仍是 Scanner sustained-health 动态预检、Web+scanner-worker 精确重发和 1800 秒/至少两个 completion advances 观察。
