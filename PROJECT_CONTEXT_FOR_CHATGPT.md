...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.32。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.37。
...
- M1.1 已建立 `SCOPE_EPOCH_V1_CRYPTO_3V` 的三 Venue Identity/Fact；M1.1A 已把 Binance/OKX/Bybit/Bitget + CoinGlass Hobbyist 按 33 类能力形成 165 行穷举登记。M1.1B 已在独立 Scope V2 层实现 15 个精确只读探针、四 Venue catalog normalizer、Bitget、多资产身份、官方 underlying mapping、listing/identity epoch、Bybit/Bitget 公告和生命周期账本。M1.1B0 首次腾讯派发在业务 artifact 前阻断；R1 形成 0/15 共同传输失败。R2 精确提交 `d557c666e2e27b67842354b869a64271c91ceae1` 与派发 `m1b0-r2-live-source-20260723t165411z` 已真实取得 14/15，Identity/CoinGlass Gate PASS，Listing Gate 因唯一 Binance 现货目录超过 8 MiB 而 BLOCKED；生产和 secret 边界未变。R3 只采用官方 `showPermissionSets=false` 查询，现场响应从约 17,407,074 bytes 降为 6,629,806 bytes，保留所需 schema，不提高上限。R3 package 24/24、fixed dispatch 21/21、V2 Ops 125/125 和独立正确分支完整 `ci:production` PASS；提交和重派发待完成。Bybit 两页 `BOUNDED_COMPLETE` 与 M1.4B 历史 bootstrap/checkpoint/gap/incremental 继续分责，Bitget 官方一个月完整 cursor 不变。
...
- 首次腾讯派发在业务 artifact 前阻断；R1 形成 0/15 共同 transport failure。R2 派发 `m1b0-r2-live-source-20260723t165411z` 已形成 14/15：Identity/CoinGlass PASS，Listing 因唯一 Binance 现货目录失败而 BLOCKED；现场生产 HEAD、11 个容器、health、timer 与 secret 边界仍匹配基线。
...
- R2 已根治 Web Fetch/WebAssembly 共同传输缺陷；R3 进一步用官方 `showPermissionSets=false` 把 Binance 现货目录从约 17.4 MB 限定到 6,629,806 bytes，而不提高 8 MiB 上限。当前 R3 24/24、21/21、V2 Ops 125/125 和完整 CI PASS，提交和重派发待执行；只有新腾讯 exact-release 实际 PASS 的 capability 才可进入 M1.4B。
...
B1-B3 已关闭 V1 M1.5-B1；P0 已执行并因容量与 recovery evidence BLOCKED。Object Lock 31 天、age Keychain 身份、source=`bed938566d242394de7f6c31b309bd9f8198b71f` 和 exact staging 已通过；已覆盖旧 staging、16 个 `/dev/shm` 旧辅助/占位文件与诊断文件均已精确清理，clean pre-STS baseline 已复核通过。当前生产 P0R 下一步是 fresh exact-plan 7200 秒 STS 的即时 server-side compile、受限上传、加密备份、精确取回、隔离恢复和 cleanup；随后刷新 topology、重跑校准与 fresh P0，只有 PASS 才能进入 P1-P4/M1.7。签名通道不能运输 P0R secret。Scope V2 的 M1.1B0 已从 R1 0/15 推进到 R2 14/15；R3 Binance 现货有界查询当前等待完整 CI、exact commit/push、fresh 生产身份绑定与腾讯 `LIVE_READ_ONLY` 重派发。M1.4B 只允许接入 live B0 实际 PASS 的 capability，并独立完成 Bybit listing 历史回填；Bitget、Listing Lifecycle、股票 Asset Domain 的证据不得混用。M3.4 草稿继续暂停；外部门 B0.2-B 未解决前 historical bulk、真实 cohort、holdout、Detector lifecycle 和 runtime 一律关闭。
