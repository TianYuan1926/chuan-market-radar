...
- M1.6 v1 日分区迁移 checksum 保持不可变；additive v2 六小时分区、无扩容 59%/67% 模型、Object Lock `COMPLIANCE` 31 天、Keychain age 身份、atomic remote session、private `/dev/shm` runner boundary 和 fresh rebind request v3 七文件合同均已形成。exact source `44e51823f11ad81cbabc632256f0402ef2d00c29` 已取得 Signed Dispatch `30474274621`、A0 `30474274716`、Full Quality `30474274718`、Independent Security `30474274719` 四门 PASS，并由 rebind `p0r-rebind-preflight-20260729t172500z-a32c3e9b` 证明 production HEAD `cec0b6572bb09ae91ff9e013f8bb160f73c045e2`、clean worktree、11 containers、P0R files/containers/volumes=0 和 health/timer 零漂移。该 source 的 run `p0r-20260729t174950z-83b68bfeebff953f49873bccf12cdacf`、plan digest `sha256:6c9909788fd0e19abffe5b206f7383412cfdc3e546f97d65f90a23d4d2406fdb`、bundle `8d8f80f8583b2608031f0883627e01ad6f5a411b4dd7843d42abb8dceb6954d5` 和独立本地 package acceptance 均 PASS，但 fixed bridge 仍默认 port 22；真实网络 A/B 随后证明当前 BoostNet SOCKS 在 SSH banner 前阻断 22。该 package 因 bridge schema v2 根治而失去执行权，未签发 STS、未触碰 COS 或数据库。当前已用 7200 秒、ubuntu public-key-only、禁止 forwarding 的独立 sshd 和唯一 `157.254.154.223/32 -> TCP 8022` 腾讯规则完成 strict known-host `ubuntu@VM-0-9-ubuntu` 预秘密握手；业务服务、数据库、Redis、仓库、env、migration 和 authority 未改变。bridge v2 固定 `8022` 与 `HostKeyAlias=43.161.202.227`；bridge 6/6、P0R 88/88、recurrence 11/11、dispatch 24/24、V2 Foundation 631 PASS / 6 explicit skips、V2 Ops 210/210、Next build、Golden 16/16 和 security 的完整本地生产 CI 均 PASS。新 clean exact commit、四门、fresh rebind、新 run/plan/bundle 和真实 recovery 仍待完成。临时 listener 与云规则必须在成功、失败或超时后显式删除并验证。
- fresh rebind request v3/result v2 已把历史 transport v1 三文件替代比较与当前 transport v2 七文件运行资格分离；七文件集包含 runner 和 atomic session helper，缺任一摘要 fail closed。该合同在 source `44e518...` 已取得远端四门与生产 rebind 历史证明，但当前 bridge v2 新 source 必须重新取得同等级资格。
...
工程描述：可运行但不完整
实战描述：不能支撑实战
V2：M0.5 已把 Market Mechanics、Microstructure、双向前兆、板块传播、补充来源、缓存和 Evidence Overlay 融入权威设计；M1.4C、M1.4D、M1.5C/M1.5D 本地包与 M2.1A 均通过各自本地出口，但真实 live、cohort/holdout、校准和 authority 仍未完成。P0R source `44e518...` 已取得本地资格、GitHub 四门、fresh rebind 和新 exact package；随后真实网络 A/B 证明 fixed bridge 的默认 port 22 无法通过当前 BoostNet SOCKS，故该 package 未签发 STS并被 bridge v2 新 source 取代。受限 8022 listener、当前代理出口 `/32` 腾讯规则与 strict host-key/public-key `ubuntu` 握手已 PASS；bridge 6/6、P0R 88/88、recurrence 11/11、dispatch 24/24 和完整本地生产 CI 已 PASS，但新 commit、四门、rebind、新 execution identity、真实恢复和临时网络清理仍待完成。M1 未完成，Detector=DRAFT、Candidate 禁发，无 Signal Grade/Strategy/Feasibility/持续 runtime/READY authority
...
当前生产存储门禁：P0_BLOCKED_CAPACITY_AND_RECOVERY；P0R_OBJECT_LOCK_31D_AGE_VAULT_PASS_SOURCE_44E_FOUR_GATES_REBIND_PACKAGE_HISTORICAL_PASS_SUPERSEDED_BY_BRIDGE_V2_8022_ROUTE_PRESECRET_ACCEPTANCE_PASS_NEW_EXACT_SOURCE_AND_REAL_RECOVERY_PENDING；没有可用 STS、backup、restore 或 fresh P0
固定生产执行通道：PRODUCTION_FIXED_DISPATCH_TIMEOUT_LOCK_RECOVERY_PASS；机器复发门禁 4 项 CLOSED、1 项 P0R secret transport REMEDIATION_IN_PROGRESS，旧 raw/tee/manual-compile、browser-state recovery、OrcaTerm secret entry、default SSH port 22、approved_orcaterm_bundle_upload 和超长复合命令均禁止复用
```
...
## 18. 当前执行入口与关键外部门
```text
V2-M1.6-P0R-B6-PROXY-COMPATIBLE-8022-TTY-BRIDGE
```
B1-B3 已关闭 V1 M1.5-B1；P0 已执行并因容量与 recovery evidence BLOCKED。Object Lock 31 天和 age Keychain 身份仍有效。`44e518...` 的四门、fresh rebind 和 exact package 是可信历史资格，但默认 port 22 被当前 BoostNet SOCKS 真实阻断，故 package 未收取 STS并由 bridge v2 根治取代。当前 8022 transient sshd、当前代理出口 `/32` 云规则、strict known-host/public-key `ubuntu` 握手已 PASS；bridge 6/6、P0R 88/88、recurrence 11/11、dispatch 24/24 和完整本地生产 CI 已 PASS，下一步是 clean exact commit、GitHub 四门、fresh rebind 和全新 execution identity。只有新 source 下的 bridge 在签发前取得 echo-disabled READY，用户才可完成 MFA 和 API Explorer 原生 Copy；之后 bridge 自动完成即时编译、Keychain age handoff和 Runner。response 后禁止 browser state/AX/OCR/screenshot/computer-use/OrcaTerm secret entry。无论结果如何都必须删除 8022 listener 与云规则并验证不存在。只有 backup、exact retrieval、isolated PG16 restore、cleanup、零漂移、fresh topology、校准和 fresh P0 全部 PASS 才能继续 P1。禁止修改 exact policy、Region、duration、source IP、resource 或 action，禁止签名通道运输 P0R secret。Scope V2 的 C1、M1.4B/M1.4C、M2.1A 和 A0 子门证据仍有效但无 runtime authority；A0 总门禁唯一剩余控制仍是 P0R 真实恢复。外部门 B0.2-B 未解决前 historical bulk、Detector lifecycle 和 runtime 一律关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
