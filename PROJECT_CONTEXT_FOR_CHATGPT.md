...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.31。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.36。
...
- M1.1 已建立 `SCOPE_EPOCH_V1_CRYPTO_3V` 的三 Venue Identity/Fact；M1.1A 已把 Binance/OKX/Bybit/Bitget + CoinGlass Hobbyist 按 33 类能力形成 165 行穷举登记。M1.1B 已在独立 Scope V2 层实现 15 个精确只读探针、四 Venue catalog normalizer、Bitget、多资产身份、官方 underlying mapping、listing/identity epoch、Bybit/Bitget 公告和生命周期账本。M1.1B0 首次腾讯派发在业务 artifact 前阻断且不计 live B0；R1 形成 0/15 PASS 的共同 `TRANSPORT_FAILURE_UNAVAILABLE` artifact/result，现场证明固定 Node `--jitless` 的 Web Fetch 因 `WebAssembly is not defined` 失败而 Node core HTTPS 返回 200。R2 保留安全硬化，以 TLS 验证、exact-host、无重定向、12 秒超时、8 MiB 上限的 `https.request` 作为 live 默认传输；Fetch 只保留 TEST_ONLY。Bybit 最新两页 `BOUNDED_COMPLETE` 与 M1.4B 历史 bootstrap/checkpoint/gap/incremental 继续分责，Bitget 官方一个月完整 cursor 不变。R2 package 24/24、fixed dispatch 21/21、V2 Foundation 422/428（6 项明确跳过）、V2 Ops 125/125、M0、Next build、Golden 16/16、安全扫描与完整 CI PASS；精确提交与腾讯新派发待执行。
...
- M1.4 已建立 21 observed / 15 eligible 的 V1 三 Venue fixture、reconciliation、quota/backpressure、故障恢复与 PG16 原子持久化。M1.4A 已把 Bitget、listing watch、股票独立资产域和 CoinGlass Hobbyist 纳入 Scope V2 T0-T3 有界调度合同：live B0、人工 rights、entitlement、地区、quota、checkpoint、baseline reserve、fairness 和 matched control 均 fail closed，五组输入内容寻址，400 subject 全量核算与定向 28/28、全 V2 424 项、ops 115 项、Next build、Golden 16/16 和 security PASS；runtime/Fact/Candidate/Strategy/READY authority 全为 false。
...
- 当前有生产运行证明的来源仍是 Binance Futures、OKX Swap、Bybit Linear Perpetual 的公开目录、mark-price、Kline 和 WebSocket。Bitget、四 Venue listing lifecycle 与股票/RWA 合约已进入 Scope V2 能力登记、本地 Adapter、身份和生命周期合同，但尚无腾讯 `LIVE_READ_ONLY` B0、Collector 或生产运行证明；Binance 股票产品已由官方资料确认，不能继续标为产品不可用。
...
工程描述：可运行但不完整
实战描述：不能支撑实战
V2：M1.1A 能力登记和 M1.1B 四 Venue/Bitget/多资产身份/上新生命周期本地实现已通过，探针证据仍未取得 live PASS；M1.1B0 首次腾讯派发已阻断且不计数，R1 形成 0/15 PASS 的共同运行时传输失败证据，R2 Node core HTTPS 已通过 24/24、21/21、V2 Foundation 422/428（6 项明确跳过）、V2 Ops 125/125、M0、Next build、Golden 16/16、安全扫描与完整 CI；精确提交与腾讯新派发仍未完成。M1.4A 已完成 28 项定向合同与独立 clone 完整 CI，仍无 runtime authority。Bitget Venue、Listing Lifecycle、股票 Asset Domain 分别核算；`SCOPE_EPOCH_V2_MULTI_ASSET_4V` 的 M1.4B Adapter/历史回填、Shadow、容量、校准和生产 authority 未开始。原 M1.1-M1.6、B1-B3、C1、M3.0-M3.3 只保留 V1 效力，B1-B1 永久不计。P0R clean pre-STS baseline 已通过但 STS/COS 恢复/fresh topology/P0 未执行，M1 未完成；历史 cohort Gate=INSUFFICIENT、Detector=DRAFT、Candidate 禁发、M3.4 草稿待 scope rebase，无 Feasibility/runtime/READY authority
...
- 首次腾讯派发 `m1b0-live-source-20260723t141526z` 在业务 artifact 前阻断；R1 派发 `m1b0-r1-live-source-20260723t155239z` 形成 0/15 PASS 的共同 transport failure artifact/result。两次均不计 live PASS，现场生产 HEAD、11 个容器、health、timer 与 CoinGlass 文件边界仍匹配基线。
...
- 根因是固定 Node `--jitless` 下 Web Fetch 依赖不可用 WebAssembly；R2 保留安全硬化并改用 Node core HTTPS，当前 package 24/24、fixed dispatch 21/21、V2 Foundation 422/428（6 项明确跳过）、V2 Ops 125/125 与完整 CI PASS，exact commit/新派发待执行；只有新腾讯 exact-release 15/15 PASS 的 capability 才可进入 M1.4B。
...
- Bitget、listing watch、股票类合约和 CoinGlass Hobbyist 已进入同一 T0-T3 有界调度，但仍按 Venue、assetDomain、rights、entitlement、quota 和 live evidence 分开核算。
- 400 subject 四 Venue全量 T0/T1 accounting、baseline reserve、matched control、429/backoff/circuit、股票 session/corporate-action block 与五组输入 lineage 已通过 28/28 定向测试。
- 本包不访问网络、不写 Fact、不启动 Worker；独立 Git clone 完整 CI 已通过，live B0、M1.4B Adapter、Shadow、容量和生产 authority 均未完成。
...
- M3.0-M3.3 只验证 V1 三 Venue加密 fixture 的决策、六族解释、双评级和 test-only Strategy；M1.1B 的 Bitget/股票/上新到本地身份与生命周期，M1.4A 到无 runtime authority 调度合同，旧 31 周期与容量不能证明 V2 扩展。真实 Adapter、Deep Validation、分域 calibration/holdout、执行成本、风险和 runtime 缺失，任何 V2 READY 声明均为 P1 风险。
...
2. `scopeEpoch` 是否覆盖所有权威对象；V1 C1/B1-B3/D0 是否保持原范围，M1.1B TEST_ONLY 是否未冒充 live；M1.4A 是否只生成 no-authority intent，Bitget/股票/上新是否仅在腾讯 B0 实际 PASS、人工 rights 和 live quota 后进入 M1.4B Adapter，任何旧 PASS 都不得改名为 V2 PASS。
...
B1-B3 已关闭 V1 M1.5-B1；P0 已执行并因容量与 recovery evidence BLOCKED。Object Lock 31 天、age Keychain 身份、source=`bed938566d242394de7f6c31b309bd9f8198b71f` 和 exact staging 已通过；已覆盖旧 staging、16 个 `/dev/shm` 旧辅助/占位文件与诊断文件均已精确清理，clean pre-STS baseline 已复核通过。当前生产 P0R 下一步是 fresh exact-plan 7200 秒 STS 的即时 server-side compile、受限上传、加密备份、精确取回、隔离恢复和 cleanup；随后刷新 topology、重跑校准与 fresh P0，只有 PASS 才能进入 P1-P4/M1.7。签名通道不能运输 P0R secret。Scope V2 的 M1.1B0 前两次派发真实阻断且不计数；R1 已证明 0/15 共同运行时传输失败，R2 Node core HTTPS 已通过独立完整 CI，下一步是 exact commit/push、fresh 生产身份绑定与腾讯 `LIVE_READ_ONLY` 新派发。M1.4B 只允许接入 live B0 实际 PASS 的 capability，并独立完成 Bybit listing 历史回填；Bitget、Listing Lifecycle、股票 Asset Domain 的证据不得混用。M3.4 草稿继续暂停；外部门 B0.2-B 未解决前 historical bulk、真实 cohort、holdout、Detector lifecycle 和 runtime 一律关闭。
