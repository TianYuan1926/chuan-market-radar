...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.86。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.91。
...
M1.6P0R_B8_EXACT_TARGET_ACCEPTANCE_HISTORICAL_PASS / M1.6P0R_B9_R1_COS_CAM_ACTION_ROOT_REMEDIATION_HISTORICAL_PASS / M1.6P0R_B9_R2_TWO_CLIPBOARD_TIMEOUTS_LATE_RESPONSE_INVALIDATED_DYNAMIC_EGRESS_DRIFT / LOCAL_BRIDGE_ZERO_AND_CLIPBOARD_CLEARED / CLOUD_LISTENER_FIREWALL_SERVER_RESIDUE_AND_PRODUCTION_ZERO_DRIFT_UNVERIFIED / ROUTE_AUTHORITY_PRODUCER_TRANSPORT_V4_P0R_139_OF_139_EXACT_MAIN_BRANCH_FULL_CI_AND_CURRENT_SOURCE_REMOTE_FOUR_GATE_PASS / FINAL_HEAD_QUALIFICATION_REBIND_AND_REAL_RECOVERY_PENDING
...
- M1.6 v1 日分区迁移 checksum 保持不可变；additive v2 六小时分区、无扩容 59%/67% 模型、Object Lock `COMPLIANCE` 31 天、Keychain age 身份和 private `/dev/shm` runner boundary 均已形成。B8 target、B9 COS CAM action 和 route evidence v1 资格只保留历史证据。2026-08-02 两个 bridge window 都在第一条 TTY READY 后 540 秒超时，后到 response 作废且未进入 Bridge 或服务器；随后双端点 egress 一致变为 `156.248.15.36`，strict 8022 不可达。当前只证明本机 Bridge=0 与 clipboard 已覆盖；云 listener、旧 `/32`、服务器 residue 和生产零漂移未复核。route authority implementation `157e9a79...` 已新增 route target、Tencent firewall capture、strict listener observer、authoritative producer 与 route evidence v2。远端首轮安全资格经 `8059edf...` 的有界脱敏 annotation 暴露真实 `js/file-system-race`，run `30725896115` 保持 FAIL；`913bae3a...` 随后以 descriptor-first `O_NOFOLLOW` 读取根除 pre-open lstat 竞争窗口，没有 suppression。当前定向 `24/24`、P0R `139/139`、exact-toolchain launcher `3/3`、V2 Ops `266/266`、完整本地 CI 和修复 source GitHub 四门全部 PASS；候选 branch-identity FAIL 与 CodeQL FAIL 均保留。生产仍未改变，fresh rebind、外部清场和 corrected real-target recovery 仍待完成。
