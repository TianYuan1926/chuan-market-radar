...
- `docs/blueprints/MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md`，内容版本 v1.66。
- `docs/blueprints/market-radar-v2-controlled-replacement-traceability.v1.json`，机器合同 v1.72。
...
工程描述：可运行但不完整
实战描述：不能支撑实战
V2：M0.5 已把 Market Mechanics、Microstructure、双向前兆、板块传播、补充来源、缓存和 Evidence Overlay 融入权威设计；M1.4C、M1.4D、M1.5C/M1.5D 本地包与 M2.1A 均通过各自本地出口，但真实 live、cohort/holdout、校准和 authority 仍未完成。P0R source `e362...` 曾完成本地 CI、GitHub 四门、fresh rebind 和新 staging，但第三枚 STS response 在 browser-state recovery 中进入工具输出，故 credential 与 run/plan/object key/bundle/staging 永久失去执行权。该 STS 未进入服务器或 COS；生产 P0R files/processes/containers/volumes 均复核为 0，双时钟已证明其过期且永久禁用。新 pre-armed local TTY bridge 和 echo-before-ready session 已通过伪 TTY 10/10、P0R 87/87、recurrence 10/10、dispatch 24/24 与完整 `ci:production`；新 clean exact source、四门、fresh rebind、新 execution identity 和真实恢复仍待完成。M1 未完成，Detector=DRAFT、Candidate 禁发，无 Signal Grade/Strategy/Feasibility/持续 runtime/READY authority
...
- 新 pre-armed local TTY bridge 固定 SSH identity/host-key/proxy/两个远端命令和 Node `v22.23.1`，只在 echo-disabled READY 后接受 API 原生 Copy，先清 clipboard 再 handoff，并从固定 Keychain 内部取 age identity。伪 TTY 10/10、P0R 87/87 与完整 `ci:production` PASS。replacement source `38b49...` 的 A0、Signed Dispatch 与 Security PASS，Full Quality 因 Ubuntu 未安装 `/usr/bin/expect` 而真实 FAIL，不能写成四门通过；当前已精确安装 Noble `expect/tcl-expect 5.45.4-3`，以材料门禁和回归锁定且本地完整 CI PASS，等待新 exact source 远端复验。
...
- 裸 `tee`、raw response 落盘、manual compile、caller-supplied clock override、AX response reconstruction 与 Compose env 重插值永久退役；本地 no-echo atomic session 已通过 P0R 81/81、Go helper、recurrence 10/10 和 dispatch 24/24，fresh rebind v2 另行绑定当前七文件运行集。
