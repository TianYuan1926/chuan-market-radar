...
- 2026-07-19 Cycle-6 observer 已在第 42 条采样 **FAIL 并自动回滚 PASS**；此前“observer active/running”的条目均为失败前历史。41 条已接受样本、12,166 秒和 5,218 次真实写入保留为失败证据，禁止复用或重标 PASS；失败原因是相邻样本之间短暂出现 `observation_unresolved_outbox`，不是 24 小时/289 样本/10,000 writes 出口通过。回滚后生产保持 clean detached commit=`72ee289388eea922d0aee58fd4ec7a3f18a91007`、tree=`bb1492d5a3c79a75c79dfa392dd9a7c2d185f70d`、Cycle-6=`legacy/frozen/epoch2`、Candidate Worker absent；600 episodes、5,218 events、10,484 outbox，Legacy lane completed=5,218/pending=48/unresolved=48，Candidate event mirror pending=5,218/non-pending=0/orphan=0/mismatch=0。`WP-G0.2-CYCLE-6-RECOVERY-AND-CYCLE-7-CONTINUATION-SUPERPACKAGE` 已完成本地实现、41/41 continuation tests、Drain 14/14、production drain 23/23、三套 PostgreSQL 16 成功/失败/相邻周期演练、Autonomy 31/31、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁；formal 未运行且禁止，生产尚未执行。本地包要求先精确 drain 48 条 Legacy pending，形成 events=5,266/outbox=10,532、Legacy unresolved=0、Candidate event pending=5,266、Cycle-6=`legacy/frozen/epoch4` 的干净基线，再以全新 request 启动 Cycle-7；observer 仅对精确 `observation_unresolved_outbox` 每 5 秒重查、最多 45 秒，其他错误立即失败。G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
