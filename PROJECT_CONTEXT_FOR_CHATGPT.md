...
- 2026-07-16 `WP-G0.2-SHADOW-CAPTURE-ACTIVATE-AND-OBSERVE` 已有两次精确生产事务在 mutation 前失败，不能写 Activation PASS。第一次绑定 source=`d07bfe37...`，database control 容器因 Web 镜像默认用户无法穿透 root-owned `0700` staging 挂载而返回 `MODULE_NOT_FOUND`；增加 staging owner UID/GID 后，第二次绑定 source=`abcc34c8f4c1dff86d960afcc24d641183a4b9aa`、Bundle=`27b64ac420a0f588fe37e416cdd031c917ddf5e86dd1072c233d3661100e3612`、request=`572c4f378087947b0f971dfb2542dfbf7b0d31ad1c77f8cf127e2fc35b24c44f`，runner 已可读取私有挂载，但 ESM 从 staging 路径解析裸 `pg` 时看不到已批准 Web 镜像的 `/app/node_modules`，control preflight 返回 `unexpected_error`。两次均发生在 lease acquire、Git checkout、control start、env switch 和 service recreate 前；第二次独立复核确认生产仍为 clean detached `cec0b657...`、`.env.production` SHA-256=`61c27fba...`、Web/Postgres/Redis healthy、Candidate worker absent，staging/secure/ops 已清理，两个旧 Bundle/request 均已消费且禁止复用。当前最小修复复用既有 Runtime Identity 模式：只给 database control 容器显式绑定 `MARKET_RADAR_APPLICATION_ROOT=/app`，runner 通过批准的 application root 加载 `pg`，缺失时返回稳定 `approved_pg_runtime_unavailable`；继续保留 read-only、cap-drop、no-new-privileges、staging UID/GID 和精确挂载。Activation 26/26、真实 PG16 migration 1-9/control start/rollback、typecheck、lint、market 960/0/4 explicit skip、workers 23/23、historical 4/4、build、Golden 16/16、Autonomy 31/31 与三项安全门禁均 PASS，且 PG16 `productionConnected=false`；current runner artifact=`9fd66066...`、activation artifact=`c6518ece...`、contract=`9180e26d...`。clean commit 已创建；提交后 commit-bound gate、main 推送和第三个全新精确 Bundle/request 尚待完成；Candidate 仍 dormant，WP-G0.2/G0 未完成。
