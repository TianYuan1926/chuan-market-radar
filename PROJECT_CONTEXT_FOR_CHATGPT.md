...
source `dd67d819...` 已从零取得 Signed Dispatch `30734390429`、Full Quality `30734390438`、A0 `30734390439`、Independent Security `30734390634` 四门，并在旧 exact 批准窗口内发布；目标以 stderr digest `sha256:d41cd1e0993f4f20b1024ac7e1c9ba17ea70cd9a1f0f5ec61fd96523bde9701f` 唯一解码为 `evidence_gateway_health_not_ready`。真实 API 是顶层 `health`，旧 parser 与 fixture 同时错成 `data.health`，测试因此没有覆盖真实生产合同。现在真实 envelope 为绿例、obsolete nested envelope 为 pre-mutation 红例；gateway `8/8`、recurrence `11/11`、dispatch `37/37`、Foundation `638 PASS / 6 skip`、Ops `275/275`、M0 `12/12`、Next build、Golden `16/16` 与 security 已在 exact full CI 中 PASS。本机没有 Docker/Caddy CLI，真实 syntax/runtime validation 仍必须在 production mutation 前使用当前生产 Caddy image、network-none/read-only 隔离执行。固定顺序调整为 `clean commit/push -> 同一 commit 四门 -> fresh production baseline -> 新 deterministic package -> 新 exact approval -> current-image 隔离 validate -> Caddy-only deploy/自动 baseline rollback -> 自动取回/解密/验签 -> fresh rebind -> 新 P0R identity/package -> backup/exact retrieval/isolated restore/cleanup -> fresh health/topology/calibration/P0`。P0R 与 fresh P0 未 PASS 前，P1、M2 runtime、页面交易计划和任何 READY authority 继续关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
