...
第一次最新字节完整 CI 已真实在 recurrence authority `10/11` 处失败，暴露三份活跃资料仍把旧 route authority 写成当前入口；同步修复已加入“新入口唯一、旧入口仅历史上游”的防复发断言。Caddyfile 变更触发 Legacy consumer map 与 reviewed commit fail closed，重新审查后 22 capabilities、539 source files、273 runtime edges 全部不变，仅受影响 capability digest 更新；M0 入口校验改为里程碑合同加精确 production-control 白名单并拒绝任意 ID、Legacy ID 和换行注入。修复后完整 `ci:production` 从头 PASS：recurrence `11/11`、dispatch `37/37`、V2 Foundation `638 PASS / 6 explicit skips`、V2 Ops `274/274`、M0 `12/12`、Next build、Golden `16/16` 和 security；clean commits 为 `1ddc0fdc...` 与 `35d7b63a...`。本机没有 Docker/Caddy CLI，真实 syntax/runtime validation 必须在 production mutation 前使用当前生产 Caddy image、network-none/read-only 隔离执行。之后固定顺序是 `GitHub 四门 -> 确定性 Caddy-only package -> 精确 production mutation 批准 -> current-image 隔离 validate -> 只重建 Caddy或自动 baseline rollback -> 本机自动取回/解密/验签 -> fresh read-only rebind 与零漂移 -> 新 P0R identity/package -> backup/exact retrieval/isolated restore/cleanup -> fresh health/topology/calibration/P0`。P0R 与 fresh P0 未 PASS 前，P1、M2 runtime、页面交易计划和任何 READY authority 继续关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
