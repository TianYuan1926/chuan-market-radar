...
- 当前状态：已有 token dossier 和 strategyV3 输出基础；狙击榜原则上只允许 `TRADE_PLAN_READY`
...
| `/signals` | 候选验证台 | `getRadarContractForPage`、`getLeaderboardContractForPage` | 信号成熟度池、狙击榜、异动候选明细 | 轻扫标记不能进主信号区；WAIT 不能进狙击榜 | 是，尤其候选/信号/计划边界 | 可访问；需继续审计展示是否强于后端 |
...
- WAIT / WATCH 不得进入狙击榜。
- 轻扫标记不允许附带完整交易计划。
- 深扫候选可以展示“验证中”，但不能包装成“狙击目标”。
- `TRADE_PLAN_READY` 才允许进入狙击榜。
...
| `npm run backtest:formal` | 正式能力验收回测 | 只在能力验收轮跑 | 否 | 已运行，但业务能力不通过 | 第三轮证据：exit=2；程序完成，裁判发现高优先级能力阻断 |
...
- 当前是否已实现：本文未确认有完整自动发布闭环。
- 还差什么：需要确认 CI 是否能自动跑测试、构建、部署到腾讯云，并且不暴露 secret。
- 是否还依赖手动执行：是，当前仍有手动同步/OrcaTerm 操作痕迹。
...
- 当前最大短板：
  1. 第三轮 formal 中 `TRADE_PLAN_READY=0`，第四轮已专项补强策略计划表达、WAIT 结构化字段和 WAIT 后验诊断，但尚未跑 formal 验收。
  2. 第三轮 formal 中 WAIT 有效率为 `0%`，触发后先止损 2 个；第四轮已把 WAIT 失败拆成触发、结构失效、止损、目标、二次确认等诊断。
  3. 分析判断有效率不足：第三轮 formal 中被选中节点真正不晚到且事后有效比例为 `23.81%`。
  4. 扫描提前发现能力不足：第三轮 formal 中结构可行动机会 TopN 捕获率为 `32.31%`，启动前捕获率为 `17.78%`。
  5. 第四轮没有改扫描和分析主干，所以扫描/分析是否改善仍需要后续专门验收。
