...
M1.6P0R_ROUTE_AUTHORITY_UPSTREAM_QUALIFIED / PRODUCTION_EVIDENCE_GATEWAY_HEALTH_FIX_COMMITTED / AUTHORITY_SYNC_SOURCE_REMOTE_3_OF_4_INDEPENDENT_SECURITY_FAIL / PUBLIC_KEY_FINGERPRINT_ARTIFACT_ROOT_REMEDIATION_COMMIT_8742828_CREATED / AUTHORITY_SYNC_FINAL_BYTES_FULL_LOCAL_CI_PASS / AUTHORITY_SYNC_PUSH_REPLACEMENT_SOURCE_REMOTE_FOUR_GATES_EXACT_PRODUCTION_APPROVAL_REAL_CADDY_VALIDATION_BOOTSTRAP_RETRIEVAL_FRESH_REBIND_AND_REAL_RECOVERY_PENDING / PRODUCTION_INTENTIONAL_MUTATION_NONE / FAILED_GATEWAY_ATTEMPT_ONLY_ZERO_DRIFT_PASS
...
source `dd67d819...` 的旧四门、旧批准和失败 dispatch 只保留历史效力；clean health remediation `51886b0...` 已解决真实 API 顶层 `health` 合同。当前 authority-sync source parts `d1c3d987cf55105e94e8 + 15992dbb989ea1045629` 远端资格为 `3/4`，Independent Security `30739919662` 的唯一 finding 是历史 generated approval request 中固定 X25519 公钥指纹被命名为 `evidenceRecipientKeySha256`。永久修复使用 `evidenceRecipientFingerprintSha256`、严格拒绝旧字段，并在 signed dispatch 发布前阻断歧义高熵非 public `*KeySha256`；exact finding 已结构化登记。clean implementation commit `8742828086041da1db51a5abeeef01ba7656e5fd` 已形成，recurrence `11/11`、dispatch `38/38`、gateway `8/8` 合计 `57/57`，最终 authority-sync 字节完整本地 CI 已 PASS。authority-sync commit/push 与 replacement 同源四门仍待完成。本机没有 Docker/Caddy CLI，真实 syntax/runtime validation 仍必须在 production mutation 前使用当前生产 Caddy image、network-none/read-only 隔离执行。固定顺序调整为 `提交并推送 authority-sync exact source -> 同源四门从零 PASS -> fresh production baseline -> 新 deterministic package -> 新 exact approval -> current-image 隔离 validate -> Caddy-only deploy/自动 baseline rollback -> 自动取回/解密/验签 -> fresh rebind -> 新 P0R identity/package -> backup/exact retrieval/isolated restore/cleanup -> fresh health/topology/calibration/P0`。生产未改变；P0R 与 fresh P0 未 PASS 前，P1、M2 runtime、页面交易计划和任何 READY authority 继续关闭。
## 19. 活跃记忆维护规则
- 本文件最多 400 行，只保留当前事实。
