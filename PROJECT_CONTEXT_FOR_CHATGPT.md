...
- 2026-07-18 Cycle-3 生产统一观察的真实结局是 **observer FAIL 后自动回滚 PASS**，不是观察通过。观察器在第 47 个样本（completed=3,705、source unresolved=0、health ready/fresh、Candidate worker healthy）把 38 条仅 29.526 秒的正常 claimed 在途任务误判为 `sample_monitor_unresolved`；随后自动恢复 clean detached baseline commit=`cec0b6572bb09ae91ff9e013f8bb160f73c045e2`、tree=`eb217a7fbaad5b464279a08d4441a8249fc266e3`、旧 Web image=`sha256:cd3652c1e72c8aabea87cee11233fb662a9209187435c14107f3da6426ba9efd`，Candidate Worker absent，Cycle-3=`legacy/frozen/epoch2`，577 episodes、3,705 events 和 7,410 outbox 全部保留，fencing token 22 已释放。当前本地 Cycle-4 最小修复包只允许 monitor ready、零 blocker/warning、零 retry/quarantine 且 `unresolved=pending+claimed+retry+unresolvedQuarantine` 时接受小于既有 300 秒阈值的短暂在途；算术漂移、300 秒及以上老化、retry 或 quarantine 仍失败关闭。定向 21/21、Production Packet 34/34、自治 31/31、PostgreSQL 16 Cycle-4 隔离演练、typecheck、lint、market 1,027/0/7、workers 23/23、historical 4/4、build、Golden 16/16 和三项安全门禁均 PASS；formal 未运行。该包目前仅为 `ready_for_gate`，尚未 clean commit、提交绑定总门禁或部署；旧 47 样本禁止复用，Cycle-4 必须从零完成 10,000 writes、1,800 秒/7 样本/2 次推进和 24 小时/289 样本双门禁。G0 主步骤仍为 7，系统仍为 `R1 / 可运行但不完整 / 不能支撑实战`。
