...
test("runPersistenceSchemaMigration sends one schema statement per query for Neon-compatible prepared execution", async () => {
  ...
  assert.match(client.calls[3]?.sql ?? "", /^create table if not exists scan_archives/i);
  assert.match(client.calls[4]?.sql ?? "", /^create index if not exists scan_archives_scope_generated_at_idx/i);
  assert.match(client.calls[5]?.sql ?? "", /^create table if not exists rank_profiles/i);
