...
export function calculateTrendHoldScore(evidence: EvidenceItem[]): ScoreResult {
  const drivers = evidence.filter((item) => /trend|pullback|support|vwap|ema/i.test(item.label) && item.direction !== "RISK");
