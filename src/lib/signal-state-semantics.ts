...
export const STATE_DEFINITION_MAP: Record<CoreSemanticState, StateDefinition> = {
  ...
  CANDIDATE: {
    ...
    boundary: "CANDIDATE 只说明值得继续验证，不是信号，更不是交易计划。",
    canTrade: false,
    canDisplay: true,
    canEnterSniper: false,
    canAttachTradePlan: false,
    canUpgrade: true,
    upgradeRule: "必须完成深扫验证、盘面结构、证据融合、RR 和风控门禁。",
    lane: "validate",
    laneLabel: "验证中",
  ...
  SIGNAL: {
    ...
    boundary: "SIGNAL 是用户可读归类，不是交易执行权限。",
    canTrade: false,
    canDisplay: true,
    canEnterSniper: false,
    canAttachTradePlan: false,
    canUpgrade: true,
    upgradeRule: "必须变成 TRADE_PLAN_READY 后才能附带完整策略计划。",
    lane: "watch",
    laneLabel: "重点观察",
    misleadingRisk: "中文“信号”天然容易被理解成买卖点，必须配套不可交易说明。",
  },
  EVIDENCE_SIGNAL: {
    ...
    boundary: "EVIDENCE_SIGNAL 不能附带完整交易计划，不能进入狙击榜。",
    canTrade: false,
    canDisplay: true,
    canEnterSniper: false,
    canAttachTradePlan: false,
    canUpgrade: true,
    upgradeRule: "必须由后端确认 RR、风控、结构、触发、失效条件全部满足。",
    lane: "watch",
    laneLabel: "重点观察",
    misleadingRisk: "被误读为已经可买卖的交易信号。",
  },
  TRADE_PLAN_READY: {
    ...
    boundary: "这是唯一可进入狙击榜的状态；仍只供人工复核，不自动下单。",
    canTrade: true,
    canDisplay: true,
    canEnterSniper: true,
    ...
    lane: "sniper",
    laneLabel: "狙击榜",
    misleadingRisk: "若没有完整计划却显示 READY，就是 P0 误导。",
  },
  BLOCKED: {
    state: "BLOCKED",
    label: "风控拦截",
    short: "拦截",
    userLabel: "不可交易",
    definition: "被风控、RR、结构、多周期、数据质量、晚到或失效条件拦截。",
...
export const MATURITY_DISPLAY_META: Record<DisplaySignalMaturity, MaturityDisplayMeta> = {
  ...
  EVIDENCE_SIGNAL: {
    ...
    userLabel: "证据观察",
    boundary: "可进主信号区，但不能进狙击榜，不能附带完整计划。",
    canTrade: false,
    canEnterSniper: false,
    canAttachTradePlan: false,
  },
