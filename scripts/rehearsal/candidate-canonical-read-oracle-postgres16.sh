...
WORK_DIR="$(mktemp -d /tmp/market-radar-candidate-oracle-pg16.XXXXXX)"
DATA_DIR="${WORK_DIR}/data"
SOCKET_DIR="$(mktemp -d /tmp/wp_g0_2_rehearsal_candidate_oracle_socket.XXXXXX)"
PORT="$((57332 + RANDOM % 300))"
DATABASE="wp_g0_2_rehearsal_candidate_canonical_oracle"
...
env -u DATABASE_URL -u POSTGRES_URL \
  APP_ENV=rehearsal NODE_ENV=test WP_G0_2_REHEARSAL=true \
  WP_G0_2_CANONICAL_ORACLE_REHEARSAL_DATABASE_URL="${DATABASE_URL}" \
  node --test .tmp/market-tests/lib/candidate-episode/canonical-read-oracle-postgres.test.js

printf '%s\n' '{"status":"pass","postgresMajor":16,"sameDatabaseSnapshot":true,"productionConnected":false}'
