...
WORK_DIR="$(mktemp -d /tmp/market-radar-canonical-rollback-pg16.XXXXXX)"
DATA_DIR="${WORK_DIR}/data"
SOCKET_DIR="$(mktemp -d /tmp/wp_g0_2_rehearsal_canonical_rollback_socket.XXXXXX)"
PORT="$((58532 + RANDOM % 300))"
DATABASE="wp_g0_2_rehearsal_canonical_rollback"
...
env -u DATABASE_URL -u POSTGRES_URL \
  APP_ENV=rehearsal NODE_ENV=test WP_G0_2_REHEARSAL=true \
  WP_G0_2_CANONICAL_ROLLBACK_REHEARSAL_DATABASE_URL="${DATABASE_URL}" \
  node --test scripts/production/candidate-canonical-rollback-remediation/runner-postgres.integration.mjs

printf '%s\n' '{"status":"pass","postgresMajor":16,"migrationCount":10,"canonicalRollback":"canonical_to_legacy_frozen","candidateDataPreserved":true,"productionConnected":false}'
