...
WORK_DIR="$(mktemp -d /tmp/market-radar-candidate-activation-pg16.XXXXXX)"
DATA_DIR="${WORK_DIR}/data"
SOCKET_DIR="$(mktemp -d /tmp/wp_g0_2_rehearsal_candidate_activation_socket.XXXXXX)"
PORT="$((56332 + RANDOM % 400))"
DATABASE="wp_g0_2_rehearsal_candidate_activation"
ADMIN_URL="postgresql://rehearsal@localhost:${PORT}/${DATABASE}?host=${SOCKET_DIR}"
RUNNER_URL="postgresql://market_radar_candidate_activation_runner@localhost:${PORT}/${DATABASE}?host=${SOCKET_DIR}"
...
env -u DATABASE_URL -u POSTGRES_URL \
  ...
  WP_G0_2_REHEARSAL_DATABASE_URL="${ADMIN_URL}" \
  node .tmp/market-tests/scripts/candidate-episode/migrate-rehearsal.js --environment rehearsal
env -u DATABASE_URL -u POSTGRES_URL \
  APP_ENV=rehearsal NODE_ENV=test WP_G0_2_REHEARSAL=true \
  WP_G0_2_ACTIVATION_ADMIN_DATABASE_URL="${ADMIN_URL}" \
  WP_G0_2_ACTIVATION_RUNNER_DATABASE_URL="${RUNNER_URL}" \
  node scripts/production/candidate-activation/runner-postgres.integration.mjs
