...
WORK_DIR="$(mktemp -d /tmp/market-radar-candidate-trusted-context-pg16.XXXXXX)"
DATA_DIR="${WORK_DIR}/data"
SOCKET_DIR="$(mktemp -d /tmp/wp_g0_2_rehearsal_candidate_trusted_context_socket.XXXXXX)"
PORT="$((57332 + RANDOM % 300))"
DATABASE="wp_g0_2_rehearsal_candidate_trusted_context"
...
env -u DATABASE_URL -u POSTGRES_URL \
  APP_ENV=rehearsal NODE_ENV=test WP_G0_2_REHEARSAL=true \
  WP_G0_2_TRUSTED_CONTEXT_REHEARSAL_DATABASE_URL="${DATABASE_URL}" \
  node --test .tmp/market-tests/lib/candidate-episode/canonical-read-trusted-context-postgres.test.js

printf '%s\n' '{"status":"pass","postgresMajor":16,"contextRole":"candidate_audit_role","productionConnected":false}'
