...
WORK_DIR="$(mktemp -d /tmp/market-radar-candidate-cycle-continuation-pg16.XXXXXX)"
DATA_DIR="${WORK_DIR}/data"
SOCKET_DIR="$(mktemp -d /tmp/wp_g0_2_rehearsal_candidate_cycle_continuation_socket.XXXXXX)"
PORT="$((57932 + RANDOM % 300))"
DATABASE="wp_g0_2_rehearsal_candidate_cycle_continuation"
...
env -u DATABASE_URL -u POSTGRES_URL \
  APP_ENV=rehearsal NODE_ENV=test WP_G0_2_REHEARSAL=true \
  WP_G0_2_CYCLE_CONTINUATION_REHEARSAL_DATABASE_URL="${DATABASE_URL}" \
  node --test scripts/production/candidate-cycle-continuation/runner-postgres.integration.mjs

printf '%s\n' '{"status":"pass","postgresMajor":16,"oldDeadlineImmutable":true,"candidateDataPreserved":true,"singleActiveCycle":true,"productionConnected":false}'
