...
WORK_DIR="$(mktemp -d /tmp/market-radar-candidate-canonical-read-pg16.XXXXXX)"
DATA_DIR="${WORK_DIR}/data"
SOCKET_DIR="$(mktemp -d /tmp/wp_g0_2_rehearsal_candidate_canonical_read_socket.XXXXXX)"
PORT="$((57032 + RANDOM % 300))"
DATABASE="wp_g0_2_rehearsal_candidate_canonical_read"
...
env -u DATABASE_URL -u POSTGRES_URL \
  APP_ENV=rehearsal NODE_ENV=test WP_G0_2_REHEARSAL=true \
  WP_G0_2_CANONICAL_READ_REHEARSAL_DATABASE_URL="${DATABASE_URL}" \
  node --test .tmp/market-tests/lib/candidate-episode/canonical-read-model-postgres.test.js

printf '%s\n' '{"status":"pass","postgresMajor":16,"readerRole":"candidate_application_reader_role","productionConnected":false}'
