...
WORK_DIR="$(mktemp -d /tmp/market-radar-candidate-reconciliation-pg16.XXXXXX)"
DATA_DIR="${WORK_DIR}/data"
SOCKET_DIR="$(mktemp -d /tmp/wp_g0_2_rehearsal_candidate_reconciliation_socket.XXXXXX)"
PORT="$((56732 + RANDOM % 300))"
DATABASE="wp_g0_2_rehearsal_candidate_reconciliation"
...
env -u DATABASE_URL -u POSTGRES_URL \
  APP_ENV=rehearsal NODE_ENV=test WP_G0_2_REHEARSAL=true \
  WP_G0_2_RECONCILIATION_DATABASE_URL="${DATABASE_URL}" \
  node scripts/production/candidate-reconciliation/runner-postgres.integration.mjs
