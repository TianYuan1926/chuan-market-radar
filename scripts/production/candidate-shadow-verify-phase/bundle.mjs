...
export const RUNNER_FILES = Object.freeze([
  ...
  "scripts/production/candidate-shadow-verify-phase/production-runner.sh",
  "scripts/production/candidate-shadow-verify-phase/runner.mjs",
]);
...
export function validateContract(contract) {
  ...
  const database = contract.databaseBoundary ?? {};
      ...
      && database.sourcePhase === "shadow_capture" && database.targetPhase === "shadow_verify"
  ...
  const observation = contract.observation ?? {};
      ...
      && observation.legacyResponseAuthority === true
      && observation.candidateCanonicalReviewUsable === false
      ...
      && observation.canonicalCompatStarted === false
  ...
  const rollback = contract.rollback ?? {};
      ...
      && rollback.beforeTransitionRestoreShadowCapture === true
  ...
  for (const forbidden of [
    ...
    "candidate_response_authority", "canonical_compat_activation", "canonical_cutover",
...
export async function validateLocalPreparation(root = process.cwd()) {
  ...
  return {
    status: "PASS_LOCAL_SHADOW_VERIFY_PHASE_TRANSITION_AND_OBSERVATION",
...
export function validateAuthorization(authorization, request, manifest) {
  for (const key of REQUIRED_PACKAGE_APPROVAL_FIELDS) {
      ...
      && authorization.gate === "G0" && authorization.actionClass === "shadow_verify_activation"
...
export async function validateApprovalRequest({ manifest, request, productionPaths = true }) {
      ...
      && /^market-radar-shadow-verify-phase-[a-z0-9][a-z0-9-]{7,48}$/u
        .test(request.runnerUnitName ?? "")
      && /^market-radar-shadow-verify-observer-[a-z0-9][a-z0-9-]{7,48}$/u
  ...
  const suffix = `${manifest.sourceCommit.slice(0, 12)}-${nonce.replaceAll("-", "").slice(0, 8)}`;
      ...
      === `/home/ubuntu/.cache/market-radar-ops/wp-g0-2-shadow-verify-phase-${suffix}`
      && request.opsRoot
        === `/home/ubuntu/.cache/market-radar-ops/shadow-verify-phase-ops/${suffix}`
      && request.secureRoot
        === `/home/ubuntu/.local/state/market-radar-shadow-verify-phase/${suffix}`
      && request.evidenceDirectory
        === `/home/ubuntu/.cache/market-radar-ops/evidence/wp-g0-2-shadow-verify-phase-${suffix}`
      && request.runnerUnitName
        === `market-radar-shadow-verify-phase-${manifest.sourceCommit.slice(0, 7)}-${nonce.replaceAll("-", "").slice(0, 8)}`
      && request.observerUnitName
        === `market-radar-shadow-verify-observer-${manifest.sourceCommit.slice(0, 7)}-${nonce.replaceAll("-", "").slice(0, 8)}`
      && request.approvalRef
        === `MR-G0-SHADOW-VERIFY-PHASE/${manifest.sourceCommit.slice(0, 12)}/${nonce.slice(0, 8)}`
  ...
  return {
    status: "PASS_SHADOW_VERIFY_PHASE_EXECUTION_REQUEST",
    manifestApprovalDigest: request.manifestApprovalDigest,
    targetAuthorityEpoch: request.targetAuthorityEpoch,
...
export function createProductionExecutionRequest({
  ...
  const manifestRaw = serializeManifest(buildShadowVerifyManifest({
  ...
  const request = {
    ...
    rollbackApprovalDigest: `sha256:${sha256(canonicalJson({
      action: "rollback_shadow_verify_to_legacy_frozen",
      migrationId: runtime.migrationId,
      releaseId: runtime.releaseId,
      targetAuthorityEpoch,
    }))}`,
    ...
    stagingDirectory: `/home/ubuntu/.cache/market-radar-ops/wp-g0-2-shadow-verify-phase-${suffix}`,
    opsRoot: `/home/ubuntu/.cache/market-radar-ops/shadow-verify-phase-ops/${suffix}`,
    secureRoot: `/home/ubuntu/.local/state/market-radar-shadow-verify-phase/${suffix}`,
    evidenceDirectory: `/home/ubuntu/.cache/market-radar-ops/evidence/wp-g0-2-shadow-verify-phase-${suffix}`,
    runnerUnitName: `market-radar-shadow-verify-phase-${manifest.sourceCommit.slice(0, 7)}-${nonce.replaceAll("-", "").slice(0, 8)}`,
    observerUnitName: `market-radar-shadow-verify-observer-${manifest.sourceCommit.slice(0, 7)}-${nonce.replaceAll("-", "").slice(0, 8)}`,
    ...
    approvalRef: `MR-G0-SHADOW-VERIFY-PHASE/${manifest.sourceCommit.slice(0, 12)}/${nonce.slice(0, 8)}`,
  ...
  request.autonomyAuthorization = {
    ...
    approvalId: `MR-G0-SHADOW-VERIFY-PHASE-${manifest.sourceCommit.slice(0, 12)}-${nonce}`,
    nonce,
    gate: "G0",
    packageId: PACKAGE_ID,
    scope: PACKAGE_ID,
...
export async function buildTransportBundle({
  ...
  const temporaryRoot = await mkdtemp(join(tmpdir(), "candidate-shadow-verify-phase-"));
  ...
  try {
    ...
    const manifest = {
      schemaVersion: "wp-g0.2-shadow-verify-phase-transport.v1",
      packageId: PACKAGE_ID,
    ...
    const outputPath = resolve(output ?? join(root,
      "reports/wp-g0-2-shadow-verify-phase-transition-and-observation",
      `shadow-verify-phase-${identity.sourceCommit.slice(0, 12)}.tar.gz`));
    await mkdir(dirname(outputPath), { recursive: true });
    await writeFile(outputPath, bytes, { mode: 0o600 });
    return {
      status: "PASS_SHADOW_VERIFY_PHASE_TRANSPORT",
      output: outputPath,
      sha256: sha256(bytes),
      size: bytes.length,
      manifest,
...
export async function verifyStagedTransport(root, manifest) {
  ensure(manifest?.schemaVersion === "wp-g0.2-shadow-verify-phase-transport.v1"
...
async function main() {
  ...
  if (command === "prepare-request") {
    ...
    process.stdout.write(`${JSON.stringify({
      status: "PASS_SHADOW_VERIFY_PHASE_REQUEST_PREPARED",
