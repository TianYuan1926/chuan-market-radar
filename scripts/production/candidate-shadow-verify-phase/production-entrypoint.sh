...
ENTRYPOINT_MODE="${SHADOW_VERIFY_PHASE_ENTRYPOINT_MODE:-launcher}"
...
RUNNER="${SOURCE_ROOT}/scripts/production/candidate-shadow-verify-phase/production-runner.sh"
...
STAGING_PREFIX="wp-g0-2-shadow-verify-phase-"
...
[[ "${RUNNER_UNIT}" =~ ^market-radar-shadow-verify-phase-[a-z0-9][a-z0-9-]{7,48}$ \
  && "${OBSERVER_UNIT}" =~ ^market-radar-shadow-verify-observer-[a-z0-9][a-z0-9-]{7,48}$ \
...
for evidence in "${LINEAGE_EVIDENCE}" "${RECONCILIATION_EVIDENCE}" "${CODE_RELEASE_EVIDENCE}"; do
...
if [[ "${ENTRYPOINT_MODE}" == "launcher" ]]; then
  ...
  sudo -n systemd-run --unit="${RUNNER_UNIT}" --collect --quiet \
    ...
    --setenv=SHADOW_VERIFY_PHASE_ENTRYPOINT_MODE=detached_worker \
    --setenv=REQUEST_FILE="${ACTUAL_REQUEST}" \
    /bin/bash "${SOURCE_ROOT}/scripts/production/candidate-shadow-verify-phase/production-entrypoint.sh"
  ...
  printf 'runner_unit=%s\nrunner_pid=%s\nDETACHED_SHADOW_VERIFY_PHASE_STARTED\n' \
    "${UNIT_NAME}" "${MAIN_PID}"
  exit 0
fi
...
[[ "${POSTGRES_CONTAINER}" =~ ^[0-9a-f]{12,64}$ ]] || fail postgres_container_invalid
{
  ...
  --mount "type=bind,src=${ACTUAL_ROOT},dst=/packet,readonly" \
  --entrypoint node "${CURRENT_WEB_IMAGE}" \
  /packet/scripts/production/candidate-shadow-verify-phase/bundle.mjs prepare-admin-url \
  --output /secure/migration-admin.url >/dev/null
chmod 600 "${SECURE_ROOT}/migration-admin.url"

set +e
SHADOW_VERIFY_PHASE_MODE=production_transition CONFIRM_SHADOW_VERIFY_PHASE=true \
