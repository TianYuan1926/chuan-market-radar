...
MODE="${SHADOW_VERIFY_OBSERVATION_MODE:-full}"
...
PRODUCTION_RUNNER="${SOURCE_ROOT}/scripts/production/candidate-shadow-verify-phase/production-runner.sh"
...
REHEARSAL=false
case "${OPS_ROOT}/" in
  /home/ubuntu/.cache/market-radar-ops/shadow-verify-phase-ops/*/) ;;
...
verify_static_identity() {
  ...
  [[ "$(${DOCKER[@]} exec "${WEB_CONTAINER}" stat -c '%u:%g:%a' "${MANIFEST_PATH}")" == "0:0:600" \
    ...
    && "$(${DOCKER[@]} exec "${WEB_CONTAINER}" sha256sum "${FULL_SNAPSHOT_PATH}" | awk '{print $1}')" \
      == "$(sha_file "${SOURCE_ROOT}/scripts/production/candidate-shadow-verify-phase/full-snapshot-observer.cjs")" ]] \
    || return 1
}
...
NODE
  ...
  jq -s --arg schemaVersion "candidate-shadow-verify-observation-sample.v1" \
    ...
    --arg phase shadow_verify --arg approvalDigest "$(jq -r '.manifestApprovalDigest' "${REQUEST_FILE}")" \
...
if [[ "${MODE}" == "sample" ]]; then
  ...
  printf '%s\n' 'PASS_IMMEDIATE_SHADOW_VERIFY_SAMPLE'
  exit 0
fi
...
automatic_rollback() {
  ...
  printf '%s\n' 'ERROR: Shadow Verify observation failed; invoking safety rollback.' >&2
  if ! SHADOW_VERIFY_PHASE_MODE=automatic_rollback CONFIRM_SHADOW_VERIFY_PHASE=true \
    REQUEST_FILE="${REQUEST_FILE}" ROOT_DIR_OVERRIDE="${ROOT_DIR}" \
    bash "${PRODUCTION_RUNNER}"; then
    printf '%s\n' 'P0_SHADOW_VERIFY_OBSERVATION_ROLLBACK_FAILED' >&2
    exit 98
  fi
  exit "${exit_code}"
}
...
for (( sample_number=1; sample_number<=289; sample_number++ )); do
  ...
  printf 'shadow_verify_sample=%s/289 status=pass\n' "${sample_number}"
...
printf '{"schemaVersion":"candidate-shadow-verify-observation-closeout.v1","outcome":"PASS_DUAL_READ_OBSERVATION","closedAt":"%s","canonicalCompatStarted":false,"secretsPrinted":false}\n' \
