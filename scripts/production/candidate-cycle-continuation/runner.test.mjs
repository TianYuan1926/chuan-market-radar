...
test("legacy epoch six materializes the exact disabled Compose defaults when overrides are absent", () => {
  const currentProduction = {
    ...valid,
    currentAuthorityEpoch: 6,
    currentPhase: "legacy",
  };
  const rendered = renderCycleContinuationEnvironment(
    "UNRELATED_VALUE=preserved\n",
    currentProduction,
  );
  ...
  assert.match(rendered, /^CANDIDATE_RUNTIME_MIGRATION_ID="candidate-episode-v1-cycle-2"$/mu);
  assert.match(rendered, /^CANDIDATE_RUNTIME_RELEASE_ID="candidate-shadow-cycle-next"$/mu);
  assert.match(rendered, /^UNRELATED_VALUE=preserved$/mu);
  assert.throws(() => renderCycleContinuationEnvironment(
    "CANDIDATE_EPISODE_CANONICAL_WRITE=true\n",
    currentProduction,
  ), /candidate_environment_source_mismatch:CANDIDATE_EPISODE_CANONICAL_WRITE/u);
  assert.throws(() => renderCycleContinuationEnvironment(
