...
import { loadContract, validateLocalPreparation } from "./candidate-dormant-deploy.mjs";

const execFileAsync = promisify(execFile);
const CONTRACT_PATH = "docs/governance/wp-g0-2-shadow-capture-dormant-runtime-deploy.v1.json";
...
export async function buildTransportBundle({
  ...
  const identity = sourceIdentity ?? (approvalEligible ? null : {
    ...
    ? identity?.sourceCommit === sourceCommit
      && [identity.sourceCommit, identity.sourceTree, identity.sourceParentCommit]
        .every((value) => /^[0-9a-f]{40}$/.test(value))
      && [identity.sourceDiffSha256, identity.sourcePathSetSha256,
        identity.gateEvidenceSha256, identity.policySha256]
        .every((value) => /^[0-9a-f]{64}$/.test(value))
    : sourceCommit === null, "source_identity_invalid");
  await validateLocalPreparation(root);
  const contract = await loadContract(root);
  const temporaryRoot = await mkdtemp(join(tmpdir(), "candidate-dormant-deploy-bundle-"));
  ...
  try {
    ...
    const manifest = {
      schemaVersion: "wp-g0.2-candidate-dormant-runtime-deploy-transport.v1",
      ...
      releaseDiffSha256: contract.releaseBoundary.releaseDiffSha256,
      ...
      repositoryMutationBoundary: "clean-detached-current-target-to-detached-exact-dormant-target-or-automatic-target-rollback",
      services: contract.deployment.serviceAllowlist,
      sessionIndependentExecutionRequired: contract.deployment.sessionIndependentExecutionRequired,
      rollbackImageRetentionRequired: contract.rollback.retainImageBeforeMutation,
    ...
    return {
      status: approvalEligible
        ? "PASS_FINAL_CANDIDATE_DORMANT_DEPLOY_TRANSPORT_BUNDLE"
        : "PASS_LOCAL_CANDIDATE_DORMANT_DEPLOY_TRANSPORT_TEMPLATE",
      output: outputPath,
...
async function currentSourceIdentity(root) {
  ...
  ensure(gateResult.gitHead === sourceCommit && gateResult.gitTree === sourceTree,
    "gate_evidence_source_identity_mismatch");
  return {
    sourceCommit,
    sourceTree,
...
async function main() {
  ...
  const bundleId = approvalEligible ? sourceCommit.slice(0, 12) : "precommit-template";
  const output = options.output ?? join(
    root,
    "reports/wp-g0-2-dormant-runtime-deploy-standing-authority-runner-refresh",
    `candidate-dormant-deploy-${bundleId}.tar.gz`,
  );
  const result = await buildTransportBundle({ root, output, sourceCommit, sourceIdentity, approvalEligible });
