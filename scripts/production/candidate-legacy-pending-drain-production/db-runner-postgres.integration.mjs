...
integrationTest("PostgreSQL 16 fail path refreezes with pending data preserved", async () => {
  ...
  try {
    ...
    await client.query(`SELECT * FROM candidate_authority.transition_migration_control_v1(
      $1,2,'shadow_capture',false,$2,$3,clock_timestamp())`, [migrationId, releaseId, digest]);
    ...
    await client.query(`SELECT * FROM candidate_authority.transition_migration_control_v1(
      $1,3,'legacy',true,$2,$3,clock_timestamp())`, [migrationId, releaseId, digest]);

    const opened = await openDrainEpoch(client, {
      approvalDigest: digest, expectedEpoch: 4, migrationId, releaseId,
    });
    assert.equal(opened.epoch, 5);
    const frozen = await rollbackDrainEpoch(client, {
      approvalDigest: digest, expectedEpoch: 5, migrationId, releaseId,
    });
    assert.equal(frozen.epoch, 6);
    ...
    assert.deepEqual(result.rows[0], {
      epoch: 6, pending: 1, phase: "legacy", total: 1, write_frozen: true,
    });
  } finally {
    client.release();
    await pool.end();
