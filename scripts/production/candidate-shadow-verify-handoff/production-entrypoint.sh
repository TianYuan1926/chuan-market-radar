...
ENTRYPOINT_MODE="${CANDIDATE_SHADOW_VERIFY_HANDOFF_ENTRYPOINT_MODE:-launcher}"
MANIFEST="${SOURCE_ROOT}/transport-manifest.json"
MARKER="${SOURCE_ROOT}/.transport-bundle.sha256"
RUNNER="${SOURCE_ROOT}/scripts/production/candidate-shadow-verify-handoff/production-runner.sh"
...
if [[ "${ENTRYPOINT_MODE}" == "launcher" ]]; then
  ...
  sudo -n systemd-run --unit="${UNIT}" --collect --quiet \
    ...
    --setenv=CANDIDATE_SHADOW_VERIFY_HANDOFF_ENTRYPOINT_MODE=detached_worker \
    --setenv=REQUEST_FILE="${ACTUAL_REQUEST}" \
    /bin/bash "${SOURCE_ROOT}/scripts/production/candidate-shadow-verify-handoff/production-entrypoint.sh"
  [[ "$(sudo -n systemctl show "${UNIT_NAME}" --property=ActiveState --value 2>/dev/null || true)" == "active" ]] \
    || fail transient_unit_not_active
  printf 'runner_unit=%s\nDETACHED_SHADOW_VERIFY_HANDOFF_STARTED\n' "${UNIT_NAME}"
  exit 0
fi
