...
VALIDATOR="${SOURCE_ROOT}/scripts/production/candidate-shadow-verify-code-presence/bundle.mjs"
EVIDENCE_VALIDATOR="${SOURCE_ROOT}/scripts/production/candidate-shadow-verify-code-presence/runner.mjs"
PRODUCTION_ROOT="/home/ubuntu/apps/chuan-market-radar"
ROOT_DIR="${ROOT_DIR_OVERRIDE:-${PRODUCTION_ROOT}}"
REHEARSAL="${CODE_PRESENCE_REHEARSAL:-false}"
...
verify_candidate_read_authority() {
  "${DOCKER[@]}" exec "${web_container}" test ! -e /run/market-radar/candidate-read-authority.json
...
jq -n \
  --arg schemaVersion "candidate-shadow-verify-code-presence-evidence.v1" \
  --arg status "PASS_PRODUCTION_SHADOW_VERIFY_CODE_PRESENCE_VERIFIED" \
  ...
  '{schemaVersion:$schemaVersion,status:$status,packageId:$packageId,
    ...
    candidateReadManifestAbsent:true,candidateReadEndpointFailClosed:true,
    scanFreshness:"fresh",verifiedAt:$verifiedAt,
    verificationMode:"read_only_existing_code_identity",requiresWebRelease:false,
...
printf 'PASS_PRODUCTION_SHADOW_VERIFY_CODE_PRESENCE_VERIFIED\n'
