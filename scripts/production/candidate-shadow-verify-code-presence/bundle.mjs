...
const STAGING =
  /^\/home\/ubuntu\/\.cache\/market-radar-ops\/wp-g0-2-shadow-verify-code-presence-[a-z0-9][a-z0-9-]{15,48}$/u;
const EVIDENCE =
  /^\/home\/ubuntu\/\.cache\/market-radar-ops\/evidence\/wp-g0-2-shadow-verify-code-presence-[a-z0-9][a-z0-9-]{15,48}$/u;
const UNIT = /^market-radar-shadow-verify-code-presence-[a-z0-9][a-z0-9-]{15,48}$/u;
...
export function validateContract(contract) {
  ...
  const pass = contract.passBoundary ?? {};
      ...
      && pass.candidateReadManifestAbsentRequired === true
      && pass.candidateReadEndpointFailClosedRequired === true
  ...
  const transport = contract.transport ?? {};
      ...
      && transport.temporaryArtifactCleanupRequired === true,
...
export async function validateLocalPreparation(root = process.cwd()) {
  ...
  return {
    status: "PASS_LOCAL_SHADOW_VERIFY_CODE_PRESENCE_PREPARATION",
...
export function createProductionVerificationRequest({
  approvalId = `MR-G0-CODE-${randomUUID()}`,
  ...
  const request = {
    ...
    stagingDirectory:
      `/home/ubuntu/.cache/market-radar-ops/wp-g0-2-shadow-verify-code-presence-${suffix}`,
    evidenceDirectory:
      `/home/ubuntu/.cache/market-radar-ops/evidence/wp-g0-2-shadow-verify-code-presence-${suffix}`,
    runnerUnitName:
      `market-radar-shadow-verify-code-presence-${manifest.sourceCommit.slice(0, 7)}-${nonce.replaceAll("-", "").slice(0, 8)}`,
    services: [],
...
export function validateProductionVerificationRequest(request, manifest, {
  ...
  const authorization = request.authorization;
      ...
      && /^MR-G0-CODE-[0-9a-f-]{36}$/u.test(authorization.approvalId ?? "")
      ...
      && authorization.riskTier === "R0_READ_ONLY" && authorization.maxExecutions === 1
      && /^[0-9a-f-]{36}$/u.test(authorization.nonce ?? "")
      && authorization.issuedAt === request.issuedAt
      && authorization.expiresAt === request.expiresAt
      && authorization.transportBundleSha256 === bundleSha256
      && authorization.targetCommit === manifest.sourceCommit
      && authorization.targetTree === manifest.sourceTree
      && authorization.runnerArtifactSha256 === manifest.runnerArtifactSha256
  ...
  return { status: "PASS_SHADOW_VERIFY_CODE_PRESENCE_REQUEST", productionMutationAllowed: false };
}
...
export async function buildTransportBundle({
  ...
  const temporaryRoot = await mkdtemp(join(tmpdir(), "candidate-shadow-verify-code-presence-"));
  ...
  try {
    ...
    const outputPath = resolve(output ?? join(root,
      "reports/wp-g0-2-shadow-verify-production-code-presence-identity-remediation",
      `shadow-verify-code-presence-${identity.sourceCommit.slice(0, 12)}.tar.gz`));
    await mkdir(dirname(outputPath), { recursive: true });
    await writeFile(outputPath, bytes, { mode: 0o600 });
    return {
      status: "PASS_SHADOW_VERIFY_CODE_PRESENCE_TRANSPORT",
      output: outputPath,
      sha256: sha256(bytes),
      size: bytes.length,
      manifest,
