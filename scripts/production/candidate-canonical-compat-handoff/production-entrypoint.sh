...
ENTRYPOINT_MODE="${CANDIDATE_CANONICAL_COMPAT_HANDOFF_ENTRYPOINT_MODE:-launcher}"
MANIFEST="${SOURCE_ROOT}/transport-manifest.json"
MARKER="${SOURCE_ROOT}/.transport-bundle.sha256"
RUNNER="${SOURCE_ROOT}/scripts/production/candidate-canonical-compat-handoff/production-runner.sh"
PRODUCTION_ROOT="/home/ubuntu/apps/chuan-market-radar"

fail() { printf 'ERROR: %s\n' "$1" >&2; exit 1; }
file_mode() { stat -c '%a' "$1" 2>/dev/null || stat -f '%Lp' "$1"; }

for command_name in docker jq realpath sudo; do
...
[[ "${BUNDLE_SHA256}" =~ ^[0-9a-f]{64}$
  && "$(jq -r '.transportBundleSha256' "${ACTUAL_REQUEST}")" == "${BUNDLE_SHA256}"
  && "$(jq -r '.stagingDirectory' "${ACTUAL_REQUEST}")" == "${ACTUAL_ROOT}" ]] \
...
if [[ "${ENTRYPOINT_MODE}" == "launcher" ]]; then
  ...
  sudo -n systemd-run --unit="${UNIT}" --collect --quiet \
    ...
    --property=KillMode=mixed --property=TimeoutStopSec=900 --property=RuntimeMaxSec=7200 \
    --property=UMask=0077 --property=StandardOutput=journal --property=StandardError=journal \
    --setenv=CANDIDATE_CANONICAL_COMPAT_HANDOFF_ENTRYPOINT_MODE=detached_worker \
    --setenv=REQUEST_FILE="${ACTUAL_REQUEST}" \
    /bin/bash "${SOURCE_ROOT}/scripts/production/candidate-canonical-compat-handoff/production-entrypoint.sh"
  [[ "$(sudo -n systemctl show "${UNIT_NAME}" --property=ActiveState --value 2>/dev/null || true)" == "active" ]] \
    || fail transient_unit_not_active
  printf 'runner_unit=%s\nDETACHED_CANONICAL_COMPAT_HANDOFF_STARTED\n' "${UNIT_NAME}"
  exit 0
fi
