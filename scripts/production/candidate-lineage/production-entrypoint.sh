...
ENTRYPOINT_MODE="${CANDIDATE_LINEAGE_CAPTURE_ENTRYPOINT_MODE:-launcher}"
BUNDLE_MARKER="${SOURCE_ROOT}/.transport-bundle.sha256"
TRANSPORT_MANIFEST="${SOURCE_ROOT}/transport-manifest.json"
PACKET_VALIDATOR="${SOURCE_ROOT}/scripts/production/candidate-lineage/bundle.mjs"
RUNNER="${SOURCE_ROOT}/scripts/production/candidate-lineage/production-runner.sh"
...
[[ "${APPROVED_STAGING}" == "${ACTUAL_SOURCE_ROOT}" \
  && "$(basename "${ACTUAL_SOURCE_ROOT}")" == wp-g0-2-lineage-capture-* \
...
[[ "${APPROVED_UNIT}" =~ ^market-radar-lineage-capture-[a-z0-9][a-z0-9-]{7,48}$ \
  ...
  && "${APPROVED_SECURE_ROOT}" == /home/ubuntu/.local/state/market-radar-lineage-capture/* \
  && "${APPROVED_OPS_ROOT}" == /home/ubuntu/.cache/market-radar-ops/lineage-capture-ops/wp-g0-2-lineage-capture-* \
  && "${APPROVED_EVIDENCE_DIRECTORY}" == /home/ubuntu/.cache/market-radar-ops/evidence/wp-g0-2-candidate-lineage-* ]] \
...
for file in "${SOURCE_EVIDENCE_FILES[@]}"; do
  [[ -f "${file}" && ! -L "${file}" && "$(realpath "${file}")" == "${file}" ]] \
    || fail "source_evidence_missing:$(basename "${file}")"
...
VALIDATOR_MOUNTS=(--mount "type=bind,src=${ACTUAL_SOURCE_ROOT},dst=/packet,readonly")
for directory in "${UNIQUE_EVIDENCE_DIRECTORIES[@]}"; do
  VALIDATOR_MOUNTS+=(--mount "type=bind,src=${directory},dst=${directory},readonly")
done
${DOCKER[@]} run --rm --network none --read-only --cap-drop ALL \
  --security-opt no-new-privileges --user "$(id -u):$(id -g)" \
  --tmpfs /tmp:rw,noexec,nosuid,size=16m "${VALIDATOR_MOUNTS[@]}" \
  --entrypoint node "${WEB_IMAGE}" \
  /packet/scripts/production/candidate-lineage/bundle.mjs validate-request \
...
cleanup() {
  ...
  [[ "${ACTUAL_SOURCE_ROOT}" == /home/ubuntu/.cache/market-radar-ops/wp-g0-2-lineage-capture-* \
    && "${APPROVED_SECURE_ROOT}" == /home/ubuntu/.local/state/market-radar-lineage-capture/* \
    && "${APPROVED_OPS_ROOT}" == /home/ubuntu/.cache/market-radar-ops/lineage-capture-ops/wp-g0-2-lineage-capture-* \
...
for index in "${!SOURCE_EVIDENCE_FILES[@]}"; do
  install -m 0600 "${SOURCE_EVIDENCE_FILES[$index]}" \
    "${APPROVED_SECURE_ROOT}/${target_files[$index]}"
done
  ...
  && ! -L "${POSTGRES_ADMIN_ENV}" ]] || fail postgres_admin_env_boundary_invalid
{
  ...
  --entrypoint node "${WEB_IMAGE}" \
  /packet/scripts/production/candidate-lineage/bundle.mjs prepare-admin-url \
    --output /secure/migration-admin.url >/dev/null
chmod 600 "${APPROVED_SECURE_ROOT}/migration-admin.url"

REQUEST_FILE="${ACTUAL_REQUEST}" \
CANDIDATE_LINEAGE_CAPTURE_MODE=production_collect \
CONFIRM_CANDIDATE_LINEAGE_CAPTURE=true \
