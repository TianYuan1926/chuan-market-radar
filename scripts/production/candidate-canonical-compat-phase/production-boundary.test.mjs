...
const file = (name) => readFile(resolve(root,
  `scripts/production/candidate-canonical-compat-phase/${name}`), "utf8");

test("production runner defaults to a no-mutation dry run", () => {
  const result = spawnSync("bash", [
    resolve(root, "scripts/production/candidate-canonical-compat-phase/production-runner.sh"),
...
test("session-independent entrypoint keeps staging for the 24-hour observer", async () => {
  ...
  assert.match(source, /CANONICAL_COMPAT_PHASE_ENTRYPOINT_MODE=detached_worker/u);
...
test("observer requires exactly 289 database-clock samples over 24 hours", async () => {
  ...
  assert.match(observer, /readSource: resource\.readSource/u);
  assert.match(observer, /candidate-canonical-compat-observation-sample\.v1/u);
});
