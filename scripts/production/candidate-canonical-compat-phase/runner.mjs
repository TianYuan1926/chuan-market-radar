...
export const SAMPLE_SCHEMA = "candidate-canonical-compat-observation-sample.v1";
export const EVIDENCE_SCHEMA = "candidate-canonical-compat-observation-evidence.v1";
...
export class CanonicalCompatPhaseError extends Error {}
...
export function validateShadowVerifyEnvironment(source, releaseId) {
  ...
  ensure(entries.get("CANDIDATE_EPISODE_DUAL_READ") === "true",
    "candidate_pretransition_dual_read_not_enabled");
  ensure(entries.get("CANDIDATE_EPISODE_CANONICAL_READ") === "false",
    "candidate_pretransition_canonical_read_not_false");
  ensure(entries.get("CANDIDATE_EPISODE_REVIEW_READ") === "false",
    "candidate_pretransition_review_read_not_false");
  return { releaseId, candidateUrlsConfigured: 3, phaseFlags: "shadow_verify" };
}

export function renderCanonicalCompatEnvironment(source, releaseId) {
  validateShadowVerifyEnvironment(source, releaseId);
  return renderEnvironment(source, CANONICAL_COMPAT_FLAGS);
}
...
export function validateDualReadEvidence(evidence, expected = {}) {
  ...
  "dual_read_observation_status_invalid");
      ...
      && evidence.maximumGapSeconds <= MAXIMUM_SAMPLE_GAP_SECONDS
      && evidence.allPagesComparedEverySample === true
      && evidence.differenceCount === 0
      && evidence.legacyResponseAuthority === true
      && evidence.candidateCanonicalReviewUsable === false
      && evidence.canAuthorizeCutover === false
      && evidence.canCreateTradePlan === false
      && evidence.canMutateLiveRanking === false
  ...
  for (const [key, value] of Object.entries(expected)) {
    ensure(evidence[key] === value, `dual_read_observation_identity_mismatch:${key}`);
  }
  return evidence;
}
...
export function validateCodeReleaseEvidence(evidence) {
  ...
  "canonical_compat_code_release_not_pass");
    ...
    "canonical_compat_code_release_identity_invalid");
  ...
  "canonical_compat_code_release_boundary_invalid");
  return evidence;
}

export function buildCanonicalCompatManifest({
  currentAuthorityEpoch,
  dualReadEvidenceHash,
  generatedAt,
  migrationId,
  ...
  ensure(SHA256.test(dualReadEvidenceHash ?? ""),
    "dual_read_evidence_hash_invalid");
...
export function serializeManifest(manifest) {
  ensure(manifest?.schemaVersion === MANIFEST_SCHEMA && manifest.phase === "canonical_compat",
...
export function validateFullSnapshotEvidence(snapshot, expected) {
  ensure(snapshot?.schemaVersion === "candidate-canonical-compat-full-snapshot-parity.v1"
  ...
  ensure(snapshot.phase === "canonical_compat"
...
export function validateObservationSample(sample, expected) {
  ...
  const api = sample.api;
      ...
      && api.status === "ready"
      && api.mode === "canonical_compat_candidate"
      && api.readSource === "candidate"
      && api.authority === "candidate_authority"
      && api.allowedUse === "candidate_lifecycle_and_review_only"
      && api.candidateCanonicalReviewUsable === true
...
export function evaluateCanonicalCompatObservation(samples, expected) {
  const violations = [];
  ...
  const evidenceBody = {
    ...
    legacyFallbackCount: unique.length === 0 ? 0 : null,
    partialResponseCount: unique.length === 0 ? 0 : null,
    unavailableResponseCount: unique.length === 0 ? 0 : null,
...
function loadPg() {
  ...
  throw new CanonicalCompatPhaseError("approved_pg_runtime_unavailable");
}
...
function assertPretransitionControl(row, request) {
      ...
      && row.phase === "shadow_verify" && row.write_frozen === false
  ...
  ensure(row.approval_digest === request.currentApprovalDigest,
    "candidate_control_approval_digest_mismatch");
...
export async function preflightControl(client, request) {
  return withMigrationRole(client, async () => {
    ...
    return {
      status: "PASS_CANONICAL_COMPAT_CONTROL_PREFLIGHT",
...
export async function transitionControl(client, request, rawManifest) {
  const manifest = JSON.parse(rawManifest);
    ...
    "manifest_approval_digest_mismatch");
      ...
      && manifest.phase === "canonical_compat",
...
export async function rollbackControl(client, request) {
  ...
  return withMigrationRole(client, async () => {
    ...
    if (current.phase === "legacy") {
      ...
      return {
        status: "PASS_CANONICAL_COMPAT_CONTROL_ROLLBACK",
    ...
    const rollbackEpoch = current.phase === "canonical_compat"
...
async function main() {
  ...
  if (command === "render-env") {
    ...
    await writeAtomic(options.output, renderCanonicalCompatEnvironment(source, request.releaseId));
    return;
  }
  ...
  if (command === "build-manifest") {
    ...
    const dualRead = validateDualReadEvidence(
      await readJson(request.dualReadEvidencePath), {
  ...
  if (command === "observe") {
    ...
    const result = evaluateCanonicalCompatObservation(samples, request);
    ensure(result.status === "PASS_CANONICAL_COMPAT_OBSERVATION", result.violations.join(","));
  ...
  throw new CanonicalCompatPhaseError("command_invalid");
}
