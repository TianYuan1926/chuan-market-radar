...
MODE="${CANONICAL_COMPAT_PHASE_MODE:-dry_run}"
CONFIRMED="${CONFIRM_CANONICAL_COMPAT_PHASE:-false}"
...
OBSERVER="${SOURCE_ROOT}/scripts/production/candidate-canonical-compat-phase/observation-runner.sh"
FULL_SNAPSHOT="${SOURCE_ROOT}/scripts/production/candidate-canonical-compat-phase/full-snapshot-observer.cjs"
...
[[ "${PACKAGE_ID}" == "WP-G0.2-CANONICAL-COMPAT-PHASE-TRANSITION-AND-OBSERVATION" \
...
case "${OPS_ROOT}/" in
  /home/ubuntu/.cache/market-radar-ops/canonical-compat-phase-ops/*/) REHEARSAL=false ;;
...
validate_request() {
  local -a mounts=(
    ...
    --mount "type=bind,src=$(jq -r '.dualReadEvidencePath' "${REQUEST_FILE}"),dst=$(jq -r '.dualReadEvidencePath' "${REQUEST_FILE}"),readonly"
...
wait_health() {
  local expected_phase="${1:-canonical_compat}"
...
if (response.status !== 200 || body.ok !== true
    || resource.mode !== "dual_read_legacy_authority"
    || resource.readSource !== "legacy"
    || resource.authority !== "legacy_projection_non_authoritative"
    || resource.parity?.status !== "pass"
    || resource.parity?.differenceCount !== 0
    || resource.candidateCanonicalReviewUsable !== false
    || resource.canAuthorizeCutover !== false
    || resource.automaticPhaseAdvance !== false) process.exit(1);
NODE
}
verify_worker() {
  local current image
...
verify_current_manifest() {
  local container="$1"
  [[ "$("${DOCKER[@]}" exec "${container}" stat -c '%u:%g:%a' "${MANIFEST_PATH}")" == "0:0:600" \
    && "$("${DOCKER[@]}" exec "${container}" sha256sum "${MANIFEST_PATH}" | awk '{print $1}')" \
      == "$(jq -r '.currentManifestSha256' "${REQUEST_FILE}")" ]]
}
...
RENDERED_ENV="${OPS_ROOT}/state/env.production.canonical-compat"
...
automatic_rollback() {
  ...
  set +e
  lease_event safety-checkpoint --checkpoint canonical_compat_rollback
  ...
  if [[ "${rollback_ok}" == "true" ]]; then
    ...
    printf '{"status":"ROLLBACK_PASS_CANONICAL_COMPAT_TO_%s","candidateDataDeleted":false,"secretsPrinted":false}\n' \
      "${phase^^}" > "${EVIDENCE_DIRECTORY}/rollback-summary.json"
    cleanup_temporary
    exit "${original_exit}"
  fi
  printf '%s\n' 'P0_CANONICAL_COMPAT_ROLLBACK_NOT_VERIFIED' >&2
  exit 98
}
...
endpoint_shadow_verify_ready "${WEB_CONTAINER}" || fail endpoint_pretransition_shadow_verify_invalid
...
jq -n --arg status prepared --arg productionCommit "${PRODUCTION_COMMIT}" \
  ...
  '{schemaVersion:"candidate-canonical-compat-phase-state.v1",status:$status,productionCommit:$productionCommit,webImageId:$webImageId,migrationId:$migrationId,releaseId:$releaseId,currentAuthorityEpoch:$currentAuthorityEpoch,targetAuthorityEpoch:$targetAuthorityEpoch}' \
  > "${STATE_FILE}"
...
lease_event checkpoint --checkpoint transition_canonical_compat
...
CANONICAL_COMPAT_OBSERVATION_MODE=sample REQUEST_FILE="${REQUEST_FILE}" \
  OBSERVATION_CONTEXT="${OBSERVATION_CONTEXT}" OUTPUT_SAMPLE="${IMMEDIATE_SAMPLE}" \
  ROOT_DIR_OVERRIDE="${ROOT_DIR}" bash "${OBSERVER}"

lease_event checkpoint --checkpoint immediate_canonical_compat_pass
if [[ "${REHEARSAL}" == "true" ]]; then
  printf '%s\n' 'PASS_REHEARSAL_CANONICAL_COMPAT_PHASE_IMMEDIATE'
  SUCCEEDED=true
  trap - ERR EXIT INT TERM HUP
  exit 0
fi
...
sudo -n systemd-run --unit="${OBSERVER_UNIT}" --collect --quiet \
  ...
  --setenv=CANONICAL_COMPAT_OBSERVATION_MODE=full \
...
printf '{"status":"PASS_IMMEDIATE_CANONICAL_COMPAT_OBSERVATION_ACTIVE","observerUnit":"%s","candidateResponseAuthority":"candidate_parity_gated","automaticPhaseAdvance":false,"secretsPrinted":false}\n' \
  "${OBSERVER_UNIT}.service" > "${EVIDENCE_DIRECTORY}/immediate-summary.json"
SUCCEEDED=true
trap - ERR EXIT INT TERM HUP
printf '%s\n' 'PASS_IMMEDIATE_CANONICAL_COMPAT_OBSERVATION_ACTIVE'
