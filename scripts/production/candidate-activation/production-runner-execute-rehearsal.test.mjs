...
if (args.includes('branch') && args.includes('--show-current')) { console.log('main'); process.exit(0); }
...
if (args.includes('merge')) { fs.writeFileSync(process.env.FAKE_GIT_STATE,process.env.FAKE_APPROVED_COMMIT+'\\n'); process.exit(0); }
if (args.includes('checkout') && args.includes('--detach')) { fs.writeFileSync(process.env.FAKE_GIT_STATE,process.env.FAKE_ROLLBACK_COMMIT+'\\n'); process.exit(0); }
process.exit(0);
`;
...
test("isolated failure restores env, web, git and database control", async () => {
  ...
  assert.match(result.docker, /candidate-activation-rollback/);
...
test("isolated image build failure restores git before control starts", async () => {
  ...
  assert.match(result.docker, /control-rollback/);
  assert.doesNotMatch(result.docker, /control-start/);
  assert.equal(result.head, "b".repeat(40));
});
