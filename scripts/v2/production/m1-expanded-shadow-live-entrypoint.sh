...
RUNNER="${SOURCE_ROOT}/scripts/v2/production/m1-expanded-shadow-live-runner.mjs"
STAGING_ROOT="/home/ubuntu/.cache/market-radar-v2"
STAGING_PREFIX="m1-expanded-shadow-"
...
cleanup_staging() {
  ...
  if [[ "$(dirname "${ACTUAL_SOURCE_ROOT}")" == "${STAGING_ROOT}" \
    ...
    echo "ERROR: expanded-shadow staging cleanup boundary rejected." >&2
    exit 1
  fi
  exit "${exit_code}"
}
...
if [[ "$(dirname "${ACTUAL_SOURCE_ROOT}")" != "${STAGING_ROOT}" \
  ...
  || ! -f "${RUNNER}" \
  || -L "${RUNNER}" ]]; then
  echo "ERROR: expanded-shadow staging boundary is invalid." >&2
  exit 1
fi
...
echo "PASS_V2_M1_5C_5D_TENCENT_EXPANDED_SHADOW"
