...
Git child operations are hard-bounded to 90 seconds, while the SSH wrapper has bounded connection attempts and keepalive failure detection. The agent lock records boot ID, PID, Linux process start identity and a random ownership token. A live owner remains fail-closed; an owner proven dead is quarantined and recovered. The empty legacy lock left by a systemd timeout is recoverable only after a four-minute stale interval, while a recent or unsafe lock remains blocked. This prevents a killed poll from permanently disabling all later signed dispatches without weakening single-agent exclusion.
