...
RUNNER="${SOURCE_ROOT}/scripts/v2/production/m1-source-conformance-live-runner.mjs"
STAGING_ROOT="/home/ubuntu/.cache/market-radar-v2"
STAGING_PREFIX="m1-1b0-source-conformance-"
...
cleanup_staging() {
  ...
  if [[ "$(dirname "${ACTUAL_SOURCE_ROOT}")" == "${STAGING_ROOT}" \
    ...
    echo "ERROR: source-conformance staging cleanup boundary rejected." >&2
    exit 1
  fi
  exit "${exit_code}"
}
...
if [[ "$(dirname "${ACTUAL_SOURCE_ROOT}")" != "${STAGING_ROOT}" \
  ...
  || ! -f "${RUNNER}" \
  || -L "${RUNNER}" ]]; then
  echo "ERROR: source-conformance staging boundary is invalid." >&2
  exit 1
fi
...
echo "PASS_V2_M1_1B0_TENCENT_LIVE_SOURCE_CONFORMANCE"
