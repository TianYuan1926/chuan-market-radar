...
export async function validateActivationRunnerPreparation(contract) {
  ...
  const violations = [];
  const [runnerArtifact, currentReleaseArtifact, shell, observer, runner, flags, migration] = await Promise.all([
  ...
  if (runnerArtifact.sha256 !== contract.runnerArtifact?.sha256 || runnerArtifact.fileCount !== 3) violations.push("runner_artifact");
  ...
  if (contract.rollback?.automaticRollbackRequired !== true
      ...
      || contract.rollback?.deleteCandidateEvidenceAllowed !== false) violations.push("rollback_boundary");
  for (const token of [
  ...
  if (/--remove-orphans/.test(shell) || /backtest:formal|candidate:migrate|persistence\/migrate/.test(shell)) {
