...
test("integration contract keeps the future chain local and production closed", async () => {
  ...
  assert.equal(contract.hardBoundaries.candidateProductionActivationAllowed, false);
  ...
  assert.ok(contract.forbiddenInThisPackage.includes("current_main_merge_before_dormant_deploy_pass"));
});
