...
## 2026-08-02 to 2026-08-03 / Autonomous Evidence Return, Runtime Preflight And Signer Identity Root Remediation
...
- request v4 将 signing key path 和 SSH SHA-256 public-key fingerprint 写入 canonical request/policy/bundle，拒绝漂移与 caller 冲突，并在任何 Caddy mutation 前执行 sanitized real sign、独立 verify 和 observed fingerprint 比对。无 caller options happy path、missing-key 与 wrong-identity 故障注入已使 gateway `14/14`、recurrence `11/11`、dispatch `38/38` PASS；该 signer 根治已以 source `4d1042618eff24d39f6ecceb93d5715e2e798b5a`、tree `c28fa60f1236e8b80a7185de4bcfdc4c084d34e0` 提交推送。
- `4d104261...` 的 Signed Dispatch `30755615525`、A0 `30755615545`、Independent Security `30755615533` PASS；Full Quality `30755615546` / job `91517146516` FAIL，远端资格严格为 `3/4`。失败用例期望 `prearmed_credential_session_disconnected`，实际得到 `prearmed_secret_session_disconnected`：bridge v5 初始 liveness 使用 session-specific reason，clipboard wait 却把 credential/age 两条会话统一重标为 generic secret session，进程调度因而决定外显错误码。
- bridge v6 现把 `{id,failure_code}` 贯穿 clipboard wait，分别覆盖 immediate credential disconnect、clipboard-wait credential disconnect 与 clipboard-wait age disconnect；directed `13/13`、完整 P0R `142/142`、Go helper 和最终字节完整 exact CI 本地 PASS。完整 CI 为 recurrence `11/11`、dispatch `38/38`、market `965/969`（4 explicit skips）、workers `23/23`、historical `4/4`、Foundation `638/644`（6 explicit skips）、Ops `283/283`、M0 `12/12`、Next build、Golden `16/16` 和 security。历史时点的新 clean commit/四门前置随后由 source `df85b485...` 独立满足；`4d104261...` 的三枚 PASS 未被拼接复用。
- bridge-v6 exact source `df85b485b6d2c0ede8cdd2b656978ba2fdc372ab`、tree `f32cc5aa70f7f1c46e772b3b7ba2abbbd7aaf2a5` 已取得 Signed Dispatch `30758244706`、Full Quality `30758244710`、Independent Security `30758244707` 和 A0 `30758244715` 四门全 PASS。deterministic package 绑定 approval `1daeb3...`、bundle `e70e638...`、manifest `4c8bba4...`、baseline `f5bde72...`、container set `f871741...`、wrapper `fb473dc...`、override `1b7f8b...` 和 signer fingerprint `SHA256:wxHx/NcT7wmgM6aOJnjgYKK4gOQGGeN44XkHYARbpbc`。
- signed dispatch `3cca64f63ea160e0b772c513c9c6624bc76a28ec` 已由生产 claim 并返回 `PASS_PRODUCTION_EVIDENCE_GATEWAY_CADDY_ONLY`。本机固定 retriever 自动取得 `22ebecfad6dd9d83b8e40203889f2f36d7c22fa7237fcf3bf63142acafe14acf.mre`，返回 `PASS_PRODUCTION_EVIDENCE_DECRYPTED_AND_VERIFIED`；payload SHA `d33c62e...`、sealed SHA `2f0ab69...` 一致，payload/receipt/sealed 三文件均为 mode `0600`。
- 独立 OrcaTerm 只读复核确认 runner `Result=success/ExecMainStatus=0`、claim/cursor exact、生产 HEAD `cec0b657...`、clean worktree、十个非 Caddy full IDs、ready/fresh health、staging/lease absence 与 expiry prune timer active；exact object 为 HTTP 200，错误 namespace 为 404。数据库、Redis、Worker、env、migration、Feature Flag、生产仓库和其他服务均未改变。三个失败 dispatch 保留且不可复用，gateway recurrence count=3 已 `CLOSED_VERIFIED`。

### 当前真值与下一步

状态是 `THREE_HISTORICAL_GATEWAY_FAILURES_PRESERVED / BRIDGE_V6_SOURCE_DF85_SAME_SOURCE_FOUR_GATES_PASS / SIGNER_BOUND_CADDY_ONLY_REAL_TARGET_ACCEPTANCE_PASS / ENCRYPTED_EVIDENCE_AUTONOMOUSLY_RETRIEVED_DECRYPTED_AND_SIGNATURE_VERIFIED / NON_TARGET_ZERO_DRIFT / GATEWAY_RECURRENCE_COUNT_3_CLOSED_VERIFIED / P0R_BACKUP_EXACT_RETRIEVAL_ISOLATED_RESTORE_NOT_EXECUTED`。下一步先形成并资格化权威同步 source，再通过已验收通道执行 fresh P0R read-only rebind，生成全新 route/lease/run/plan/transport 和 no-secret target；之后才允许一次 fresh STS、加密 backup、COS exact-version retrieval、isolated PG16 restore、全清理与 fresh P0。gateway PASS 不得冒充 P0R、P0、M1 或实战准入。
