...
状态是 `AUTONOMOUS_EVIDENCE_RETURN_LOCAL_IMPLEMENTATION_AND_SECURITY_ROOT_REMEDIATION_FULL_CI_PASS / FIRST_REMOTE_QUALIFICATION_THREE_OF_FOUR_PASS_INDEPENDENT_SECURITY_FAIL / NEW_CLEAN_COMMIT_AND_REMOTE_FOUR_GATE_REQUALIFICATION_PENDING / EXACT_APPROVAL_REAL_CADDY_VALIDATION_BOOTSTRAP_RETRIEVAL_AND_FRESH_REBIND_PENDING / PRODUCTION_INTENTIONAL_MUTATION_NONE / FRESH_PRODUCTION_ZERO_DRIFT_UNVERIFIED / P0R_BACKUP_RETRIEVAL_RESTORE_NOT_EXECUTED`。先固化并推送同一 clean remediation source，再从零取得四门，禁止借用前三门；只有 exact production package 与批准成立后才允许 Caddy-only bootstrap。成功自动取回并验证 gateway evidence 后，使用同一通道 fresh rebind，再恢复 P0R。
