...
状态：`FROZEN_TASK_CONTRACT / M1.5A_LOCAL_ENGINEERING_AND_POSTGRES16_PASS / M1.5B0_RELEASE_SAFETY_LOCAL_PASS / LIVE_EGRESS_UNAVAILABLE / PRODUCTION_UNCHANGED`
...
M1.5-B0 已把入口收紧为显式 capability role、secret-file database URL、完整 observation envelope、有限 Shadow profile 和独立最小权限容器。详细合同见 `M1_5_B0_SHADOW_RELEASE_SAFETY_CONTRACT_V1.md`。

审计确认当前高频 Fact 单表只有 retention metadata，没有可执行物理 purge；因此 30 分钟 early Shadow 与长期存储治理分开推进，24 小时持续 Shadow 必须等待分区/retention 完成。本合同不把 B0、本地静态容器检查或 early Shadow 写成 M1 PASS。
