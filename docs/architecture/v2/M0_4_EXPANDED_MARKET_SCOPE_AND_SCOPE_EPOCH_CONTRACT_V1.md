...
状态：`DESIGN_SCOPE_AMENDMENT_PASS / M1.1A_CAPABILITY_REGISTRY_LOCAL_PASS / LIVE_SCOPE_V2_IMPLEMENTATION_UNPROVEN / PRODUCTION_UNCHANGED`
...
| --- | --- | --- | --- | --- |
| Binance | 官方 USD-M 永续 API 已确认 | `PENDING_TRADING/TRADING/.../CLOSE` 与 contract information stream 可表达待上市、规则变化和下架 | 官方 Stock/TradFi Perpetuals 已确认 | Crypto + Equity product target；Scope V2 identity/Adapter/运行仍未证明 |
...
- 四 Venue + CoinGlass 的 33 类 capability 已形成 165 行穷举登记，缺失和重复均 fail closed。
- 每个 endpoint/channel 的权限、限速、历史范围、套餐、运行状态和失败语义都有显式字段；无法取得精确资料时保持 `UNVERIFIED/UNAVAILABLE`。
- Binance、OKX、Bybit、Bitget 的股票永续均已有官方产品证明；四者的 Scope V2 identity、Adapter、地区可用性和运行探测仍为未证明。
- 详细合同为 `docs/architecture/v2/M1_1A_FOUR_VENUE_SOURCE_CAPABILITY_REGISTRY_V1.md`。
...
四 Venue + CoinGlass 的来源能力分母已由 165 行机器登记表完整核算。
官方能力与 Scope V2 实现/运行能力已分层，Binance 股票永续旧结论已纠正。
```
