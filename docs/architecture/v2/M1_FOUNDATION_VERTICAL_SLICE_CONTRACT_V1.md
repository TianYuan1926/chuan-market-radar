...
状态：`FROZEN_TASK_CONTRACT / M1.1-M1.6_LOCAL_PASS / M2.0-M2.1_LOCAL_PASS / DETECTORS_DRAFT_NO_EMISSION / LIVE_EGRESS_UNAVAILABLE / M1.5B1_AND_M1.7_PENDING / PRODUCTION_UNCHANGED`
...
- 每个 instrument 的同一 cutoff `MARK_PRICE` fixture 和 quality metadata。
...
- 三家公开 catalog/mark-price Adapter、GET-only allowlist Transport、Identity/Fact builder 已在独立 `src/v2` 中实现。
...
- 唯一首批 Feature 为 `cross-venue-mark-price-dispersion.v1`，使用十进制整数运算计算 `(max - min) / median`，不经过浮点价格计算。
- Feature builder 要求每个 eligible instrument 恰好一个同 cutoff `MARK_PRICE` Fact，拒绝缺失、重复、晚 cutoff 和在 feature computedAt 之后才产生的 Fact。
...
- Collector 明确区分 `providerObserved / accounted / eligible / collected / usablePrice / fresh`，ratio 同时携带分子和分母，0 分母返回 null。
- 冷启动、增量 mark-price snapshot、周期 reconciliation、provider quota、global/per-provider concurrency、队列深度/等待、数据库失败和 recovery 状态机已通过定向测试。
...
- PostgreSQL 16 真实关闭读写连接后重新创建进程边界，成功从 checkpoint 继续 `INCREMENTAL_MARK_PRICE` 且不重复 catalog；checkpoint 不存在时只能冷启动，checkpoint append 失败时 Worker 停止。
...
- v1 partition migration checksum 保持不变；新增 `v2-m1-partitioned-fact-store.v2` additive migration。迁移后新 Fact 只能进入无 DEFAULT 的 UTC 六小时分区，旧账本 trigger 拒绝旁路新 Fact；非空 v1 日分区状态禁止直接升级。
- 活动身份注册表提供保留期内全局 fact/idempotency 唯一性，并随受控 partition DROP 原子收缩；不可变 DROPPED 事件永久拒绝旧 source day 重建和重灌，不把容量债转移到无限身份表。
- Reader、Writer、Replay、Audit、Retention、Migration 六类能力分离；Audit 只能登记 restore evidence，Retention 只能经 security-definer 函数 ensure/inspect/drop，Writer/Audit 无 DROP 权限。
- Capacity 合同要求分区连续、写入窗口已预建、单分区与总字节不过线；空清单为证据不足，estimated rows 不冒充精确计数。
- PG16 真实演练证明迁移前旧 Fact 可读、8 个连续六小时分区、新旧读取、幂等/污染、保留中和活跃 replay 阻断、真实 `pg_dump -> pg_restore -> replay parity`、原子 DROP、身份同步收缩和防重灌。
- clean source `15746813245744af4f4ba73f61a976b722ad9a21` 已完成 8 周期/11,552 Fact 生产形状校准；六小时本地无扩容模型稳态/峰值 59%/67%，低于固定 70% 门槛。该 PASS 不替代真实 recovery、fresh topology 或 fresh P0。

M1 仍未完成。M1.5-B1 30 分钟 live Shadow 已通过；M1.6 仍等待真实 recovery、fresh P0、生产分阶段启用和 M1.7 24 小时持续 SLO。M2.0 合同与 M2.1 DRAFT 纯回放内核已本地完成，Detector 仍未校准且 Candidate 禁发；等待外部 Gate 时只允许继续不读取生产 authority 的合同、fixture、测试和 no-authority 工具，不开放 Detector runtime、Candidate Store、API、页面或交易计划。
