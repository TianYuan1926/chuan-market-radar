...
状态：`FROZEN_TASK_CONTRACT / M1.1-M1.6_LOCAL_PASS / LIVE_EGRESS_UNAVAILABLE / M1.5B1_AND_M1.7_PENDING / PRODUCTION_UNCHANGED`
...
M1 仍未完成。M1.6 已解决本地长期存储结构，但 M1.5-B1 30 分钟 live Shadow 与 M1.7 24 小时持续 SLO 仍未通过。等待外部 Gate 时只允许并行 M2.0 合同/黄金样本，不开放 Detector runtime、API、页面或交易计划。
