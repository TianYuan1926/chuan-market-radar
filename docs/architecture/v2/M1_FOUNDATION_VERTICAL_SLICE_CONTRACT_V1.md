...
- 独立 `v2-m1-partitioned-fact-store.v1` additive migration 保持 M1.3/M1.5 checksum 不变；迁移后新 Fact 只能进入无 DEFAULT 的 UTC 日分区，旧账本 trigger 拒绝旁路新 Fact。
...
- PG16 真实演练证明迁移前旧 Fact 可读、两日跨分区、新旧读取、幂等/污染、保留中和活跃 replay 阻断、真实 `pg_dump -> pg_restore -> replay parity`、原子 DROP、身份同步收缩和防重灌。
- 定向 5/5、M1.6 PG16 1/1、全 V2 141 pass / 0 fail / 5 explicit external-dependency skips、完整 `ci:production` PASS；生产 migration、真实容量和持续 SLO 未证明。

M1 仍未完成。M1.6 已解决本地长期存储结构，但 M1.5-B1 30 分钟 live Shadow 与 M1.7 24 小时持续 SLO 仍未通过。M2.0 合同与 M2.1 DRAFT 纯回放内核已本地完成，Detector 仍未校准且 Candidate 禁发；等待外部 Gate 时只允许继续 M2.2 historical replay/lifecycle Gate，不开放 Detector runtime、M1 authority 读取、Candidate Store、API、页面或交易计划。
