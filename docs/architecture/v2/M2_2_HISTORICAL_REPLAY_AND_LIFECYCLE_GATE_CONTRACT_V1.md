...
日期：2026-07-20
状态：`M2.2-A_LOCAL_HARNESS_PASS / B0.1_POLICY_FREEZE_LOCAL_PASS / REAL_COHORT_MISSING / GATE_INSUFFICIENT / DETECTORS_DRAFT / NO_CANDIDATE_EMISSION / PRODUCTION_UNCHANGED`
...
- target-blind diagnostic strength、Top20 排序合同和全部 sensitivity trial registry 已冻结，但当前没有真实 cohort 可生成真实 Top20 或 sensitivity 结果。
...
4. event time、knowledge cutoff 和 lineage 完整；knowledge time 必须明确为 observed receivedAt 或冻结的保守 modeled availability，二者不得混写。
...
- `src/v2/research/historical-cohort-construction-policy.ts`
- `src/v2/research/historical-replay-gate.test.ts`
- `src/v2/research/historical-cohort-construction-policy.test.ts`
- `npm run test:v2-m2-historical-replay`

22 项定向测试覆盖：不可变 dataset identity、未来字段拒绝、purge/embargo、固定 Detector 分母、holdout group isolation、holdout 载荷物理分离与单次打开、三分母、背景误报、unavailable 保留、以 knowledge time 计算提前量、六维 TRAIN-only 阈值、任意构造政策和 trial drift 拒绝、合成样本禁晋级与 report 防篡改。
...
3. 按冻结精确对象清单取得不可变 raw archive；来源权利和历史身份未通过前继续阻断。
4. 构建完整 point-in-time feature observations、完整背景、Event 与 Matched non-event；实际执行必须遵守 B0.1 已冻结政策。
5. 冻结 train/validation、独立 holdout artifact 和外部 custodian/access ledger；代码合同已经能拒绝 inline 假隔离，但当前没有真实工件。
6. 运行全部预登记 sensitivity trials，不能漏报失败试验。
7. 单次打开真实 holdout，输出 Top20、失败案例、分层指标和独立审计。
