...
M3.1 只建立六类机会的独立结构解释层：把同一 Candidate Episode 的 Opportunity Thesis、完整 Evidence Package、point-in-time Market Context 和有事实来源的结构位，解释成一个可审计的 `AnalysisSnapshot v2`。
...
## 5. AnalysisSnapshot v2

M3.1 把 AnalysisSnapshot 从 v1 升级为 v2，新增三个不可省略的真值：
...
输出还包含 family、direction bias、structure state、market stage、location quality、结构位、支持/反证、late/fakeout/noise risk 和四维 uncertainty。输出不包含 grade、交易计划或个人风险参数。
...
- Evidence Grade 与 Setup Grade 的独立校准、置信区间和 abstain policy。
...
M3.0 回归扩展到 17/17，新增“隐藏 counter-evidence”和“未校准 Analysis 进入 authorized replay”拒绝用例；合计 M3 定向 38/38。完整出口还要求 typecheck、lint、`test:market`、全 V2、ops、M0、production build、Golden 16/16 和 security PASS。`backtest:formal` 不属于本包。
