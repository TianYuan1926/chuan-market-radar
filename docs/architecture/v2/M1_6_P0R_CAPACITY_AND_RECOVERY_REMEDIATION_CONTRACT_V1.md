...
状态：`COS_OBJECT_LOCK_COMPLIANCE_31D_PASS / AGE_X25519_KEYCHAIN_IDENTITY_PASS / EXACT_TRANSPORT_BUNDLE_PASS / SIX_HOUR_NO_COST_CAPACITY_LOCAL_MACHINE_PROOF_PASS / STS_RECOVERY_AND_FRESH_P0_PENDING / P0_BLOCKED`
...
免费保管路线固定为可信 Apple Silicon Mac 的登录 Keychain。生成工具必须验证官方 `age v1.3.1` darwin/arm64 archive SHA-256、独立推导 recipient、Keychain 读回与 recipient 一致性，只输出 mode-600 公钥和无私钥 attestation；重复项、输出覆盖、工具链漂移或读回不一致全部失败并回滚新建项。真实 age X25519 身份已经生成并完成 Keychain 读回验证；私钥未进入 Git、Bundle 或生产持久盘，该事实不等于 STS、备份或恢复 PASS。

## 6. P0R-E 生产验收

P0R-D0 零付费容量本地机器证明通过后，生产仍必须按顺序验证；六小时 schema 只能在 fresh P0 旧现场真值与新容量组合准入同时 PASS 后由 P1/P3 分阶段应用：
...
当前达到 `P0R_COS_OBJECT_LOCK_COMPLIANCE_31D_PASS / AGE_X25519_KEYCHAIN_IDENTITY_PASS / EXACT_TRANSPORT_BUNDLE_PASS / SIX_HOUR_NO_COST_CAPACITY_AND_FRESH_P0_ADMISSION_LOCAL_ENGINEERING_PASS / STS_RECOVERY_AND_FRESH_P0_PENDING`：
...
- 运行级 provisioning plan/STS credential compiler 已实现；单 AZ/region、policy/request/plan digest、上传前对象不存在和 exact version 均进入 strict evidence。macOS Keychain age vault 和 fresh admission 已加入同一门禁，P0R 定向测试当前 59/59 通过。
- 香港单 AZ 私有/versioned/SSE-COS 桶已启用并回读 Object Lock=`COMPLIANCE` 31 天；真实 age X25519 身份只保存在 macOS Keychain，checksum-bound 无 secret transport bundle 已通过。STS、P0R 对象、加密备份、exact retrieval 和隔离恢复仍未执行。
...
- fresh P0 组合准入工具已本地通过：它只替代三个旧日分区容量计算，继承所有非容量 blocker，并固定稳态 60% / 峰值 70%。fresh production topology、真实 recovery evidence 和 exact-release 校准尚未组合取得生产 PASS；因此旧 P0 仍为 `BLOCKED`，P1 仍不允许启动。

下一生产动作只能为 frozen plan 签发一组 fresh 7200 秒、精确对象范围的临时 COS 凭证，并在签发后立即于服务器 `/dev/shm` 完成 plan-bound 编译；任何既往短期 STS 均已失效且不得复用。随后执行真实同快照加密备份、远端按 exact version 取回、隔离 PG16 restore 和 fingerprint parity，并清理明文、临时凭证和隔离恢复资源；再刷新 boot/filesystem/Docker/Postgres/Redis/app health 与 topology，完整重跑 P0。任何长期凭证、付费、关机或 P1 生产写入都不由本合同隐式授权。
