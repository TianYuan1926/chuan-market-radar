...
9. 非 TEST_ONLY scope 必须同时具有 scope-matched calibrated Analysis 和 Signal Qualification authority。
...
- Analysis 具有明确方向，且与 StrategyDraft 相同。
...
- 各机会族 Strategy template、结构 RR 计算和反例集。
...
定向测试必须覆盖：授权 REPLAY READY、当前 closed state、伪造 READY、DRAFT Detector、WAIT/OBSERVE/BLOCKED/READY、低质量评级、计划篡改、release/id 拼接、时间倒流、原因隐藏、矛盾权限、未知 future 字段、确定性与深冻结。
