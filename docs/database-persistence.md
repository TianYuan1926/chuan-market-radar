...
6. 迁移成功后接口会返回当前持久化表清单，包括 `journal_events`、`scan_archives`、`rank_profiles`、`v3_forward_map_snapshots`、`ohlcv_candle_cache`、`scan_asset_states`、`macro_market_snapshots`、`frontend_ui_states` 和每日异动归因复盘相关表。
7. 部署后访问 `/api/health`。
