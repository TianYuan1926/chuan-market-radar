...
> 最后整理日期：2026-07-02。当前阶段：腾讯云香港单机生产主线，GitHub `main` 为代码正本。
...
- 风控边界不变：最低结构 RR 仍为 `3:1`；`WAIT_PULLBACK` / `WAIT_RETEST` 仍不能冒充 `TRADE_PLAN_READY`；早期放量进入 TopN 只代表“值得验证/复盘”，不代表可以直接交易。
...
- 已修正审计候选排序：新增安静压缩但方向待确认的启动前机会保底入口；新增 RR 合格、`WAIT_PULLBACK` / `WAIT_RETEST` 条件计划的保底入口。它们只能提高进入候选/深扫的机会，不能直接进入狙击榜，不能绕过 `3:1` 和风控门禁。
...
- 下一步固定为：`目标位/止损位质量复核 -> WAIT 触发有效性修复 -> 启动前机会排序配额 -> neutral_direction 早期机会误杀复查`。仍然禁止降低 `3:1 RR` 或把 WAIT 包装成狙击榜就绪计划。
...
- 当前结论：本轮不是“完成”，状态仍是 `不能支撑实战`。下一步必须优先修分析质量过滤：方向不明确、结构完整度、确认等待、趋势完整度和假阳性压制。禁止为了提高计划数量降低 RR 或把 WATCH_ONLY 包装成狙击榜。
...
- 已经晚了的币可以展示为趋势观察、等待回踩、等待反抽、风险复盘或错过机会；不能进入优质启动前候选、交易计划就绪或狙击榜。
...
- 噪音不进入主信号区，不进入狙击榜，不生成交易计划；但不一定直接删除，可以作为低权重背景记录，用于后续复盘识别是否有重复线索。
