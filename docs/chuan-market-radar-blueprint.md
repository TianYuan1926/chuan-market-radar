...
2026-06-20 最新校准：**当前前端已清空**。旧前端设计要求已失效，历史 UI 方向、历史视觉方案、历史组件壳、历史素材、历史 QA 记录和旧前端沟通结论不再作为后续前端事实源。

本次清空只针对前端展示层，不删除后端核心能力：
...
- 首页只保留最小占位页，用于证明应用仍可构建和后端入口仍可验证。

后续新前端必须从零重新设计，但不能绕开后端：

- 新前端必须消费后端只读契约，不能复制旧组件里的临时逻辑。
- 新前端不得伪造扫描、伪造 K 线、伪造信号、伪造运行状态。
- 新前端必须清楚区分全市场资产池、当前扫描批次、当前候选信号、深扫候选、人工复盘样本和策略演化样本。
- 新前端不能因为布局、分页、Top N、折叠或视觉选择，让用户误以为系统只扫描了当前可见的几个币。
- 新前端设计开始前，先重新确认产品信息架构和数据契约；确认后再进入视觉设计和实现。
- 旧前端文件、旧 UI 方案和旧 QA 文档不得作为恢复对象，除非用户明确要求查看历史。
...
2026-06-20 最新校准：当前前端已清空。后续互动只允许服务真实系统状态、风险提醒、数据新鲜度和复盘动作。
...
| 阶段 8：前端重建 | 当前前端已清空；旧首页组件、旧视觉素材、旧 UI QA 和旧 UI reset 方案已删除；首页只保留最小占位页 | 后续从零重新设计前端信息架构和视觉系统。新前端必须接入 `/api/radar/backend-contract`、`/api/radar/dossier`、`/api/health`、`/api/scan` 等后端契约，先保证信息完整、运行可信、可读性和稳定性，再做视觉精修 |
...
- 生产访问地址已生成。
- 当前前端已清空，首页只保留最小占位页，避免旧视觉和旧交互继续影响新设计。
- 后端事实出口已落地：`GET /api/radar/backend-contract` 输出扫描证明、轻扫全市场状态、深扫候选、状态池分配、轮转健康审计、数据质量、v3 覆盖和进化闭环边界；`GET /api/radar/dossier?symbol=SYMBOL` 输出单标的 TradingView 外链、可用周期、v3 关键位/Forward Map、Evidence 和 Journal 样本。后续前端重建必须优先消费这些只读契约，避免绕开扫描、分析和复盘主链路形成“两张皮”。
...
- 2026-06-21 已落地 CoinGlass 请求 pacing MVP：新增 `COINGLASS_REQUEST_INTERVAL_MS`，生产默认 `500ms`。主深扫和 daily mover 的 CoinGlass 请求会按间隔排队，避免 30 次/分钟预算被瞬间打爆。该 pacing 只服务稳定性，不提高策略信号等级，也不绕过每日预算、分钟限速、缓存和失败降级。
...
2026-06-20 最新校准：当前前端展示层已经清空。旧 UI 组件、旧视觉素材、旧前端 QA 记录和旧 UI reset 方案不再作为当前事实源。

当前状态：

- 首页只保留最小占位页，显示“前端已清空”。
- 后端 API、扫描、数据库、复盘、分析引擎、告警策略和 Worker 保留。
- 后续新前端必须重新设计信息架构、页面结构、视觉系统和交互方式。
- 后续新前端必须优先消费后端只读契约，不能复活旧组件或复制旧前端临时逻辑。
