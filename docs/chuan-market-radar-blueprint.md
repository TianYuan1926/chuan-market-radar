...
- 剔除宠物、小人、副驾驶、S680、座驾、装备、角色台词和娱乐彩蛋作为后续主线模块；后续 UI 只保留真实运行状态、提示音、扫描动效和可关闭的状态反馈。
...
2026-06-19 已落地免费公共轻扫层 MVP：生产环境默认使用 Binance public futures 24h ticker 做全市场轻扫，快速获取 USDT 永续候选、成交额、24h 波动、靠近高低点和压缩/异动标签。轻扫结果只能生成 `public light scan priority hints`、补充 universe instruments 和状态诊断，不能直接生成 EvidenceItem、交易方向、买卖建议或绕过 CoinGlass 深扫。CoinGlass 仍是深扫资金质量确认器，轻扫只负责“全市场先看一遍”和“别让深扫永远停在固定几个币”。
...
后续 UI 不是传统 dashboard，而是 **Living Radar UI 活体雷达界面**：合约雷达台、交易复盘舱、运行监控台的结合体。它要动态、有交互、有运行感，但所有动态都服务识别机会和理解风险。
...
2026-06-17 进一步校准：这次前端工作定义为 **UI Reset**，不是在旧页面上继续加样式类名。后端能力、API、CoinGlass、Neon、扫描、日记、段位、回放和规则引擎保留；首页信息架构、视觉系统、组件壳、动效反馈、响应式布局和 CSS 组织重建。正式设计依据见 `docs/superpowers/specs/2026-06-17-ui-reset-living-radar-cockpit-design.md`。

2026-06-19 最新校准：用户明确要求“当前前端 UI 全部剔除，按照 Figma Make 黑金雷达稿搭建”。2026-06-18 的浅色 Light Liquid-Glass 方案降级为历史探索，不再作为当前首页方向。当前正式方向是 **CHUANSCAN Black/Gold Anomaly Radar**：黑色专业数据底、金色/琥珀强调色、顶部 CHUANSCAN/川 品牌导航、市场行情带、KPI 墙、主信号卡网格、右侧实时预警/市场热力/扫描证明和信号档案抽屉。旧的 2 : 6 : 2 cockpit、浅色液态玻璃 banner、左侧扫描控制台、中心 chart-first 主舞台、右侧 Action Rail、旧大面积互动角色面板均不再作为首页首屏骨架。

新 UI 骨架规则：

- 技术方向必须真实接入 **Tailwind CSS + daisyUI** 后才能宣称使用；Element UI / Element Plus 只作为组件参考，不作为主实现栈，因为当前项目是 Next.js / React。
- 顶部使用 **compact navbar / product banner**，承载“川 / CHUANSCAN”品牌、主导航、刷新动作、设置入口和运行状态；禁止回到纯文字导航或浅色旧 banner。
- 首页主体使用 Figma 黑金雷达的信息架构：行情 ticker strip -> KPI wall -> 主信号卡网格 -> 选中计划 dock -> 右侧实时预警/市场热力/扫描证明/助手。桌面可以左右分栏，但不再固定旧 2 : 6 : 2 cockpit。
- 首页第一屏必须以信号卡网格为主，而不是把图表、复盘、日记、段位、回放全部堆在一起；完整 Signals、Review、Journal、Evolution、Settings 进入功能抽屉或二级页。
- 用户提供的视觉图和 Figma Make 导出只作为视觉方向和信息结构参考；不能复制 mock 数字、`Math.random()` K 线、伪造交易标签或 Figma 静态业务逻辑。
- 参考图应体现为黑金材质、紧凑数据面、信号卡密度、右侧运行状态和“川”品牌，不作为全屏背景；首屏必须优先保证行情、信号、关键位、策略和运行状态可读。
...
- 首页主舞台允许只展示 Top 候选，但必须同时显示完整候选池入口、当前总信号数、首页展示数、隐藏数和分组总数。Signals / Scan / Coverage / Dossier 等完整工作区必须承接全量数据，不允许核心信号只存在于后端或隐藏抽屉深处且无入口提示。
- 首页和二级模块允许做 Top N 预览，但 **不允许静默截断**：任何 `slice/top/limit/max-height/line-clamp` 只要影响候选、证据、风险、关键位、复盘样本或执行计划，都必须同时给出总数、当前展示数、隐藏数、完整入口或展开路径。
- 前端信息架构必须明确区分三层：全市场资产池、当前深扫批次、当前候选信号。三者不能混成一个“当前几个币”的视觉印象；用户必须能看到本轮、下一轮、排队、久未扫、被预算限制、被风控阻断和复盘复活的状态。
- TradingView 和图表命名必须诚实。真实 TradingView 嵌入才可称为 “TradingView 实时图”；系统自己绘制的 K 线/结构层必须称为“系统结构图”或“只读结构预览”；占位图、简化预览或外链按钮不能冒充站内真实图表。长期方向是 TradingView 负责真实盘面，自研结构图负责关键位、Evidence、Risk Gate、Forward Map 和复盘轨迹。
...
```text
Compact Navbar / CHUANSCAN Banner
-> Market Ticker Strip
-> KPI Wall
-> Main Signal Card Grid
-> Selected Plan Dock
-> Right Rail: Alerts / Market Heat / Scan Proof / System Status
-> Drawers: Signals / Review / Journal / Evolution / Settings
-> Signal Dossier 深挖
```
...
信号档案应展示：当前信号状态、多周期证据、v2 证据审计、v3 关键位地图、Forward Map、CoinGlass 衍生品证据、TradingView/K 线入口、日记历史、涨跌榜归因记录、告警状态、复盘样本和策略版本关联。
...
2026-06-20 最新校准：宠物、小人、副驾驶、S680、装备升级、角色台词和娱乐彩蛋不再作为后续搭建项。后续互动只保留与真实系统状态相关的运行反馈。
...
- 宠物角色、小人角色、副驾驶角色、S680 座驾、装备系统、角色台词和娱乐彩蛋。
...
- 生产访问地址已生成。
- 页面首屏现在以主雷达工作台为核心：顶部品牌/运行状态、市场 ticker、KPI 墙、主信号卡网格、选中计划 dock、右侧预警/热力/扫描证明、功能抽屉、Macro Weather 和生命周期预览。复盘日记、段位、扫描回放、每日异动归因等完整模块不再全部堆在首屏，而是通过导航、功能抽屉、信号档案或二级页面承接。
- 信号档案基础已落地：点击候选池、信号地图或热区匹配项可打开同一标的档案；桌面为右侧抽屉，移动端为底部上滑面板；档案复用现有扫描、日记、每日异动、告警、TradingView 链接和 v3 关键位地图，不新增 CoinGlass 请求。
...
- 生产环境默认接入 Binance public futures 24h ticker 作为免费全市场轻扫层；轻扫结果会补充 universe、生成深扫优先级提示并写入 `metadata.lightScan`，但不直接生成交易信号。
...
当前已有日记、段位和信号生命周期复盘基础层；早期宠物方向已废弃，不再作为复盘进化主线：
...
2026-06-20 最新校准：早期角色化互动方向已废弃。后续 UI 不再建设宠物、小人、副驾驶、座驾、装备、角色台词或娱乐彩蛋；历史实现只能作为旧代码清理参考，不能作为新路线。

2026-06-18 已按用户当时选定参考图完成浅色首屏 UI Reset 探索版，并完成 Product Design QA。该方向在 2026-06-19 已被 Figma Make 黑金 CHUANSCAN 方向覆盖，以下内容只作为历史记录，不再作为当前首页事实源：
...
2026-06-19 已按 Figma Make 黑金 CHUANSCAN 参考完成当前首页重构，并完成 Product Design QA：

- 首页渲染入口已从 `RadarWorkspace` 切换为 `ChuanScanWorkspace`；旧浅色 cockpit、左侧扫描控制台、中心 chart-first 主舞台、右侧 Action Rail 和大面积互动角色面板不再作为当前首屏。
- 当前首页结构为 compact navbar、行情 ticker strip、KPI wall、主信号卡网格、选中计划 dock、右侧实时预警、市场热力、扫描证明和系统状态。
- Signals、Review、Journal、Evolution、Settings 保留为功能抽屉；信号详情进入 `chuan-dossier` 档案层；首页不再承载所有完整模块。
- 新 UI 全部消费现有 `snapshot`、`health`、`backendContract`、`journalEvents` 和 `dailyMoverArchive`；不复制 Figma mock 数组、不使用 `Math.random()` 伪造 K 线、不绕开后端扫描契约。
- 信号卡支持真实筛选：全部、看多候选、看空候选、突破/临界、观察池、高危预警；隐藏候选必须显示“查看剩余 N 个候选”，不能静默截断。
...
- 当前已经通过 `npm run test:market`、`npm run lint`、`npm run build` 和单独 `npm run typecheck`；`design-qa.md` 记录本轮视觉验收，最终结果为 `passed`。后续还需要继续做 Signal Dossier 视觉精修和更专业的真实图表表现。
...
- 执行策略、证据链、每日异动关联、复盘记录和告警状态被分成更清晰的功能区，继续围绕同一标的上下文联动，避免首页重新堆满细节。
- 移动端继续保持底部上滑档案形态；桌面端保持右侧深挖抽屉。后续正确顺序转为更专业图表和交互式关键位选中态。
...
- 主视觉是黑金 CHUANSCAN 全市场山寨异动雷达，不是传统后台表格、PPT 页面、游戏化页面，也不是 2026-06-18 的浅色液态玻璃 cockpit。
- “川”是核心品牌符号，顶部、启动动画、favicon、雷达刻印和关键入口都应逐步体现。
- 用户提供的图像和 Figma Make 导出用于视觉方向、材质和信息架构参考，不能铺满成壁纸、遮挡行情、削弱专业度或带入静态 mock 业务逻辑。
- 首页必须有运行感：行情带、刷新状态、候选变化、实时预警、扫描证明、数据新鲜度、stale 降级和 session clock。
- 首页不展示所有功能；完整复盘、每日异动、段位、回放、AI 反证和策略进化进入导航、功能抽屉、信号档案或二级页面。
- 宠物、小人、副驾驶、座驾、装备和娱乐彩蛋不再作为 UI 主线；任何动效必须服务状态理解、数据可信度和风险提醒。
...
- UI 不能为了视觉趣味破坏核心市场分析稳定性、可读性和状态边界。
...
- V1.6 已落地：主要面板中文优先；早期角色化宠物方向已在 2026-06-20 废弃，不再作为后续目标。
- V1.8 历史验证过 `PixelCopilot` 角色边界；2026-06-20 后该方向废弃，`PixelCopilot`、宠物、小人、副驾驶、S680、装备和角色台词都不再进入后续 UI 主线。
...
- Phase 3.9 已落地：右栏新增 `MacroWeatherPanel`，用现有扫描快照中的 BTC/ETH ticker、funding、OI、杠杆风险字段和扫描状态生成顺风、逆风、震荡、杠杆拥挤、去杠杆、波动扩张、未知等天气层；该层只用于解释山寨候选的顺逆风和风险环境，不新增 CoinGlass 请求，不修改真实策略权重。
- Phase 8.2e 已落地：按用户选定浅色专业工作台参考图重建首屏信息架构。`TopRadarBar` 统一品牌 banner、液态玻璃雷达之眼、主导航、运行状态条和 ticker；`RadarWorkspace` 首屏保留左侧雷达控制台、中央信号主舞台、候选横条、首屏 Chart/Strategy 和右侧 Action Rail；Altcoin Opportunity Board 下沉为主图后的辅助筛选区；完整 DailyMover/Journal/Replay/Rank 不再常驻首页，而是进入功能抽屉、信号档案或二级页面。该浅色方向已是历史探索，不再作为当前 UI 主线。
- 2026-06-19 当前前端基线已切换：用户要求剔除现有前端 UI 后，首页入口改为 `ChuanScanWorkspace`，按 Figma Make 黑金 CHUANSCAN 参考重建为 compact navbar、行情带、KPI 墙、信号卡网格、选中计划 dock、右侧预警/热力/扫描证明/系统状态和功能抽屉。该基线使用真实 `snapshot` / `health` / `backendContract` / `dailyMoverArchive`，不复制 Figma mock 业务逻辑。旧浅色 cockpit 只保留为历史探索记录，不再作为当前 UI 方向。
...
1. **Phase 8.2h：Signal Dossier Visual Upgrade**
   - 当前状态：已完成。信号档案已按浅色工作台风格升级为证据室，关键位、Forward Map、证据链、交易计划、复盘关联和纪律状态同一上下文联动。
   - 深层证据继续放档案，不回到首页堆叠。

2. **Phase 8.2i：运行状态反馈边界**
   - 当前状态：已重新校准。旧 Pixel Copilot Motion / Equipment 方向废弃。
   - 后续只保留真实运行反馈、数据新鲜度、扫描心跳、告警提示和可关闭动效；不再建设宠物、小人、副驾驶、装备或角色台词。
...
38. **Phase 4V3-25：AI Counter-Evidence Production Boundary**
   - `AiSignalReview` 已新增 `boundary`：固定 `allowedUse=counter_evidence_review_only`、`canOverrideDecision=false`、`canMutateLiveRanking=false`、`canCreateTradeSignal=false`、`canAutoExecute=false`。
   - `reviewSignalWithAi()` 已把缺 key、关闭、prompt 超预算、模型失败、解析失败和成功复核全部转成可见边界；超过 `AI_REVIEW_MAX_SIGNALS` 的候选也会显示预算保护，而不是静默没有 AI。
...
40. **Phase Backend-1：后端事实契约与单信号档案 API（已落地）**
