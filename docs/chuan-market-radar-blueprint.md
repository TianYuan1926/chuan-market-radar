...
2026-06-19 最新校准：用户明确要求“当前前端 UI 全部剔除，按照 Figma Make 黑金雷达稿搭建”。2026-06-18 的浅色 Light Liquid-Glass 方案降级为历史探索，不再作为当前首页方向。当前正式方向是 **CHUANSCAN Black/Gold Anomaly Radar**：黑色专业数据底、金色/琥珀强调色、顶部 CHUANSCAN/川 品牌导航、市场行情带、KPI 墙、主信号卡网格、右侧实时预警/市场热力/扫描证明、小型川川助手和信号档案抽屉。旧的 2 : 6 : 2 cockpit、浅色液态玻璃 banner、左侧扫描控制台、中心 chart-first 主舞台、右侧 Action Rail、旧大面积宠物 dock 均不再作为首页首屏骨架。
...
-> Selected Plan Dock
-> Right Rail: Alerts / Market Heat / Scan Proof / Chuan Assistant
...
- 首页渲染入口已从 `RadarWorkspace` 切换为 `ChuanScanWorkspace`；旧浅色 cockpit、左侧扫描控制台、中心 chart-first 主舞台、右侧 Action Rail 和大面积宠物 dock 不再作为当前首屏。
- 当前首页结构为 compact navbar、行情 ticker strip、KPI wall、主信号卡网格、选中计划 dock、右侧实时预警、市场热力、扫描证明和小型川川助手。
...
- 主视觉是黑金 CHUANSCAN 全市场山寨异动雷达，不是传统后台表格、PPT 页面、低龄像素游戏页，也不是 2026-06-18 的浅色液态玻璃 cockpit。
...
- 宠物/助手主角是小型川川助手，以 BTC/加密识别、装备升级、情绪话唠和纪律反馈为核心；它是互动层，不是主信息架构，不能占大面积空间。
