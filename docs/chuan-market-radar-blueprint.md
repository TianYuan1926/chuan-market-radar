...
2026-06-24 起，网站唯一核心目标固定为：

```text
提前发现有潜力的山寨币异动，并判断它有没有交易价值。
```
...
- 有用：保留，但必须挂到链路中的某一环。
- 有点用但不核心：降级为 supporting 或观察状态。
- 重复：合并，避免同类信息多处重复展示。
- 会误导：重构，让它只能展示真实边界和 partial/waiting。
- 没实战价值：删除。
...
- 全市场扫描：发现所有可交易山寨，不只盯熟悉币。
- 秒级轻扫：用 Binance / OKX / Bybit WebSocket 或 public ticker 找价格、成交、盘口、放量和异常。
- 候选池：只放“值得继续验证”的币，不能伪装成交易信号。
- CoinGlass 深扫：验证 OI、Funding、多空拥挤和资金质量。
- 结构分析：判断趋势、压缩、突破、假突破、关键位、回踩和衰竭。
- 风险赔率：最低结构 RR `3:1`，不够直接拦截。
- 单币档案：每个币必须讲清楚为什么看、怎么做、哪里错。
- 交易计划：只有证据完整才生成，不能由前端编。
- 复盘进化：验证系统准不准，找漏判和错判。
- 数据源健康：明确哪些数据 live、cached、partial、stale、failed 或 unavailable。

辅助功能，保留但不能抢主线：

- 榜单：只能作为市场观察，不等于推荐。
- 大盘环境：BTC、ETH、BTC.D、TOTAL2、TOTAL3 只判断顺风逆风。
- DEX 新币观察：只做早期观察池，不直接进狙击榜。
- AI 反证：只审查成熟候选，不替代规则引擎。
- 告警：只提醒状态变化，不制造交易结论。
- 段位 / 宠物 / 彩蛋：只做纪律反馈、复盘反馈和系统状态提示。
...
1. `LIGHT_SCAN_MARK`：只说明这个币有异常；不能给方向，不能给计划。
2. `DEEP_SCAN_CANDIDATE`：说明值得验证；可以展示原因，但不能当交易信号。
3. `EVIDENCE_SIGNAL`：已有结构和数据支持；可以重点观察，但不一定能交易。
4. `TRADE_PLAN_READY`：必须具备真实数据源、当前阶段、支持证据、反证、多周期结构、关键位、入场条件、止损、目标、`RR >= 3:1`、风险门控通过、失效条件和复盘追踪。只有这一层能进狙击榜。

### 当前工程落地

- 新增 `src/lib/api/core-chain-governance.ts`，把核心链路、功能分级、页面职责、清理规则和分析顺序变成后端只读治理报告。
- `/api/radar/backend-contract` 通过 `analysis.coreChainGovernance` 暴露该报告。
- `RadarContract.coreChainGovernance` 已接入前端合同，后续总控、系统页或审查面板可以直接读取该字段展示核心/辅助/降级/合并/重构/删除分类，不需要前端重新推理。
- `coreChainGovernance` 只做产品治理和验收，不创建交易信号、不改变 live ranking、不自动下单、不自动调权。
- 后续所有新功能必须先通过该治理层判断：核心、辅助、降级、合并、重构或删除。
...
4. **分析推理报告展示缺口**：前端关于分析、推理、反证、风险门控和交易计划的展示过于简陋，不能体现后端 Evidence / Market Reading / Key Level / Forward Map / Risk Gate / Review 的能力。后续必须把“事实、证据、推理、阻断、计划、复盘反馈”分层展示，并保证每条中文解释可追溯到后端 EvidenceItem 或只读 review 样本。
   ...
   - 2026-06-24 已完成第三轮合同修复：Token Dossier 的 `reportSections` 标题和首条信息改为更直接的实战问答层：`当前结论`、`为什么值得看`、`哪里可能错`、`现在能不能做`、`怎么做`、`复盘与 AI 边界`。该层只翻译后端 `signal`、Evidence、Risk Gate 和 v3 Trade Plan，不允许前端编造入场、止损、目标或交易方向。
...
- 2026-06-21 已落地 CoinGlass 请求 pacing MVP，并在 2026-06-24 生产复测后把默认值从 `500ms` 逐步加固到 `3000ms`。原因是 Hobbyist 标称 `30 调用/分钟`，但部署重启、worker、能力探针和深扫可能共享同一账号额度；过高瞬时速率会触发 `429 Too Many Requests`。主深扫、能力探针和 daily mover 的 CoinGlass 请求必须按间隔排队；该 pacing 只服务稳定性，不提高策略信号等级，也不绕过每日预算、分钟限速、缓存和失败降级。
