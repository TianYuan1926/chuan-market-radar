...
- 强制整改方向：
  ...
  2. 下一包必须重整“启动前质量判定主干”：压缩、早期放量、关键位靠近、相对强弱、盘口/成交压力必须形成组合评分，不能让弱组合进入 `premium_early_setup`。
  3. `structure_repair_pending` 必须再分层：真修复机会、无效反抽/反弹、风险复盘教材。不能统一塞进 WAIT。
  4. WAIT 条件计划必须加二次确认质量：触发后不能先止损；如果触发即先止损，要回查止损位置、触发 K 线、目标位和波动缓冲。
