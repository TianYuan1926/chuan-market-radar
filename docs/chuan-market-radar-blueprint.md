...
AI 输出必须绑定证据项、信号 id、复盘样本或已知后端事实。
...
- `npm run backtest:professional` 是专业回测审计 v2 的正式命令。它当前复用生产 `analyzeMarketAnomaly`、技术指标、多周期、v3 dossier、RR、交易计划和成熟度分类，并把历史衍生品缺失明确输出为待整改问题。
...
- `/review` 的历史回测面板必须兼容 `auditV2`，展示专业回测 v2 的问题归因和整改方案；如果只有旧版历史回放报告，则只能显示 v1 能力边界。
