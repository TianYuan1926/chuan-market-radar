...
| 阶段 2：真正多周期分析引擎 | 基础已落地，受限主候选已接入真实多周期 OHLCV profile 和指标矩阵摘要 | 尚未完成前端多周期矩阵、完整权重校验和全量候选覆盖 |
| 阶段 3：合约 universe registry | 基础、三交易所自动发现、分层币池、低频轮转、覆盖差异、quota 护栏、动态优先级和 repository hints 基础已落地 | 尚未完成高优先级加密扫描和覆盖差异前端展示 |
| 阶段 4：OHLCV 与技术指标 | 基础已落地，受限主候选已接入 `1m/5m/15m/30m/1h/4h/1d/1w` candles、MACD、近似成交量分布、指标矩阵摘要和策略卡前端矩阵基础展示 | 尚未完成完整权重校验、交互式多周期图表和更专业的成交量分布模型 |
...
但这仍不等于完整多周期融合：当前只覆盖受限主候选，前端还没有完整多周期矩阵，指标矩阵也只是摘要型 evidence。后续必须补齐 UI 展示、复盘权重校验和更细的候选分层策略，才能算完整专业闭环。
...
当前已具备基础技术指标计算，并已把受限主候选的多周期 candles 传入指标证据构建器。当前输出已经包含多周期指标矩阵摘要、MACD 动能和近似成交量分布，策略卡已能展示紧凑前端指标矩阵；但它仍是 evidence 摘要级 UI，不是完整交互式指标图表、专业成交量分布系统或权重校验闭环。后续必须补齐结构确认能力、权重校验和更专业的成交量分布模型。
...
当前状态：基础已完成，受限主候选多周期 candles 接入和指标矩阵摘要已完成，完整多周期融合未完成。
...
- 多周期 candles 已同时进入 technical indicators 和 timeframe profile，并能输出指标矩阵摘要；后续要补齐矩阵展示和复盘权重校验。
...
- 已完成：策略卡前端紧凑指标矩阵基础展示。
- 已完成：策略不直接由单指标触发。
- 未完成：指标与多周期 Profile 的统一权重校验、交互式多周期图表、更专业的成交量分布模型。
