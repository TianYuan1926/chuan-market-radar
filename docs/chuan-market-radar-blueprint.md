...
- 结论：本轮整改修复了报告可读性、v2 合同映射和漏判归因透明度，排序捕获率有小幅改善，但没有根治核心选币能力。radar 仍未跑赢动量基线，Top10 捕获率仍远低于 45% 验收线，RR/计划就绪问题仍是阻断项。
- 下一轮不能继续只调单一分数；必须做更深层整改：分短线/中线/大周期候选池、把回踩确认与大周期低位机会从统一 Top10 中拆出专用 lane、并修复 RR/目标位生成和计划就绪诊断。
