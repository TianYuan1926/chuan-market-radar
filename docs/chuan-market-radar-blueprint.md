...
| 阶段 6B：每日异动归因复盘 | 逻辑、数据源适配器、抓取写入服务、受保护 API、公开只读 API、外部 cron 策略、schema、repository、公开复盘面板、扫描归档/复盘日记只读关联摘要及其 UI 展示已落地 | 尚未完成历史样本选择、单样本详情和规则校准建议 |
...
- 在 UI 中增加历史样本选择、单样本详情和“为什么漏判”的联动说明。
