...
| 阶段 4：OHLCV 与技术指标 | 基础已落地，受限主候选已接入 `1m/5m/15m/30m/1h/4h/1d/1w` candles、MACD、近似成交量分布和指标矩阵摘要 | 尚未完成前端指标矩阵、完整权重校验和更专业的成交量分布模型 |
...
当前已具备基础技术指标计算，并已把受限主候选的多周期 candles 传入指标证据构建器。当前输出已经包含多周期指标矩阵摘要、MACD 动能和近似成交量分布，但仍是 evidence 摘要，不是完整前端矩阵和专业成交量分布系统。后续必须补齐结构确认能力、权重校验和 UI 矩阵。
...
- 前端多周期指标矩阵：`1m/5m/15m/30m/1h/4h/1d/1w` 同币种指标证据可视化。
...
- 已完成：策略不直接由单指标触发。
- 未完成：前端矩阵展示、指标与多周期 Profile 的统一权重校验、更专业的成交量分布模型。
