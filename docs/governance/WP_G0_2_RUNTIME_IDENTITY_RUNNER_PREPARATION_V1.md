...
当前 8 文件 artifact SHA-256 为 `0d40fdf0dc1739daf7d9b4facf2d69db7a97993e013ab1b56c1fd1f72cfe0650`。旧 checksum `d471bd5a7c0c062715946b07359ccf0777ee998d826d2161572875f101cdbd49` 及更早值只保留为历史事实，不得再用于后续身份审批。
...
```text
PASS_LOCAL_RUNTIME_IDENTITY_WEB_READINESS_REMEDIATION
```

它证明 runner、回滚和当前 release 合同可进入精确生产授权准备，并证明 Web 重建后必须等待容器 healthy 且容器内 health 达到 ready/database ready/fresh，随后才能执行身份探针；同一等待门禁也保护回滚验证。它不证明生产角色或 URL 已配置。生产运输、外部授权、lease/fencing 和 transient systemd 入口另由 `wp-g0-2-runtime-identity-production-execution.v1.json` 约束；两份合同必须同时 PASS。
