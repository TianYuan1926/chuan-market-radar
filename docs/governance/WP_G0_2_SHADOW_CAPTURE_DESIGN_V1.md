...
2. Quarantine 的审批化 forward-fix/resolution ledger 尚未实现；当前设计选择永久阻断晋级，而不是静默跳过。
3. Production API/worker/composition root 尚未接线，五个 Candidate Feature Flag 继续关闭。
...
下一包仍是本地准备：补 quarantine resolution ledger、生产 composition 的 fail-closed 接线设计、009 精确 checksum/权限/回滚/监控/审批材料。全部通过后才允许向用户申请生产审批。
