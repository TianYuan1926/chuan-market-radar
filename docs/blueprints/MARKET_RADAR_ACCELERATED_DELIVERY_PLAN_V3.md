...
恢复后的基线健康不再依赖生产宿主机上的 Node 或 Compose 环境插值；Drain 与 Cycle-7 runner 都直接验证 `/api/health`、三个 Radar 合同端点、Postgres `pg_isready` 和 Redis `PONG`。任一真实条件不满足仍分别按原 1,200 秒恢复上限和 600 秒启动上限失败并回滚，门槛没有降低。
