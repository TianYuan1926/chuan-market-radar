...
- 固定通道不得接收任意命令、命令参数、`.env`、Token、数据库 URL、COS STS 或私钥，不开放入站端口，也不得取代 package runner 自身的 lease/fencing、checkpoint、rollback 和 evidence；生产 timer 已安装，首个真实 signed dispatch 已完成 publish/pull/verify/launch/package acceptance，普通无 secret 运输闭环通过，但任何业务包仍必须单独通过自身生产验收。
