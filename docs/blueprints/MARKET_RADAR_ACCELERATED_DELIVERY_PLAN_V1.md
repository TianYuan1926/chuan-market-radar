...
| 11 | WP-G0.2 production Web identity recovery | B -> A | local runner/transport full gates PASS / production prohibited | exact HEAD、runner commit、Recovery/contract/bundle、override/wrapper/Compose/env/image 指纹；脱敏 bundle 只进入仓库外 staging 并在执行后清理；只允许 no-build recreate Web；成功证明 health ready/fresh 与其它容器不变，失败恢复 pre-recovery baseline；仍缺最终 commit、动态预检与 exact approval |
