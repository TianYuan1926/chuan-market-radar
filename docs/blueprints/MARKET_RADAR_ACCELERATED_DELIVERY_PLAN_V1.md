...
容量/恢复、Add Schema、production verify-only、Shadow Safety Schema 009、本地 Composition Wiring 和 Dormant Deploy 本地准入已形成闭环证据。Dormant 生产只读预检确认生产仍为旧 release、worktree clean、11 个容器正常、Candidate 休眠边界和核心 health/contract PASS；runner/production-check 统一按 `.env` -> `.env.production` 加载，Bash 3.2 隔离演练覆盖 web-only 成功与自动回滚。最新审计发现 14 文件 artifact 无法证明完整 Web release 未夹带其它代码，已新增 release-diff 祖先/allowlist/path-set 门禁；包含 Review、Canonical read、activation 或 reconciliation 的后续功能分支不再可能仅凭相同 14 文件 checksum 获批。同时修复 market test 编译目录不清理和 TypeScript 增量缓存导致的跨分支旧测试/0 测试假绿灯，真实本分支基线为 952 pass。首次 Web-only 生产尝试还暴露了新容器启动竞态和回滚后误用旧 verification script 两个 P1，现 runner 必须先等待 Web readiness，并以批准源码中的 production-check 对回滚后的生产根目录验真。当前 Dormant artifact=`e56d37ff17a34b60e65bdfdb86865691e9b91cdb160b5afaa7940a027deb2b0a`，Runtime Identity artifact=`855f8e0d72bb30cb65852c91efa6f89d5c325d9c8eb91f51e02acc7f028070a2`；旧值全部失效。生产 Candidate schema 仍是 migration 1-9 applied/verified/dormant，runtime deployment=false、control lifecycle 未启动。下一步只能在最终修复 commit 进入 GitHub main 后，重新只读确认生产 rollback 和 health ready，再申请 exact commit + artifact + release diff + web-only + 90 分钟审批；即时检查后还必须做 ledger/control 只读核验和 30-60 分钟观察。之后才可单独申请 Runtime Identity and Permission。Writer、backfill、dual read 和 read cutover 继续禁止。
