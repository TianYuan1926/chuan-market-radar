...
| 11 | WP-G0.2 production Web identity recovery | B -> A | fingerprint truth remediation full gates 14/14 PASS / production prohibited | 最终动态预检发现旧合同的 override/wrapper SHA 为人工转录错误而非生产漂移；旧 Recovery/contract/bundle 全部失效，真实指纹与新 artifact=`340ab9db...` 已重锁，定向 12/12、基础与自治 14/14 PASS。仍需最终 commit/main、clean final bundle、执行前动态再确认和 exact approval；生产只允许 no-build recreate Web，persistence 失败才回滚，persistence 已恢复但 scan 未 fresh 时保留正确身份并返回 PARTIAL/阻断晋级；只有 health ready/fresh 才 PASS |
...
容量/恢复、Add Schema、production verify-only、Shadow Safety Schema 009、本地 Composition Wiring 和 Dormant Deploy 本地准入已形成闭环证据。首次 Web-only 生产尝试虽自动回滚到旧 release，却暴露回滚/重建未带上仓库外最小权限身份 override，Web 因此重新拿到旧数据库凭据，`scan_archives` 与 `journal_events` 持久化读取认证失败，生产 health 真实降级；`marketDataQuality=degraded` 是并存的数据质量事实，不是总 health 的计算根因。部署器现要求身份 override 为绝对路径普通 `0600` 文件并绑定 SHA-256，正常部署与回滚使用同一 Compose 数组，启动后比较 Compose 预期身份与 Web 实际身份指纹；checksum 漂移在任何生产 Git/Docker mutation 前 fail closed。最终动态预检又发现旧合同的两条身份文件 SHA 来自错误人工转录；生产文件自 2026-07-11 创建后未漂移，真实 override=`1b7f8ba4...c478bacf`、wrapper=`fb473dc3...a369f1f3` 已重新锁定，旧 Recovery/contract/bundle 一律失效。release-diff 继续锁定祖先、149 个 A/M allowlist 路径和 path-set checksum；真实 market test 基线仍为 952 pass。当前 Dormant artifact=`a82ed943e7eae1df94bacb0f0c11439586e14fc3127564556215162bb8d82a50`，Runtime Identity artifact=`95c50a233b4587234ab574d76de0a02fc870c481ed3fed04317112044cb40178`；旧值全部失效。生产 Candidate schema 仍是 migration 1-9 applied/verified/dormant，runtime deployment=false、control lifecycle 未启动。Web identity recovery 使用脱敏 checksum bundle、仓库外 `0700` staging、exact source commit/contract/bundle 绑定与 success/failure 自动清理，避免“禁止生产 source sync 后 runner 无法到达”或临时脚本污染服务器。下一步在完整本地门禁与最终 GitHub main commit 后生成 clean final bundle并单独批准仅重建旧 Web；确认 health ready 后，才能申请 exact commit + 两项 artifact/release diff + identity override checksum + web-only + 90 分钟 Dormant 审批。即时检查后仍必须做 ledger/control 只读核验和 30-60 分钟观察。之后才可单独申请 Runtime Identity and Permission。Writer、backfill、dual read 和 read cutover 继续禁止。
