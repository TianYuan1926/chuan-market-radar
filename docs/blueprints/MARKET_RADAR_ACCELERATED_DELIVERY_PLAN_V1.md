...
| 11 | WP-G0.2 dormant runtime deploy | A | identity-hardening artifact refresh PASS / production prohibited | 预检与 Bash 3.2 隔离执行/回滚保持 PASS；当前 14 文件 artifact 纳入安全关键 `transaction-adapter.ts`，旧 13 文件 checksum 已失效且不得再审批；仍需新 exact commit + current checksum + web-only + 90 分钟独立审批 |
...
| 13 | WP-G0.2 activation + observation runner preparation | B | local PASS / production prohibited | 双重授权、future artifact、即时/观察失败自动回滚、24h/289 样本 evaluator 和真实 PG16 control start/rollback 均通过；当前代码授权保持 false |
| 14 | WP-G0.2 activate + shadow observation | A | prohibited | dormant + runtime identity final PASS、future activation release + 独立审批；启动 72h lifecycle 和不少于 24h clean window |
| 15 | WP-G0.2 shadow_verify/reconciliation runner preparation | B | local PASS / production prohibited | 逐笔 Source payload hash、唯一 Event、完整 projection command hash、24h/289 样本、>=10,000 writes、0 difference、只读事务和不自动切 phase 已实现；完整基础/安全门禁 PASS |
| 16 | WP-G0.2 shadow_verify/reconciliation | A | prohibited | shadow_capture 稳定、>=10,000 compared writes、0 difference、无 unresolved 状态 + 独立审批 |
| 17 | WP-G0.2 canonical compat read model local preparation | B | local PASS / production prohibited | 固定 asOf/release/observation+due cohort/checkpoint/eg.v1；partial/unavailable、Reader 最小权限、双 24h 零差异窗口和 Fail-Closed 路由完整门禁 PASS |
| 18 | WP-G0.2 canonical read compatibility Oracle local preparation | B | local PASS / production prohibited | Legacy diagnostic-only；Candidate 主查询与独立 Raw Oracle 同快照比较，毫秒/8 位 metric/reference parity v2、PG16 0 difference 和完整门禁 PASS |
| 19 | WP-G0.2 canonical API resource contract local preparation | B | local PASS / production prohibited | 资源信封显式区分 mode/source/authority/status/policy/parity；非法组合、伪造 parity hash 和 Legacy 权威抬高均 fail closed；未接现有 API/前端 |
| 20 | WP-G0.2 canonical API route adapter local preparation | B | local PASS / production prohibited | 公共 query 仅 bounded limit/cursor，可信 policy/control、代码锁、lazy Legacy、2s/15s deadline+AbortSignal、no-store、400/503 语义均锁定；现有 API 0 修改 |
| 21 | WP-G0.2 canonical cutover | A | prohibited | reconciliation、dual-read 24h、canonical-compat 24h、生产 Reader/API/前端合同 PASS + 独立审批 |
| 22 | WP-G0.3/G0.4/G0.5 | A/B | queued | WP-G0.2 完成并按独立包执行 |
...
容量/恢复、Add Schema、production verify-only、Shadow Safety Schema 009、本地 Composition Wiring 和 Dormant Deploy 本地准入已形成闭环证据。Dormant 生产只读预检确认生产仍为旧 release、worktree clean、11 个容器正常、Candidate 休眠边界和核心 health/contract PASS；同时发现并立即修复单独 `.env.production` 无法解析 Compose 的阻断，runner/production-check 现统一按 `.env` -> `.env.production` 加载。隔离执行演练进一步在 Bash 3.2 上真实覆盖 web-only 成功路径与即时验证失败自动回滚路径。Runtime Identity 本地准备又发现并关闭 NOINHERIT LOGIN 无显式能力角色切换的真实缺口；source/consumer/monitor 固定 `SET LOCAL ROLE`、3 个单 membership LOGIN、跨角色拒绝和 source 最小 archive 权限已在隔离 PG16 证明，生产身份仍未创建。身份加固改变了 Candidate runtime database 并依赖安全关键事务适配器，因此 Dormant current artifact 已刷新为 14 文件；旧 13 文件 checksum 只能作为历史证据，禁止再获批。Activation/Observation 本地 runner 又固定 future code + exact artifact 双重授权，只允许 shadow write 与 `web/candidate-shadow-worker`，即时失败、镜像构建失败或 24 小时观察任一样本失败都会恢复 env/Web/Git 并把 control 回退为 `legacy/write_frozen=true`；12 项定向、隔离 shell 成功/回滚/observer 和真实 PG16 lifecycle 均已通过，但当前代码授权仍为 false。等待生产审批期间，Reconciliation 本地 runner 已实现逐笔 Source Outbox -> Candidate Event 的 payload/command hash 对账；纯函数 10,000 条和隔离 PG16 10,000 条均为 0 difference，数据库只读事务写入被拒绝且 phase 保持 `shadow_capture`，完整定向、基础和安全门禁均 PASS，但这仍不能冒充生产 reconciliation。Canonical Compat Read Model 本地包关闭了全历史无 cohort 聚合的 P1；Oracle 后续包又证明 Legacy 无法提供 Episode/Outcome 权威字段，禁止补值冒充 parity。Canonical 0 difference 现只允许主聚合 Read 与同一只读快照内独立 Raw Episode/Checkpoint/Outcome Oracle 比较；毫秒 cohort、8 位 metric、policy 和 referenceStatus v2 均纳入，PG16 已本地 0 difference。API Resource Contract 本地包进一步固定 mode/source/authority/status/policy/parity 与 Candidate/Legacy 双槽；伪造 parity proof、Legacy 权威抬高、Candidate partial/unavailable 包装和 silent fallback 都 fail closed。API Route Adapter 本地包又固定公共 query 只能控制 bounded limit/cursor，policy/control/code authorization 均为可信服务端输入；当前代码锁强制 lazy Legacy，Candidate/Oracle 调用为 0；2 秒控制 deadline、15 秒数据 deadline、AbortSignal、no-store 与 400/503 已验证。两包都没有接现有 API 或前端。当前代码 canonical read 授权仍为 false。生产 Reader/API/前端与真实双 24 小时窗口尚未实现，旧 Review 的 missing direction 偏 long/null metric 补 0 仍是切换 blocker。生产 Candidate schema 仍是 migration 1-9 applied/verified/dormant；runtime deployment=false、control lifecycle 未启动。下一步只能另行审批新的 exact commit + 当前 14 文件 artifact checksum + web-only + 90 分钟 dormant runtime deploy；即时检查后还必须做 ledger/control 只读核验和 30-60 分钟观察。之后必须完成生产 Runtime Identity and Permission 包，才能制作并审批 future activation release。Writer、backfill、dual read 和 read cutover 继续禁止。
