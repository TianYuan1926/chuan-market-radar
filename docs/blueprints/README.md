...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.5](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.7](./market-radar-v2-controlled-replacement-traceability.v1.json) | 18 个 Module、5 维状态、4 类不确定性、硬门槛和 M0-M7 的机器合同 |
...
| 36 | [M0 开工基线报告](./V2_M0_FOUNDATION_START_DELIVERY_REPORT.md) | 干净分支、初始合同和 M0.0-M0.2 历史证据 |
...
M0.0-M0.3、M1.1-M1.5-B0、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1、B0.2-A 和 B0.2-C 已通过各自本地工程出口；生产仍为零变更。B0.2-C 已建立三 Venue 前向 raw evidence、完整分母、身份 epoch、连续性 checkpoint 和 append-only journal，但本机两轮真实请求均因 egress 失败，三家 complete snapshot 均为 0，因此运行捕获起点仍未通过。五个历史来源候选均为 `RESEARCH_ONLY`，bulk acquisition=false、cohort freeze=false；accepted real cohort=0，Gate=INSUFFICIENT、Detector 仍为 DRAFT。当前可执行运行入口是：

```text
V2-M2.2-B0.2-C1-EGRESS-CAPABLE-FORWARD-CAPTURE-START
```

C1 只恢复可信 egress 并用同一 no-authority runner 取得连续完整 Snapshot；不能回填过去，也不能把失败 journal 当作 live coverage。关键路径外部门 `V2-M2.2-B0.2-B-EXACT-SOURCE-RIGHTS-AND-CAPABILITY-RESOLUTION` 仍必须取得人工来源权利结论和合格历史身份；Agent、公开下载、当前 snapshot、archive presence 和厂商宣传都不能自批。外部 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE` 在可信通道恢复后执行固定 31 个一分钟周期；B1 与 M1.6 都通过后才进入 M1.7 24 小时 Shadow。任何生产 Shadow、identity 或 migration 必须另过独立 Gate；M1.7 前不得让 M2 runtime 读取 M1 authority、写 Candidate Store、接入页面或生成等级/交易计划。
