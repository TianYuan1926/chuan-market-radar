...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M0.4_EXPANDED_SCOPE_DESIGN_PASS / M0.5_MARKET_MECHANICS_MICROSTRUCTURE_PRECURSOR_ATLAS_DESIGN_PASS / A0_ENGINEERING_SUBGATES_PASS_TOTAL_GATE_INCOMPLETE_P0R_PENDING / M1.1B0_R3_LIVE_15_OF_15_ALL_GATES_PASS / M1.4A_M1.4D_LOCAL_RUNTIME_CONTRACTS_PASS_NO_AUTHORITY / M1.5C_M1.5D_LOCAL_RUNTIME_EVIDENCE_AND_EXACT_PACKAGE_PASS_LIVE_ZERO_OF_31_EACH / M2.1A_LOCAL_RESEARCH_CONTRACT_PASS_REAL_COHORT_MISSING_NO_CANDIDATE_EMISSION / M1.5-B1_EARLY_SHADOW_BUSINESS_GATE_PASS_V1_ONLY / M1.6-P0_EXECUTED_BLOCKED_CAPACITY_AND_RECOVERY / P0R_ROUTE_AUTHORITY_UPSTREAM_QUALIFIED / PRODUCTION_EVIDENCE_GATEWAY_HEALTH_FIX_COMMITTED / AUTHORITY_SYNC_SOURCE_REMOTE_3_OF_4_INDEPENDENT_SECURITY_FAIL / PUBLIC_KEY_FINGERPRINT_ARTIFACT_ROOT_REMEDIATION_COMMIT_8742828_AND_AUTHORITY_SYNC_FINAL_BYTES_FULL_LOCAL_CI_PASS / AUTHORITY_SYNC_PUSH_REPLACEMENT_SOURCE_REMOTE_FOUR_GATES_EXACT_APPROVAL_REAL_CADDY_VALIDATION_BOOTSTRAP_RETRIEVAL_FRESH_REBIND_AND_RECOVERY_PENDING / PRODUCTION_INTENTIONAL_MUTATION_NONE / FAILED_GATEWAY_ATTEMPT_ONLY_ZERO_DRIFT_PASS / M2.2-B0.2-C1_FORWARD_ONLY_READY_NO_AUTHORITY / M3.0-M3.3_LOCAL_CONTRACT_PASS_TEST_ONLY_UNCALIBRATED_NO_READY_AUTHORITY / M3.3E_STRATEGY_ARCHETYPE_LABELING_DESIGN_AUTHORITY_ONLY / M3.4-R0_SCOPE_REBASE_GOVERNANCE_PASS_R1_PARTIAL_TEST_ONLY_DRAFT / EXTERNAL_RIGHTS_AND_HISTORICAL_SOURCE_BLOCKED / GATE_INSUFFICIENT / DETECTORS_DRAFT / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED
V2 生产权限：false
自动交易：永久禁止
最新生产存储门禁：P0_BLOCKED_CAPACITY_AND_RECOVERY / APPLICATION_HEALTH_NOT_EVALUATED
```

2026-08-02 最新覆盖：当前执行入口仍为 `V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY`，但已经从 health-envelope 修复推进到生成制品公钥指纹根治。`dd67d819...` 的旧四门、旧批准、失败 dispatch 和 clean health fix `51886b0...` 只保留历史效力。authority-sync source parts `d1c3d987cf55105e94e8 + 15992dbb989ea1045629` 已取得 Signed Dispatch `30739919674`、Full Quality `30739919649`、A0 `30739919664` PASS，Independent Security `30739919662` 因唯一历史 `approval-request.json:1 / generic-api-key` finding FAIL，当前资格严格为 `3/4`。脱敏 artifact 证明该值是固定 X25519 recipient 公钥指纹而非 credential，但旧名 `evidenceRecipientKeySha256` 具有 secret-shaped 语义。永久修复使用 `evidenceRecipientFingerprintSha256`、拒绝旧字段，并在 fixed dispatch 发布前阻断歧义非 public `*KeySha256` 高熵值；历史只登记 exact finding。clean implementation commit `8742828086041da1db51a5abeeef01ba7656e5fd` 已形成，recurrence `11/11`、dispatch `38/38`、gateway `8/8` 合计 `57/57`，最终 authority-sync 字节完整 exact-toolchain CI 已 PASS。authority-sync commit/push、replacement exact source 同源四门、fresh baseline、新 package、新批准、bootstrap、自动取回、fresh rebind 和真实 recovery 尚未完成。生产保持不变。
...
route target、Tencent firewall capture、route evidence、descriptor-first 安全修复、source `dd67d819...` 的四门和 health fix `51886b0...` 只保留为已资格化上游及事故证据。authority-sync source 远端当前严格为 `3/4`：前三门 PASS，Independent Security 的 full-history Gitleaks 因历史 generated approval request 的 secret-shaped X25519 公钥指纹字段 FAIL。当前新字段 `evidenceRecipientFingerprintSha256`、旧字段拒绝、发布前扫描阻断和 exact historical review 已通过 recurrence `11/11`、dispatch `38/38`、gateway `8/8`，合计 `57/57`，首轮完整 CI 也已 PASS；必须继续完成最终权威字节完整 CI、clean replacement source 和该同源四门从零 PASS，随后才允许 fresh baseline、新 deterministic package、新 exact approval、current-image 隔离验证、Caddy-only deploy/自动回滚、自动取回、fresh rebind 与真实 P0R recovery。只有 P0R 与 fresh P0 均 PASS 才能请求 P1，并严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 推进。
