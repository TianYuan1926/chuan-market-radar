...
P0R 本地恢复与云资源前置安全工程现已通过：同快照加密备份、数据库 fingerprint、运行级 COS/STS plan、香港单 AZ/region 验证、上传前对象不存在、私有 exact-version retrieval、隔离 PG16 restore、证据 verifier、checksum-bound bundle 和失败注入均已自动验证。当前 COS inventory=0；真实 bucket、密钥、STS、生产对象、restore parity、容量扩展和 fresh P0 尚未发生，因此不得把本地 PASS 写成生产恢复 PASS，P1 继续关闭。
...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.13](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.15](./market-radar-v2-controlled-replacement-traceability.v1.json) | 18 个 Module、5 维状态、4 类不确定性、硬门槛和 M0-M7 的机器合同 |
...
| 46 | [M1.6-P0R 容量与恢复整改合同](../architecture/v2/M1_6_P0R_CAPACITY_AND_RECOVERY_REMEDIATION_CONTRACT_V1.md) | 180 GiB 容量目标、离机加密备份、隔离恢复和 P0 重跑边界 |
...
| 49 | [M1.6-P0R 生产恢复运行手册](../runbooks/V2_M1_6_P0R_PRODUCTION_RECOVERY_RUNBOOK.md) | 离机密钥保管、临时凭证、staging、执行、清理、扩容和 fresh P0 的固定顺序 |
...
M0.0-M0.3、M1.1-M1.5-B0、M1.5-B1-A/B0/B2/B3、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1、B0.2-A、B0.2-C 和 C1 已通过对应本地、技术、业务或运行起点出口。B1-B1 因证据包装失败永久 `INVALID_NOT_COUNTED`；B1-B3 已以新 mark-price schema 取得 31/31 READY 和四项 100% 业务 PASS。M1.6-P0 已真实执行但因容量与 recovery evidence 准入 `BLOCKED`；P0R 本地恢复与云资源前置安全工程已通过，外部 COS/密钥/STS、生产恢复和容量动作仍待执行。五个历史来源仍为 `RESEARCH_ONLY`，bulk/cohort blocked；Gate=INSUFFICIENT、Detector=DRAFT。当前执行入口是：

```text
V2-M1.6-P0R-B1-COS-KEY-STS-EXTERNAL-PROVISIONING
```

P0R 下一步按 checksum-bound plan 建立香港单 AZ 私有 COS、离机 age 身份和 7200 秒运行级 STS，取得真实加密离机备份、精确 version retrieval 与独立 PG16 restore parity，再由用户把文件系统提升到至少 161,643,694,113 bytes，并重新运行全部 P0。不得清缓存冒充永久容量，不得用本地测试或 backup exit 0 冒充生产可恢复；只有新 P0 PASS 才能请求 P1。随后严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 串行推进。关键路径外部门 `V2-M2.2-B0.2-B-EXACT-SOURCE-RIGHTS-AND-CAPABILITY-RESOLUTION` 仍必须取得人工来源权利结论和合格历史身份；M1.7 前不得让 M2 runtime 读取 M1 authority、写 Candidate Store、接入页面或生成等级/交易计划。
