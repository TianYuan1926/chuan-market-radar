...
2026-08-02 最新覆盖：当前执行入口为 `V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY`。route authority 的 implementation `157e9a79...`、真实 CodeQL 失败 `8059edf...` 和 descriptor-first remediation `913bae3a...` 保留为已资格化上游；所有旧 response、route、run、plan 和 staging 继续失效。新根因是 fixed dispatch 只能送达包、不能自主返回严格脱敏结果，继续依赖 Edge/API response、AX/browser-state、人工复制或 OrcaTerm 结果运输必然重复浪费。新通道用生产 SSH Ed25519 host key 对 canonical evidence 作 namespace 签名，以 X25519/HKDF-SHA256/AES-256-GCM 加密，只通过既有 Caddy 的高熵精确路径提供 ciphertext；本机固定 SOCKS poller 校验 pinned host identity、dispatch/schema/time/hash 并以 mode-600 no-clobber 保存，生产对象由 exact transient systemd timer 两小时后删除。网关包只允许复用当前 image 并重建 Caddy，失败自动恢复 baseline Caddy；仓库、数据库、Redis、Worker、env、migration、Feature Flag 和非目标容器不得改变。implementation `1ddc0fdc...` 与 authority-sync remediation `35d7b63a...` 已形成 clean commits。资料同步 source `d67c0e87...` 的远端资格为 3/4：Signed Dispatch、Full Quality、A0 PASS，Independent Security 因私钥测试 path-read race 和一个 X25519 公钥指纹误报 FAIL。两项已分别用 descriptor-first `O_NOFOLLOW` 稳定读取、历史 exact fingerprint 加活动源码两段表示完成本地根治；官方 Gitleaks `8.30.1` 完整历史 finding=0，recurrence `11/11`、dispatch `37/37`、gateway `7/7`、P0R rebind `14/14`、V2 Foundation `638 PASS / 6 explicit skips`、V2 Ops `274/274`、M0 `12/12`、Next build、Golden `16/16` 和 security 全部 PASS。新的 clean commit、GitHub 四门重验、精确生产批准、真实 current-image Caddy validation、bootstrap、自动取回、fresh rebind 和真实 recovery 尚未完成。生产没有本轮有意 mutation，fresh zero-drift 尚未证明。
...
route target v1、Tencent firewall capture v1、strict listener observer、route evidence v2、implementation `157e9a79...`、CodeQL 诊断 `8059edf...` 和 descriptor-first 修复 `913bae3a...` 保留为已资格化上游，不再是当前实施入口。当前网关根治 fixed dispatch 只能送达、不能自主回传严格脱敏结果的问题：生产 SSH Ed25519 host key 对 canonical evidence 作 namespace 签名，固定本机 X25519 recipient 负责加密接收，既有 Caddy 只提供高熵 exact ciphertext path，本机 SOCKS poller 校验 host/dispatch/schema/time/hash 后以 mode-600 no-clobber 保存，exact transient systemd timer 两小时后删除生产对象。网关包只允许使用当前 Caddy image 并重建 Caddy，任何失败自动恢复 baseline Caddy；仓库、数据库、Redis、Worker、env、migration、Feature Flag 与非目标容器不得改变。首次最新字节完整 CI 在 recurrence authority `10/11` 处真实失败，原因是本节等三处活跃资料仍把旧 route authority 标为当前入口；同步修复加入唯一入口防复发断言，并让 Legacy consumer map/reviewed commit 与 M0 精确 production-control 白名单 fail closed。资料同步 source `d67c0e87...` 随后只取得远端 3/4，Independent Security 因测试私钥 path-read race 和 X25519 公钥指纹误报 FAIL。当前本地根治不增加 CodeQL suppression、不扩大 Gitleaks allowlist；官方同版本完整历史 finding=0 且最终字节完整 CI PASS。必须先形成新的 clean source 并从零取得四门，前三门不可借用；精确生产批准、真实 current-image Caddy validation、bootstrap、自动取回、fresh rebind 与真实 P0R recovery 仍未完成。只有 P0R 与 fresh P0 均 PASS 才能请求 P1，并严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 推进。
