...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M0.4_EXPANDED_SCOPE_DESIGN_PASS / M0.5_MARKET_MECHANICS_MICROSTRUCTURE_PRECURSOR_ATLAS_DESIGN_PASS / A0_ENGINEERING_SUBGATES_PASS_TOTAL_GATE_INCOMPLETE_P0R_PENDING / M1.1B0_R3_LIVE_15_OF_15_ALL_GATES_PASS / M1.4A_M1.4D_LOCAL_RUNTIME_CONTRACTS_PASS_NO_AUTHORITY / M1.5C_M1.5D_LOCAL_RUNTIME_EVIDENCE_AND_EXACT_PACKAGE_PASS_LIVE_ZERO_OF_31_EACH / M2.1A_LOCAL_RESEARCH_CONTRACT_PASS_REAL_COHORT_MISSING_NO_CANDIDATE_EMISSION / M1.5-B1_EARLY_SHADOW_BUSINESS_GATE_PASS_V1_ONLY / M1.6-P0_EXECUTED_BLOCKED_CAPACITY_AND_RECOVERY / P0R_ROUTE_AUTHORITY_UPSTREAM_QUALIFIED / PRODUCTION_EVIDENCE_GATEWAY_TWO_EXACT_ATTEMPTS_FAILED_PRE_CADDY_MUTATION / FIRST_HEALTH_ENVELOPE_SECOND_RUNTIME_CAPABILITY_AND_COMPOSE_IDENTITY / TWO_ATTEMPT_ZERO_DRIFT / RUNTIME_PREFLIGHT_RECURRENCE_STOP_THE_LINE / ROOT_REMEDIATION_COMMIT_BB9ABE5_FULL_LOCAL_CI_PASS / FINAL_AUTHORITY_COMMIT_PUSH_REPLACEMENT_SOURCE_REMOTE_FOUR_GATES_FRESH_BASELINE_NEW_PACKAGE_NEW_APPROVAL_REAL_CADDY_ACCEPTANCE_RETRIEVAL_FRESH_REBIND_AND_RECOVERY_PENDING / PRODUCTION_INTENTIONAL_MUTATION_NONE / M2.2-B0.2-C1_FORWARD_ONLY_READY_NO_AUTHORITY / M3.0-M3.3_LOCAL_CONTRACT_PASS_TEST_ONLY_UNCALIBRATED_NO_READY_AUTHORITY / M3.3E_STRATEGY_ARCHETYPE_LABELING_DESIGN_AUTHORITY_ONLY / M3.4-R0_SCOPE_REBASE_GOVERNANCE_PASS_R1_PARTIAL_TEST_ONLY_DRAFT / EXTERNAL_RIGHTS_AND_HISTORICAL_SOURCE_BLOCKED / GATE_INSUFFICIENT / DETECTORS_DRAFT / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED
V2 生产权限：false
自动交易：永久禁止
最新生产存储门禁：P0_BLOCKED_CAPACITY_AND_RECOVERY / APPLICATION_HEALTH_NOT_EVALUATED
```

2026-08-02 最新覆盖：当前执行入口仍为 `V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY`。两个 exact dispatch 均在任何 Caddy mutation 前失败、已 claim 且不可复用：首次是 production 顶层 health envelope 偏差；第二次 source `d9a1e5fedf481f00e3cc88705c7e2b884add3ebe` 虽从零通过同源四门，仍暴露 current Caddy image capability 与 root-owned Compose identity 两项生产预演偏差。两次 fresh read-only 复核均证明 production HEAD、clean worktree、exact 11-container identity、ready/fresh health、404 route 与非目标面零漂移，第二次唯一空 outbound residue 已清理。`REC-2026-08-02-PRODUCTION-GATEWAY-PREFLIGHT-EQUIVALENCE` occurrence 2 已触发 generic release STOP-THE-LINE。根治 commit `bb9abe5cf0404ca7ce9af55f6ed99a6e3f646d76` 绑定 exact wrapper/override identity、最小 `NET_BIND_SERVICE` capability、稳定 command fingerprint、owned-state cleanup 和 checksum-bound recurrence gate。gateway `12/12`、recurrence `11/11`、dispatch `38/38` 与完整 exact-toolchain CI 全部 PASS；CI 精确结果为 market `965/969`（4 explicit skips）、workers `23/23`、historical `4/4`、Foundation `638/644`（6 explicit skips）、Ops `279/279`、M0 `12/12`、Next build、Golden `16/16` 和 security。final authority commit/push、replacement exact source 同源四门、fresh baseline、新 package、新批准、真实 Caddy acceptance、自动取回、fresh rebind 和真实 recovery 尚未完成。生产保持不变。
...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.94](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.99](./market-radar-v2-controlled-replacement-traceability.v1.json) | Scope Epoch、A0、18 个 Module、5 维状态、硬门槛和 M0-M7 的机器合同 |
...
route target、Tencent firewall capture、route evidence 与 descriptor-first 安全修复只保留为已资格化上游。两个 gateway exact dispatch 均在 Caddy mutation 前失败、已 claim 且不可复用：首次是 production 顶层 health envelope 偏差；第二次 source `d9a1e5fedf481f00e3cc88705c7e2b884add3ebe` 虽通过同源四门，仍直接暴露 current Caddy image capability 与 root-owned Compose identity 两项预演偏差。两次 production fresh read-only 复核均为 exact 11-container、ready/fresh health、404 route 与非目标零漂移，第二次唯一空 outbound residue 已清理。`REC-2026-08-02-PRODUCTION-GATEWAY-PREFLIGHT-EQUIVALENCE` occurrence 2 已使 generic release STOP-THE-LINE。local remediation `bb9abe5cf0404ca7ce9af55f6ed99a6e3f646d76` 使用 request v3 绑定 wrapper/override exact identity、最小 Caddy capability、稳定 command fingerprint、owned-state cleanup，并把 checksum-bound recurrence gate 同时接入构包和生产 pre-mutation；gateway `12/12`、recurrence `11/11`、dispatch `38/38` 与完整 exact-toolchain CI PASS。必须继续完成 final exact source commit/push 和同源四门从零 PASS，随后才允许 fresh baseline、双构建 deterministic package、新 exact approval、登记过的 remediation execution/自动回滚、自动取回、fresh rebind 与真实 P0R recovery。只有 gateway real-target acceptance、P0R 与 fresh P0 均 PASS 才能请求 P1，并严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 推进。
