...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M2.0_LOCAL_CONTRACT_PASS / M2.1_LOCAL_DRAFT_KERNEL_PASS / M2.2-A_LOCAL_HARNESS_PASS / M2.2-B0_LOCAL_SOURCE_GATE_AND_TECHNICAL_PILOT_PASS / M2.2-B0.1_LOCAL_CONTRACT_PASS / M2.2-B0.2-A_LOCAL_EVIDENCE_GATE_PASS / M2.2-B0.2-C_LOCAL_ENGINEERING_PASS / C1_OPERATIONAL_CAPTURE_START_PASS / FORWARD_ONLY_READY / FIVE_SOURCE_CANDIDATES_RESEARCH_ONLY / EXTERNAL_RIGHTS_AND_HISTORICAL_SOURCE_BLOCKED / BULK_AND_COHORT_BLOCKED / REAL_COHORT_MISSING / GATE_INSUFFICIENT / DETECTORS_STILL_DRAFT / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED / COLLECTOR_DOCKER_AND_SHADOW_UNPROVEN
V2 生产权限：false
自动交易：永久禁止
...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.6](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.8](./market-radar-v2-controlled-replacement-traceability.v1.json) | 18 个 Module、5 维状态、4 类不确定性、硬门槛和 M0-M7 的机器合同 |
...
M0.0-M0.3、M1.1-M1.5-B0、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1、B0.2-A、B0.2-C 和 C1 已通过对应本地/运行起点出口；生产仍为零变更。C1 已用 release-bound/config-bound runner 连续取得两轮三 Venue COMPLETE，每家 2/2、跨度约 368.5 秒、raw 可复核且 gap/unresolved/conflict/blocker=0，三个 Continuity 均为 `FORWARD_ONLY_READY`。五个历史来源候选仍为 `RESEARCH_ONLY`，bulk acquisition=false、cohort freeze=false；accepted real cohort=0，Gate=INSUFFICIENT、Detector 仍为 DRAFT。当前可执行本地入口是：

```text
V2-M1.5-B1-A-REACHABLE-DOCKER-RUNNER-PREFLIGHT
```

B1-A 只在 branch-scoped GitHub-hosted no-authority runner 构建 exact source image，并证明三家 Collector 四分母、身份隔离和脱敏 artifact；预检通过后 B1-B 才执行固定 31 个一分钟周期。关键路径外部门 `V2-M2.2-B0.2-B-EXACT-SOURCE-RIGHTS-AND-CAPABILITY-RESOLUTION` 仍必须取得人工来源权利结论和合格历史身份；Agent、公开下载、当前 snapshot、archive presence 和厂商宣传都不能自批。B1 与 M1.6 都通过后才进入 M1.7 24 小时 Shadow。任何生产 Shadow、identity 或 migration 必须另过独立 Gate；M1.7 前不得让 M2 runtime 读取 M1 authority、写 Candidate Store、接入页面或生成等级/交易计划。
