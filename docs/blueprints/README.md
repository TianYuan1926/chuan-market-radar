...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M2.0_LOCAL_CONTRACT_PASS / M2.1_LOCAL_DRAFT_KERNEL_PASS / M2.2-A_LOCAL_HARNESS_PASS / M2.2-B0_LOCAL_SOURCE_GATE_AND_TECHNICAL_PILOT_PASS / M2.2-B0.1_LOCAL_CONTRACT_PASS / BULK_AND_COHORT_BLOCKED / REAL_COHORT_MISSING / GATE_INSUFFICIENT / DETECTORS_STILL_DRAFT / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED / LIVE_MARKET_AND_SHADOW_UNPROVEN
V2 生产权限：false
自动交易：永久禁止
...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.3](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.5](./market-radar-v2-controlled-replacement-traceability.v1.json) | 18 个 Module、5 维状态、4 类不确定性、硬门槛和 M0-M7 的机器合同 |
...
| 26 | [M0 开工基线报告](./V2_M0_FOUNDATION_START_DELIVERY_REPORT.md) | 干净分支、初始合同和 M0.0-M0.2 历史证据 |
...
M0.0-M0.3、M1.1-M1.5-B0、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0 和 B0.1 已通过各自本地出口；生产仍为零变更。B0 已真实证明 Binance Vision 精确对象下载/checksum/删 raw 技术链，B0.1 已冻结 target-blind relative-rule-margin、固定分母 Top20 和样本构造政策；但来源权利与 point-in-time instrument history 未通过，因此 bulk acquisition=false、cohort freeze=false。当前 accepted real cohort=0，真实 Top20/sensitivity/holdout 证据缺失，所以 Gate=INSUFFICIENT、Detector 仍为 DRAFT。M1.6 只证明分区/retention 本地实现和真实隔离 dump/restore/replay，不证明生产容量。本机 provider live probe 连接超时，Edge OrcaTerm 为 0 会话，本机无 Docker CLI，故真实镜像、live market 和 Shadow/SLO 均未证明。当前下一入口是：

```text
V2-M2.2-B0.2-RIGHTS-AND-POINT-IN-TIME-INSTRUMENT-METADATA-RESOLUTION
```

该入口必须取得人工来源权利结论和可审计历史合约身份；Agent、公开下载、当前 snapshot 和 archive presence 都不能自批。外部 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE` 在可信通道恢复后执行固定 31 个一分钟周期；B1 与 M1.6 都通过后才进入 M1.7 24 小时 Shadow。任何生产 Shadow、identity 或 migration 必须另过独立 Gate；M1.7 前不得让 M2 runtime 读取 M1 authority、写 Candidate Store、接入页面或生成等级/交易计划。
