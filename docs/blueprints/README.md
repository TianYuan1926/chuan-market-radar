...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M0.4_EXPANDED_SCOPE_DESIGN_PASS / M0.5_MARKET_MECHANICS_MICROSTRUCTURE_PRECURSOR_ATLAS_DESIGN_PASS / A0_MATERIALS_SUPPLY_CHAIN_INDEPENDENT_SECURITY_REPRODUCIBLE_RELEASE_AND_RESOURCE_BASELINE_PASS_TOTAL_GATE_INCOMPLETE_P0R_PENDING / M1.1A_REGISTRY_LOCAL_PASS / M1.1B_MULTI_ASSET_IDENTITY_LISTING_AND_EXACT_PROBE_LOCAL_IMPLEMENTATION_PASS_TEST_ONLY_CONFORMANCE_PASS / M1.1B0_R1_LIVE_0_OF_15_COMMON_RUNTIME_TRANSPORT_BLOCKED_R2_LIVE_14_OF_15_LISTING_GATE_BLOCKED_R3_LIVE_15_OF_15_ALL_GATES_PASS / M1.4A_ADAPTIVE_MULTI_ASSET_COLLECTOR_LOCAL_CONTRACT_AND_FULL_CI_PASS_NO_RUNTIME_AUTHORITY / M1.4B_TENCENT_BOOTSTRAP_AND_CHECKPOINT_BOUND_RESUME_PASS_NO_RUNTIME_AUTHORITY_15_LIVE_PROFILES_14_ROUTE_ELIGIBLE / M1.4C_MICROSTRUCTURE_FACT_FEATURE_CACHE_LOCAL_CONTRACT_PASS_22_OF_22_NO_RUNTIME_AUTHORITY / M1.4D_MULTI_ASSET_IDENTITY_CATALOG_BASE_FACT_LOCAL_RUNTIME_PASS_NO_AUTHORITY / M1.5C_M1.5D_LOCAL_RUNTIME_EVIDENCE_AND_EXACT_PACKAGE_PASS_LIVE_ZERO_OF_31_EACH / M2.1A_BIDIRECTIONAL_PRECURSOR_ATLAS_LOCAL_RESEARCH_CONTRACT_PASS_13_OF_13_REAL_COHORT_MISSING_NO_CANDIDATE_EMISSION / SCOPE_EPOCH_V1_EVIDENCE_PRESERVED / M1.5-B1_EARLY_SHADOW_BUSINESS_GATE_PASS_V1_ONLY / M1.6-P0_EXECUTED_BLOCKED_CAPACITY_AND_RECOVERY / M1.6-P0R_LOCAL_AND_EXACT_SOURCE_REMOTE_QUALIFICATION_PASS_PREVIOUS_DISPATCH_CONFIRMED_NOT_EXECUTED_EXPIRED_NOT_CLAIMED_FIXED_DISPATCH_TIMEOUT_LOCK_RECOVERY_PASS_FRESH_REDISPATCH_REQUIRED_LEGACY_BED938_STAGING_REJECTED_STS_AND_RECOVERY_PENDING / M2.2-B0.2-C1_FORWARD_ONLY_READY_V1_ONLY / C1_REFRESH_FOUR_CYCLE_FULL_HISTORY_INTEGRITY_AND_SCOPE_V2_DOMAIN_REPLAY_PASS_NO_AUTHORITY / M3.0-M3.3_LOCAL_CONTRACT_PASS_V1_ONLY_TEST_ONLY_UNCALIBRATED_NO_READY_AUTHORITY / M3.1A-M3.3D_FOUR_LANE_LOCAL_RESEARCH_CONTRACT_SCAFFOLD_PASS_NO_REAL_CALIBRATION_OR_AUTHORITY / M3.4-R0_SCOPE_REBASE_GOVERNANCE_PASS_R1_PARTIAL_TEST_ONLY_DRAFT_NO_DIRECTED_EXIT / PRODUCTION_FIXED_DISPATCH_TIMEOUT_LOCK_RECOVERY_PASS / EXTERNAL_RIGHTS_AND_HISTORICAL_SOURCE_BLOCKED / BULK_AND_COHORT_BLOCKED / GATE_INSUFFICIENT / DETECTORS_DRAFT / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED
V2 生产权限：false
自动交易：永久禁止
...
P0R 本地恢复、六小时无扩容容量和 fresh P0 组合准入工程已通过；真实 COS 已启用并回读 Object Lock=`COMPLIANCE` 31 天，age X25519 身份仅保存在 macOS Keychain。历史 `bed938...` staging 的完整性保留，但其运行文件早于当前安全修复，已标记 `REJECTED_SUPERSEDED_SECURITY_SOURCE`，禁止执行或绑定新 STS。source parts `408803e0bdc21051124a + 79e307db8e9eb39c793c` 已实现无 secret 只读重绑定包；本地 package `9/9`、P0R `70/70`、V2 Ops `179/179` 和完整 CI PASS，GitHub exact-source Full Quality `30219999104`、A0 Release Qualification `30219999094` 和 Independent Security `30219999063` 也全部 PASS。首次 signed dispatch 已确认因固定通道旧代理超时遗留空锁而未 claim、未解包、未执行；4,526 次重复失败的根因已由 90 秒 Git 硬上限与 owner-aware stale-lock recovery 永久修复并完成腾讯验收。旧 dispatch 已过期且不可复用，当前必须 fresh redispatch；没有 fresh STS、生产对象、backup/retrieval/restore、fresh topology、exact-release 校准或 fresh P0，P1 继续关闭。M1.1B0 R3 已真实取得 15/15 exact endpoint conformance。M1.4B 已以 source `3c21a75009aeb4f4f7d9fd8954245238c38d9636` 完成腾讯 bootstrap 与 checkpoint-bound resume；两轮均为 14/14 route PASS、0 failed、1 registry blocked、零 listing gap 和两个 committed checkpoint，生产应用身份不变。`BINANCE_SPOT_CATALOG` 请求数固定为 0。M1.4C 22/22 与 M2.1A 13/13 本地合同出口均已通过。M1.4D 已形成四 Venue catalog/listing/identity 与 point-in-time Base Fact runtime；Base Fact 23/23、M1.5C/M1.5D Expanded Shadow 70/70、exact no-secret package 12/12 均通过，真实 live cycle 均为 0。旧 M1.4B/conformance evidence 不能跨提交绑定当前包；A0/P0R 关闭后仍须刷新 same-commit upstream。C1 最新 evidence release 已取得四轮三 Venue COMPLETE，并由正式 verifier 全链复核 4 条 journal、28 个 artifact 引用、12 个 raw 引用、37 个精确文件；Scope V2 资产域重放证明旧 `CANONICAL_TARGET` 不是资产域结论，三家 normalizer 均保持 `PARTIAL`。verifier source 的 A0 `30223737098`、Full Quality `30223737124`、Security `30223737105` 和 Signed Dispatch Quality `30223737131` 全部 PASS；该包仍为 forward-only/no-authority。A0 材料、供应链和独立安全已通过，最新 source parts `9ef63b85d1a76f3ad7ac + 815e081506c5dbc074a5` 又由 A0 `30217335595` 关闭双 RootFS/确定性胶囊/回滚与冻结 1,440 instrument 性能资源基线，同源 Full Quality `30217335543` 和 Security `30217335622` 全部 PASS。A0 现在只缺 P0R 真实恢复。Bitget Venue、Listing Lifecycle、Equity Asset Domain 与 Data Maximization 四轴独立核算；股票 tradable Fact 仍为 0。原三 Venue 加密证据只保留 `SCOPE_EPOCH_V1_CRYPTO_3V` 效力。Scope V2 四轨决策研究合同已完成 28/28，但真实 cohort、holdout、校准和 authority 均为 0；M3.4-R0 只完成治理门禁，R1 定向出口仍为 0。

固定生产派发通道的首个真实 signed dispatch 已完成端到端验收；本轮又关闭了 Git fetch 超时后遗留空锁的生产根因。生产 agent、SSH wrapper 与 README 使用精确哈希升级，timer 恢复稳定 `IDLE_NO_NEW_DISPATCH`，应用 HEAD、clean worktree、11 个容器、health、数据库、Redis 和六 Worker 零漂移；机器复发门禁现为 4 项 CLOSED、0 项 open。该结果只证明普通无 secret 包的运输与独立启动地基，不提升任何 G0、M1 或交易能力状态，也不运输 P0R 临时凭证。
...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.53](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.59](./market-radar-v2-controlled-replacement-traceability.v1.json) | Scope Epoch、A0、18 个 Module、5 维状态、硬门槛和 M0-M7 的机器合同 |
...
M0.0-M0.5、M1.1-M1.6、M2.0-M2.2 已列本地包、C1、M3.0-M3.3 已通过各自工程、业务、运行起点或合同出口。M1.4C 已取得 22/22 本地合同与完整 CI 出口，M1.4D 及 M1.5C/M1.5D 的本地 runtime/evidence/exact package 已通过，M2.1A 已取得 13/13 research-only 合同出口；但 M1.5C/M1.5D 真实 live cycle 均为 0，故无持续 runtime/Fact/Candidate/Signal/READY authority。C1 最新四轮三 Venue evidence 已通过全历史完整性审计与 Scope V2 资产域重放，并取得 exact-source 四条 GitHub 质量 PASS；它仍只有约 16.6 分钟、没有 Bitget、官方 mapping 完整性、Fact 或 authority。A0 工程材料、供应链、exact-runtime remote CI、完整历史 secret、CodeQL、collector 镜像扫描、可重复制品 provenance、隔离回滚和冻结性能资源基线已 PASS；唯一剩余控制是 P0R 真实恢复，总门禁保持 `INCOMPLETE_P0R_PENDING`。Scope V2 的 M3.1A-D-R0 四轨 research contract scaffold 已通过定向测试与完整 CI，但真实 M3.1A-M3.3D 仍等待 M2.3/M2.4 cohort、untouched holdout 和分域校准。B1-B1 永久 `INVALID_NOT_COUNTED`，B1-B3 已取得 V1 业务 PASS。M1.6-P0 因容量与 recovery evidence `BLOCKED`；Object Lock 31 天和 age Keychain 身份保持有效，历史 `bed938...` staging 已拒绝执行，current-source 重绑定包已通过本地和 exact-source 远端资格门禁。signed read-only dispatch 已发布并过期，但腾讯目标 receipt 未读，执行结果为 `UNKNOWN`；STS、生产恢复、fresh topology/calibration/P0 均无确认回执。历史来源仍 `RESEARCH_ONLY`，Gate=INSUFFICIENT、Detector=DRAFT；M3.4-R0 只完成 Scope V2 治理门禁，R1 草稿没有独立测试出口，仍无 Feasibility/runtime/READY authority。当前生产执行入口是：

```text
V2-M1.6-P0R-R0-READ-ONLY-SOURCE-REBIND
```

exact-source GitHub revalidation 已通过；下一步先只读取得已发布 dispatch 的目标 receipt。若目标已有结果，先验证身份、时效与零漂移；只有 receipt 证明未领取或不存在时，才生成新的 exact signed read-only dispatch，禁止盲目重发覆盖现场事实。现场重绑定 PASS 后，才可从 current source 重建 plan/bundle，随后通过 `/dev/shm` 单独注入 fresh 7200 秒 STS 与 age identity，执行受限上传、真实加密离机备份、exact version retrieval、独立 PG16 restore parity 和 cleanup。再刷新 production health/topology，在 exact clean release 重跑容量校准和 fresh P0；不得清缓存、缩短核心扫描分母或改阈值。只有新 P0 PASS 才能请求 P1，并严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 推进。固定通道只作为普通无 secret 包的默认运输层，不替代 P0R `/dev/shm` 凭证边界或任一业务包自身 Gate。M1.4D 与 M1.5C/M1.5D 本地 exact package 已通过，但真实派发还必须在 A0/P0R 关闭后刷新同一 clean commit 的 conformance + M1.4B upstream，再构建相同 release 的两包并分别验收。只允许 live PASS 且 registry route eligible 的 capability 进入 batch；Bybit 只声明 provider-available history，Bitget 只声明官方一个月窗口，Binance spot 仍须修订 registry 后重跑。M1.6-D1 必须使用 M1.5C/M1.5D 的真实事实率。外部门 `V2-M2.2-B0.2-B-EXACT-SOURCE-RIGHTS-AND-CAPABILITY-RESOLUTION` 仍需人工来源权利和合格历史身份；M1.7 前不得让 M2/M3 runtime 写 Candidate、接页面或生成真实等级/计划。
