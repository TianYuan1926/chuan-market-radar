...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M1_NOT_COMPLETE / LIVE_MARKET_AND_SHADOW_UNPROVEN
V2 生产权限：false
自动交易：永久禁止
...
| 22 | [M0 开工基线报告](./V2_M0_FOUNDATION_START_DELIVERY_REPORT.md) | 干净分支、初始合同和 M0.0-M0.2 历史证据 |
...
M0.0-M0.3、M1.1-M1.5-B0 和 M1.6 已通过本地合同、冻结 fixture、隔离 PostgreSQL 16 与 Shadow 安全门禁；生产仍为零变更。M1.6 只证明分区/retention 本地实现和真实隔离 dump/restore/replay，不证明生产容量。本机 provider live probe 连接超时，Edge OrcaTerm 为 0 会话，本机无 Docker CLI，故真实镜像、live market 和 Shadow/SLO 均未证明。等待外部 Gate 时，当前允许并行的本地下一包是：

```text
V2-M2.0-DISCOVERY-CONTRACTS-AND-GOLDEN-FIXTURES
```

该包只冻结六类机会、Detector 输入、Candidate/Episode/Thesis 生命周期、三分母和 point-in-time 黄金样本，不启动 Detector runtime。外部 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE` 在可信通道恢复后执行固定 31 个一分钟周期；B1 与 M1.6 都通过后才进入 M1.7 24 小时 Shadow。任何生产 Shadow、identity 或 migration 必须另过独立 Gate；M1.7 前不得让 M2 runtime 读取 M1 authority，也不得接入页面或生成交易计划。
