...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.68](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
...
M0.0-M0.5、M1.1-M1.6、M2.0-M2.2 已列本地包、C1、M3.0-M3.3 已通过各自工程、业务、运行起点或合同出口。M1.4C 已取得本地合同与完整 CI 出口，M1.4D 及 M1.5C/M1.5D 的本地 runtime/evidence/exact package 已通过，M2.1A 已取得 research-only 合同出口；但真实 live、cohort/holdout、校准和 authority 仍未完成。A0 唯一剩余控制是 P0R 真实恢复，总门禁保持 `INCOMPLETE_P0R_PENDING`。M1.6-P0 因容量与 recovery evidence `BLOCKED`；Object Lock 31 天和 age Keychain 身份保持有效。source `e362...` 的本地 CI、GitHub 四门、fresh rebind 和 staging 曾真实 PASS，但第三枚 STS response 在 post-response browser-state recovery 中进入工具输出，因此 credential 与该 run/plan/object key/bundle/staging 永久失去执行权。该 STS 未进入服务器或 COS，生产 P0R files/processes/containers/volumes 均为 0。新 pre-armed fixed local TTY bridge、echo-before-ready session 和 request v3 高层 release 根治已通过伪 TTY 10/10、P0R 88/88、recurrence 10/10、dispatch 24/24 和完整 `ci:production`；新 clean exact source、远端资格、fresh rebind、新 execution identity 和真实 recovery 仍待完成。历史来源仍 `RESEARCH_ONLY`，Gate=INSUFFICIENT、Detector=DRAFT；M3.3E 只完成标签设计权威，M3.4-R1 草稿没有独立测试出口，仍无 Feasibility/runtime/READY authority。当前生产执行入口是：

```text
V2-M1.6-P0R-B4-PREARMED-LOCAL-TTY-BRIDGE-RECOVERY
```

第三枚 STS 已由本机 UTC 与腾讯 HTTPS Date 独立证明超过 exact expiry，现为 `EXPIRED_FORBIDDEN_REUSE`。bridge 的完整本地门禁和权威同步已经通过；现在形成新 clean exact commit，再完成 GitHub Full Quality、Signed Production Dispatch Quality、Independent Security、A0 Release Qualification、fresh production read-only rebind 和全新 run/object key/plan/14-member bundle staging。只有 fixed local TTY bridge 在签发前取得远端 echo-disabled READY，用户才可完成 MFA 并点击 API Explorer 原生 Copy；bridge 自动即时编译 STS、从固定 Keychain 内部 handoff age identity并启动 helper。response 后禁止 browser state、截图、AX/OCR、computer-use 和 OrcaTerm secret entry。helper 必须完成真实加密离机备份、exact version retrieval、独立 PG16 restore parity、全量 cleanup 和零漂移复核。随后刷新 production health/topology，在 exact clean release 重跑容量校准和 fresh P0；不得清缓存、缩短核心扫描分母或改阈值。只有新 P0 PASS 才能请求 P1，并严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 推进。固定通道只作为普通无 secret 包的默认运输层，不替代 P0R `/dev/shm` 凭证边界或任一业务包自身 Gate。外部门 `V2-M2.2-B0.2-B-EXACT-SOURCE-RIGHTS-AND-CAPABILITY-RESOLUTION` 仍需人工来源权利和合格历史身份；M1.7 前不得让 M2/M3 runtime 写 Candidate、接页面或生成真实等级/计划。
