...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M1.5-B1-B0_EARLY_SHADOW_EVIDENCE_LOCAL_PASS / B1-B1_EMPIRICAL_CAPTURE_PENDING / BUSINESS_SLO_UNPROVEN / M2.0_LOCAL_CONTRACT_PASS / M2.1_LOCAL_DRAFT_KERNEL_PASS / M2.2-A_LOCAL_HARNESS_PASS / M2.2-B0.2-C1_FORWARD_ONLY_READY / FIVE_SOURCE_CANDIDATES_RESEARCH_ONLY / EXTERNAL_RIGHTS_AND_HISTORICAL_SOURCE_BLOCKED / BULK_AND_COHORT_BLOCKED / GATE_INSUFFICIENT / DETECTORS_DRAFT / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED
V2 生产权限：false
自动交易：永久禁止
...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.8](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.10](./market-radar-v2-controlled-replacement-traceability.v1.json) | 18 个 Module、5 维状态、4 类不确定性、硬门槛和 M0-M7 的机器合同 |
...
M0.0-M0.3、M1.1-M1.5-B0、M1.5-B1-A/B0、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1、B0.2-A、B0.2-C 和 C1 已通过对应本地、技术或运行起点出口。B1-B0 已冻结 exact、原子 31 周期证据和隔离 Runner，但还没有执行真实 31 周期，业务 SLO 仍未证明。五个历史来源仍为 `RESEARCH_ONLY`，bulk/cohort blocked；Gate=INSUFFICIENT、Detector=DRAFT。当前执行入口是：

```text
V2-M1.5-B1-B1-31-CYCLE-EMPIRICAL-CAPTURE
```

B1-B1 按冻结合同原样实测，任何中断都必须清理并从第 1 周期重跑。若失败，必须先修 freshness 语义，再用同一门槛复验，禁止放宽阈值制造 PASS。关键路径外部门 `V2-M2.2-B0.2-B-EXACT-SOURCE-RIGHTS-AND-CAPABILITY-RESOLUTION` 仍必须取得人工来源权利结论和合格历史身份；Agent、公开下载、当前 snapshot、archive presence 和厂商宣传都不能自批。B1-B 与 M1.6 production Gate 都通过后才进入 M1.7 24 小时 Shadow；M1.7 前不得让 M2 runtime 读取 M1 authority、写 Candidate Store、接入页面或生成等级/交易计划。
