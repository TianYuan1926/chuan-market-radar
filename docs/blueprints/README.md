...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M1.5-B1_EARLY_SHADOW_BUSINESS_GATE_PASS / B1-B1_EXECUTION_INVALID_NOT_COUNTED / B1-B3_PASS / M1.6-P0_EXECUTED_BLOCKED_CAPACITY_AND_RECOVERY / M1.6-P0R_SIX_HOUR_CAPACITY_AND_FRESH_ADMISSION_LOCAL_ENGINEERING_PASS_OBJECT_LOCK_WHITELIST_REQUIRED_RECOVERY_PENDING / M2.0_LOCAL_CONTRACT_PASS / M2.1_LOCAL_DRAFT_KERNEL_PASS / M2.2-A_LOCAL_HARNESS_PASS / M2.2-B0.2-C1_FORWARD_ONLY_READY / FIVE_SOURCE_CANDIDATES_RESEARCH_ONLY / EXTERNAL_RIGHTS_AND_HISTORICAL_SOURCE_BLOCKED / BULK_AND_COHORT_BLOCKED / GATE_INSUFFICIENT / DETECTORS_DRAFT / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED
V2 生产权限：false
自动交易：永久禁止
...
P0R 本地恢复、云资源前置、六小时无扩容容量和 fresh P0 组合准入工程现已通过：组合准入会重建旧 P0 报告、继承全部非容量 blocker，只替代三个旧日分区容量计算，并固定稳态 60% / 峰值 70%。真实专用空桶已按香港单 AZ、私有、versioning、SSE-COS 创建并复核；精确名称只保存在 Git 外受限事实文件。当前账号没有 Object Lock 入口，白名单工单已填写脱敏草稿但尚未提交；macOS Keychain age vault 工具本地 PASS，真实身份未生成。Object Lock、STS、生产对象、restore parity、fresh topology、exact-release 校准和 fresh P0 生产结论尚未发生，因此不得把空桶或本地 PASS 写成生产恢复/容量 PASS，P1 继续关闭。
...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.17](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.19](./market-radar-v2-controlled-replacement-traceability.v1.json) | 18 个 Module、5 维状态、4 类不确定性、硬门槛和 M0-M7 的机器合同 |
...
M0.0-M0.3、M1.1-M1.5-B0、M1.5-B1-A/B0/B2/B3、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1、B0.2-A、B0.2-C 和 C1 已通过对应本地、技术、业务或运行起点出口。B1-B1 因证据包装失败永久 `INVALID_NOT_COUNTED`；B1-B3 已以新 mark-price schema 取得 31/31 READY 和四项 100% 业务 PASS。M1.6-P0 已真实执行但因容量与 recovery evidence 准入 `BLOCKED`；P0R 六小时容量模型和 fresh admission 工具本地 PASS，空桶已创建，Object Lock 白名单待开通，age vault 工具本地 PASS 但身份未生成，STS、生产恢复、fresh topology、exact-release calibration 和 fresh P0 结论仍待执行。五个历史来源仍为 `RESEARCH_ONLY`，bulk/cohort blocked；Gate=INSUFFICIENT、Detector=DRAFT。当前执行入口是：

```text
V2-M1.6-P0R-B1B-OBJECT-LOCK-WHITELIST-AND-AGE-VAULT-QUALIFICATION
```

P0R 下一步先补齐腾讯账号联系方式并提交/通过 Object Lock 白名单，再独立确认不可逆 COMPLIANCE 31 天动作，生成离机 age 身份和 7200 秒运行级 STS，取得真实加密离机备份、exact version retrieval 与独立 PG16 restore parity。随后刷新生产 health/topology，在 exact clean release 重跑容量校准并执行 fresh P0 组合准入；不得清缓存、缩短核心扫描分母或改阈值冒充容量。只有新 P0 PASS 才能请求 P1。随后严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 串行推进。关键路径外部门 `V2-M2.2-B0.2-B-EXACT-SOURCE-RIGHTS-AND-CAPABILITY-RESOLUTION` 仍必须取得人工来源权利结论和合格历史身份；M1.7 前不得让 M2 runtime 读取 M1 authority、写 Candidate Store、接入页面或生成等级/交易计划。
