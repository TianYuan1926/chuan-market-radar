...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.4_LOCAL_PASS / LIVE_MARKET_AND_SHADOW_UNPROVEN
V2 生产权限：false
自动交易：永久禁止
...
| 18 | [M0 开工基线报告](./V2_M0_FOUNDATION_START_DELIVERY_REPORT.md) | 干净分支、初始合同和 M0.0-M0.2 历史证据 |
...
M0.0-M0.3、M1.1-M1.4 和 M1.5-A 已通过本地合同、冻结 fixture 与隔离 PostgreSQL 16 出口；M1.5-A 已具备 checkpoint、单实例 Worker、强制 telemetry 和 SLO evaluator。本机对三家 provider 的 live probe 全部连接超时，故 live market 和连续 Shadow/SLO 仍未证明，生产仍为零变更。唯一下一包是：

```text
V2-M1.5-LIVE-SHADOW-GATE External Egress Rehearsal and Production No-Authority Shadow Gate
```

该包复用同一 Worker，先在 provider 可达环境证明真实 observed/accounted/eligible/collected/fresh，再绑定精确 release、migration checksum、分离身份、资源预算和回滚目标进入 no-authority Shadow。任何生产 Shadow 或 migration 必须另过独立 Gate；不得接入页面、删除 Legacy 或生成 Candidate、方向、Signal、交易计划。
