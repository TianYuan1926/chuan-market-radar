...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M1_READY_TO_START_LOCAL_ONLY
V2 生产权限：false
自动交易：永久禁止
...
| 8 | [M0 开工基线报告](./V2_M0_FOUNDATION_START_DELIVERY_REPORT.md) | 干净分支、初始合同和 M0.0-M0.2 历史证据 |
...
M0.0-M0.3 已通过本地工程出口，生产仍为零变更。唯一下一包是：

```text
V2-M1.1 Three-Venue Identity and Fact Slice
```

该包按已冻结的 M1 合同先实现 Binance Futures、OKX Swap、Bybit Linear Perpetual 同一 BTC 线性永续的身份、Point-in-Time Fact 与 Quality 最小纵切。它不修改生产、不执行 migration、不接入页面、不删除 Legacy 运行代码、不生成 Candidate、Signal 或交易计划。
