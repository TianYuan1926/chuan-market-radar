...
M0.0-M0.3、M1.1-M1.5-A 和 M1.5-B0 已通过本地合同、冻结 fixture、隔离 PostgreSQL 16 与 Shadow 安全门禁；生产仍为零变更。本机 provider live probe 连接超时，Edge OrcaTerm 为 0 会话，本机无 Docker CLI，故真实镜像、live market 和 Shadow/SLO 均未证明。当前唯一工程下一包是：

```text
V2-M1.6-PARTITIONED-FACT-STORAGE Retention and Capacity Foundation
```

该包解决高频 Fact 单表 append-only 但无物理 purge 的长期容量问题。外部 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE` 在可信通道恢复后并行执行固定 31 个一分钟周期；两条路径都通过后才进入 M1.7 24 小时 Shadow。任何生产 Shadow、identity 或 migration 必须另过独立 Gate；不得接入页面、删除 Legacy 或生成 Candidate、方向、Signal、交易计划。
