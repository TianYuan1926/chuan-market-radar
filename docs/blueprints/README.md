...
| 16 | [M0 开工基线报告](./V2_M0_FOUNDATION_START_DELIVERY_REPORT.md) | 干净分支、初始合同和 M0.0-M0.2 历史证据 |
...
M0.0-M0.3、M1.1-M1.4 已通过本地合同、冻结 fixture 与隔离 PostgreSQL 16 出口；live market 和连续 Shadow/SLO 仍未证明，生产仍为零变更。唯一下一包是：

```text
V2-M1.5 Live No-Authority Collector Rehearsal and Shadow/SLO Entry
```

该包只把 M1.4 Runtime 组合成无读取权威的 V2 Collector Worker，先证明 live provider 合同、真实规模、持续 freshness/coverage、容量、限流、重连、资源和成本。任何生产 Shadow 或 migration 必须另过独立 Gate；不得接入页面、删除 Legacy 或生成 Candidate、方向、Signal、交易计划。
