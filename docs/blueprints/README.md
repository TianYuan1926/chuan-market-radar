...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M2.0_LOCAL_CONTRACT_PASS / M2.1_LOCAL_DRAFT_KERNEL_PASS / M2.2-A_LOCAL_HARNESS_PASS / REAL_COHORT_MISSING / GATE_INSUFFICIENT / DETECTORS_STILL_DRAFT / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED / LIVE_MARKET_AND_SHADOW_UNPROVEN
V2 生产权限：false
自动交易：永久禁止
...
M0.0-M0.3、M1.1-M1.5-B0、M1.6、M2.0、M2.1 和 M2.2-A 已通过各自本地出口；生产仍为零变更。M2.2-A 只证明真实数据接纳合同、target-blind harness、固定 Detector/完整背景分母、sealed holdout 物理隔离、knowledge-time lead、统计与 lifecycle proposal Gate；当前 accepted real cohort=0，Top20/sensitivity/真实 holdout 证据缺失，所以 Gate=INSUFFICIENT、Detector 仍为 DRAFT。M1.6 只证明分区/retention 本地实现和真实隔离 dump/restore/replay，不证明生产容量。本机 provider live probe 连接超时，Edge OrcaTerm 为 0 会话，本机无 Docker CLI，故真实镜像、live market 和 Shadow/SLO 均未证明。等待外部 Gate 时，当前允许并行的本地下一包是：
