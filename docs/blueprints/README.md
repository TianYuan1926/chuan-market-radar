...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M0.4_EXPANDED_SCOPE_DESIGN_PASS / M0.5_MARKET_MECHANICS_MICROSTRUCTURE_PRECURSOR_ATLAS_DESIGN_PASS / A0_ENGINEERING_SUBGATES_PASS_TOTAL_GATE_INCOMPLETE_P0R_PENDING / M1.1B0_R3_LIVE_15_OF_15_ALL_GATES_PASS / M1.4A_M1.4D_LOCAL_RUNTIME_CONTRACTS_PASS_NO_AUTHORITY / M1.5C_M1.5D_LOCAL_RUNTIME_EVIDENCE_AND_EXACT_PACKAGE_PASS_LIVE_ZERO_OF_31_EACH / M2.1A_LOCAL_RESEARCH_CONTRACT_PASS_REAL_COHORT_MISSING_NO_CANDIDATE_EMISSION / M1.5-B1_EARLY_SHADOW_BUSINESS_GATE_PASS_V1_ONLY / M1.6-P0_EXECUTED_BLOCKED_CAPACITY_AND_RECOVERY / M1.6-P0R_ROUTE_AUTHORITY_PRODUCER_TRANSPORT_V4_17_MEMBER_P0R_138_OF_138_AND_EXACT_MAIN_BRANCH_FULL_CI_PASS / REMOTE_QUALIFICATION_REBIND_AND_RECOVERY_PENDING / PRODUCTION_UNCHANGED / CLOUD_LISTENER_FIREWALL_SERVER_RESIDUE_AND_PRODUCTION_ZERO_DRIFT_UNVERIFIED / M2.2-B0.2-C1_FORWARD_ONLY_READY_NO_AUTHORITY / M3.0-M3.3_LOCAL_CONTRACT_PASS_TEST_ONLY_UNCALIBRATED_NO_READY_AUTHORITY / M3.3E_STRATEGY_ARCHETYPE_LABELING_DESIGN_AUTHORITY_ONLY / M3.4-R0_SCOPE_REBASE_GOVERNANCE_PASS_R1_PARTIAL_TEST_ONLY_DRAFT / EXTERNAL_RIGHTS_AND_HISTORICAL_SOURCE_BLOCKED / GATE_INSUFFICIENT / DETECTORS_DRAFT / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED
V2 生产权限：false
自动交易：永久禁止
最新生产存储门禁：P0_BLOCKED_CAPACITY_AND_RECOVERY / APPLICATION_HEALTH_NOT_EVALUATED
```

2026-08-02 最新覆盖：B8 exact target、B9 COS CAM action 和 route evidence v1 资格均只保留历史证据。两个 Bridge 窗口只预建 credential TTY，并在 native Copy 到达前分别等待 540 秒超时；后到 response 已作废且未进入 Bridge 或服务器。之后双端点 egress 一致漂移为 `156.248.15.36`，旧 exact `/32` 下的 8022 已不可达。当前仅证明本机 Bridge=0、clipboard 已覆盖；云 listener、旧规则、服务器 residue 和生产零漂移尚未复核。bridge/session v5、transaction v1 与 rebind v4/v3 已关闭双 TTY、状态机、控制文件和显式 8022/unit absence 缺口。继续权威审计发现旧 route evidence 只有 validator、没有 producer，测试曾允许任意 final hash 冒充 firewall identity。route authority implementation commit `157e9a79a635d8f31857481fbd6a51d10df51160` 已在唯一 V2 实施分支加入 route target v1、Tencent firewall capture v1、strict listener observer、authoritative route producer 和 route evidence v2；route target、lease、firewall capture 与 listener observation 必须 canonical，provisioning plan 保持稳定安全读取和精确摘要绑定，transaction 从完整 provider/server 观察重算 identity/digest。101 条两页分页、官方协议 `Port` 语义、RequestId 唯一性和 SSH identity TOCTOU 防护均有回归。observer 进入 transport v4，内包 17 members，current runtime source 9 files。定向 `23/23`、P0R `138/138`、launcher `3/3`、V2 Ops `263/263`、Go helper、ESLint 与 Biome PASS；默认错误工具链运行真实 FAIL 且不计资格，固定入口现自动选取锁定版本。M0 进一步以 `6/6` 防绕过回归锁死外层 launcher、内层 14 项有序生产门禁与 self-building verifier。隔离候选曾在唯一 V2 分支身份门 fail closed，Foundation=`643 total / 636 pass / 6 explicit skip / 1 expected blocker`；该历史失败未被覆盖。精确合入后，官方分支完整 `ci:production` 已从头 PASS：recurrence `11/11`、dispatch `25/25`、market `965 PASS / 4 explicit skip`、workers `23/23`、historical `4/4`、V2 Foundation `643 total / 637 PASS / 6 explicit skip`、V2 Ops `263/263`、M0 `12/12`、Next production build、Golden `16/16` 和 security 全部通过。GitHub 四门、fresh rebind、外部清场、新 execution identity 和真实恢复仍待完成；GitHub 与生产未改变。
...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.85](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.90](./market-radar-v2-controlled-replacement-traceability.v1.json) | Scope Epoch、A0、18 个 Module、5 维状态、硬门槛和 M0-M7 的机器合同 |
...
当前根治在 plan v4、credential v3、bridge/session v5、transaction v1 和 rebind v4/v3 之上新增 route target v1、Tencent firewall capture v1、strict listener observer、authoritative route producer 与 route evidence v2。transaction 不再信任调用者提供的 final hash，而是从完整 provider response 与服务器 observation 重算 identity/digest；route target、lease、firewall capture 与 listener observation 必须 canonical，provisioning plan 保持既有精确 SHA-256 合同，输出必须 no-clobber mode-600。observer 纳入 transport v4，内包 17 members、current runtime 9 files。implementation commit `157e9a79...` 已精确合入唯一 V2 实施分支；定向 `23/23`、P0R `138/138`、Go helper、ESLint、Biome 和官方分支最终字节完整 `ci:production` 全部 PASS。候选分支的预期身份门失败保留为历史审计证据。当前仅剩本轮 exact source 的 GitHub 四门、fresh read-only rebind、新 route target/lease/run/v4 plan/transport/staging 和 corrected recovery；外部全清场前不得创建新 route 或 secret。只有这些资格和全量 cleanup 同时成立，才允许用户完成唯一不可替代的一次 MFA + API Explorer 原生 Copy；其余 capture、observer、evidence、上传、执行、验证和清理均由代理完成。P0R 还必须完成真实加密离机备份、exact version retrieval、独立 PG16 restore parity、全量 cleanup 和零漂移复核；随后刷新 health/topology、容量校准和 fresh P0。只有新 P0 PASS 才能请求 P1，并严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 推进。
