...
当前根治在 plan v4、credential v3、bridge/session v5、transaction v1 和 rebind v4/v3 之上新增 route target v1、Tencent firewall capture v1、strict listener observer、authoritative route producer 与 route evidence v2。transaction 不再信任调用者提供的 final hash，而是从完整 provider response 与服务器 observation 重算 identity/digest；route target、lease、firewall capture 与 listener observation 必须 canonical，provisioning plan 保持既有精确 SHA-256 合同，输出必须 no-clobber mode-600。observer 纳入 transport v4，内包 17 members、current runtime 9 files。implementation `157e9a79...`、诊断 `8059edf...` 与 descriptor-first remediation `913bae3a...` 已形成完整修复链；当前定向 `24/24`、P0R `139/139`、V2 Ops `266/266`、完整本地 CI 和修复 source GitHub 四门全部 PASS。候选分支身份门与首轮 CodeQL 失败保留为历史审计证据。当前剩余最终资料 HEAD 资格、fresh read-only rebind、新 route target/lease/run/v4 plan/transport/staging 和 corrected recovery；外部全清场前不得创建新 route 或 secret。只有这些资格和全量 cleanup 同时成立，才允许用户完成唯一不可替代的一次 MFA + API Explorer 原生 Copy；其余 capture、observer、evidence、上传、执行、验证和清理均由代理完成。P0R 还必须完成真实加密离机备份、exact version retrieval、独立 PG16 restore parity、全量 cleanup 和零漂移复核；随后刷新 health/topology、容量校准和 fresh P0。只有新 P0 PASS 才能请求 P1，并严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 推进。
