...
P0R 本地恢复与云资源前置安全工程现已通过：同快照加密备份、数据库 fingerprint、运行级 COS/STS plan、香港单 AZ/region 验证、上传前对象不存在、私有 exact-version retrieval、隔离 PG16 restore、证据 verifier、checksum-bound bundle 和失败注入均已自动验证。真实专用空桶已按香港单 AZ、私有、versioning、SSE-COS 创建并复核；精确名称只保存在 Git 外受限事实文件。当前账号没有 Object Lock 入口，白名单工单已填写脱敏草稿但尚未提交；macOS Keychain age vault 工具本地 41/41 PASS，真实身份未生成。Object Lock、STS、生产对象、restore parity、零付费容量证明和 fresh P0 尚未发生，因此不得把空桶或本地 PASS 写成生产恢复 PASS，P1 继续关闭。
...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.15](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.17](./market-radar-v2-controlled-replacement-traceability.v1.json) | 18 个 Module、5 维状态、4 类不确定性、硬门槛和 M0-M7 的机器合同 |
