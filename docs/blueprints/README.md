...
V2 实现状态：M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M2.0_LOCAL_CONTRACT_PASS / M2.1_LOCAL_DRAFT_KERNEL_PASS / M2.2-A_LOCAL_HARNESS_PASS / M2.2-B0_LOCAL_SOURCE_GATE_AND_TECHNICAL_PILOT_PASS / BULK_AND_COHORT_BLOCKED / REAL_COHORT_MISSING / GATE_INSUFFICIENT / DETECTORS_STILL_DRAFT / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED / LIVE_MARKET_AND_SHADOW_UNPROVEN
V2 生产权限：false
自动交易：永久禁止
...
M0.0-M0.3、M1.1-M1.5-B0、M1.6、M2.0、M2.1、M2.2-A 和 M2.2-B0 已通过各自本地出口；生产仍为零变更。B0 已真实证明 Binance Vision 精确对象下载/checksum/删 raw 技术链，但来源权利与 point-in-time instrument history 未通过，因此 bulk acquisition=false、cohort freeze=false。当前 accepted real cohort=0，M2.1 也没有 target-blind diagnostic strength，Top20/sensitivity/真实 holdout 证据缺失，所以 Gate=INSUFFICIENT、Detector 仍为 DRAFT。M1.6 只证明分区/retention 本地实现和真实隔离 dump/restore/replay，不证明生产容量。本机 provider live probe 连接超时，Edge OrcaTerm 为 0 会话，本机无 Docker CLI，故真实镜像、live market 和 Shadow/SLO 均未证明。等待外部 Gate 时，当前允许继续的本地下一包是：

```text
V2-M2.2-B0.1-TARGET-BLIND-DIAGNOSTIC-STRENGTH-AND-CONSTRUCTION-POLICY-FREEZE
```

该包只建立真实冻结历史 cohort、candidate/event/matched-non-event 三分母、分层指标、threshold sensitivity 和 untouched holdout；不得用合成 fixture 自批 Detector 晋级。外部 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE` 在可信通道恢复后执行固定 31 个一分钟周期；B1 与 M1.6 都通过后才进入 M1.7 24 小时 Shadow。任何生产 Shadow、identity 或 migration 必须另过独立 Gate；M1.7 前不得让 M2 runtime 读取 M1 authority、写 Candidate Store、接入页面或生成等级/交易计划。
