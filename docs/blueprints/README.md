...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.84](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.89](./market-radar-v2-controlled-replacement-traceability.v1.json) | Scope Epoch、A0、18 个 Module、5 维状态、硬门槛和 M0-M7 的机器合同 |
...
当前隔离根治在 plan v4、credential v3、bridge/session v5、transaction v1 和 rebind v4/v3 之上新增 route target v1、Tencent firewall capture v1、strict listener observer、authoritative route producer 与 route evidence v2。transaction 不再信任调用者提供的 final hash，而是从完整 provider response 与服务器 observation 重算 identity/digest；route target、lease、firewall capture 与 listener observation 必须 canonical，provisioning plan 保持既有精确 SHA-256 合同，输出必须 no-clobber mode-600。observer 纳入 transport v4，内包 17 members、current runtime 9 files。定向 `23/23`、P0R `138/138`、Go helper、ESLint 与 Biome PASS；隔离候选 CI 在唯一 V2 实施分支身份门安全阻断，候选审计、精确合入、主线最终 CI、clean exact source、GitHub 四门、fresh read-only rebind、新 route target/lease/run/v4 plan/transport/staging 和 corrected recovery 尚未完成。外部全清场前不得创建新 route 或 secret。只有这些资格和全量 cleanup 同时成立，才允许用户完成唯一不可替代的一次 MFA + API Explorer 原生 Copy；其余 capture、observer、evidence、上传、执行、验证和清理均由代理完成。P0R 还必须完成真实加密离机备份、exact version retrieval、独立 PG16 restore parity、全量 cleanup 和零漂移复核；随后刷新 health/topology、容量校准和 fresh P0。只有新 P0 PASS 才能请求 P1，并严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 推进。
