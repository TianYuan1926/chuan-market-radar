...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.96](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.101](./market-radar-v2-controlled-replacement-traceability.v1.json) | Scope Epoch、A0、18 个 Module、5 维状态、硬门槛和 M0-M7 的机器合同 |
...
route target、Tencent firewall capture、route evidence 与 descriptor-first 安全修复只保留为已资格化上游。前三个 gateway dispatch 均已 claim 且永久不可复用；第三次 source `ad9aeb9842f6b57d934e7729e5e8a5cf2680a3b7` 通过同源四门后真实完成 Caddy-only deploy、健康、mount 和 route probe，随后因 `evidence_signing_key_path_invalid` 在 evidence seal 失败。自动 rollback 恢复 baseline Caddy，十个非 Caddy IDs、生产 HEAD、clean worktree、健康和非目标面零漂移；该事实不能被旧“两次 pre-mutation failure”覆盖。生产诊断已排除 Caddy route 与 host key 故障，直接缺口是 request v3/default signer 没有绑定并传递 signing key path，而本地测试手工 options 造成假等价。occurrence 3 已退役 runtime-identity-only operation；request v4 绑定 key path/fingerprint，并在 mutation 前真实 sign+verify。gateway `14/14`、recurrence `11/11`、dispatch `38/38` 与最终字节完整 exact-toolchain CI 均 PASS。必须继续完成 clean commit/push、同源四门、fresh baseline、双构建、新批准、signer-bound execution/自动回滚、自动取回、fresh rebind 与真实 P0R recovery。只有 gateway real-target acceptance、P0R 与 fresh P0 均 PASS 才能请求 P1，并严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 推进。
