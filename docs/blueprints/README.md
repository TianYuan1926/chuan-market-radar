...
P0R 本地恢复、六小时无扩容容量和 fresh P0 组合准入工程已通过；真实 COS 已启用并回读 Object Lock=`COMPLIANCE` 31 天，age X25519 身份仅保存在 macOS Keychain。历史 `bed938...` staging 保持 `REJECTED_SUPERSEDED_SECURITY_SOURCE`。source `94118...` 的 v2 Region 失败和 `bd20...` 的四门、fresh rebind、生产零漂移、13-member v3 plan/bundle staging PASS 均保留为历史证据。第一枚 STS 因 receiver 未运行而暴露并已过期；第二枚 STS 虽进入 receiver，却因超过签发后 5 分钟被真实时钟编译器正确 BLOCKED，未使用伪造时钟，也未继续 COS、数据库或 Runner。其 exact expiry `2026-07-28T20:57:29Z` 已由本机 UTC `2026-07-28T20:57:48Z` 与腾讯 STS HTTPS Date `2026-07-28T20:57:56Z` 双重证明超过，现永久禁止复用。raw path 删除后仍发现 PID `1175383` 的 `tee` 持有 unlink 文件；核对身份并终止后，生产 P0R 文件和进程均复核为 0。由此裸 `tee`、raw 落盘、manual compile、caller-supplied clock override、AX reconstruction 和 Compose env 重插值永久退役，`bd20...` bundle 不再拥有执行权。本地 replacement no-echo atomic session 已通过 P0R 81/81、Go helper、recurrence 10/10、dispatch 24/24 和完整 `ci:production`；Runner 内部连接描述和 canary 只进入 owner-bound mode-700 私有 `/dev/shm` 目录，以 no-clobber mode-600 文件生成并在成功前验证整目录删除；fresh rebind schema v2 另行绑定 transport v2 全部七个运行文件，历史 transport v1 仍只按原三文件比较，禁止混算。新 clean commit、GitHub 四门、fresh rebind、新 14-member plan/bundle、真实 target acceptance、backup、exact retrieval、isolated restore、fresh topology、calibration 和 fresh P0 仍未发生，P1 关闭。M1.1B0 R3 已真实取得 15/15 exact endpoint conformance；M1.4B 腾讯 bootstrap/resume 两轮均为 14/14 route PASS、0 failed、1 registry blocked，Binance spot 请求数固定为 0。M1.4C 22/22 与 M2.1A 13/13 本地合同出口通过；M1.4D Base Fact 23/23、M1.5C/M1.5D Expanded Shadow 70/70、exact no-secret package 12/12 均通过，但真实 live cycle 均为 0。C1 四轮三 Venue COMPLETE 与 Scope V2 资产域重放仍为 forward-only/no-authority；Bitget、Listing Lifecycle、Equity Asset Domain 和 Data Maximization 独立核算，股票 tradable Fact=0。真实 cohort、holdout、校准和 authority 仍为 0。
...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.60](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.66](./market-radar-v2-controlled-replacement-traceability.v1.json) | Scope Epoch、A0、18 个 Module、5 维状态、硬门槛和 M0-M7 的机器合同 |
...
M0.0-M0.5、M1.1-M1.6、M2.0-M2.2 已列本地包、C1、M3.0-M3.3 已通过各自工程、业务、运行起点或合同出口。M1.4C 已取得 22/22 本地合同与完整 CI 出口，M1.4D 及 M1.5C/M1.5D 的本地 runtime/evidence/exact package 已通过，M2.1A 已取得 13/13 research-only 合同出口；但 M1.5C/M1.5D 真实 live cycle 均为 0，故无持续 runtime/Fact/Candidate/Signal/READY authority。C1 最新四轮三 Venue evidence 已通过全历史完整性审计与 Scope V2 资产域重放，并取得 exact-source 四条 GitHub 质量 PASS；它仍只有约 16.6 分钟、没有 Bitget、官方 mapping 完整性、Fact 或 authority。A0 工程材料、供应链、exact-runtime remote CI、完整历史 secret、CodeQL、collector 镜像扫描、可重复制品 provenance、隔离回滚和冻结性能资源基线已 PASS；唯一剩余控制是 P0R 真实恢复，总门禁保持 `INCOMPLETE_P0R_PENDING`。Scope V2 的 M3.1A-D-R0 四轨 research contract scaffold 已通过定向测试与完整 CI，但真实 M3.1A-M3.3D 仍等待 M2.3/M2.4 cohort、untouched holdout 和分域校准。M3.3E 策略类型标签只完成设计权威，schema、runtime、Outcome 和前端消费尚未实现。B1-B1 永久 `INVALID_NOT_COUNTED`，B1-B3 已取得 V1 业务 PASS。M1.6-P0 因容量与 recovery evidence `BLOCKED`；Object Lock 31 天和 age Keychain 身份保持有效，历史 `bed938...` staging 已拒绝执行。历史 `94118...` Region 失败和 `bd20...` 四门/rebind/bundle PASS 只保留为根因与资格证据。第二枚 STS 因延迟编译正确 BLOCKED，旧双会话 receiver 路线已失效；replacement no-echo atomic session 已完成定向门禁和完整本地 CI，但尚无新 exact source、远端资格、fresh rebind、plan/bundle 或恢复证据。历史来源仍 `RESEARCH_ONLY`，Gate=INSUFFICIENT、Detector=DRAFT；M3.4-R0 只完成 Scope V2 治理门禁，R1 草稿没有独立测试出口，仍无 Feasibility/runtime/READY authority。当前生产执行入口是：

```text
V2-M1.6-P0R-B3-RECOVERY-EXECUTION
```

第二枚 STS 的可信时钟过期证明已经完成；完整本地 CI 已 PASS。现在把 no-echo atomic session 形成 clean exact commit，再完成 GitHub Full Quality、Signed Production Dispatch Quality、Independent Security、A0 Release Qualification、fresh production read-only rebind 和新 run-id/plan/14-member bundle staging 验证，之后才允许签发新的 exact 7200 秒 STS。两个 fresh OrcaTerm 会话分别执行唯一 no-echo session 入口：第一会话以内存即时编译 STS，第二会话注入临时 age identity；两项 secret 到齐后 helper 自动执行真实加密离机备份、exact version retrieval、独立 PG16 restore parity、全量 cleanup 和零漂移复核。raw response 不得落盘；断线、超时、无 EOF、编译超窗或失败必须自动清理。随后刷新 production health/topology，在 exact clean release 重跑容量校准和 fresh P0；不得清缓存、缩短核心扫描分母或改阈值。只有新 P0 PASS 才能请求 P1，并严格按 `P1 schema -> P2 identities -> P3 partitions+dormant Worker -> P4 isolated-write Shadow -> M1.7 24h` 推进。固定通道只作为普通无 secret 包的默认运输层，不替代 P0R `/dev/shm` 凭证边界或任一业务包自身 Gate。M1.4D 与 M1.5C/M1.5D 本地 exact package 已通过，但真实派发还必须在 A0/P0R 关闭后刷新同一 clean commit 的 conformance + M1.4B upstream，再构建相同 release 的两包并分别验收。只允许 live PASS 且 registry route eligible 的 capability 进入 batch；Bybit 只声明 provider-available history，Bitget 只声明官方一个月窗口，Binance spot 仍须修订 registry 后重跑。M1.6-D1 必须使用 M1.5C/M1.5D 的真实事实率。外部门 `V2-M2.2-B0.2-B-EXACT-SOURCE-RIGHTS-AND-CAPABILITY-RESOLUTION` 仍需人工来源权利和合格历史身份；M1.7 前不得让 M2/M3 runtime 写 Candidate、接页面或生成真实等级/计划。
