...
P0R 本地恢复工程现已通过：同快照加密备份、数据库 fingerprint、私有 COS version retrieval、隔离 PG16 restore、证据 verifier、可复现 bundle 和失败注入均已自动验证。真实生产 COS 对象、restore parity、容量扩展和 fresh P0 尚未发生，因此不得把本地 PASS 写成生产恢复 PASS，P1 继续关闭。
...
| 优先级 | 文档 | 唯一职责 |
| ---: | --- | --- |
| 1 | [V2 受控替换工程与运行蓝图 v1.12](./MARKET_RADAR_V2_CONTROLLED_REPLACEMENT_BLUEPRINT_V1.md) | 当前唯一产品、领域、工程、研究、运行与切换设计权威 |
| 2 | [V2 机器追踪矩阵 v1.14](./market-radar-v2-controlled-replacement-traceability.v1.json) | 18 个 Module、5 维状态、4 类不确定性、硬门槛和 M0-M7 的机器合同 |
