...
`V2-M1.1A Four-Venue Capability Registry` 已达到 `LOCAL_CONTRACT_PASS / OFFICIAL_DOCUMENTS_REVIEWED`：4 个第一方 Venue + CoinGlass Hobbyist、33 类目标能力、165 个 source-capability 组合完整记账，官方文档行 110，unavailable/unlicensed 57，Scope V2 live runtime PASS=0。旧 V1 的 Binance/OKX/Bybit catalog 与 mark-price 六行只保留 V1 证据。

`V2-M1.1B Exact Source Conformance + Multi-Asset Identity + Listing Intelligence` 已达到 `LOCAL_IMPLEMENTATION_PASS / TEST_ONLY_CONFORMANCE_PASS / LIVE_B0_ATTEMPTS_BLOCKED_NOT_COUNTED`：15 个探针、四 Venue catalog normalizer、Bitget、官方资产映射、listing/identity epoch、Bybit/Bitget announcement normalizer 和生命周期账本已进入独立 Scope V2 层，R2 定向 24/24 PASS。它不修改旧 V1 eligibility，也没有 live、Shadow、容量、校准、Candidate 或 Strategy authority。首次腾讯尝试未形成业务 artifact/result；R1 腾讯尝试形成 0/15 PASS 的共同运行时传输失败 artifact/result。两次均不能算 live PASS；下一 Scope V2 入口是 R2 Node core HTTPS exact clean commit 的腾讯隔离 `LIVE_READ_ONLY` B0，只有实际 PASS 的能力才可进入 M1.4B runtime Adapter。

`V2-M1.1B0 Tencent Live Source Conformance No-Secret Dispatch Package` 当前为 `ATTEMPT_1_BLOCKED_NOT_COUNTED / R1_LIVE_0_OF_15_PASS_15_OF_15_COMMON_TRANSPORT_FAILURE / JITLESS_WEB_FETCH_ROOT_CAUSE_PROVEN / R2_NODE_HTTPS_DIRECTED_24_OF_24_AND_FULL_CI_PASS / COMMIT_AND_NEW_DISPATCH_PENDING`：正式包继续绑定 exact commit/tree/ref、15 项固定分母、announcement filters、并发策略、最小 Zod 运行树和生产前后身份；CoinGlass key 不进入运输层或 evidence。R1 明确 Bybit 最新两页 `BOUNDED_COMPLETE` 与 M1.4B 完整历史回填的职责分离，并为前 artifact 故障持久化 phase/reason；R1 现场进一步证明 Web Fetch 在固定 Node `--jitless` 下依赖不可用 WebAssembly。R2 保留安全硬化并使用 Node core HTTPS，生产 transport 身份进入 `probePlanDigest`。本地网络的 14 项 reset/timeout 仍只登记为非权威诊断；两次生产阻断也不能被新代码覆盖成 PASS。R2 精确提交推送和新的腾讯现场执行仍是后续独立 Gate。
...
**当前执行入口**：`V2-M1.6-P0R-C-STS-ENCRYPTED-BACKUP-EXACT-RETRIEVAL-AND-ISOLATED-RESTORE`。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 fresh 证据通过前不变。香港单 AZ 私有 COS 已启用并回读 `COMPLIANCE` 31 天，真实 age X25519 身份仅保存在 macOS Keychain；exact commit `6a81e865e61569f7d2d7c3bb3be1d78db72a9eab` 的 plan 与无 secret transport bundle 已复核。下一步只能签发与该 plan 完全一致的 7200 秒 STS，执行真实加密离机备份、精确 version retrieval、独立 PostgreSQL 16 restore parity 与清理证据；随后刷新 health/topology，在 exact clean release 重跑容量校准和 fresh P0 组合准入。只有新的 P0 PASS 才能请求 P1 v1+v2 schema，之后仍须依次执行 P2 最小权限身份、P3 六小时分区与 dormant Worker、P4 有界 isolated-write Shadow 和 M1.7 同 release 24h Shadow。B0.2-B 外部权利与历史来源 Gate 继续并行 blocked；M2 runtime、页面和交易计划仍关闭。

**V2 扩展子包**：`M1.1A Four-Venue Capability Registry [LOCAL_PASS] -> M1.1B local implementation [LOCAL_PASS_TEST_ONLY] -> M1.4A capability-independent scheduler contract [LOCAL_CONTRACT_AND_FULL_CI_PASS] + M1.1B0 Tencent isolated live source conformance [R1_0_OF_15_RUNTIME_TRANSPORT_BLOCKED / R2_NODE_HTTPS_FULL_CI_PASS_NEW_DISPATCH_PENDING] -> M1.4B endpoint batching/runtime Adapter profiles + listing-history bootstrap -> M1.5C Four-Venue Multi-Asset Shadow -> M1.6-D1 Expanded-Scope No-Cost Capacity Proof`。Bitget Venue、Listing Lifecycle 与股票 Asset Domain 分别核算；M1.4A 已冻结失败关闭的调度合同，但只有 live B0 实际 PASS 的 capability 才能进入 M1.4B runtime Adapter。Bybit 完整公告历史必须在 M1.4B 以 checkpoint/gap/incremental 证据验收，不能由 B0 两页窗口冒充；原 B1-B3 和 D0 只属于 V1，V2 未完成新 Shadow 和容量证明前不得进入扩展生产范围。
...
`V2-M3.3 Strategy Construction` 已达到 `LOCAL_CONTRACT_PASS / TEST_ONLY_UNCALIBRATED / NO_READY_AUTHORITY`。六族 long/short 使用独立模板；entry/stop/target 必须引用 Analysis exact level，stop 只能在结构失效基准外扩，缺字段时必须 no-draft abstain。`StrategyDraft v2` 写入 reference/cost/buffer/RR lineage，Final Decision 重新核对 level price 与 exact RR。M3.3 20/20、M3.2 18/18、M3.1 21/21、M3.0 22/22，合计 81/81；当前只有带 blocker 的测试草案，没有真实成本、Execution Feasibility、Risk、runtime 或 READY authority。

V2 多资产需要 `M3.1A-M3.3A` 逐域扩展 Analysis/Qualification/Strategy，再由 `M3.4-M3.6` 完成执行可行性、Personal/Portfolio Risk 与 runtime Gate。股票必须新增传统市场 session、公司行动、FX、mark/index/reference、休市 basis、multiplier、funding/fee/slippage 和地区可用性；这些缺失时只能 OBSERVE/WAIT/BLOCKED。当前未提交的 M3.4 草稿起源于 V1 范围，提交前必须进行 scope rebase review。
...
M0.0-M0.4、M1.1A、M1.1B 本地实现、M1.1-M1.5-B0、M1.5-B1-A/B0/B2/B3、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1、B0.2-A、B0.2-C/C1 和 M3.0-M3.3 已通过各自设计、本地、技术、业务、运行起点或合同出口；B1-B1 因证据包装失败永久记为不计数，B1-B3 已用新 schema 重新取得完整业务 PASS。M1.1B0 首次腾讯派发已真实阻断且不计 live B0；R1 随后形成 0/15 PASS 的共同传输失败证据，并证明固定 Node `--jitless` 下 Web Fetch 的 WebAssembly 依赖是共同根因。R2 保留安全硬化、改用 Node core HTTPS，当前通过本地 24/24、固定派发 21/21、V2 Foundation 422/428（6 项明确跳过）、V2 Ops 125/125、M0、Next build、Golden 16/16、安全扫描与完整 CI；精确提交与新派发仍待收口。M1.1B/M1.4A 只证明 `SCOPE_EPOCH_V2_MULTI_ASSET_4V` 的本地合同和无 authority 调度能力；Bitget Venue、股票 Asset Domain、上新 Lifecycle 的腾讯 live、Shadow、容量、校准和生产证明都未完成。M1.6-P0 已执行并因容量与 recovery evidence 真实 BLOCKED；P0R-D0 和 fresh P0 组合准入工具本地 PASS，Object Lock 31 天、age Keychain 身份、exact transport bundle 和 clean pre-STS baseline 已通过，STS、对象、真实恢复、fresh topology、exact-release 校准与 fresh P0 生产结论仍待执行。M2.2 真实能力 Gate、外部来源解决、bulk historical acquisition、生产存储和持续 SLO 尚未通过；M3.0-M3.3 不构成 M3 runtime、真实校准、真实可执行 Strategy 或 READY authority。生产当前执行入口是：
...
Scope V2 后续先完成 R2 exact clean commit/push，再执行新的腾讯隔离 `M1.1B0 LIVE_READ_ONLY source conformance`；只有实际 PASS 的目录、时间、现货、公告和 CoinGlass capability 才能进入 `M1.4B endpoint batching/runtime Adapter`。M1.4B 同时负责 Bybit listing history 的一次性回填、持久 checkpoint、缺口检测和增量页，不得把 B0 的两页 `BOUNDED_COMPLETE` 改写成完整历史。Bitget、Listing Lifecycle 与股票 Asset Domain 的运行/研究/决策证据继续分开，任何一条链失败都只能影响对应 capability，不能用其他链的 PASS 补偿。M1.4A 只提供 no-authority 调度合同，当前 TEST_ONLY 结果不能替代 live Gate。同时保持 `B0.2 rights + point-in-time instrument metadata -> B1 immutable raw acquisition -> B2 cohort construction -> B3 split + sealed holdout -> C registered replay` 的真实研究顺序。M3.4 旧范围草稿暂停在未提交状态，必须先完成 scope rebase review；不能接页面或产生 READY。全程禁止读取 Legacy 运行模块或 M1 runtime、写 Candidate Store、自动调权/晋级、生成真实 Signal/Plan、删除 Legacy 代码或宣称四 Venue/股票/上新 live 能力完成。
