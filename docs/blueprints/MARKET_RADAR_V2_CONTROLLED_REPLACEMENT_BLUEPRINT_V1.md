...
v1.36 把 M1.4B 现场出口从文字计划升级为无 secret、内容寻址的固定派发包。该包保持 15 个 live-conformant Profile 完整分母，只执行 14 个 route-eligible Profile，`BINANCE_SPOT_CATALOG` 请求数固定为 0；五个 source group 跨来源并行、同源并发 1，Bybit/Bitget listing 各有 64 页有界预算。Bitget Venue、Listing Lifecycle、Equity Asset Domain、Data Maximization 四轴独立核算，任何一轴不能借 PASS。Bundle 绑定 R3 artifact、registry/probe digest、exact source commit/tree/ref 及生产 HEAD、容器、listener、timer、health；CoinGlass key 只进入目标机一次性子进程。blocked segment 不晋级 checkpoint，续跑必须同时绑定原 checkpoint 与精确 `PASS` result SHA-256。定向包 9/9 PASS；正式实施分支完整 CI、GitHub 同步和腾讯真实执行尚未完成，生产服务、数据与 authority 不变。
...
`V2-M1.4B Endpoint Batching + Runtime Adapter Profiles + Listing History Runtime` 当前为 `LOCAL_CORE_AND_EXACT_DISPATCH_PACKAGE_PASS / FULL_CI_REVERIFY_PENDING / LIVE_NO_AUTHORITY_RUNTIME_UNPROVEN`：本地已把 exact conformance、registry/probe digest、HTTPS endpoint、分页、credential、恢复和四条独立验收轴冻结成内容寻址 Profile，并把 M1.4A ready intent 精确一次合并为 source-capability batch。snapshot 请求节省与 listing-history 分页预算分开；Bybit provider-available history 和 Bitget 官方一个月窗口已有 bootstrap/resume/gap/incremental 状态机。固定现场包只执行 R3 15/15 endpoint PASS 中 14 个 route-eligible Profile，`BINANCE_SPOT_CATALOG` 因 registry 仍为 `UNAVAILABLE` 保持零请求；Bundle 无 secret，执行前后绑定宿主身份，blocked segment 不晋级 checkpoint，续跑只接受绑定精确 PASS result 的 checkpoint。Bitget Venue、Listing Lifecycle、Equity Asset Domain 与 Data Maximization 分别记账，股票只做 catalog accounting、tradable Fact batch 为 0。定向包 9/9 PASS；正式实施分支完整 CI、腾讯隔离执行、真实 checkpoint、请求率/配额、四 Venue Shadow、容量和下游 authority 均未证明。
...
**V2 扩展子包**：`M1.1A Four-Venue Capability Registry [LOCAL_PASS] -> M1.1B local implementation [LOCAL_PASS_TEST_ONLY] -> M1.4A capability-independent scheduler contract [LOCAL_CONTRACT_AND_FULL_CI_PASS] + M1.1B0 Tencent isolated live source conformance [R1_0_OF_15_RUNTIME_TRANSPORT_BLOCKED / R2_14_OF_15_LISTING_BLOCKED / R3_15_OF_15_ALL_GATES_PASS] -> M1.4B local endpoint/profile/history core + exact fixed-dispatch package [LOCAL_PACKAGE_PASS / FULL_CI_REVERIFY_PENDING / LIVE_RUNTIME_UNPROVEN] -> M1.4B Tencent isolated no-authority runtime -> M1.5C Four-Venue Multi-Asset Shadow -> M1.6-D1 Expanded-Scope No-Cost Capacity Proof`。Bitget Venue、Listing Lifecycle、Equity Asset Domain 与 Data Maximization 分别核算。R3 的 15/15 endpoint conformance 不能掩盖当前 14/15 route eligibility；Binance spot 必须完成 registry 修订和新 digest live conformance。Bybit 完整公告历史必须以真实 checkpoint/gap/incremental 证据验收，Bitget 只能声明官方一个月窗口完整；原 B1-B3 和 D0 只属于 V1，V2 未完成新 runtime、Shadow 和容量证明前不得进入扩展生产范围。
...
Scope V2 的 M1.4B 本地核心和 exact fixed-dispatch package 已通过定向门禁，正式实施分支完整 CI 与 GitHub 同步仍待完成；随后才执行腾讯隔离 no-authority runtime。只允许 live conformance PASS 且 registry route eligible 的 capability 进入 batch；当前 Binance spot 请求数固定为 0，必须先修订 registry 并用新 digest 重跑 conformance。真实 Bybit listing history 必须完成一次性回填、持久 checkpoint、缺口检测和增量页，Bitget 只能按官方一个月窗口验收，不得把 B0 两页或本地 fixture 改写成完整历史。Bitget Venue、Listing Lifecycle、Equity Asset Domain 与 Data Maximization 的运行/研究/决策证据继续分开，任何一条链失败都只能影响对应 capability；失败 segment 不晋级 checkpoint，续跑只接受绑定原精确 PASS result 的 checkpoint。M1.4A/M1.4B 本地实现和 M1.1B0 PASS 都不能替代持续 Adapter、Shadow、容量或校准 Gate。同时保持 `B0.2 rights + point-in-time instrument metadata -> B1 immutable raw acquisition -> B2 cohort construction -> B3 split + sealed holdout -> C registered replay` 的真实研究顺序。M3.4 旧范围草稿暂停在未提交状态，必须先完成 scope rebase review；不能接页面或产生 READY。全程禁止读取 Legacy 运行模块或 M1 runtime、写 Candidate Store、自动调权/晋级、生成真实 Signal/Plan、删除 Legacy 代码或宣称四 Venue/股票/上新 runtime 能力完成。
