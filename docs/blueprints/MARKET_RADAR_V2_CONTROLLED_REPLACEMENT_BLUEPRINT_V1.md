...
**当前进度**：`V2-M1.5-A Durable Worker Local Exit` 已达到 `LOCAL_ENGINEERING_AND_POSTGRES16_RESTART_REHEARSAL_PASS`。在 M1.4 四分母 Collector 之上增加独立 checksum 的 append-only checkpoint migration、artifact 外键/lineage guard、config/sequence/content digest、精确 release 恢复、固定节拍 skip-missed Worker、优雅停止、强制 telemetry sink、NO_AUTHORITY 进程入口和三态 SLO evaluator。真实临时 PostgreSQL 16 已证明 checkpoint 不能领先 artifact、只读/写入身份分离、append-only、幂等和关闭连接后的精确增量恢复。生产仍为零变更。
...
**当前入口**：`V2-M1.5-LIVE-SHADOW-GATE External Egress Rehearsal and Production No-Authority Shadow Gate`。先在可达网络环境复用同一 entrypoint 和 migration artifact，证明 live provider 合同与真实四分母；再经独立生产 Gate 启动无读取权威 Shadow，验证连续 freshness/coverage、限流、重连、资源和成本。不得接入 Candidate、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.4 和 M1.5-A 已通过各自本地出口，生产零变更；M1.5 live/SLO 尚未通过。当前唯一下一包是：
...
- 复用已通过本地/PG16 出口的单一 V2 Collector Worker composition，不新增平行采集实现。
- 先在 provider 可达环境以 no-read-authority 模式验证 live catalog/ticker 合同、真实 instrument 数量、请求预算和端到端延迟。
- 绑定精确 commit、镜像、checkpoint migration checksum、分离身份、资源预算和回滚目标后，再提交独立 Shadow 发布 Gate。
- 输出连续 coverage/freshness、provider failures、queue lag、recovery、release identity 和 checkpoint evidence；缺失必须 partial/unavailable。
- 观察窗口可与后续本地合同设计并行，但 M1 authority 与 M2 读取许可必须等待 SLO 出口。

该包的本地阶段仍不允许修改生产；任何 no-authority Shadow 部署和数据库变更必须另过精确发布 Gate。全程禁止接入现有页面、读取 Legacy 运行模块、生成 Candidate/Signal/Plan、删除 Legacy 代码或宣称 live 全市场能力完成。
