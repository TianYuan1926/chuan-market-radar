...
`V2-M1.4B Endpoint Batching + Runtime Adapter Profiles + Listing History Runtime` 当前为 `LOCAL_ENGINEERING_AND_FULL_CI_PASS / LIVE_NO_AUTHORITY_RUNTIME_UNPROVEN`：本地已把 exact conformance、registry/probe digest、HTTPS endpoint、分页、credential、恢复和独立验收轴冻结成内容寻址 Profile，并把 M1.4A ready intent 精确一次合并为 source-capability batch。snapshot 请求节省与 listing-history 分页预算分开；Bybit provider-available history 和 Bitget 官方一个月窗口已有 bootstrap/resume/gap/incremental 状态机。R3 15/15 endpoint PASS 中当前 route eligible 为 14/15，`BINANCE_SPOT_CATALOG` 因 registry 仍为 `UNAVAILABLE` 保持 blocked，禁止旧 digest 越权。股票只做 catalog accounting，tradable Fact batch 为 0。完整 `ci:production` 已通过；腾讯隔离持续执行、真实 checkpoint、请求率/配额、四 Venue Shadow、容量和下游 authority 均未证明。
