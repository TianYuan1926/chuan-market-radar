...
P0R 不允许以 Object Lock、Keychain 身份、transport bundle、canary、临时清理、阈值下调或 backup exit 0 冒充生产可恢复；临时凭证、对象上传、exact version retrieval、隔离 restore 和 cleanup 均是独立、可审计动作。第三枚 STS 已由本机 UTC 与腾讯 HTTPS Date 独立证明超过 exact expiry，现为 `EXPIRED_FORBIDDEN_REUSE`。当前顺序是 `新 clean exact commit -> GitHub 四门 -> fresh read-only rebind -> 全新 run-id/object key/plan/14-member bundle staging -> 签发前启动 fixed local TTY bridge 并取得 echo-disabled READY -> 用户 MFA + API Explorer 原生 Copy -> bridge 即时编译 STS并从固定 Keychain 内部注入 age identity -> helper 自动 backup / exact retrieval / isolated PG16 restore -> cleanup / zero drift -> fresh topology / calibration / P0`。response 出现后禁止 screenshot、AX/OCR、browser state read、computer-use 和 OrcaTerm secret entry；native Copy 无法完成时停止。raw STS response 不得落盘；超时、断线、无 EOF、编译超窗或失败必须自动清理。不得复用任何旧 token、`e362...` run、`bd20...` bundle、旧计划、裸 `tee`、断线 receiver、超长复合命令或空 `/dev/shm` 占位。付费扩容已退出活跃路线，六小时零付费本地机器模型与组合准入实现已通过，但这不取消原 P0 blocker：fresh 证据必须同时满足所有旧非容量门禁、稳态 60% / 峰值 70%、完整恢复和健康门禁，否则 P1 继续 BLOCKED。P0R 生产动作完成后只允许重新执行 fresh P0，不直接授权 migration、身份创建、分区预建、Worker 启动或生产写入。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 historical B1 acquisition 的硬前置，C1 不能替代或解锁它。
