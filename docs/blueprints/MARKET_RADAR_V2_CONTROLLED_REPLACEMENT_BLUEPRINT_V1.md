# Market Radar V2 受控替换工程与运行蓝图 v1.7

状态：`ACTIVE_DESIGN_AUTHORITY / M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M1.5-B1-A_REACHABLE_DOCKER_RUNNER_TECHNICAL_PASS / BUSINESS_READINESS_FAIL / B1-B_EARLY_SHADOW_PENDING / M2.2-B0.2-C1_FORWARD_ONLY_READY / B0.2_EXTERNAL_RESOLUTION_BLOCKED / M1_NOT_COMPLETE / PRODUCTION_SERVICES_DATA_AND_AUTHORITY_UNCHANGED`
...
**当前进度**：`V2-M1.5-B1-A Reachable Docker Runner Preflight` 已达到 `TECHNICAL_RUNNER_PASS / BUSINESS_READINESS_FAIL`。在 B0 安全边界上，exact commit `97f10e75ce296b07d933e9c362c40ba2be0997ea` 已于腾讯宿主机隔离 no-authority Runner 完成专用 image build、两周期三 Venue live Collector、checkpoint/persistence、内容寻址 evidence 和宿主机精确恢复。两周期 eligible/collected 均为 1,444/1,444、provider failure=0，但 fresh 分别为 1,441 和 1,274、READY=0/2；技术通路通过，业务 SLO 明确失败。
...
**当前本地工程入口**：`V2-M1.5-B1-B0-EARLY-SHADOW-EVIDENCE-CONTRACT`。先实现固定 31 周期的完整 observation、每 Venue 分母、exact release/image/config、资源与调度指标、SLO 独立结论、内容寻址 evidence、断点恢复和宿主机精确清理；再执行 B1-B1 原始实测。若业务 Gate FAIL，B1-B2 必须先拆分 provider snapshot receipt、per-instrument market-event recency 与 downstream price usability，定点修复后以同一 31 周期 Gate 做 B1-B3 复验；不得放宽 120 秒、把 duplicate/carried-forward 标为 FRESH 或删除失败分母。B1-B PASS 后才允许 M1.6 production storage 分阶段启用，随后 M1.7 24h Shadow。B0.2-B 外部权利与历史来源 Gate 继续并行 blocked；M2 runtime、页面和交易计划仍关闭。
...
M0.0-M0.3、M1.1-M1.5-B0、M1.5-B1-A、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1、B0.2-A、B0.2-C 和 C1 已通过对应本地、技术或运行起点出口；B1-A 只达到 Docker Runner technical PASS，业务 readiness/SLO 为 FAIL。M2.2 真实能力 Gate、外部来源解决、bulk historical acquisition、31 周期业务 Gate、生产存储和持续 SLO 尚未通过。当前本地入口是：

```text
V2-M1.5-B1-B0-EARLY-SHADOW-EVIDENCE-CONTRACT
```

B1-B0 只建设 31 周期证据与 Runner 合同；B1-B1 才执行原始实测。实测失败时进入 B1-B2 freshness 语义整改和 B1-B3 同门槛复验，实测直接通过时跳过两个条件包。技术 Runner PASS 永远不能替代业务 Gate。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 historical B1 acquisition 的硬前置，C1 不能替代或解锁它。
