...
**当前执行入口**：`V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY` 的 production-runtime preflight 永久根治与 replacement exact-source 重新资格化。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 recovery evidence 通过前不变。香港单 AZ 私有 COS 的 `COMPLIANCE` 31 天和 macOS Keychain age X25519 身份保持有效。首个 health-envelope 失败与第二个 runtime-identity/capability 失败都只保留历史证据；两个 dispatch 均已 claim、`FAIL_DISPATCH_NOT_REUSABLE`，两个 exact approval 均已过期且禁止复用。第二次 source `d9a1e5fedf481f00e3cc88705c7e2b884add3ebe` 的同源四门全部 PASS，但生产在任何 Caddy mutation 前直接暴露 current-image capability 和 root-owned Compose identity 两项预演偏差。fresh read-only 复核证明两个失败窗口都保持 production HEAD `cec0b657...`、clean worktree、exact 11-container identity、ready/fresh health、404 route 与全部非目标面零漂移；唯一空 outbound residue 已在批准的自动清理边界内删除。occurrence 2 已触发 `REC-2026-08-02-PRODUCTION-GATEWAY-PREFLIGHT-EQUIVALENCE` STOP-THE-LINE。local remediation commit `bb9abe5cf0404ca7ce9af55f6ed99a6e3f646d76` 将 request 升为 v3，绑定 wrapper/override exact hash 与 root mode，保留全部 Caddy isolation 且只加 `NET_BIND_SERVICE`，使用 exact wrapper 完成 config/deploy/rollback，输出稳定 command digest，并让 checksum-bound recurrence registry 在构包及生产 gateway state 创建前各验证一次。gateway `12/12`、recurrence `11/11`、dispatch `38/38` 及完整 exact-toolchain CI 均 PASS；完整 CI 的精确分母为 market `965/969`（4 explicit skips）、workers `23/23`、historical `4/4`、Foundation `638/644`（6 explicit skips）、Ops `279/279`、M0 `12/12`、Golden `16/16`，构建与安全检查也通过。最终 authority commit/push、当前源码同源四门、fresh baseline、新 deterministic package 和新批准仍待完成。固定后续顺序为 `提交推送当前 exact source -> 同源四门从零 PASS -> fresh production baseline -> 双构建 deterministic package -> 新 exact approval -> 受控 remediation execution/自动回滚 -> 自动取回验签 -> fresh rebind -> P0R recovery`；只有 gateway real-target acceptance、P0R 与 fresh P0 均 PASS 才能请求 P1，M2 runtime、页面和交易计划继续关闭。
