...
**V2 扩展子包**：`M1.1A Four-Venue Capability Registry [LOCAL_PASS] -> M1.1B local implementation [LOCAL_PASS_TEST_ONLY] -> M1.4A capability-independent scheduler contract [LOCAL_CONTRACT_AND_FULL_CI_PASS] + M1.1B0 Tencent isolated live source conformance [R1_0_OF_15_RUNTIME_TRANSPORT_BLOCKED / R2_14_OF_15_LISTING_BLOCKED / R3_15_OF_15_ALL_GATES_PASS] -> M1.4B endpoint batching/runtime Adapter profiles + listing-history bootstrap -> M1.5C Four-Venue Multi-Asset Shadow -> M1.6-D1 Expanded-Scope No-Cost Capacity Proof`。Bitget Venue、Listing Lifecycle 与股票 Asset Domain 分别核算；M1.4A 已冻结失败关闭的调度合同，M1.4B 现在只允许接入 R3 live PASS 的 capability。Bybit 完整公告历史必须在 M1.4B 以 checkpoint/gap/incremental 证据验收，不能由 B0 两页窗口冒充；原 B1-B3 和 D0 只属于 V1，V2 未完成新 Shadow 和容量证明前不得进入扩展生产范围。
...
Scope V2 后续进入 `M1.4B endpoint batching/runtime Adapter`，且只接入 R3 exact live PASS 的目录、时间、现货、公告和 CoinGlass capability。M1.4B 同时负责 Bybit listing history 的一次性回填、持久 checkpoint、缺口检测和增量页，不得把 B0 的两页 `BOUNDED_COMPLETE` 改写成完整历史。Bitget、Listing Lifecycle 与股票 Asset Domain 的运行/研究/决策证据继续分开，任何一条链失败都只能影响对应 capability，不能用其他链的 PASS 补偿。M1.4A 只提供 no-authority 调度合同，M1.1B0 PASS 也不能替代 Adapter、Shadow、容量或校准 Gate。同时保持 `B0.2 rights + point-in-time instrument metadata -> B1 immutable raw acquisition -> B2 cohort construction -> B3 split + sealed holdout -> C registered replay` 的真实研究顺序。M3.4 旧范围草稿暂停在未提交状态，必须先完成 scope rebase review；不能接页面或产生 READY。全程禁止读取 Legacy 运行模块或 M1 runtime、写 Candidate Store、自动调权/晋级、生成真实 Signal/Plan、删除 Legacy 代码或宣称四 Venue/股票/上新 runtime 能力完成。
