...
状态：`ACTIVE_DESIGN_AUTHORITY / M0_ENGINEERING_EXIT_LOCAL_PASS / M1.3_STORE_REPLAY_RUNTIME_TRUTH_LOCAL_PASS / LIVE_INGESTION_UNPROVEN / PRODUCTION_UNCHANGED`
...
本文是 V2 当前唯一设计权威。M0 已在干净实施分支通过本地工程出口；M1.1-M1.3 已完成冻结 fixture 的 Identity、Fact、单一价格分散 Feature、保守 Context、append-only Store、双 cutoff durable replay 和五维 Runtime Truth 本地纵切。这些只证明合同、纯函数和隔离 PostgreSQL 16 演练，不代表 live 全市场能力、生产变更、盈利能力或自动下单授权。
...
**当前进度**：`V2-M1.3 Fact Store, Replay Manifest and Runtime Truth Rehearsal` 已达到 `LOCAL_POSTGRES16_REHEARSAL_PASS`。三家交易所冻结 Identity/Fact/Feature/Context 已作为 8 个 artifact 原子写入临时 PostgreSQL 16；相同重试幂等、同 ID 异内容冲突、UPDATE/DELETE 与跨身份写入被拒绝，强制污染由 full storage digest 识别；event/knowledge 双 cutoff Manifest 从账本完成两次独立 replay 并通过 parity。Runtime Truth 即使所有本地技术证据通过仍诚实保持 `REHEARSAL/PARTIAL`。当前没有 live ingestion、全 eligible Universe、生产 migration、Worker 或生产 authority。

**当前入口**：`V2-M1.4 Full Eligible Universe and Collector Runtime`。只建设全 observed instrument accounting、全量/增量 reconciliation、受限并发、限速、背压、冷启动、恢复和 coverage telemetry 的本地采集运行纵切；不得执行生产 migration、接入 Candidate、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.3 已通过各自本地出口，生产零变更。当前唯一下一包是：
...
- 从单 BTC fixture 扩大为三 Venue 全 observed instrument accounting，并清楚区分 observed、eligible、collected、fresh 四个分母。
- 建立启动全量 catalog、增量 ticker、周期 reconciliation、provider quota、受限并发、背压、冷启动和恢复状态机。
- 让 Collector 只调用 Adapter 和 M1 Store，不读取页面、Legacy provider、Detector、Analysis 或 Strategy。
- 输出 coverage、freshness、provider failures、queue lag 和 recovery telemetry；缺失必须 partial/unavailable，不能静默丢币。
- 用确定性多标的 fixture、故障矩阵和本地 runtime rehearsal 验证；真实生产 Worker、migration 和 authority 仍须独立 Gate。

该包仍不允许修改生产、执行生产 migration、接入现有页面、读取 Legacy 运行模块、生成 Candidate/Signal/Plan、删除 Legacy 代码或宣称 live 全市场能力完成。
