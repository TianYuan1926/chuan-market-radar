...
状态：`ACTIVE_DESIGN_AUTHORITY / M0_ENGINEERING_EXIT_LOCAL_PASS / M1.2_FEATURE_CONTEXT_LOCAL_PASS / LIVE_CONNECTIVITY_UNPROVEN / PRODUCTION_UNCHANGED`
...
本文是 V2 当前唯一设计权威。M0 已在干净实施分支通过本地工程出口；M1.1-M1.2 已完成冻结 fixture 的 Identity、Fact、单一价格分散 Feature、独立 replay 证据和保守 Context 本地纵切。这些只证明合同与纯函数行为，不代表 live 全市场能力、生产变更、盈利能力或自动下单授权。
...
**当前进度**：`V2-M1.2 Point-in-Time Feature and Context Slice` 已达到 `LOCAL_PASS_REPLAY_PROVEN_FIXTURE_SLICE`。三家交易所 Identity/Fact 之上已形成一个精确跨 Venue 价格分散 Feature；ONLINE 与两次 REPLAY 具有独立 run、engine version 和语义哈希证据；Context 不能从低分散推断健康流动性，也不输出方向。当前网络环境仍没有取得 live provider 响应，且未接页面、数据库、Worker 或生产 authority。

**当前入口**：`V2-M1.3 Fact Store, Replay Manifest and Runtime Truth Rehearsal`。只建设 append-only Fact artifact、幂等/去重、可验证 replay manifest、最小权限身份合同和五类 Runtime Truth 的本地 store/replay rehearsal；不得执行生产 migration、接入 Candidate、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.2 已通过各自本地出口，生产零变更。当前唯一下一包是：
...
- 冻结 append-only Fact Store、artifact integrity、幂等键、去重和 retention 合同。
- 用本地临时 PostgreSQL 或等价隔离 fixture rehearsal 写入、读取、重复写、损坏检测和恢复；不触碰生产数据库。
- 生成可验证 replay manifest，使 replay 只读取 cutoff 当时可见的 Universe/Fact，并调用 M1.2 同一 Feature 纯函数。
- 分开生成 liveness、dependency readiness、business readiness、data freshness 和 release validity，不用 HTTP 200 冒充业务 ready。
- 定义最小权限 writer/reader/replay 身份合同；真实 migration 仍须独立包、rehearsal 和精确生产授权。

该包仍不允许修改生产、执行生产 migration、接入现有页面、读取 Legacy 运行模块、生成 Candidate/Signal/Plan、删除 Legacy 代码或宣称全市场能力完成。
