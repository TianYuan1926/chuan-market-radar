...
**当前执行入口**：`V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY`。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 recovery evidence 通过前不变。香港单 AZ 私有 COS 的 `COMPLIANCE` 31 天和 macOS Keychain age X25519 身份保持有效。B8 target、B9 COS action、route authority implementation `157e9a79...`、CodeQL 诊断 `8059edf...` 与 descriptor-first 修复 `913bae3a...` 只保留为已资格化上游；旧 response、route、lease、run、plan、object key 和 staging 均无当前执行权。当前结构性根因是 fixed dispatch 可以送达签名包，却不能自主取回严格脱敏结果；继续依赖 Edge/API response、AX/browser-state、人工复制或 OrcaTerm result transport 会重复同一故障。新网关使用生产 SSH Ed25519 host key namespace 签名、固定本机 X25519 recipient、HKDF-SHA256 + AES-256-GCM、既有 Caddy 高熵 exact ciphertext route、本机 SOCKS poller、pinned host identity、mode-600 no-clobber receipt 和两小时 exact systemd TTL cleanup。生产包只允许复用当前 Caddy image 并重建 Caddy，失败自动恢复 baseline Caddy；生产仓库、数据库、Redis、Worker、env、migration、Feature Flag 和非目标容器 identity 禁止改变。首次最新字节完整 CI 在 recurrence authority `10/11` 处真实失败，暴露活跃权威资料仍残留旧入口；同步修复已加入唯一入口防复发断言、Legacy consumer map/reviewed commit 复核和 M0 production-control 精确白名单。implementation `1ddc0fdc...` 与 authority-sync remediation `35d7b63a...` 已形成 clean commits；修复后 recurrence `11/11`、dispatch `37/37`、gateway `7/7`、P0R rebind `14/14`、V2 Foundation `638 PASS / 6 explicit skips`、V2 Ops `274/274`、M0 `12/12`、Next build、Golden `16/16` 和 security 全部 PASS。GitHub 四门、精确生产批准、真实 current-image Caddy validation、bootstrap、自动取回、fresh rebind 和 P0R recovery 仍待完成；只有 P0R 与 fresh P0 均 PASS 才能请求 P1，M2 runtime、页面和交易计划继续关闭。
