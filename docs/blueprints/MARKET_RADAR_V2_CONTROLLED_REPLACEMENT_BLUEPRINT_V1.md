...
v1.79 根除最终预提交审计发现的三个输出与输入边界缺口：status allowlist 不等于合法生命周期，单个 pending fragment 上限不等于 stdout/stderr 累计上限，path `lstat` 后另行 `readFile` 也不等于稳定安全读取。transaction v1 现强制唯一 child 序列 `PREARMED -> READY -> HEARTBEAT* -> CREDENTIAL_HANDOFF -> AGE_HANDOFF -> LOCAL_PASS`，BLOCKED/PASS 均为终态；累计输出最多 `65,536` bytes、状态最多 `64`。plan、lease、route 和 cleanup 控制文件必须 canonical、current-user、mode-600、普通文件，使用 `O_NOFOLLOW`、1 MiB 硬上限和读取前后 descriptor/path identity 一致性验证。原有 transaction `10/10` 已加入乱序、重复、终态后写、状态洪泛、symlink、空文件和超限文件红例，完整 P0R 保持 `124/124` 加 Go helper；外部清场和生产零漂移仍未证明，当前不得进入 secret 或 recovery 动作。
...
**当前执行入口**：`V2-M1.6-P0R-B9-R2-LEASED-DUAL-TTY-TRANSACTION-ROOT-REMEDIATION`。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 fresh recovery evidence 通过前不变。香港单 AZ 私有 COS 的 `COMPLIANCE` 31 天和 macOS Keychain age X25519 身份保持有效。B8 target 和 B9 COS action 修复只保留历史证据；2026-08-02 两个 bridge window 连续在 native Copy 到达前超时，后到 response 作废，随后 dynamic egress 漂移使旧 exact `/32` 路线不可达。当前只证明本机 Bridge=0、clipboard 已覆盖；云 listener、旧 firewall rule、服务器 residue 和生产零漂移尚未复核。本地 plan v4、credential v3、bridge/session v5 和 transaction v1 保持严格状态机、输出与稳定读取合同；fresh rebind v4/v3 进一步明确拒绝任何 TCP 8022 listener 或 exact transient unit residue。rebind `13/13`、P0R `125/125`、Go helper 与当前最终字节完整 CI PASS；外部清场、clean exact source、GitHub 四门、fresh read-only rebind、新 lease/run/package/staging 和 corrected recovery 尚未完成。clean source 和四门可在不触碰敏感 Edge 页时先行，但外部全清场前不得创建新 route、STS、数据库或 COS 动作。只有 P0R 与 fresh P0 均 PASS 才能请求 P1，M2 runtime、页面和交易计划仍关闭。
