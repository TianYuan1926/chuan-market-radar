...
V2 多资产需要 `M3.1A-M3.3A` 逐域扩展 Analysis/Qualification/Strategy，再由 `M3.4-M3.6` 完成执行可行性、Personal/Portfolio Risk 与 runtime Gate。股票必须新增传统市场 session、公司行动、FX、mark/index/reference、休市 basis、multiplier、funding/fee/slippage 和地区可用性；这些缺失时只能 OBSERVE/WAIT/BLOCKED。M3.4-R0 已完成 scope rebase review 并机器化阻断条件；当前未提交草稿继续作为 `QUARANTINED_NOT_COMMIT_ELIGIBLE`，不得进入主线。
