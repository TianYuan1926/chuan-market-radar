...
**当前执行入口**：`V2-M1.6-P0R-B6-PROXY-COMPATIBLE-8022-TTY-BRIDGE`。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 fresh 证据通过前不变。香港单 AZ私有 COS 已启用并回读 `COMPLIANCE` 31 天，真实 age X25519 身份仅保存在 macOS Keychain。source `44e51823f11ad81cbabc632256f0402ef2d00c29` 的 GitHub 四门、fresh rebind `p0r-rebind-preflight-20260729t172500z-a32c3e9b`、生产 11 容器零漂移和 run `p0r-20260729t174950z-83b68bfeebff953f49873bccf12cdacf` 的 exact package acceptance 都是真实历史 PASS；但签发前网络 A/B 证明当前 BoostNet SOCKS 在 SSH banner 前阻断 fixed bridge 默认 port 22，因此该 package 未接收 STS并由 bridge schema v2 取代。当前只启动一个 7200 秒、ubuntu public-key-only、root/password/keyboard-interactive/forwarding 全禁用的独立 sshd，并只添加 `157.254.154.223/32 -> TCP 8022` 腾讯规则；strict known_hosts、ed25519 与 `HostKeyAlias=43.161.202.227` 的本机握手已真实返回 `ubuntu / VM-0-9-ubuntu / active`。bridge v2 固定 port 8022 和 HostKeyAlias，完整 SSH 参数进入两条伪 TTY 测试并拒绝 `-p 22` fallback；当前 bridge 6/6、P0R 88/88、recurrence 11/11、dispatch 24/24 与完整本地生产 CI 均 PASS，新 clean exact commit、四门、fresh rebind、新 run/plan/bundle 和真实 recovery 仍待完成。新 source 全部资格成立后，必须先通过 8022 bootstrap gate并启动 bridge取得 echo-disabled READY，才可签发 STS；response 后仍禁止 browser state、screenshot、AX/OCR、computer-use 和 OrcaTerm secret entry。helper 才能自动执行真实加密离机备份、exact version retrieval、独立 PostgreSQL 16 restore parity、全量清理和零漂移验证。无论成功、失败或超时，都必须显式删除 8022 listener 与云规则并证明不存在。只有 P0R 与 fresh P0 均 PASS 才能请求 P1 v1+v2 schema；B0.2-B 外部权利与历史来源 Gate 继续并行 blocked，M2 runtime、页面和交易计划仍关闭。
...
P0R 不允许以 Object Lock、Keychain 身份、transport bundle、canary、临时清理、阈值下调或 backup exit 0 冒充生产可恢复；临时凭证、对象上传、exact version retrieval、隔离 restore 和 cleanup 均是独立、可审计动作。第三枚 STS 已由本机 UTC 与腾讯 HTTPS Date 独立证明超过 exact expiry，现为 `EXPIRED_FORBIDDEN_REUSE`。当前顺序是 `bridge v2 完整本地门禁 -> 新 clean exact commit -> GitHub 四门 -> 仅通过 request v3 高层入口执行 fresh read-only rebind -> 全新 run-id/object key/plan/14-member bundle staging -> 8022 bootstrap gate -> fixed local TTY bridge echo-disabled READY -> 用户 MFA + API Explorer 原生 Copy -> bridge 即时编译 STS并从固定 Keychain内部注入 age identity -> helper 自动 backup / exact retrieval / isolated PG16 restore -> cleanup / zero drift -> 删除 8022 listener 与云规则并证明不存在 -> fresh topology / calibration / P0`。response 出现后禁止 screenshot、AX/OCR、browser state read、computer-use 和 OrcaTerm secret entry；native Copy 无法完成时停止。raw STS response 不得落盘；超时、断线、无 EOF、编译超窗或失败必须自动清理。不得复用任何旧 token、`44e518...` package、`e362...` run、`c85ec1a4` dispatch、`bd20...` bundle、旧计划、port 22 fallback、裸 `tee`、断线 receiver、超长复合命令或空 `/dev/shm` 占位。付费扩容已退出活跃路线，六小时零付费本地机器模型与组合准入实现已通过，但这不取消原 P0 blocker：fresh 证据必须同时满足所有旧非容量门禁、稳态 60% / 峰值 70%、完整恢复和健康门禁，否则 P1 继续 BLOCKED。P0R 生产动作完成后只允许重新执行 fresh P0，不直接授权 migration、身份创建、分区预建、Worker 启动或生产写入。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 historical B1 acquisition 的硬前置，C1 不能替代或解锁它。
