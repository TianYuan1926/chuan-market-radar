...
**当前本地工程入口**：M2.0 发现合同已本地冻结；下一包为 `V2-M2.1-PRE-MOVE-BREAKOUT-REPLAY-KERNELS`，只允许读取 test-only point-in-time fixture 的纯函数/纯回放内核，不得启动读取 M1 authority 的 Detector runtime。外部通道恢复后并行执行 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE`，只运行固定 31 个一分钟周期；B1 与 M1.6 都通过后进入 M1.7 24 小时持续 Shadow。M1.7 前不得开放 M2 runtime、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.5-A、M1.5-B0、M1.6 和 M2.0 已通过各自本地出口，生产零变更；live、生产存储和持续 SLO 尚未通过。等待外部 B1 时，当前唯一允许并行的本地下一包是：
...
- 在冻结的 `M2DetectorReadInput` 和 19 个 test-only fixture 上实现 Pre-Move 与 Breakout/Retest 多空/UNKNOWN 纯函数检测内核。
- 输出可复现的模式、reason code、counter hint 和 Candidate 草稿字段，但不得绕过 M2.0 strict decoder。
- 明确区分 early、late、noise、fakeout 和 unavailable；空结果必须保留完整评价分母。
- 做多与做空分别实现和验收，禁止简单符号反转；同一冻结输入必须确定性重放。
- 可与外部 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE` 并行，但任何 Detector runtime、M1 authority 读取和 24 小时 Shadow 必须等待 M1.7。

该包的本地阶段仍不允许修改生产。全程禁止接入现有页面、读取 Legacy 运行模块或 M1 runtime、写 Candidate Store、生成等级/Signal/Plan、使用 Outcome、删除 Legacy 代码或宣称 live 全市场能力完成。
