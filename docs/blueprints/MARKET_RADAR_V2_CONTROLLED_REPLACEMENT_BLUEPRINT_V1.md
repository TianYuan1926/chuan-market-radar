...
`V2-M1.1B Exact Source Conformance + Multi-Asset Identity + Listing Intelligence` 已达到 `LOCAL_IMPLEMENTATION_PASS / TEST_ONLY_CONFORMANCE_PASS / LIVE_B0_NOT_RUN`：15 个探针、四 Venue catalog normalizer、Bitget、官方资产映射、listing/identity epoch、Bybit/Bitget announcement normalizer 和生命周期账本已进入独立 Scope V2 层，定向 22/22 PASS。它不修改旧 V1 eligibility，也没有 live、Shadow、容量、校准、Candidate 或 Strategy authority。下一 Scope V2 入口是绑定 exact clean commit 的腾讯隔离 `LIVE_READ_ONLY` B0；只有实际 PASS 的能力才可进入 M1.4B runtime Adapter。

`V2-M1.1B0 Tencent Live Source Conformance No-Secret Dispatch Package` 已达到 `LOCAL_IMPLEMENTATION_PASS / DIRECTED_20_OF_20_PASS / FIXED_CHANNEL_REHEARSAL_PASS / INDEPENDENT_FULL_CI_PASS / LIVE_NOT_RUN`：正式包绑定 exact commit/tree/ref、15 项固定分母、announcement filters、分页、并发策略、最小 Zod 运行树和生产前后身份；CoinGlass key 不进入运输层或 evidence。本地网络的 14 项 reset/timeout 仅登记为非权威诊断，不冒充 provider 失败。精确提交推送和腾讯现场执行仍是后续独立 Gate。
...
**V2 扩展子包**：`M1.1A Four-Venue Capability Registry [LOCAL_PASS] -> M1.1B local implementation [LOCAL_PASS_TEST_ONLY] -> M1.4A capability-independent scheduler contract [LOCAL_CONTRACT_AND_FULL_CI_PASS] + M1.1B0 Tencent isolated live source conformance [PENDING] -> M1.4B endpoint batching/runtime Adapter profiles -> M1.5C Four-Venue Multi-Asset Shadow -> M1.6-D1 Expanded-Scope No-Cost Capacity Proof`。M1.4A 已冻结失败关闭的调度合同，但只有 live B0 实际 PASS 的 capability 才能进入 M1.4B runtime Adapter；原 B1-B3 和 D0 只属于 V1，V2 未完成新 Shadow 和容量证明前不得进入扩展生产范围。
