...
**当前进度**：`V2-M1.4 Full Eligible Universe and Collector Runtime` 已达到 `LOCAL_POSTGRES16_REHEARSAL_PASS`。确定性三 Venue fixture 含 21 observed / 15 eligible；Collector 区分 providerObserved、accounted、eligible、collected、fresh，执行启动全量、增量 ticker、周期 reconciliation、配额、并发、背压、目录 tombstone、数据库失败和恢复。真实临时 PostgreSQL 16 已证明启动、增量和全 catalog 故障三个 durable cycle；M1.3 Store/Replay 回归仍通过。当前没有 live provider 证据、连续生产 Worker、Shadow/SLO、生产 migration 或 authority。

**当前入口**：`V2-M1.5 Live No-Authority Collector Rehearsal and Shadow/SLO Entry`。只把 M1.4 运行时组合为可发布但无读取权威的采集 Worker，先验证 live provider 合同、真实规模容量、连续 freshness/coverage、限流、重连、资源和成本，再经独立 Gate 进入 Shadow；不得接入 Candidate、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.4 已通过各自本地出口，生产零变更。当前唯一下一包是：
...
- 建立单一 V2 Collector Worker composition，只调用冻结 Adapter、Collector Runtime 和 M1 Store。
- 以 no-read-authority 模式验证 live catalog/ticker 合同、真实 instrument 数量、请求预算、端到端延迟、资源和成本。
- 先在隔离 rehearsal 证明启动、重启、断流、429、分页漂移、数据库失败和 recovery，再提交独立 Shadow 发布 Gate。
