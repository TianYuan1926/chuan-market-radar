...
**当前执行入口**：`V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY` 的 public-key-fingerprint artifact 根治与 replacement exact-source 重新资格化。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 recovery evidence 通过前不变。香港单 AZ 私有 COS 的 `COMPLIANCE` 31 天和 macOS Keychain age X25519 身份保持有效。上游 route authority、安全根治、gateway source `dd67d819...` 的四门、失败生产尝试与 health-contract clean commit `51886b0...` 均只保留为历史有效证据；旧 exact 批准已过期，signed dispatch `a52aa6a...` 已被 claim 且 `FAIL_DISPATCH_NOT_REUSABLE`。authority-sync source parts `d1c3d987cf55105e94e8 + 15992dbb989ea1045629` 已推送，远端只有 Signed Dispatch、Full Quality、A0 三门 PASS，Independent Security 因历史 `approval-request.json` 把固定 X25519 公钥指纹命名为 `evidenceRecipientKeySha256` 而真实 FAIL。永久修复使用 `evidenceRecipientFingerprintSha256`、严格拒绝旧字段，并在 signed dispatch 发布前阻断歧义高熵 `*KeySha256`；exact 历史 finding 单独登记。clean implementation commit `8742828086041da1db51a5abeeef01ba7656e5fd` 已形成，recurrence `11/11`、dispatch `38/38`、gateway `8/8` 合计 `57/57`，最终 authority-sync 字节完整 exact-toolchain CI 已 PASS。authority-sync commit、push 和 replacement source 四门尚未完成，生产未改变。固定后续顺序为 `提交并推送 authority-sync exact source -> 同源四门从零 PASS -> fresh production baseline -> 新 exact package -> 当前批准 -> current-image 隔离验证 -> Caddy-only deploy/自动回滚 -> 自动取回验签 -> fresh rebind -> P0R recovery`；只有 P0R 与 fresh P0 均 PASS 才能请求 P1，M2 runtime、页面和交易计划继续关闭。
