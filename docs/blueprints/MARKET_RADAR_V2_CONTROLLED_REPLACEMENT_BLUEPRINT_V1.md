...
| 全面捕捉其他机会 | 六类独立机会族 | family/direction/regime 分层指标 | 一个万能总分或涨跌榜 |
...
**当前本地工程入口**：`V2-M2.0-DISCOVERY-CONTRACTS-AND-GOLDEN-FIXTURES`。只允许冻结六类机会 taxonomy、Detector 输入边界、Candidate/Episode/Thesis 生命周期、三分母和 point-in-time 黄金样本，不得启动读取 M1 authority 的 Detector runtime。外部通道恢复后并行执行 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE`，只运行固定 31 个一分钟周期；B1 与 M1.6 都通过后进入 M1.7 24 小时持续 Shadow。M1.7 前不得开放 M2 runtime、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.5-A、M1.5-B0 和 M1.6 已通过各自本地出口，生产零变更；live、生产存储和持续 SLO 尚未通过。等待外部 B1 时，当前唯一允许并行的本地下一包是：
...
- 冻结 Pre-Move、Breakout/Retest、Trend Continuation、Key-Level Reversal、Range Edge、Relative Strength/Derivatives Flow 六类机会的互斥/重叠关系和明确未知态。
- 冻结 Detector 只读输入、`DiscoveryCandidate -> CandidateEpisode -> OpportunityThesis` 生命周期、去重/时效/资源优先级和 discovered/deep-validated/actionable 三分母。
- 建立纯 point-in-time 黄金样本、反例、晚到/噪音/假突破标签和 no-future-input 证明。
- Candidate 不得携带 EvidenceGrade、SetupGrade、ActionState 或交易计划；Thesis 不是方向真值。
- 可与外部 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE` 并行，但任何 Detector runtime、M1 authority 读取和 24 小时 Shadow 必须等待 M1.7。

该包的本地阶段仍不允许修改生产。全程禁止接入现有页面、读取 Legacy 运行模块或 M1 runtime、生成 Signal/Plan、使用 Outcome、删除 Legacy 代码或宣称 live 全市场能力完成。
