...
**当前本地工程入口**：M2.1 五个 DRAFT 纯回放内核已本地完成且 Candidate emission=false；下一包为 `V2-M2.2-HISTORICAL-REPLAY-AND-DETECTOR-LIFECYCLE-GATE`，只允许建立冻结历史 cohort、三分母、分层指标和 untouched holdout，不得用合成 fixture 自批 `REPLAY_VALIDATED`。外部通道恢复后并行执行 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE`，只运行固定 31 个一分钟周期；B1 与 M1.6 都通过后进入 M1.7 24 小时持续 Shadow。M1.7 前不得开放 M2 runtime、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.5-A、M1.5-B0、M1.6、M2.0 和 M2.1 已通过各自本地出口，生产零变更；live、生产存储和持续 SLO 尚未通过。等待外部 B1 时，当前唯一允许并行的本地下一包是：
...
- 同时保存 candidate、event、matched non-event 三分母，报告 family/direction/regime 分层 recall、precision、lead time、late/noise 和 unavailable。
- 做 threshold sensitivity、失败案例、数据覆盖与 untouched holdout；所有试验登记，禁止只保留最佳阈值。
- 输出 PASS/FAIL/INSUFFICIENT 与升级提案；Gate 不得自动改变 Detector lifecycle，人工/独立审计前仍为 DRAFT。
