...
**当前执行入口**：`V2-M1.6-P0R-B3-NOECHO-MEMORY-INGRESS-ATOMIC-RECOVERY-SESSION`。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 fresh 证据通过前不变。香港单 AZ 私有 COS 已启用并回读 `COMPLIANCE` 31 天，真实 age X25519 身份仅保存在 macOS Keychain；历史 `bed938...` staging 继续隔离。`bd20...` 的四条 GitHub 门禁、fresh read-only rebind、生产 11 容器零漂移和 v3 plan/bundle staging 都是真实历史 PASS，但它的裸 `tee`、raw 落盘、manual compile 与 Compose 重插值路径已被第二次 STS 的真实失败证伪，因此不再拥有执行权。第二枚 STS 的 exact expiry `2026-07-28T20:57:29Z` 已由本机与腾讯可信时钟双重证明超过，现永久禁止复用；生产 P0R 文件和进程已复核为 0。no-echo atomic session 已取得 81/81 定向 PASS、完整 `ci:production` PASS 且拒绝 caller clock override；fresh rebind v2 同时冻结历史三文件比较集与当前七文件运行集。当前先形成 clean commit，完成 GitHub 四门、fresh read-only rebind 和新 plan/bundle，再通过两个不超过 200 UTF-8 字节的精确 session 命令完成新 STS 内存即时编译与 age handoff，随后由 helper 自动执行真实加密离机备份、exact version retrieval、独立 PostgreSQL 16 restore parity、全量清理和零漂移验证。只有 P0R 与 fresh P0 均 PASS 才能请求 P1 v1+v2 schema；B0.2-B 外部权利与历史来源 Gate 继续并行 blocked，M2 runtime、页面和交易计划仍关闭。
...
```text
V2-M1.6-P0R-B3-NOECHO-MEMORY-INGRESS-ATOMIC-RECOVERY-SESSION
```

P0R 不允许以 Object Lock、Keychain 身份、transport bundle、canary、临时清理、阈值下调或 backup exit 0 冒充生产可恢复；临时凭证、对象上传、exact version retrieval、隔离 restore 和 cleanup 均是独立、可审计动作。STS Region 与第四门触发合同根因已经关闭，但 `bd20...` 只保留历史资格，不再拥有执行权。第二枚 STS 的精确过期前置已经完成证明；完整本地 CI 已 PASS。当前顺序是 `最终权威文档同步与复核 -> replacement clean exact commit -> GitHub 四门 -> fresh read-only rebind -> 新 run-id/plan/14-member bundle staging -> 新 exact 7200 秒 STS 通过 no-echo memory ingress 即时编译 -> 第二 fresh session 注入 age identity -> helper 自动 backup / exact retrieval / isolated PG16 restore -> cleanup / zero drift -> fresh topology / calibration / P0`。raw STS response 不得落盘；超时、断线、无 EOF、编译超窗或失败必须自动清理。两个 OrcaTerm session 入口仍须 `<=200` UTF-8 字节并执行 clear/settle/exact-preview，凭证可见期间禁止截图、AX/OCR、日志和回显。不得复用任何旧 token、`bd20...` bundle、旧 v2 计划、裸 `tee`、断线 receiver、超长复合命令或空 `/dev/shm` 占位。付费扩容已退出活跃路线，六小时零付费本地机器模型与组合准入实现已通过，但这不取消原 P0 blocker：fresh 证据必须同时满足所有旧非容量门禁、稳态 60% / 峰值 70%、完整恢复和健康门禁，否则 P1 继续 BLOCKED。P0R 生产动作完成后只允许重新执行 fresh P0，不直接授权 migration、身份创建、分区预建、Worker 启动或生产写入。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 historical B1 acquisition 的硬前置，C1 不能替代或解锁它。
