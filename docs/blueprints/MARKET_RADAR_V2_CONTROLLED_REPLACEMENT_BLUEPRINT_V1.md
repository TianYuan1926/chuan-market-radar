...
- `M0.5`：`DESIGN_AND_LOCAL_CONTRACT_PASS / M1.5D_FORWARD_EXECUTION_NOT_STARTED / REAL_COHORT_NOT_AVAILABLE / NO_AUTHORITY / PRODUCTION_UNCHANGED`。Market Mechanics 三轴、LiquidityWallEpisode、压力-价格响应、双向前兆图谱、板块传播、补充来源、缓存和 Evidence Overlay 已落入现有 18 Module；M1.4C 与 M2.1A 本地合同已通过，M1.5D 真实前向证据尚未开始。详细权威为 `docs/architecture/v2/M0_5_MARKET_MECHANICS_MICROSTRUCTURE_AND_PRECURSOR_ATLAS_AMENDMENT_V1.md`。
...
**当前执行入口**：`V2-M1.6-P0R-R0-READ-ONLY-SOURCE-REBIND`。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 fresh 证据通过前不变。香港单 AZ 私有 COS 已启用并回读 `COMPLIANCE` 31 天，真实 age X25519 身份仅保存在 macOS Keychain；历史 `bed938...` staging 已因安全源码被替代而隔离，禁止执行。current-source 重绑定已通过本地与 exact-source GitHub 资格门禁，signed read-only dispatch 已发布并过期，但目标 receipt 未读，现场领取、启动和结果均为 `UNKNOWN`。下一步必须先只读取得 receipt；若已有结果则验收身份与零漂移，若未领取才生成新时效派发。现场重绑定 PASS 后才可由 current source 重建 plan/bundle，再单独注入 fresh 7200 秒 STS 和 age identity，执行真实加密离机备份、精确 version retrieval、独立 PostgreSQL 16 restore parity 与清理证据；随后刷新 health/topology，在 exact clean release 重跑容量校准和 fresh P0 组合准入。只有新的 P0 PASS 才能请求 P1 v1+v2 schema，之后仍须依次执行 P2 最小权限身份、P3 六小时分区与 dormant Worker、P4 有界 isolated-write Shadow 和 M1.7 同 release 24h Shadow。B0.2-B 外部权利与历史来源 Gate 继续并行 blocked；M2 runtime、页面和交易计划仍关闭。

**V2 扩展子包**：`M1.1A Four-Venue Capability Registry [LOCAL_PASS] -> M1.1B local implementation [LOCAL_PASS_TEST_ONLY] -> M1.4A capability-independent scheduler contract [LOCAL_CONTRACT_AND_FULL_CI_PASS] + M1.1B0 Tencent isolated live source conformance [R1_0_OF_15_RUNTIME_TRANSPORT_BLOCKED / R2_14_OF_15_LISTING_BLOCKED / R3_15_OF_15_ALL_GATES_PASS] -> M1.4B endpoint/profile/history core + Tencent bootstrap/resume [PASS_NO_AUTHORITY] -> M1.5C Four-Venue Multi-Asset Shadow -> M1.6-D1 Expanded-Scope No-Cost Capacity Proof`。Bitget Venue、Listing Lifecycle、Equity Asset Domain 与 Data Maximization 分别核算。R3 的 15/15 endpoint conformance 不能掩盖当前 14/15 route eligibility；Binance spot 必须完成 registry 修订和新 digest live conformance。M1.4B 已用真实 checkpoint/gap/resume 证据验收 Bybit provider-available history 与 Bitget 官方一个月窗口；原 B1-B3 和 D0 只属于 V1，V2 未完成 Shadow 和容量证明前不得进入扩展生产范围。
