# Market Radar V2 受控替换工程与运行蓝图 v1.96
...
**当前执行入口**：`V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY` 的 signer-identity 根治与 replacement exact-source 重新资格化。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 recovery evidence 通过前不变。前三个 gateway dispatch 均已 claim、`FAIL_DISPATCH_NOT_REUSABLE` 且批准已消费并过期；第三次 source `ad9aeb9842f6b57d934e7729e5e8a5cf2680a3b7` 的同源四门全部 PASS，并真实完成 target Caddy recreate、健康、mount 和 route probe，随后因 request v3 未绑定 signing key path 在 evidence seal 失败。自动 baseline rollback 已恢复原 Caddy config/image；十个非 Caddy full IDs、production HEAD `cec0b657...`、clean worktree、ready/fresh health 与全部非目标面不变，exact staging/gateway/outbound/diagnostic state 均 absent。本轮是“受权临时 Caddy mutation 后回滚”，不是 pre-mutation failure，也不是 retained deployment。生产隔离诊断证明 current Caddy route 与 host key sign/verify 均正常；直接根因是默认 signer 未收到 `keyPath`，且本地 happy test 手工注入 caller options 掩盖生产差异。`REC-2026-08-02-PRODUCTION-GATEWAY-PREFLIGHT-EQUIVALENCE` occurrence 3 继续 STOP-THE-LINE，旧 runtime-identity-only operation 已退役。当前 request v4 将 key path/fingerprint 写入 canonical request 和 bundle，并在任何 Caddy mutation 前执行真实 sign、独立 verify 与 observed fingerprint 比对；gateway `14/14`、recurrence `11/11`、dispatch `38/38` 和最终字节完整 exact-toolchain CI 均 PASS，CI 精确分母为 market `965/969`（4 explicit skips）、workers `23/23`、historical `4/4`、Foundation `638/644`（6 explicit skips）、Ops `281/281`、M0 `12/12`、Golden `16/16`，生产构建与安全检查也通过。固定后续顺序为 `clean commit/push -> 同源四门从零 PASS -> fresh production baseline -> 双构建 deterministic package -> 新 exact approval -> signer-identity-bound execution/自动回滚 -> 自动取回解密验签 -> fresh rebind -> P0R recovery`；只有 gateway real-target acceptance、P0R 与 fresh P0 均 PASS 才能请求 P1，M2 runtime、页面和交易计划继续关闭。
