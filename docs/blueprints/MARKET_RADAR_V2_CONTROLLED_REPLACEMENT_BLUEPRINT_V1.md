# Market Radar V2 受控替换工程与运行蓝图 v1.9

状态：`ACTIVE_DESIGN_AUTHORITY / M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M1.5-B1-B2_MARK_PRICE_SEMANTICS_LOCAL_ENGINEERING_EXIT_PASS / B1-B1_EXECUTION_INVALID_NOT_COUNTED / B1-B3_RETEST_PENDING / BUSINESS_SLO_UNPROVEN / M2.2-B0.2-C1_FORWARD_ONLY_READY / B0.2_EXTERNAL_RESOLUTION_BLOCKED / M1_NOT_COMPLETE / PRODUCTION_SERVICES_DATA_AND_AUTHORITY_UNCHANGED`
...
**当前进度**：`V2-M1.5-B1-B2 Mark Price Snapshot Semantics Remediation` 已达到 `LOCAL_ENGINEERING_EXIT_PASS / BUSINESS_SLO_UNPROVEN`。旧 `LAST_PRICE` 已替换为三 Venue 统一 `MARK_PRICE / MARK_PRICE_SNAPSHOT`；Collector 明确拆分 providerObserved/accounted/eligible/collected/usablePrice/fresh 六计数，任何聚合都必须等于 Venue 求和。Runner 和 validator 共用一个冻结 environment 合同，旧 schema 证据禁止进入新 Gate；完整 `ci:production` 已通过。
...
**当前执行入口**：`V2-M1.5-B1-B3-MARK-PRICE-SAME-GATE-31-CYCLE-RETEST`。绑定 B1-B2 exact clean commit，在腾讯隔离 Runner 从第 1 周期执行一个不可拼接的 31 周期窗口；必须同时保存原始 artifact、domain/runner evidence、六计数、独立 SLO 重算和宿主恢复。不得放宽 freshness、把 duplicate/carried-forward 标为 FRESH、删除失败分母或用旧 schema 证据。B1-B3 PASS 后才允许 M1.6 production storage 分阶段启用，随后 M1.7 24h Shadow。B0.2-B 外部权利与历史来源 Gate 继续并行 blocked；M2 runtime、页面和交易计划仍关闭。
...
M0.0-M0.3、M1.1-M1.5-B0、M1.5-B1-A/B0/B2、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1、B0.2-A、B0.2-C 和 C1 已通过对应本地、技术或运行起点出口；B1-B1 因证据包装失败不计完成，业务 SLO 仍未证明。M2.2 真实能力 Gate、外部来源解决、bulk historical acquisition、B1-B3、生产存储和持续 SLO 尚未通过。当前执行入口是：

```text
V2-M1.5-B1-B3-MARK-PRICE-SAME-GATE-31-CYCLE-RETEST
```

B1-B3 使用新 `MARK_PRICE` schema 和原有 31 周期/60 秒业务门槛，并增加 price-usability=100% 独立门槛。技术 Runner 或 31 周期捕获 PASS 永远不能替代业务 Gate。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 historical B1 acquisition 的硬前置，C1 不能替代或解锁它。
