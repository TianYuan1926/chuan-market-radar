...
状态：`ACTIVE_DESIGN_AUTHORITY / M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.4_LOCAL_PASS / LIVE_MARKET_AND_SHADOW_UNPROVEN / PRODUCTION_UNCHANGED`
...
**当前本地工程入口**：M2.2-B0 历史来源资格与采集安全已本地完成。Binance Vision 单个 BTCUSDT 1m 月文件真实验证为 1,838,455 bytes，官方 SHA-256 与实际 SHA-256 一致，验证后原始 ZIP 强制删除；该证据只证明技术链。来源权利仍需人工审查，point-in-time onboard/delist/contract/status/underlying class 仍缺，L2 Liquidity Shift 不受 Kline 支持，bulk acquisition 与 cohort freeze 均 fail closed。M2.1 还缺 target-blind 候选强度，无法诚实形成 Top20；当前本地下一包因此是 `V2-M2.2-B0.1-TARGET-BLIND-DIAGNOSTIC-STRENGTH-AND-CONSTRUCTION-POLICY-FREEZE`，同时外部解决 B0.2 来源权利和历史合约身份。M2.2 Gate 继续 `INSUFFICIENT`，五个 Detector 仍为 DRAFT 且 Candidate emission=false。外部通道恢复后并行执行 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE`，只运行固定 31 个一分钟周期；B1 与 M1.6 都通过后进入 M1.7 24 小时持续 Shadow。M1.7 前不得开放 M2 runtime、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.5-A、M1.5-B0、M1.6、M2.0、M2.1、M2.2-A 和 M2.2-B0 已通过各自本地出口，生产零变更；M2.2 真实能力 Gate、bulk historical acquisition、live、生产存储和持续 SLO 尚未通过。等待外部 B0.2 来源权利/历史身份与 M1.5-B1 时，当前唯一允许继续的本地下一包是：

```text
V2-M2.2-B0.1-TARGET-BLIND-DIAGNOSTIC-STRENGTH-AND-CONSTRUCTION-POLICY-FREEZE
```

M2.2-A 已完成合同与 harness，M2.2-B0 已完成来源与采集安全 Gate。B0.1 只允许：

- 为每个 DRAFT Detector 建立只读、target-blind、可复现的 diagnostic strength；只能使用 cutoff 时已知 observation 和质量，不得读取 target、eventStart、Outcome、MFE/MAE 或未来标签。
- 冻结 ranking policy、train-only 事件阈值拟合、matched control、完整背景枚举、pre-cutoff regime/liquidity、modeled knowledge-time 和全部 trial/sensitivity 参数身份。
- 将 policy id/digest 绑定到 replay dataset manifest；单个 record 的 arbitrary threshold/matching/sampling id 必须被拒绝。
- 数据来源未批准、point-in-time instrument history 不完整、ranking 不可解释或任何 future leak 时，必须停止在 `BLOCKED/INSUFFICIENT`。
- 本包不批量下载、不构造真实 cohort、不打开 holdout、不运行最终 Gate。
- 可与外部 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE` 并行，但任何 Detector runtime、M1 authority 读取和 24 小时 Shadow 必须等待 M1.7。
