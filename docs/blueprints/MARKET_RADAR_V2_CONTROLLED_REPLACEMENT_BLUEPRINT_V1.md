...
**当前执行入口**：`V2-M1.6-P0R-B9-R2-ROUTE-AUTHORITY-PRODUCER-ROOT-REMEDIATION`。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 fresh recovery evidence 通过前不变。香港单 AZ 私有 COS 的 `COMPLIANCE` 31 天和 macOS Keychain age X25519 身份保持有效。B8 target、B9 COS action 和 source `6c7f382...` 的本地/远端资格只保留为历史或已被新源码替代的前置证据；2026-08-02 两个 bridge window 的后到 response 已作废，dynamic egress 漂移后旧 exact `/32` 不可达。当前只证明本机 Bridge=0、clipboard 已覆盖；云 listener、旧 firewall rule、服务器 residue 和生产零漂移尚未复核。隔离分支在 plan v4、credential v3、bridge/session v5、transaction v1 和 rebind v4/v3 之上新增 route target v1、Tencent firewall capture v1、strict listener observer、authoritative producer 与 route evidence v2，transport 升为 v4/17 members，current runtime source 分母升为 9。定向 `23/23`、P0R `138/138`、Go helper、ESLint 与 Biome PASS；隔离候选完整 CI 已在唯一 V2 实施分支身份门 fail closed，必须精确合入主线后重跑最终字节完整 CI。clean exact source、GitHub 四门、fresh read-only rebind、新 lease/run/package/staging 和 corrected recovery 尚未完成。任何旧 route evidence v1、transport v3、16-member package、旧 lease/run 或手填 final hash 均无执行权。外部全清场前不得创建新 route、STS、数据库或 COS 动作。只有 P0R 与 fresh P0 均 PASS 才能请求 P1，M2 runtime、页面和交易计划仍关闭。
