# Market Radar V2 受控替换工程与运行蓝图 v1.4

状态：`ACTIVE_DESIGN_AUTHORITY / M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M2.2-B0.2-A_LOCAL_EVIDENCE_GATE_PASS / B0.2_EXTERNAL_RESOLUTION_BLOCKED / LIVE_MARKET_AND_SHADOW_UNPROVEN / PRODUCTION_UNCHANGED`
...
**当前本地工程入口**：M2.2-B0 来源资格与一文件技术链、B0.1 target-blind diagnostic strength/构造策略、B0.2-A 权利与历史身份机器证据门禁均已通过本地合同出口。B0.2-A 已证明当前 snapshot、archive presence、Agent/合成审批、状态缺口、晚到知识和 symbol reuse 不能通过；它没有替代外部审查或提供真实历史身份。五个已登记来源候选全部为 `RESEARCH_ONLY`，因此 B0.2-B 仍受外部权利结论和 exact historical source 阻断，bulk acquisition 与 cohort freeze 均为 false。可并行本地建设三 Venue 前向 instrument capture，但不得回填过去。M2.2 Gate 继续 `INSUFFICIENT`，五个 Detector 仍为 DRAFT 且 Candidate emission=false。外部通道恢复后也可并行执行 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE`；M1.7 前不得开放 M2 runtime、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.5-A、M1.5-B0、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1 和 B0.2-A 已通过各自本地出口，生产零变更；M2.2 真实能力 Gate、外部来源解决、bulk historical acquisition、live、生产存储和持续 SLO 尚未通过。当前串行外部入口是：

```text
V2-M2.2-B0.2-B-EXACT-SOURCE-RIGHTS-AND-CAPABILITY-RESOLUTION
```

B0.2-A 已完成机器拒绝门禁；B0.2-B 仍必须由账户所有者或合格审查者给出 exact source 的 retention/replay 权利结论，并取得可审计 point-in-time onboard/delist/contract type/settlement/underlying class/trading status。Agent 不能把公开下载、仓库许可证、当前 instrument snapshot、归档 presence、厂商宣传或推断当成批准证据。外部门等待期间，唯一不冲突的本地入口是 `V2-M2.2-B0.2-C-FIRST-PARTY-FORWARD-INSTRUMENT-CAPTURE`，其数据从真实 capture start 起有效，不能解锁过去窗口。
