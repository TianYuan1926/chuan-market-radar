...
**当前执行入口**：`V2-M1.6-P0R-B9-R2-ROUTE-AUTHORITY-PRODUCER-ROOT-REMEDIATION`。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 fresh recovery evidence 通过前不变。香港单 AZ 私有 COS 的 `COMPLIANCE` 31 天和 macOS Keychain age X25519 身份保持有效。B8 target、B9 COS action 和 source `6c7f382...` 的资格只保留为历史前置证据；2026-08-02 两个 bridge window 的后到 response 已作废，dynamic egress 漂移后旧 exact `/32` 不可达。当前只证明本机 Bridge=0、clipboard 已覆盖；云 listener、旧 firewall rule、服务器 residue 和生产零漂移尚未复核。route authority producer、transport v4/17 members 和 current runtime 9-file 边界已由 `157e9a79...` 实现；首轮远端安全 run `30725896115` 真实报出 public identity 的 `js/file-system-race`，没有被覆盖。最终 `913bae3a2db4d8f172d0de9be87305ad21b53884` 以 descriptor-first `O_NOFOLLOW` 读取根除该竞争窗口，定向 `24/24`、P0R `139/139`、V2 Ops `266/266`、完整本地 CI 和 GitHub 四门全部 PASS。当前剩余从 fresh signed read-only rebind 开始，包括外部零残留、新 lease/run/package/staging 和 corrected recovery；任何旧 route evidence v1、transport v3、16-member package、旧 lease/run 或手填 final hash 均无执行权。外部全清场前不得创建新 route、STS、数据库或 COS 动作。只有 P0R 与 fresh P0 均 PASS 才能请求 P1，M2 runtime、页面和交易计划仍关闭。
