# Market Radar V2 受控替换工程与运行蓝图 v1.86

状态：`ACTIVE_DESIGN_AUTHORITY / M0.4_EXPANDED_SCOPE_DESIGN_PASS / M0.5_MARKET_MECHANICS_MICROSTRUCTURE_AND_PRECURSOR_ATLAS_LOCAL_CONTRACT_PASS / A0_ENGINEERING_SUBGATES_PASS_TOTAL_GATE_INCOMPLETE_P0R_PENDING / M1.1B0_R3_LIVE_15_OF_15_ALL_GATES_PASS / M1.4A_M1.4D_LOCAL_RUNTIME_CONTRACTS_PASS_NO_AUTHORITY / M1.5C_M1.5D_LOCAL_RUNTIME_AND_EXACT_PACKAGE_PASS_LIVE_ZERO_OF_31 / M2.1A_RESEARCH_CONTRACT_PASS_REAL_COHORT_MISSING / M3.1A_M3.3D_RESEARCH_SCAFFOLD_PASS_NO_REAL_CALIBRATION / M3.3E_STRATEGY_ARCHETYPE_LABELING_DESIGN_AUTHORITY_ADDED_IMPLEMENTATION_NOT_STARTED / M1.6_P0_EXECUTED_BLOCKED_CAPACITY_AND_RECOVERY / P0R_B8_EXACT_TARGET_ACCEPTANCE_HISTORICAL_PASS / P0R_B9_R1_COS_CAM_ACTION_ROOT_REMEDIATION_HISTORICAL_PASS / P0R_B9_R2_ROUTE_AUTHORITY_PRODUCER_ROOT_REMEDIATION / LOCAL_BRIDGE_ZERO_AND_CLIPBOARD_CLEARED / CLOUD_LISTENER_FIREWALL_SERVER_RESIDUE_AND_PRODUCTION_ZERO_DRIFT_UNVERIFIED / ROUTE_AUTHORITY_PRODUCER_TRANSPORT_V4_17_MEMBER_P0R_139_OF_139_EXACT_MAIN_BRANCH_FULL_CI_AND_CURRENT_SOURCE_GITHUB_FOUR_GATES_PASS / FRESH_REBIND_EXTERNAL_CLEANUP_AND_REAL_RECOVERY_PENDING / PRODUCTION_UNCHANGED_BUT_FULL_ZERO_DRIFT_NOT_YET_PROVEN / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED`
...
**v1.86 当前覆盖事实**：首个远端安全资格在 run `30725896115` / job `91437419288` 以 CodeQL `js/file-system-race`、security severity `7.7`、route evidence line 112 真实失败。`8059edf37fd0076b36cb7c57f233525c0d3056f3` 只建立有界脱敏诊断 annotation；`913bae3a2db4d8f172d0de9be87305ad21b53884` 随后以先 open exact path、再用 descriptor identity 判定并在读取后复核的实现根除 pre-open lstat 竞争窗口，没有 suppression。定向 route authority/producer/transaction=`24/24`、P0R=`139/139`、V2 Ops=`266/266` 和最终字节完整本地 CI PASS；相同 source 的 Signed Dispatch `30726178696`、Full Quality `30726178679`、A0 `30726178659`、Independent Security `30726178672` 全部 PASS，CodeQL job `91438221977` 无 annotation。该证据关闭当前源码远端资格，不关闭 fresh production rebind、external cleanup、真实 backup/retrieval/restore 或 P0；生产和 Edge 均未改变。

```text
V2-M1.6-P0R-B9-R2-ROUTE-AUTHORITY-PRODUCER-ROOT-REMEDIATION
```

P0R 不允许以 Object Lock、Keychain 身份、transport bundle、target staging、canary、临时清理、阈值下调或 backup exit 0 冒充生产可恢复；临时凭证、对象上传、exact version retrieval、隔离 restore 和 cleanup 均是独立、可审计动作。当前顺序已收敛为 `权威资料最终 HEAD 本地/远端资格 -> fresh production read-only rebind 证明服务器 8022/unit/P0R residue 为零 -> 本人仅确认可能含 response 的 Edge 页已关闭 -> 代理自动删除旧腾讯规则并完成 external zero drift -> 新 route target/lease/run/object key/v4 plan/transport-v4 -> fixed-dispatch 17-member staging 与独立 target acceptance -> API Explorer 未调用页预备 -> 双端点 egress 一致 -> 7200 秒 listener + 唯一 exact /32 -> 代理保存完整 Tencent firewall capture + 生成 exact listener observation -> 生成 120 秒内 route evidence v2 -> transaction 启动并让两条 TTY PREARMED -> coordinator 授权并放出 READY -> 一次 fresh STS native Copy + 内部 Keychain age handoff -> COS preflight -> read-only encrypted backup / exact retrieval / isolated PG16 restore -> lease-bound secret/process/container/volume/staging/listener/rule cleanup -> production zero drift -> fresh health/topology -> calibration -> fresh P0`。route authority 实现和 descriptor-first race remediation 已通过官方分支完整本地与远端门禁；历史红灯均保留。旧响应页关闭前禁止 browser state/screenshot/AX/OCR/computer-use；本地资格和服务器只读 rebind 不依赖该页面，可以先行；当前不请求 STS。不得复用任何旧 token、失败 run/package、旧 dispatch、旧计划、route evidence v1、transport v3/16-member package、手填 final hash、生产 node_modules host-path 假设、port 22 fallback、裸 `tee`、断线 receiver、单 TTY prearm、签发后第二次 SSH、无 route authority producer 的 execute、直接转发 child READY、超长复合命令或空 `/dev/shm` 占位。付费扩容已退出活跃路线，六小时零付费本地机器模型与组合准入实现已通过，但这不取消原 P0 blocker：fresh 证据必须同时满足所有旧非容量门禁、稳态 60% / 峰值 70%、完整恢复和健康门禁，否则 P1 继续 BLOCKED。P0R 生产动作完成后只允许重新执行 fresh P0，不直接授权 migration、身份创建、分区预建、Worker 启动或生产写入。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 historical B1 acquisition 的硬前置，C1 不能替代或解锁它。
