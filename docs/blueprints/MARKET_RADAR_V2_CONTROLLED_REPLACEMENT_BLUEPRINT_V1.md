...
- `M0.4`：`DESIGN_SCOPE_AMENDMENT_PASS / M1.1A_LOCAL_CONTRACT_PASS / SCOPE_V2_RUNTIME_UNPROVEN`。Bitget、上新生命周期、受控数据最大化和股票合约已进入 `SCOPE_EPOCH_V2_MULTI_ASSET_4V`；V1 三 Venue 加密证据保持原效力但不能跨 epoch。M1.1A 已完成四 Venue + CoinGlass 的 165 行来源能力登记；详细合同为 `docs/architecture/v2/M0_4_EXPANDED_MARKET_SCOPE_AND_SCOPE_EPOCH_CONTRACT_V1.md` 与 `docs/architecture/v2/M1_1A_FOUR_VENUE_SOURCE_CAPABILITY_REGISTRY_V1.md`。
...
`V2-M1.1A Four-Venue Capability Registry` 已达到 `LOCAL_CONTRACT_PASS / OFFICIAL_DOCUMENTS_REVIEWED`：4 个第一方 Venue + CoinGlass Hobbyist、33 类目标能力、165 个 source-capability 组合完整记账，官方文档行 110，unavailable/unlicensed 57，Scope V2 runtime PASS=0。旧 V1 的 Binance/OKX/Bybit catalog 与 mark-price 六行只保留 V1 证据。Bitget、股票和上新仍未接入；下一本地超级包为 `M1.1B0 Exact Source Conformance + M1.1B1 Multi-Asset Identity and Listing Intelligence`。
...
**V2 扩展子包**：`M1.1A Four-Venue Capability Registry [LOCAL_PASS] -> M1.1B0 Exact Source Conformance -> M1.1B1 Multi-Asset Identity and Listing Intelligence -> M1.4A Adaptive Multi-Asset Collector -> M1.5C Four-Venue Multi-Asset Shadow -> M1.6-D1 Expanded-Scope No-Cost Capacity Proof`。B0/B1 合并为同一实现超级包以减少重复 CI，但 B0 失败能力不能在 B1 中假定可用。原 B1-B3 和 D0 只属于 V1；V2 未完成新 Shadow 和容量证明前不得进入扩展生产范围。
