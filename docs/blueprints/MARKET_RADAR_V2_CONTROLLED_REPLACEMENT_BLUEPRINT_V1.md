...
**当前执行入口**：`V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY` 的 health-envelope 根因修复与重新资格化。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 recovery evidence 通过前不变。香港单 AZ 私有 COS 的 `COMPLIANCE` 31 天和 macOS Keychain age X25519 身份保持有效。上游 route authority、安全根治与 gateway source `dd67d819...` 的本地完整 CI及 GitHub 四门均为历史有效证据；它们不能授权当前新字节。旧 exact 批准已经过期，signed dispatch `a52aa6a...` 已被目标 claim 且状态为 `FAIL_DISPATCH_NOT_REUSABLE`，不得再次发布或执行。该尝试在任何 gateway/Caddy mutation 前因 runner 错把真实顶层 `health` 响应读取为 `data.health` 而 fail closed；生产 health/scan/persistence ready、exact 11-container identity、baseline 404 route、仓库和非目标服务均由 fresh read-only 证据证明零漂移。当前 parser/fixture 修复已通过 gateway `8/8`、recurrence `11/11` 和最终字节完整 `ci:production`，但尚未形成 clean commit、推送、取得同一 commit GitHub 四门、新 deterministic package 或新 exact approval。固定后续顺序为 `clean commit/push -> 同一 commit GitHub 四门 -> fresh production baseline -> 新 exact package -> 当前批准 -> current-image 隔离验证 -> Caddy-only deploy/自动回滚 -> 自动取回验签 -> fresh rebind -> P0R recovery`；只有 P0R 与 fresh P0 均 PASS 才能请求 P1，M2 runtime、页面和交易计划继续关闭。
