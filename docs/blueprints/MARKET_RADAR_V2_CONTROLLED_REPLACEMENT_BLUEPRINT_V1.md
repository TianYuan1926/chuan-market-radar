# Market Radar V2 受控替换工程与运行蓝图 v1.8

状态：`ACTIVE_DESIGN_AUTHORITY / M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M1.5-B1-B0_EARLY_SHADOW_EVIDENCE_LOCAL_PASS / B1-B1_EMPIRICAL_CAPTURE_PENDING / BUSINESS_SLO_UNPROVEN / M2.2-B0.2-C1_FORWARD_ONLY_READY / B0.2_EXTERNAL_RESOLUTION_BLOCKED / M1_NOT_COMPLETE / PRODUCTION_SERVICES_DATA_AND_AUTHORITY_UNCHANGED`
...
**当前进度**：`V2-M1.5-B1-B0 Early Shadow Evidence Contract` 已达到 `LOCAL_ENGINEERING_PASS / BUSINESS_SLO_UNPROVEN`。它在 B1-A 真实失败基线上冻结单进程 31 周期、60 秒 cadence、完整 collection/freshness 分母、strict process summary、独立 SLO 重算、内容寻址 domain/runner evidence、隔离临时 PostgreSQL 和宿主 Docker 精确恢复。中断或短包禁止拼接；技术捕获成功与业务 Gate PASS 使用不同状态。M1 专用 68/68、全 V2 274/0/5 explicit skip、ops 31/31 与完整 `ci:production` PASS。
...
**当前执行入口**：`V2-M1.5-B1-B1-31-CYCLE-EMPIRICAL-CAPTURE`。绑定 B1-B0 exact commit，在腾讯隔离 Runner 原样执行一个不可拼接的 31 周期窗口；只有原始 artifact、独立重算、运行边界和宿主清理同时成立，技术捕获才算 PASS。若业务 Gate FAIL，B1-B2 必须先拆分 provider snapshot receipt、per-instrument market-event recency 与 downstream price usability，定点修复后以同一 31 周期 Gate 做 B1-B3 复验；不得放宽 freshness、把 duplicate/carried-forward 标为 FRESH 或删除失败分母。B1-B PASS 后才允许 M1.6 production storage 分阶段启用，随后 M1.7 24h Shadow。B0.2-B 外部权利与历史来源 Gate 继续并行 blocked；M2 runtime、页面和交易计划仍关闭。
...
M0.0-M0.3、M1.1-M1.5-B0、M1.5-B1-A/B0、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1、B0.2-A、B0.2-C 和 C1 已通过对应本地、技术或运行起点出口；B1-B0 只完成可信执行与证据合同，业务 SLO 仍未证明。M2.2 真实能力 Gate、外部来源解决、bulk historical acquisition、31 周期业务 Gate、生产存储和持续 SLO 尚未通过。当前执行入口是：

```text
V2-M1.5-B1-B1-31-CYCLE-EMPIRICAL-CAPTURE
```

B1-B1 只执行冻结合同的原始实测，不先改 freshness 或 cadence。实测失败时进入 B1-B2 freshness 语义整改和 B1-B3 同门槛复验，实测直接通过时跳过两个条件包。技术 Runner 或 31 周期捕获 PASS 永远不能替代业务 Gate。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 historical B1 acquisition 的硬前置，C1 不能替代或解锁它。
