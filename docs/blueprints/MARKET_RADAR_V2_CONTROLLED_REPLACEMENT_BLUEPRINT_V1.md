...
**V2 扩展子包**：`M1.1A Four-Venue Capability Registry [LOCAL_PASS] -> M1.1B local implementation [LOCAL_PASS_TEST_ONLY] -> M1.1B0 Tencent isolated live source conformance [PENDING] -> M1.4A Adaptive Multi-Asset Collector -> M1.5C Four-Venue Multi-Asset Shadow -> M1.6-D1 Expanded-Scope No-Cost Capacity Proof`。B0/B1 共享实现与完整 CI 以减少重复成本，但 live B0 失败能力不能在 B1 或 M1.4A 中假定可用。原 B1-B3 和 D0 只属于 V1；V2 未完成新 Shadow 和容量证明前不得进入扩展生产范围。
