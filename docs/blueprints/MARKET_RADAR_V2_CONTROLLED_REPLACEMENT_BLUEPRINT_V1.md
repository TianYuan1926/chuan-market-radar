# Market Radar V2 受控替换工程与运行蓝图 v1.99

状态：`ACTIVE_DESIGN_AUTHORITY / M0.4_EXPANDED_SCOPE_DESIGN_PASS / M0.5_MARKET_MECHANICS_MICROSTRUCTURE_AND_PRECURSOR_ATLAS_LOCAL_CONTRACT_PASS / A0_ENGINEERING_SUBGATES_PASS_TOTAL_GATE_INCOMPLETE_P0R_PENDING / M1.1B0_R3_LIVE_15_OF_15_ALL_GATES_PASS / M1.4A_M1.4D_LOCAL_RUNTIME_CONTRACTS_PASS_NO_AUTHORITY / M1.5C_M1.5D_LOCAL_RUNTIME_AND_EXACT_PACKAGE_PASS_LIVE_ZERO_OF_31 / M2.1A_RESEARCH_CONTRACT_PASS_REAL_COHORT_MISSING / M3.1A_M3.3D_RESEARCH_SCAFFOLD_PASS_NO_REAL_CALIBRATION / M3.3E_STRATEGY_ARCHETYPE_LABELING_DESIGN_AUTHORITY_ADDED_IMPLEMENTATION_NOT_STARTED / M1.6_P0_EXECUTED_BLOCKED_CAPACITY_AND_RECOVERY / P0R_ROUTE_AUTHORITY_UPSTREAM_QUALIFIED / PRODUCTION_EVIDENCE_GATEWAY_DF85_SAME_SOURCE_FOUR_GATES_AND_SIGNER_BOUND_CADDY_ONLY_REAL_TARGET_ACCEPTANCE_PASS / ENCRYPTED_EVIDENCE_RETRIEVED_DECRYPTED_AND_SIGNATURE_VERIFIED / NON_TARGET_ZERO_DRIFT / GATEWAY_RECURRENCE_CLOSED_VERIFIED / P0R_BACKUP_EXACT_RETRIEVAL_ISOLATED_RESTORE_AND_FRESH_P0_NOT_EXECUTED / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED`
...
v1.76 根据 2026-08-02 同一 secret handoff 故障类再次复发触发 STOP-THE-LINE。source `6a70b8d3a964dd109d5051ba731813c4bccda62d` 的两个窗口都只预建 credential TTY，分别在 540 秒 clipboard deadline 后安全退出；后到的 native Copy 已作废，从未进入 Bridge、`/dev/shm` 或生产服务器，也没有 Runner、数据库读取、COS、backup、retrieval 或 restore。随后两个独立公网端点都测得 BoostNet egress 已变为 `156.248.15.36`，而 8022 路线使用更早的精确 `/32`；严格 SSH 在 banner exchange 超时。该证据把根因从“用户复制稍晚”收紧为三个跨层缺口：只预建第一条 TTY、第二条 age SSH 仍发生在签发后、短 clipboard wait 缺少双会话心跳；动态 egress 可在两次连接之间使精确防火墙失效。本机已确认 Bridge 进程为 0 并覆盖 clipboard，但 API response 页尚需本人关闭，云 listener、旧 `/32`、服务器 secret/process/container/volume 和生产零漂移尚未复核，因此旧 cleanup/zero-drift PASS 不得沿用。永久整改将 bridge/session 升到 v5：primary 在任何 secret ingress 前创建 PID/start-token/source binding，credential 与 age 两条 strict no-echo TTY 均在 operator READY 前建立，1200 秒无 secret 窗口持续检查两条本地 SSH 并输出有界心跳，签发后不再发生网络 reconnect。新增 transaction v1，以 7200 秒租约绑定 exact clean source、plan SHA-256、双端点 egress、8022 listener、唯一 `/32` rule、两条 TTY、clipboard、`/dev/shm`、process、container、volume、run staging 和显式零残留 closure；自动到期不能替代删除证明。focused bridge/session `22/22`、transaction `6/6`、完整 P0R `120/120` 与 Go helper PASS。两次不完整 PATH 运行分别因 Node 和 Go 解析失败，均保留为 FAIL，不计入最终 PASS。完整 `ci:production`、外部清场、clean commit、GitHub 四门、fresh rebind、新 lease/run/plan/package/staging、real-target recovery 和 fresh P0 仍未完成。

v1.77 关闭 transaction v1 初版中“存在授权函数但 execute 未消费外部 route authority”的合同缺口。execute 现在强制接收 mode-600 的 `market-radar-v2-m1-p0r-external-route-evidence.v1`，并精确验证 lease/source/run/plan、两个一致 egress、listener unit/port/count、唯一 firewall rule 的 identity hash、remark、source `/32`、direction/protocol/port 和 120 秒 freshness；任一漂移都在 Bridge 启动前 fail closed。协调器不会直接转发子 Bridge 的 operator READY：只有先观察到且仅观察到一次 `PASS_P0R_BOTH_TTY_SESSIONS_PREARMED`，再重新验证 route evidence 并内部形成 issuance authority 后，才输出授权与 READY；重复、乱序或过期状态均杀死 Bridge。子进程启动失败、信号或输出边界失败也进入 finally clipboard 清理。新增 2 个红绿回归后 transaction `8/8`、完整 P0R `122/122` 与 Go helper PASS；完整 `ci:production` 仍必须针对该新源码从头执行，外部清场和生产零漂移仍未证明。

v1.78 完成 transaction v1 的预提交合同加固。人工审计发现 detached `authorize` CLI 可绕开事务形成误导性授权结果、Bridge 行解析接受未声明字段和任意状态形状、cleanup evidence 不校验未来时间或陈旧度、lease 嵌套合同不够精确，以及 preflight 在 child spawn 前失败时 clipboard cleanup 没有统一覆盖；这些缺口均在 clean commit 和生产执行前被阻断。当前 CLI 只保留 `plan/create/execute/close`，授权只允许由 execute 在 fresh route 与单次双 TTY PREARMED 顺序内内部形成；lease、plan 和 cleanup scope 使用 exact-key 验证，Bridge 输出使用固定 status/key allowlist，cleanup evidence 必须非未来且不超过 120 秒，outer finally 覆盖 preflight 失败，create/execute 均独立验证完整 COS plan。transaction `10/10`、完整 P0R `124/124`、Go helper 和独立 V2 Ops `246/246` 已 PASS；完整 `ci:production` 必须对当前源码重新从头执行，外部清场、clean exact source 和生产零漂移仍未证明。

v1.79 根除最终预提交审计发现的三个输出与输入边界缺口：status allowlist 不等于合法生命周期，单个 pending fragment 上限不等于 stdout/stderr 累计上限，path `lstat` 后另行 `readFile` 也不等于稳定安全读取。transaction v1 现强制唯一 child 序列 `PREARMED -> READY -> HEARTBEAT* -> CREDENTIAL_HANDOFF -> AGE_HANDOFF -> LOCAL_PASS`，BLOCKED/PASS 均为终态；累计输出最多 `65,536` bytes、状态最多 `64`。lease、route 和 cleanup 控制文件必须 canonical、current-user、mode-600、普通文件，provisioning plan 保持 exact SHA-256 绑定；全部使用 `O_NOFOLLOW`、1 MiB 硬上限和读取前后 descriptor/path identity 一致性验证。原有 transaction `10/10` 已加入乱序、重复、终态后写、状态洪泛、symlink、空文件和超限文件红例，完整 P0R 保持 `124/124` 加 Go helper；外部清场和生产零漂移仍未证明，当前不得进入 secret 或 recovery 动作。

v1.80 发现并关闭“listener 总哈希前后相同却不能证明 8022 为零”的验收缺口。fresh rebind request/result 分别升级为 v4/v3，canonical request 现在精确绑定 `forbiddenListenerPort=8022` 和 `forbiddenListenerUnit=market-radar-p0r-8022.service`；只读 Runner 必须分别证明 TCP 8022 监听数量为 0、unit 为 `LoadState=not-found / ActiveState=inactive`，再核对其他 server residue 与生产前后 identity。IPv4、IPv6、多 listener、active unit、畸形 inventory 和任意 systemctl mutation 均有失败关闭回归；rebind 定向 `13/13`、完整 P0R `125/125` 与 Go helper PASS，当前源码完整生产 CI 尚待重跑。施工顺序作正向提速调整：不触碰敏感 Edge 页时可先完成 clean commit 与 GitHub 四门，随后用 signed fixed dispatch 做服务器只读零残留复核；旧腾讯云规则仍须在页面安全关闭后自动删除，且在外部全清场前不得创建新 listener、规则、STS、数据库或 COS 动作。

v1.81 完成 rebind v4/result v3 当前最终字节的完整本地生产门禁：精确 Node `22.23.1`、npm `10.9.8` 与本机 Go `GOTOOLCHAIN=local` 下，recurrence `11/11`、production dispatch `25/25`、V2 Foundation `631 PASS / 6 explicit skips`、V2 Ops `247/247`、M0、Next production build、Golden `16/16` 和 security 全部 PASS。该晋级只关闭本地最终字节资格，不证明云 listener、旧 firewall rule、服务器 residue、production zero drift、clean exact source、远端四门、fresh read-only rebind 或真实 recovery；这些外部与生产门禁保持 BLOCKED/PENDING。

v1.82 根据继续施工前的权威链审计根治 route evidence 只有 validator、没有 producer 的缺口。旧 transaction 测试曾允许调用者以任意 64 位十六进制字符串填充 `firewallRuleIdentityHash`，无法证明该值来自正确腾讯 Lighthouse 实例、完整 `DescribeFirewallRules` 分页、唯一 exact `/32` 规则和 exact 8022 listener。隔离分支现新增 route target v1、Tencent firewall capture v1、strict listener observer、authoritative route producer 和 route evidence v2；lease 与 transaction 绑定 provider/action/version/region/instance、完整 RequestId/FirewallVersion、observer 源码摘要、exact unit/MainPID、一个 IPv4 listener、零 IPv6 listener、strict SSH identity，并重算 firewall identity 与 listener observation digest。route target、lease、firewall capture 和 listener observation 必须已经是单一 canonical object，既有 provisioning plan 则继续按安全稳定读取和精确 SHA-256 绑定；输出为 no-clobber mode-600 canonical JSON。observer 进入 transport v4，内包从 16 个升级为 17 个成员，current runtime source 分母从 8 个升级为 9 个。定向 route authority/producer/transaction `21/21`、完整 P0R `136/136`、Go helper、ESLint 与 Biome PASS；最终字节完整 CI、clean commit、远端四门、fresh rebind 和真实恢复仍待完成，已资格化主线与生产均未改变。

v1.83 在不改变 v1.82 权威边界的前提下完成官方协议语义、SSH 身份 TOCTOU 与本地工具链派发加固。Tencent firewall authority 现以 101 条跨两页回归证明完整分页，按官方协议语义验证 TCP/UDP/ALL/ICMP/ICMPv6 的 `Port`，拒绝重复 RequestId、非法端口和分页不一致；public SSH identity 使用同一 `O_NOFOLLOW` descriptor 有界稳定读取，private identity、public key 与 known_hosts 在远程观察前后均复核完整文件身份。定向 route authority/producer/transaction 升为 `23/23`，完整 P0R 升为 `138/138`。本机默认 Node/npm 的一次错误运行真实失败且不计 PASS；随后新增受限 exact-toolchain launcher，自动选择已安装的 Node `22.23.1` / npm `10.9.8`，仅允许三条固定资格入口，独立 `3/3` 与全 V2 Ops `263/263` PASS。隔离候选完整 CI 已运行并在 M0 唯一 V2 实施分支身份门 fail closed；该结果证明分支门有效，不得绕过。正确路线调整为先完成隔离候选审计与 clean commit，再精确合入 `codex/market-radar-v2-implementation` 执行最终字节完整 CI。GitHub、生产与外部云状态仍未改变。

v1.84 根治 exact-toolchain launcher 改变 `ci:production` 文本形态后 M0 无法证明完整质量链的问题。M0 现在精确绑定唯一外层 launcher、14 项有序生产门禁、self-building verifier 和唯一 compiled authority，任何绕过 launcher、删除或重排门禁、取消 verifier 自构建或漂移 compiled target 均 fail closed；新增 `6/6` 防绕过回归。隔离候选完整 CI 再次自动选择 Node `22.23.1` / npm `10.9.8`，前序门全部通过，V2 Foundation `643 total / 636 pass / 6 explicit skip / 1 expected branch-identity blocker`，唯一阻断仍是必须在 `codex/market-radar-v2-implementation` 执行最终字节资格。该失败不计 PASS；生产、GitHub、Edge 和已资格化主线均未改变。

v1.85 完成 route authority implementation 的本地官方分支收口。候选经污染、安全和 diff 审计形成 exact commit `157e9a79a635d8f31857481fbd6a51d10df51160`，并保持同一 commit identity 精确合入 `codex/market-radar-v2-implementation`。官方分支最终字节 `ci:production` 已从头 PASS：recurrence `11/11`、production dispatch `25/25`、market `965 PASS / 4 explicit skip`、workers `23/23`、historical `4/4`、V2 Foundation `643 total / 637 PASS / 6 explicit skip`、V2 Ops `263/263`、M0 `12/12`、Next production build、Golden `16/16` 和 security 全部通过。候选 branch-identity failure 保留为历史红证据；远端四门、fresh rebind、外部零残留和真实恢复仍待完成，GitHub、生产和 Edge 未改变。

v1.86 登记远端安全红灯、可自主诊断链和无 suppression 根因治理。诊断提交 `8059edf37fd0076b36cb7c57f233525c0d3056f3` 将 CodeQL 失败以有界脱敏 annotation 暴露为 `js/file-system-race`、`scripts/v2/production/m1-production-storage-p0r-route-evidence.mjs:112`、security severity `7.7`；run `30725896115` / job `91437419288` 保持 FAIL。永久修复 `913bae3a2db4d8f172d0de9be87305ad21b53884` 改为先以 `O_NOFOLLOW` 打开 exact path，再以 descriptor identity 为 authority，并在有界读取前后复核 path/realpath；新增 open-before-lstat 防复发回归，没有关闭规则、隐藏 annotation 或缩小扫描。定向 route authority `24/24`、P0R `139/139`、V2 Ops `266/266` 和完整 `ci:production` PASS；同一 source 的 Signed Dispatch `30726178696`、Full Quality `30726178679`、A0 `30726178659`、Independent Security `30726178672` 四门全部 PASS，CodeQL job `91438221977` 无 annotation。生产、腾讯 firewall/listener、STS、COS、数据库、服务和 Edge 均未改变；fresh rebind、外部零残留和真实恢复仍待完成。

v1.87 根据固定生产派发“任务可送达、脱敏结果只滞留服务器”的重复阻断，停止把 Edge/API Explorer response、AX/browser-state、人工复制或 OrcaTerm 文件回传当作验收通道。新增 `V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY`：生产结果先由生产 SSH Ed25519 host key 对 canonical statement 作 namespace 签名，再以本机固定 X25519 recipient 经 HKDF-SHA256 + AES-256-GCM 加密；Caddy 只在既有 HTTP 入口的高熵精确 `.mre` 路径提供 ciphertext，禁止目录、重定向、明文、secret 和任意文件读取。本机固定 poller 经既有 SOCKS 下载，校验 pinned host key/fingerprint、dispatch/schema/time/hash 后以 mode-600、no-clobber 方式保存 payload、sealed object 和 receipt；每个对象由 exact transient systemd timer 在两小时租约后删除。生产包只允许复用当前 Caddy image、只重建 Caddy、挂载只读 outbox；数据库、Redis、Worker、env、migration、Feature Flag、生产仓库和其他容器均禁止改变，任何验证失败都必须恢复 baseline Caddy 并证明非目标容器 identity 不变。审批与发布控制文件统一采用 descriptor-first `O_NOFOLLOW`、mode-600、大小上限和读取前后 inode/mtime 稳定校验。该版本冻结时 dispatch `37/37`、gateway `7/7`、P0R rebind `14/14`、V2 Ops `274/274` 与故障注入通过；本机没有 Docker/Caddy CLI，因此真实 Caddy syntax/runtime validation 被放入生产变更前、当前生产 Caddy image 的隔离 network-none preflight。其后完整 CI 与 clean commit 资格由 v1.88 更新；生产没有发生本轮有意 mutation，P0R backup、exact retrieval 和独立 restore 仍未执行。

v1.88 登记 authority surface 与 M0/Legacy fail-closed 根治。第一次最新字节完整 CI 在 recurrence authority `10/11` 真实失败，证明 PROJECT_CONTEXT、蓝图索引和旧防复发测试仍把 B9 R2 route authority 标为当前入口；同步修复把 gateway 锁为唯一当前入口，旧 route authority 只保留为已资格化上游。Caddyfile 变更同时触发 Legacy consumer map 与 reviewed commit 失败；重生成并人工复核后 22 capabilities、539 source files、273 runtime edges 全部不变，仅受影响 capability digest 更新。M0 对生产控制 ID 使用精确白名单并新增任意 ID、Legacy ID 与换行注入红例。implementation `1ddc0fdc9408c697fcb73c42fe2b4c6e98b34e3e` 与 authority-sync remediation `35d7b63a40f9a21a07a44a974c0a1250b1c6c9be` 已形成唯一 V2 分支 clean commits；修复后完整 `ci:production` 从头 PASS：recurrence `11/11`、dispatch `37/37`、V2 Foundation `644 total / 638 PASS / 6 explicit skips`、V2 Ops `274/274`、M0 `12/12`、Next build、Golden `16/16` 和 security。GitHub 四门、精确生产批准、真实 current-image Caddy validation、gateway bootstrap、自动取回、fresh rebind 与 P0R recovery 仍待完成。

v1.89 保留首轮 exact remote qualification 的真实 3/4 结果并根治两项独立安全红灯。source `d67c0e87d5de3914a433d3bcc3f94ac5ead54bb0` 的 Signed Dispatch `30733232254`、Full Quality `30733232256` 和 A0 Release Qualification `30733232263` PASS；Independent Security `30733232255` FAIL，不能拼接为四门通过。CodeQL job `91457099084` 在私钥 no-clobber 测试中发现两次 path-based read 的 `js/file-system-race`；测试现与生产实现一致，先以 `O_NOFOLLOW` 打开 descriptor，再核对 regular file、mode、大小及读取前后 dev/ino/mtime/size，且 recipient 私钥的所有测试读取均走同一安全入口。Gitleaks job `91457099116` 的唯一 finding 精确定位到 `1ddc0fdc...:scripts/v2/production/m1-p0r-rebind-evidence-export.mjs:generic-api-key:35`；审查确认它是固定 X25519 recipient 公钥的 SHA-256 指纹而非 credential，历史提交只登记唯一 fingerprint，活动源码改为两段非 credential-shaped 表示，并新增 `PUBLIC_KEY_FINGERPRINT` 结构化分类，禁止 path/rule/commit 宽泛 allowlist。官方 Gitleaks `8.30.1` 校验发布 checksum 后本地重放全部 reachable history 为 finding=`0`，定向 `33/33` 和最终字节完整 `ci:production` 从头 PASS。新的 clean remediation commit 与同一 source 四门重验仍待完成；生产、Caddy、仓库、数据库、Redis、Worker、env、migration、Feature Flag 和流量均未改变。

v1.90 登记首个 exact gateway 生产尝试的真实失败与健康合同永久修复。最终 source `dd67d81910f5696049d4271d527c264f2e42115f` 已从零取得 Signed Dispatch `30734390429`、Full Quality `30734390438`、A0 `30734390439` 和 Independent Security `30734390634` 四门，并在 `2026-08-02T05:44:30Z..07:09:30Z` 精确批准窗口内发布 signed dispatch `a52aa6acfbe2f4f5d26cb15a82701195ca2f32df`。目标在 `06:12:33.853Z` 以 `FAIL_DISPATCH_NOT_REUSABLE / dispatch_entrypoint_launch_failed` 退出；stderr digest `sha256:d41cd1e0993f4f20b1024ac7e1c9ba17ea70cd9a1f0f5ec61fd96523bde9701f` 唯一对应 canonical `evidence_gateway_health_not_ready`。真实 `/api/health` 合同为顶层 `{ok, health}`，runner 与测试 fixture 却错误读取 `data.health`，因此在初始 health preflight fail closed。生产当时及事后只读复核均为 health ready、scan ready/fresh、persistence ready、exact 11-container identity 不变、目标 route 仍 404、gateway/outbound/staging absent、global lease absent；Caddy、仓库、数据库、Redis、Worker、env、migration、Feature Flag 和流量均无 mutation。旧批准已过期、旧 dispatch 永久不可复用。当前 parser 与 fixture 已按真实顶层合同修复，并加入旧 nested envelope 必须在 mutation 前失败且 mutation/root creation 均为 0 的红例；定向 gateway `8/8` 和 ESLint PASS，但这些新字节尚未完成完整 CI、clean commit、GitHub 四门或新生产批准。详细证据见 `V2_PRODUCTION_EVIDENCE_GATEWAY_HEALTH_CONTRACT_REMEDIATION_REPORT.md`。

v1.91 登记 health-contract remediation 第一轮最终字节完整质量门：冻结 Node `22.23.1` / npm `10.9.8` 下 `ci:production` 从头退出 0，recurrence `11/11`、dispatch `37/37`、V2 Foundation `644 total / 638 PASS / 6 explicit skips`、V2 Ops `275/275`、M0 `12/12`、Next production build、Golden `16/16` 和 security 全部 PASS。该证据只把本地状态提升为 `LOCAL_RUNTIME_PASS`；clean commit、push、同一 commit GitHub 四门、fresh production baseline、新 deterministic package、新 exact approval、current-image validation、Caddy-only deploy、自动证据取回、fresh rebind 与 P0R recovery 仍未完成，生产保持不变。

v1.92 health-contract remediation 已形成 clean implementation commit `51886b038b6dfa5e1bf0169abc518810de7eb2d5`。权威资料随后绑定该 commit、旧失败身份、旧批准过期和 failed-attempt-only zero drift，并在相同冻结工具链下对 authority-sync 最终字节完整 `ci:production` 再次从头 PASS；生产未改变。当前只允许推送 authority-sync source 并等待该精确 source 的四个 GitHub 门；`51886b0...` 本身以及任何旧 PASS 不得拼接为新 source 资格。

v1.93 登记 authority-sync exact source 的真实 `3/4` 远端资格和第二项永久根治。source parts `d1c3d987cf55105e94e8 + 15992dbb989ea1045629` 的 Signed Dispatch `30739919674`、Full Quality `30739919649` 与 A0 `30739919664` PASS，Independent Security `30739919662` FAIL；CodeQL 和 image scan 通过，Gitleaks `8.30.1` full-history job `91475363046` 发现唯一 `approval-request.json:1 / generic-api-key`。脱敏 artifact `8830917655` 证明该值是历史 signed dispatch 中固定 X25519 recipient 公钥的 SHA-256 指纹，不是 credential；但旧字段 `evidenceRecipientKeySha256` 具有密钥形高熵语义，scanner 的 fail-closed 结果有效。当前实现将新请求改为 `evidenceRecipientFingerprintSha256`，严格拒绝旧字段，并在 signed dispatch 发布前阻断任何歧义的非 public `*KeySha256` 64-hex 值；历史仅保留 exact commit/path/rule/line ignore 和结构化 `PUBLIC_KEY_FINGERPRINT` 审查。recurrence `11/11`、dispatch `38/38`、gateway `8/8`，合计 `57/57` PASS；首轮完整 exact-toolchain CI 也从头通过 Foundation `638 PASS / 6 skip`、Ops `275/275`、M0 `12/12`、Next build、Golden `16/16` 和 security。最终权威字节仍须再跑完整 CI，clean replacement commit 和同源四门仍待完成；前三门不得拼接复用，生产未改变。

v1.94 登记 public-key-fingerprint artifact 永久根治的 clean implementation commit `8742828086041da1db51a5abeeef01ba7656e5fd`。最终 authority-sync 字节已在冻结 Node `22.23.1` / npm `10.9.8` 下再次完成完整 `ci:production`：recurrence `11/11`、dispatch `38/38`、V2 Foundation `644 total / 638 PASS / 6 explicit skips`、V2 Ops `275/275`、M0 `12/12`、Next production build、Golden `16/16` 和 security 全部 PASS。该状态只证明本地精确源码具备推送资格；authority-sync commit、push 和 replacement exact source 的同源 GitHub 四门仍待完成，`d1c3d987...` 的 `3/4` 不得拼接复用。生产、Caddy、仓库、数据库、Redis、Worker、env、migration、Feature Flag 和流量均未改变。

v1.95 登记第二次 exact gateway 生产尝试与 production-runtime preflight 复发根治。source `d9a1e5fedf481f00e3cc88705c7e2b884add3ebe` 已从零通过 Signed Dispatch `30743899298`、Full Quality `30743899329`、A0 `30743899304` 和 Independent Security `30743899295`；dispatch `39732e2e943e7b32648d1f0ca43c477900133acb` 于 `2026-08-02T11:39:28.743Z` 被目标 claim，并在 `11:39:39.416Z` 以 `FAIL_DISPATCH_NOT_REUSABLE / dispatch_entrypoint_launch_failed` 退出。stderr digest `67269043...` 唯一对应旧 runner 的 canonical `unexpected_error`，未被伪装成可诊断 PASS。生产等价无 mutation A/B 直接证明两个根因：current Caddy image 在 `cap-drop ALL` 下丢失执行所需能力并以 RC 255 失败；runner 绕过 root-owned production Compose identity wrapper 后缺少 `POSTGRES_USER` 并以 RC 1 失败。相同 Caddy isolation 仅恢复 `NET_BIND_SERVICE` 后 validate PASS，exact wrapper 携 gateway override 后 `config --quiet` PASS。生产 HEAD、clean worktree、11-container identity、health ready/fresh 和 404 route 均不变；已清理唯一空 outbound residue，gateway/staging/lock/diagnostic container 全部 absent。该问题与首次 health-envelope 偏差共同登记为 occurrence 2，generic gateway release STOP-THE-LINE。local remediation commit `bb9abe5cf0404ca7ce9af55f6ed99a6e3f646d76` 绑定 wrapper/override exact hash、最小 capability、稳定 command fingerprint、owned-state cleanup，并在构包和生产 mutation 前双重执行 checksum-bound recurrence gate；gateway `12/12`、recurrence `11/11`、dispatch `38/38`、lint 和 tracked-source secret scan PASS。冻结 Node `22.23.1` / npm `10.9.8` 的完整 `ci:production` 也从头退出 0：market `965/969`（4 explicit skips）、workers `23/23`、historical `4/4`、V2 Foundation `638/644`（6 explicit skips）、V2 Ops `279/279`、M0 `12/12`、Next production build、Golden `16/16` 和 security 全部 PASS。final authority commit/push、同源四门、新 deterministic package、新批准和真实目标验收仍待完成。详细证据见 `V2_PRODUCTION_EVIDENCE_GATEWAY_RUNTIME_PREFLIGHT_REMEDIATION_REPORT.md`。

v1.96 登记第三次 exact gateway 生产尝试、自动 baseline rollback 和 signer-identity 永久根治。source `ad9aeb9842f6b57d934e7729e5e8a5cf2680a3b7` 已从零通过 Signed Dispatch `30749690584`、A0 `30749690576`、Full Quality `30749690577` 和 Independent Security `30749690593`；signed dispatch `700b994066309aa242484d978f712f6e2b3f0775` 真实通过 current-image Caddy validate、exact Compose identity、Caddy-only recreate、ready/fresh health、mount 和 route probe，随后在 final evidence seal 以 `evidence_gateway_seal_failed:evidence_signing_key_path_invalid` 失败。该次发生受权的 Caddy 临时 mutation，自动回滚恢复 baseline config/image；十个非 Caddy full IDs、production HEAD `cec0b657...`、clean worktree、health 与全部非目标面不变，exact staging/gateway/outbound/diagnostic state 均 absent。生产隔离诊断分别证明目标 Caddy route 与 root-owned mode-600 host key sign/verify 正常，签名公钥指纹为 `SHA256:wxHx/NcT7wmgM6aOJnjgYKK4gOQGGeN44XkHYARbpbc`。直接根因是 request v3 未绑定 signer identity、默认 signer 未获得 `keyPath`，而本地 happy test 手工注入 `signerOptions` 掩盖了生产差异。occurrence 现为 3，runtime-identity-only operation 永久退役；request v4 绑定 key path/fingerprint，在任何 Caddy mutation 前执行真实 sign、独立 verify 和指纹比对。无 caller options happy path、missing-key 与 wrong-identity 故障注入已使 gateway `14/14`、recurrence `11/11`、dispatch `38/38` PASS；冻结 Node `22.23.1` / npm `10.9.8` 的最终字节完整 CI 也已通过 market `965/969`（4 explicit skips）、workers `23/23`、historical `4/4`、Foundation `638/644`（6 explicit skips）、Ops `281/281`、M0 `12/12`、Next build、Golden `16/16` 与 security。clean commit/push、同源四门、fresh baseline、双构建、新批准和 signer-bound real-target acceptance 仍待完成。详细证据见 `V2_PRODUCTION_EVIDENCE_GATEWAY_SIGNER_IDENTITY_REMEDIATION_REPORT.md`。

v1.97 登记 signer remediation clean source 的真实远端 `3/4` 和跨工作包 CI 竞态根治。commit `4d1042618eff24d39f6ecceb93d5715e2e798b5a`、tree `c28fa60f1236e8b80a7185de4bcfdc4c084d34e0` 已推送；Signed Dispatch `30755615525`、A0 `30755615545` 与 Independent Security `30755615533` PASS，Full Quality `30755615546` / job `91517146516` FAIL，因此不得拼接为四门资格。失败唯一落在 P0R bridge 的 `prearmed-disconnect` 故障注入：同一 credential TTY 断开若在首次 liveness check 被观察会返回 `prearmed_credential_session_disconnected`，若调度稍晚进入 clipboard wait loop 则被旧通用标签改写为 `prearmed_secret_session_disconnected`。这不是 signer/Caddy 回归，而是 bridge v5 在不同检查阶段重复归类同一已知会话的非确定性合同。bridge v6 现让 credential 与 age 会话在 prearm 和 clipboard-wait 全程携带各自固定 failure code，并新增立即 credential 断开、clipboard wait 中 credential 断开和 age 断开三条可控时序红例；focused bridge `13/13`、完整 exact-toolchain P0R `142/142` 与 Go helper PASS。当前最终字节完整 `ci:production`、clean v6 commit 和同源四门仍待完成；`4d104...` 的 `3/4` 不得重跑冒充通过，也不拥有新的生产批准。生产自第三次自动回滚后没有新增变更。

v1.98 登记 bridge-v6 最终权威字节的完整本地资格。冻结 Node `22.23.1` / npm `10.9.8` 下 `ci:production` 从头退出 0：recurrence `11/11`、production dispatch `38/38`、market `965/969`（4 explicit skips）、workers `23/23`、historical `4/4`、V2 Foundation `638/644`（6 explicit skips）、V2 Ops `283/283`、M0 `12/12`、Next production build、Golden `16/16` 和 security 全部 PASS；bridge directed `13/13`、完整 P0R `142/142` 与 Go helper 同样 PASS。该证据只提升 bridge-v6 本地资格，不修补 `4d104...` 的远端 `3/4`，也不产生生产授权。下一入口缩短为 clean commit/push 与该精确 source 的四门从零复验；fresh baseline、package、批准、Caddy acceptance、自动取回、fresh rebind 和 P0R recovery 继续关闭。

v1.99 登记 production evidence gateway 的第四次 exact real-target acceptance。bridge-v6 source `df85b485b6d2c0ede8cdd2b656978ba2fdc372ab`、tree `f32cc5aa70f7f1c46e772b3b7ba2abbbd7aaf2a5` 已独立通过 Signed Dispatch `30758244706`、Full Quality `30758244710`、Independent Security `30758244707` 和 A0 `30758244715`；deterministic package 绑定 approval `1daeb3...`、bundle `e70e638...`、manifest `4c8bba4...`、baseline `f5bde72...` 与 container set `f871741...`。signed dispatch `3cca64f63ea160e0b772c513c9c6624bc76a28ec` 真实完成 signer-bound preflight、Caddy-only recreate、route probe 和 ready/fresh health，返回 `PASS_PRODUCTION_EVIDENCE_GATEWAY_CADDY_ONLY`。本机 fixed retriever 自动取得对象 `22ebec...mre`，以 pinned production host identity 解密验签，payload `d33c62e...`、sealed `2f0ab69...` 一致，三份本地证据均为 mode `0600`。独立只读复核确认生产 HEAD `cec0b657...`、clean worktree、十个非 Caddy full IDs、健康、exact staging/lease absence 与 expiry prune timer；数据库、Redis、Worker、env、migration、Feature Flag、生产仓库和其他服务均未变。复发 `REC-2026-08-02-PRODUCTION-GATEWAY-PREFLIGHT-EQUIVALENCE` 以 recurrence count 3 关闭为 `CLOSED_VERIFIED`；前三个失败 dispatch 永久保留且不可复用。当前入口切换到 fresh P0R read-only rebind、全新 route/lease/run/plan/transport、加密备份、COS exact-version retrieval、隔离 PostgreSQL 16 restore、清理和 fresh P0，任何 gateway PASS 均不得冒充 P0R 或实战准入。
...
**当前执行入口**：`V2-M1.6-P0R-FRESH-REBIND-AND-RECOVERY`。production evidence gateway 已由 source `df85b485b6d2c0ede8cdd2b656978ba2fdc372ab` 的同源四门、deterministic package、signer-bound Caddy-only 真实发布、自动密文取回、独立解密验签和非目标零漂移验收关闭；前三个失败 dispatch 仍保留且永久不可复用。该 PASS 只建立可靠生产证据返回通道，不构成 P0R、P0、M1 或交易 authority。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 recovery evidence 通过前不变。固定后续顺序为 `权威同步 source 资格 -> fresh P0R read-only rebind/零残留基线 -> 全新 route/lease/run/object key/v4 plan/transport -> fixed-dispatch no-secret target acceptance -> 双 TTY PREARMED 与腾讯权威 /32 路由 -> 一次 fresh exact-plan STS native Copy + age handoff -> read-only encrypted backup -> COS exact-version retrieval -> isolated PostgreSQL 16 restore/integrity -> lease-bound cleanup -> fresh health/topology/capacity -> fresh P0`。只有 P0R 与 fresh P0 均 PASS 才能请求 P1；M2 runtime、页面和交易计划继续关闭。
...
`M2.3 Listing/Venue and Equity Event Detection`、`M2.4 Multi-Asset Cohort and Domain-Sealed Holdout` 当前均为 `DESIGN_ONLY / NOT_STARTED / NO_CANDIDATE_EMISSION`。它们必须使用 V2 四 Venue facts，按 assetDomain、listing warm-up/mature、Venue、direction、regime 和 liquidity 分层，禁止复用 V1 holdout 结论。
...
`V2-M3.3 Strategy Construction` 已达到 `LOCAL_CONTRACT_PASS / TEST_ONLY_UNCALIBRATED / NO_READY_AUTHORITY`。六族 long/short 使用独立模板；entry/stop/target 必须引用 Analysis exact level，stop 只能在结构失效基准外扩，缺字段时必须 no-draft abstain。`StrategyDraft v2` 写入 reference/cost/buffer/RR lineage，Final Decision 重新核对 level price 与 exact RR。M3.3 20/20、M3.2 18/18、M3.1 21/21、M3.0 22/22，合计 81/81；M3.4-R0 Scope Rebase Gate 已本地通过 12/12，但只有治理权限。当前仍只有带 blocker 的 V1 测试草案，没有 Scope V2 Analysis/Strategy、真实成本、Execution Feasibility、Risk、runtime 或 READY authority。

`V2-M3.3E Strategy Archetype Labeling and Outcome Attribution` 已达到 `DESIGN_AUTHORITY_ADDED / IMPLEMENTATION_NOT_STARTED / NO_RUNTIME_OR_PRODUCTION_AUTHORITY`。合同要求每份完整策略恰好一个后端 canonical 主标签、有界辅助标签和 Action State 派生状态标签，并沿 StrategyDecision、DecisionSnapshot 与 Outcome 原样冻结；前端推导、Outcome 事后改写、单币种专属类别和标签驱动提级全部禁止。初始词表覆盖突破回踩、跌破反抽、支撑反弹、压力受阻、趋势回撤/反抽延续、假突破/假跌破反转、区间边界反转、压缩扩张、流动性扫单收复/拒绝、相对强弱延续与衍生品资金流延续。该项当前只有设计合同，schema、builder、校验、Outcome 分层和 UI 消费尚未实现，不计入 M3 完成。
...
**v1.78 当前覆盖事实**：2026-08-02 的两次新窗口均在第一条 credential TTY READY 后等待 540 秒超时，后到 native Copy 永久作废且未进入 Bridge 或服务器。之后双端点 egress 一致变为 `156.248.15.36`，strict 8022 在 banner exchange 超时；旧 `/32` 不能支撑签发后的第二条 age SSH。当前只证明本机 bridge process=0 和 clipboard 已覆盖，云端 listener/firewall/server residue 与 production zero drift 尚未验证。bridge/session v5 现把 primary identity binding 和两条 strict no-echo TTY 全部前移到 operator READY 之前，1200 秒 no-secret window 检查双会话并输出 heartbeat，签发后不再 reconnect。transaction v1 以 7200 秒 exact lease 绑定全资源，execute 强制消费 120 秒内 mode-600 的精确 route evidence；detached authorize CLI 已删除，Bridge 输出只接受固定键和状态，cleanup evidence 必须非未来且不超过 120 秒，outer finally 覆盖 preflight 失败。focused bridge/session `22/22`、transaction `10/10`、P0R `124/124`、Go helper 与当前源码完整 `ci:production` 均 PASS；完整 CI 包含 recurrence `11/11`、dispatch `25/25`、Foundation `631 PASS / 6 explicit skips`、V2 Ops `246/246`、Next build、Golden `16/16` 和 security。外部清场、clean source、四门、fresh rebind、新 execution identity 和 recovery 均待完成。

**v1.79 当前覆盖事实**：最终预提交审计进一步发现 transaction v1 虽已限制单行与状态 allowlist，但尚未把 child 输出强制为唯一合法终态序列，也没有限制两个输出流的累计字节和状态总数；控制文件仍采用 path `lstat` 后另行 `readFile`，缺少 no-follow、读取前后同一 inode 证明和硬大小上限。现协调器只接受 `PREARMED -> READY -> HEARTBEAT* -> CREDENTIAL_HANDOFF -> AGE_HANDOFF -> LOCAL_PASS`，`BLOCKED` 在任一未终结阶段均为终态；乱序、重复、PASS/BLOCKED 后继续输出全部 fail closed。stdout+stderr 合计最多 `65,536` bytes、状态最多 `64` 个；lease、route 与 cleanup 控制文件必须为 canonical 当前用户 mode-600 普通文件，provisioning plan 维持 exact SHA-256 绑定；全部使用 `O_NOFOLLOW` 打开、最多 `1,048,576` bytes，并在有界读取前后及路径复核时保持 dev/inode/mode/link/owner/group/size/mtime/ctime 完全一致。内部 issuance authorization 不再混入 child `safeStatuses`。原有 transaction `10/10` 现同时覆盖乱序、重复、终态后写、状态洪泛、symlink、空文件和超限文件；完整 P0R 仍为 `124/124` 加 Go helper。该加强不改变外部真值：旧敏感页、云 listener/rule、服务器 residue 和 production zero drift 仍未复核，clean exact source、四门、fresh rebind、新执行身份和真实恢复仍待完成。

**v1.82 当前覆盖事实**：权威审计证明 route evidence v1 只有消费校验器而没有生产者，测试使用任意 64 位字符串作为 firewall identity，故旧 route JSON 不具备独立来源权威。隔离分支新增的 route authority 链从 canonical route target、lease、腾讯 `DescribeFirewallRules` 完整分页响应和 checksum-bound canonical listener observation 构造 route evidence v2；既有 provisioning plan 保持精确 SHA-256 绑定。任何宽 CIDR、多端口、重复/额外 8022 ACCEPT、RequestId/FirewallVersion/分页漂移、unit/MainPID/socket/host identity 漂移或 route 控制输入非 canonical 均失败关闭。listener observer 纳入 transport v4，内包 17 members，rebind current runtime 9 files。当前定向 `21/21`、P0R `136/136`、Go helper、ESLint 与 Biome PASS；完整 `ci:production` 尚未针对最终字节执行，主线、GitHub 资格和生产没有改变。

**v1.83 当前覆盖事实**：route authority 已加入 101 条两页分页、官方 ICMP 空 `Port`、协议端口边界、RequestId 唯一性和 SSH identity 无跟随稳定读取/观察前后漂移回归，定向 `23/23`、P0R `138/138` 与 Go helper PASS。错误默认工具链运行真实 FAIL 且未计入资格；受限 exact-toolchain launcher 自动切换锁定版本，独立 `3/3` 与全 V2 Ops `263/263` PASS。隔离候选完整 CI 已通过前序门并在 M0 要求的唯一 V2 实施分支身份门安全阻断；这不是业务失败，也不是完整 CI PASS。候选必须先经污染、安全与 diff 审计形成 clean commit，再精确合入 `codex/market-radar-v2-implementation`，从头运行最终字节 `ci:production`。已资格化主线、GitHub 资格、生产、腾讯 firewall/listener、STS、COS 和数据库均未改变。

**v1.84 当前覆盖事实**：M0 已把 `ci:production` 外层 exact-toolchain launcher 与内层 14 项有序生产质量链精确绑定，并锁定 self-building/compiled verifier；防绕过定向回归 `6/6 PASS`。隔离候选的完整 CI 再次通过分支身份之前的全部门，Foundation 为 `643 total / 636 pass / 6 explicit skip / 1 expected branch-identity blocker`，不再存在 CI 包装器识别失败。该结果仍不等于 full CI PASS；必须合入唯一 V2 实施分支后从头重跑最终字节门禁。

**v1.85 当前覆盖事实**：route authority implementation commit `157e9a79a635d8f31857481fbd6a51d10df51160` 已经污染、安全、diff 与定向证据审计后精确合入 `codex/market-radar-v2-implementation`。官方分支最终字节 `ci:production` 已从头 PASS：recurrence `11/11`、production dispatch `25/25`、market `965 PASS / 4 explicit skip`、workers `23/23`、historical `4/4`、V2 Foundation `643 total / 637 PASS / 6 explicit skip`、V2 Ops `263/263`、M0 `12/12`、Next production build、Golden `16/16` 和 security 全部通过。候选分支曾出现的唯一 branch-identity failure 保留为历史红证据，不被当前 PASS 覆盖。当前仍无远端资格、fresh production rebind、外部零残留或真实恢复证据；GitHub 和生产未改变，P0R/P0 继续 BLOCKED。

**v1.86 历史覆盖事实**：首个远端安全资格在 run `30725896115` / job `91437419288` 以 CodeQL `js/file-system-race`、security severity `7.7`、route evidence line 112 真实失败。`8059edf37fd0076b36cb7c57f233525c0d3056f3` 只建立有界脱敏诊断 annotation；`913bae3a2db4d8f172d0de9be87305ad21b53884` 随后以先 open exact path、再用 descriptor identity 判定并在读取后复核的实现根除 pre-open lstat 竞争窗口，没有 suppression。定向 route authority/producer/transaction=`24/24`、P0R=`139/139`、V2 Ops=`266/266` 和最终字节完整本地 CI PASS；相同 source 的 Signed Dispatch `30726178696`、Full Quality `30726178679`、A0 `30726178659`、Independent Security `30726178672` 全部 PASS，CodeQL job `91438221977` 无 annotation。该证据只作为当前 evidence-return 工程的已资格化上游，不关闭 fresh production rebind、external cleanup、真实 backup/retrieval/restore 或 P0。

**v1.89 当前覆盖事实**：fixed dispatch 的 package delivery 已有真实目标验收，但 sanitised result 过去只能留在生产服务器；这使 Edge/API response、AX/browser-state、人工复制和 OrcaTerm result transport 成为重复故障源。当前本地实现以生产 host key 签名、X25519 加密、现有 Caddy 高熵 ciphertext route、本机固定 SOCKS poller、pinned host identity、mode-600 no-clobber receipt 和 exact two-hour TTL cleanup 形成自主证据返回闭环。Caddy-only package 在 mutation 前使用当前生产 image 做隔离 validate，在 mutation 后核对 route、health、mount、image 和非目标 full container IDs；任一失败自动恢复 baseline Caddy。首轮 source `d67c0e...` 远端资格只得到 Signed Dispatch、Full Quality、A0 三门 PASS，Independent Security 真实 FAIL：CodeQL 抓到测试私钥 path-read 竞态，Gitleaks 抓到一个 X25519 公钥指纹形似 credential。当前本地根治已把测试读取统一为 descriptor-first `O_NOFOLLOW` 稳定读取，并把活动公钥指纹改为两段表示；历史 finding 仅登记 exact fingerprint。官方同版本 Gitleaks 完整历史 finding=`0`、定向 `33/33` 和完整 `ci:production` PASS。新的 clean commit 与四门重验仍待完成，不能沿用前三门拼接资格。本机无 Docker/Caddy CLI，故真实 runtime validation 尚未发生；精确生产批准、gateway bootstrap、自动取回和 fresh rebind 均待完成。生产没有本轮有意 mutation，fresh zero-drift 未证明，P0R recovery 未执行。

```text
V2-M1.6-P0R-FRESH-REBIND-AND-RECOVERY
```

P0R 不允许以 Object Lock、Keychain 身份、transport bundle、target staging、gateway PASS、canary、临时清理、阈值下调或 backup exit 0 冒充生产可恢复；临时凭证、对象上传、exact version retrieval、隔离 restore 和 cleanup 均是独立、可审计动作。production evidence gateway 已完成真实目标验收，后续顺序收敛为 `权威同步 exact-source 四门 -> fresh read-only rebind 与零残留基线 -> 新 route target/lease/run/object key/v4 plan/transport-v4 -> fixed-dispatch target acceptance -> 双 TTY PREARMED 与腾讯权威路由 -> 仅一次不可替代 MFA/native Copy -> read-only encrypted backup -> COS exact-version retrieval -> isolated PostgreSQL 16 restore/integrity -> lease-bound 全清理 -> fresh health/topology/capacity -> calibration -> fresh P0`。route authority、descriptor-first、安全 bridge-v6 和 evidence return 保持已资格化上游；旧敏感响应页仍禁止 browser state/screenshot/AX/OCR/computer-use。除精确 production mutation 批准及平台强制本人 MFA/native Secret Copy 外，调查、构建、派发、下载、验证、清理和回滚均由代理自主完成。不得复用任何旧 token、失败 run/package、旧 dispatch、旧计划、route evidence v1、transport v3/16-member package、手填 final hash、生产 node_modules host-path 假设、port 22 fallback、裸 `tee`、断线 receiver、单 TTY prearm、签发后第二次 SSH、无 route authority producer 的 execute、直接转发 child READY、超长复合命令或空 `/dev/shm` 占位。付费扩容已退出活跃路线，六小时零付费本地机器模型与组合准入实现已通过，但这不取消原 P0 blocker：fresh 证据必须同时满足所有旧非容量门禁、稳态 60% / 峰值 70%、完整恢复和健康门禁，否则 P1 继续 BLOCKED。P0R 完成后只允许重新执行 fresh P0，不直接授权 migration、身份创建、分区预建、Worker 启动或生产写入。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 historical B1 acquisition 的硬前置，C1 不能替代或解锁它。
