# Market Radar V2 受控替换工程与运行蓝图 v1.16
...
**当前执行入口**：`V2-M1.6-P0R-B1B-OBJECT-LOCK-WHITELIST-AND-AGE-VAULT-QUALIFICATION`。P0R-D0 六小时无扩容本地机器证明已 PASS，但原 P0 在 fresh 证据通过前不变。香港单 AZ 私有 COS 空桶已经建立并核验，当前账号没有 Object Lock 入口，必须先补齐联系方式、提交并通过腾讯白名单工单；工单草稿不是开通证据。白名单确认后仍须在动作时独立确认不可逆 COMPLIANCE 31 天，才可用已通过本地门禁的 macOS Keychain 工具生成真实离机 age 身份，并签发与 checksum-bound plan 完全一致的 7200 秒 STS。随后执行 P0R-C 真实加密离机备份、精确 version retrieval、独立 PostgreSQL 16 restore parity 与清理证据，再刷新 boot/filesystem/Docker/Postgres/Redis/app health 和 topology。只有新的 P0 PASS 才能请求 P1 v1+v2 schema，之后仍须依次执行 P2 最小权限身份、P3 六小时分区与 dormant Worker、P4 有界 isolated-write Shadow 和 M1.7 同 release 24h Shadow。B0.2-B 外部权利与历史来源 Gate 继续并行 blocked；M2 runtime、页面和交易计划仍关闭。
