# Market Radar V2 受控替换工程与运行蓝图 v1.68

状态：`ACTIVE_DESIGN_AUTHORITY / M0.4_EXPANDED_SCOPE_DESIGN_PASS / M0.5_MARKET_MECHANICS_MICROSTRUCTURE_AND_PRECURSOR_ATLAS_LOCAL_CONTRACT_PASS / A0_ENGINEERING_SUBGATES_PASS_TOTAL_GATE_INCOMPLETE_P0R_PENDING / M1.1B0_R3_LIVE_15_OF_15_ALL_GATES_PASS / M1.4A_M1.4D_LOCAL_RUNTIME_CONTRACTS_PASS_NO_AUTHORITY / M1.5C_M1.5D_LOCAL_RUNTIME_AND_EXACT_PACKAGE_PASS_LIVE_ZERO_OF_31 / M2.1A_RESEARCH_CONTRACT_PASS_REAL_COHORT_MISSING / M3.1A_M3.3D_RESEARCH_SCAFFOLD_PASS_NO_REAL_CALIBRATION / M3.3E_STRATEGY_ARCHETYPE_LABELING_DESIGN_AUTHORITY_ADDED_IMPLEMENTATION_NOT_STARTED / M1.6_P0_EXECUTED_BLOCKED_CAPACITY_AND_RECOVERY / P0R_E362_REMOTE_GATES_REBIND_AND_STAGING_HISTORICAL_PASS_RUN_INVALIDATED / P0R_THIRD_STS_POST_RESPONSE_DISCLOSED_NEVER_USED_EXPIRED_FORBIDDEN_REUSE_DUAL_CLOCK_PROOF_PASS / PROD_P0R_FILES_PROCESSES_CONTAINERS_VOLUMES_ZERO / P0R_0E035_REMOTE_FOUR_GATES_PASS / P0R_REBIND_C85EC1A4_BLOCKED_PACKAGE_BINDING_BEFORE_MUTATION_STAGING_CLEANED_PRODUCTION_ZERO_DRIFT / P0R_CROSS_LAYER_DISPATCH_BINDING_ROOT_FIX_FULL_LOCAL_CI_PASS_NEW_EXACT_SOURCE_PENDING / PRODUCTION_BUSINESS_SERVICES_DATA_AND_AUTHORITY_UNCHANGED / M1_NOT_COMPLETE / M2_RUNTIME_BLOCKED`
...
**当前执行入口**：`V2-M1.6-P0R-B4-PREARMED-LOCAL-TTY-BRIDGE-RECOVERY`。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 fresh 证据通过前不变。香港单 AZ私有 COS 已启用并回读 `COMPLIANCE` 31 天，真实 age X25519 身份仅保存在 macOS Keychain。source `e362...` 的 GitHub 四门、fresh read-only rebind、生产 11 容器零漂移和 staged transport-v2 都是真实历史 PASS，但第三枚 STS response 在 post-response browser-state recovery 中进入工具输出，因此该 credential 及其 run/plan/object key/bundle/staging 永久失去执行权。它从未进入服务器或 COS；生产 P0R files/processes/containers/volumes 已复核为 0。该 STS exact expiry=`2026-07-29T06:09:17Z`，本机 UTC `2026-07-29T10:32:53Z` 与腾讯 STS HTTPS Date `2026-07-29T10:35:42Z` 已独立证明过期，状态为 `EXPIRED_FORBIDDEN_REUSE`。新 fixed local TTY bridge 在签发前建立 exact SSH session，只在远端 echo-disabled READY 后接受 API Explorer 原生 Copy，先清 clipboard 再 handoff，并从固定 Keychain 内部注入 age identity；request v3 高层 release 根治又拒绝所有人工重复绑定。当前伪 TTY 10/10、跨层定向 29/29、P0R 88/88、recurrence 10/10、dispatch 24/24 和精确 Node/npm/Go 完整 `ci:production` 均已 PASS，新的 clean exact commit 和远端资格仍待完成。下一次必须先取得新 clean commit、GitHub 四门、fresh rebind和新 run/object key/plan/bundle/staging，再启动 bridge；response 后禁止 browser state、screenshot、AX/OCR、computer-use 和 OrcaTerm secret entry。随后 helper 才能自动执行真实加密离机备份、exact version retrieval、独立 PostgreSQL 16 restore parity、全量清理和零漂移验证。只有 P0R 与 fresh P0 均 PASS 才能请求 P1 v1+v2 schema；B0.2-B 外部权利与历史来源 Gate 继续并行 blocked，M2 runtime、页面和交易计划仍关闭。
...
```text
V2-M1.6-P0R-B5-CROSS-LAYER-DISPATCH-BINDING-ROOT-REMEDIATION
```

P0R 不允许以 Object Lock、Keychain 身份、transport bundle、canary、临时清理、阈值下调或 backup exit 0 冒充生产可恢复；临时凭证、对象上传、exact version retrieval、隔离 restore 和 cleanup 均是独立、可审计动作。第三枚 STS 已由本机 UTC 与腾讯 HTTPS Date 独立证明超过 exact expiry，现为 `EXPIRED_FORBIDDEN_REUSE`。当前顺序是 `新 clean exact commit -> GitHub 四门 -> 仅通过 request v3 高层入口执行 fresh read-only rebind -> 全新 run-id/object key/plan/14-member bundle staging -> 签发前启动 fixed local TTY bridge 并取得 echo-disabled READY -> 用户 MFA + API Explorer 原生 Copy -> bridge 即时编译 STS并从固定 Keychain内部注入 age identity -> helper 自动 backup / exact retrieval / isolated PG16 restore -> cleanup / zero drift -> fresh topology / calibration / P0`。response 出现后禁止 screenshot、AX/OCR、browser state read、computer-use 和 OrcaTerm secret entry；native Copy 无法完成时停止。raw STS response 不得落盘；超时、断线、无 EOF、编译超窗或失败必须自动清理。不得复用任何旧 token、`e362...` run、`c85ec1a4` dispatch、`bd20...` bundle、旧计划、裸 `tee`、断线 receiver、超长复合命令或空 `/dev/shm` 占位。付费扩容已退出活跃路线，六小时零付费本地机器模型与组合准入实现已通过，但这不取消原 P0 blocker：fresh 证据必须同时满足所有旧非容量门禁、稳态 60% / 峰值 70%、完整恢复和健康门禁，否则 P1 继续 BLOCKED。P0R 生产动作完成后只允许重新执行 fresh P0，不直接授权 migration、身份创建、分区预建、Worker 启动或生产写入。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 historical B1 acquisition 的硬前置，C1 不能替代或解锁它。
