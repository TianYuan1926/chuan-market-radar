# Market Radar V2 受控替换工程与运行蓝图 v1.88
...
**v1.88 当前覆盖事实**：fixed dispatch 的 package delivery 已有真实目标验收，但 sanitised result 过去只能留在生产服务器；这使 Edge/API response、AX/browser-state、人工复制和 OrcaTerm result transport 成为重复故障源。当前本地实现以生产 host key 签名、X25519 加密、现有 Caddy 高熵 ciphertext route、本机固定 SOCKS poller、pinned host identity、mode-600 no-clobber receipt 和 exact two-hour TTL cleanup 形成自主证据返回闭环。Caddy-only package 在 mutation 前使用当前生产 image 做隔离 validate，在 mutation 后核对 route、health、mount、image 和非目标 full container IDs；任一失败自动恢复 baseline Caddy。首次完整 CI 的 recurrence authority `10/11` 红灯触发 authority surface、Legacy map/reviewed commit 和 M0 production-control 白名单根治；implementation `1ddc0fdc...` 与 remediation `35d7b63a...` 已形成 clean commits。修复后完整 `ci:production` 从头 PASS：recurrence `11/11`、dispatch `37/37`、V2 Foundation `638 PASS / 6 explicit skips`、V2 Ops `274/274`、M0 `12/12`、Next build、Golden `16/16` 和 security。本机无 Docker/Caddy CLI，故真实 runtime validation 尚未发生；GitHub 四门、精确生产批准、gateway bootstrap、自动取回和 fresh rebind 均待完成。生产没有本轮有意 mutation，fresh zero-drift 未证明，P0R recovery 未执行。
