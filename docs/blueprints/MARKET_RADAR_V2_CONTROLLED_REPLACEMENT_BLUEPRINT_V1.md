# Market Radar V2 受控替换工程与运行蓝图 v1.44
...
- `A0`：`ENGINEERING_MATERIALS_AND_SUPPLY_CHAIN_LOCAL_PASS / TOTAL_GATE_INCOMPLETE / PRODUCTION_UNCHANGED`。精确版本、零高危依赖、许可证、SBOM、Actions/runner/Docker pin、双重 lint、Next 构建、原生材料烟测和 exact Node 22.23.1/npm 10.9.8 本地完整 CI 已通过；GitHub 远端 exact-runtime、独立 secret/SAST/镜像扫描、性能/资源基线、制品 provenance/回滚以及 P0R 真实恢复仍未形成完整证据。
...
当前第一批子门禁已本地通过并固化为 `v2:a0:materials:verify`、`test:v2-a0-materials`、运行烟测和 `v2-full-quality.yml`。通过 npx 隔离运行时取得 exact Node `22.23.1` / npm `10.9.8` 本地完整 CI 证据，固定 GitHub runner 的远端独立证据仍待取得；P0R 的真实离机备份与独立 PostgreSQL 16 恢复仍是独立生产第一关键路径。A0 总 PASS 前只允许继续无 authority 的本地合同与扫描工程，不允许启动 M1.5C/M1.5D 或宣称工程地基完整。
