...
后续必须按 `B0.2 rights + point-in-time instrument metadata -> B1 immutable raw acquisition -> B2 cohort construction -> B3 split + sealed holdout -> C registered replay` 串行推进。所有本地阶段仍不允许修改生产。M3 本地下一包是 `M3.3 Strategy Construction Contract`，只能消费冻结 fixture 和 M3.2 test-only artifact，不能接页面或产生 READY。全程禁止接入现有页面、读取 Legacy 运行模块或 M1 runtime、写 Candidate Store、自动调权/晋级、生成真实 Signal/Plan、删除 Legacy 代码或宣称 live 全市场能力完成。
