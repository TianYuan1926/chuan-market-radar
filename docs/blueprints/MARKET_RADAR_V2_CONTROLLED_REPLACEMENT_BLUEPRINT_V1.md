# Market Radar V2 受控替换工程与运行蓝图 v1.3

状态：`ACTIVE_DESIGN_AUTHORITY / M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M2.2-B0.1_LOCAL_CONTRACT_PASS / LIVE_MARKET_AND_SHADOW_UNPROVEN / PRODUCTION_UNCHANGED`
...
**当前本地工程入口**：M2.2-B0 来源资格与一文件技术链、B0.1 target-blind diagnostic strength 与样本构造策略均已通过本地合同出口。来源权利仍需人工审查，point-in-time onboard/delist/contract/status/underlying class 仍缺，L2 Liquidity Shift 不受 Kline 支持，bulk acquisition 与 cohort freeze 均 fail closed。下一入口是 `V2-M2.2-B0.2-RIGHTS-AND-POINT-IN-TIME-INSTRUMENT-METADATA-RESOLUTION`，它受外部权利结论和历史身份数据阻断。M2.2 Gate 继续 `INSUFFICIENT`，五个 Detector 仍为 DRAFT 且 Candidate emission=false。外部通道恢复后可并行执行 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE`；M1.7 前不得开放 M2 runtime、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.5-A、M1.5-B0、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0 和 M2.2-B0.1 已通过各自本地出口，生产零变更；M2.2 真实能力 Gate、bulk historical acquisition、live、生产存储和持续 SLO 尚未通过。当前下一入口是：

```text
V2-M2.2-B0.2-RIGHTS-AND-POINT-IN-TIME-INSTRUMENT-METADATA-RESOLUTION
```

B0.1 已完成上述 target-blind strength 和构造策略冻结。B0.2 必须由账户所有者或合格审查者给出 retention/replay 权利结论，并取得可审计 point-in-time onboard/delist/contract type/settlement/underlying class/trading status。Agent 不能把公开下载、仓库许可证、当前 instrument snapshot、归档 presence 或推断当成批准证据。
