# Market Radar V2 受控替换工程与运行蓝图 v1.5

状态：`ACTIVE_DESIGN_AUTHORITY / M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M2.2-B0.2-C_LOCAL_ENGINEERING_PASS / FORWARD_CAPTURE_START_BLOCKED_ON_EGRESS / B0.2_EXTERNAL_RESOLUTION_BLOCKED / LIVE_MARKET_AND_SHADOW_UNPROVEN / PRODUCTION_UNCHANGED`
...
**当前进度**：`V2-M1.5-B0 Shadow Release Safety Local Exit` 已达到 `LOCAL_ENGINEERING_PASS`。在 M1.5-A durable Worker 之上补齐显式 reader/writer role assumption 与会话身份验证、secret-file database URL、完整 strict observation JSONL、固定 30 分钟/24 小时 SLO profile、有限周期和专用非 root/read-only/no-Legacy-secret 容器边界。定向 41/41、全 V2 136 pass / 0 fail / 4 explicit external-dependency skips、三项隔离 PostgreSQL 16 回归与完整 `ci:production` 均通过；Legacy Consumer Map 保持 539。生产仍为零变更。
...
2026-07-20 新鲜 Edge 预检确认 OrcaTerm 为 0 会话且没有连接配置，本机也无 Docker CLI；因此 image build、Compose merge、live egress 和生产 Shadow 均保持未证明。审计同时确认高频 Fact 仍进入无物理 purge 的单一 append-only ledger，不能承接长期全市场一分钟写入。
...
**当前本地工程入口**：M2.2-B0、B0.1、B0.2-A 与 B0.2-C 已通过各自本地工程出口。B0.2-C 已建立三 Venue 前向 raw evidence、完整分母、身份 epoch、持续缺席语义、链式 continuity checkpoint 和 append-only journal；两轮本机真实请求都因 egress 失败，三家 complete snapshot 均为 0，所以 operational capture start 保持 blocked。下一运行入口是 `V2-M2.2-B0.2-C1-EGRESS-CAPABLE-FORWARD-CAPTURE-START`，只能恢复可信出口并使用同一 no-authority runner 连续实采，不得回填过去。五个来源候选仍全部为 `RESEARCH_ONLY`，B0.2-B 继续受外部权利结论和 exact historical source 阻断，bulk acquisition 与 cohort freeze 均为 false。M2.2 Gate=`INSUFFICIENT`，Detector=DRAFT、Candidate emission=false；M1.7 前不得开放 M2 runtime、页面或交易计划。
...
`V2-M2.2-B0.2-C First-Party Forward Instrument Capture` 已达到 `LOCAL_ENGINEERING_PASS / OPERATIONAL_CAPTURE_START_BLOCKED_ON_EGRESS`：三 Venue 现有 catalog Adapter 可按调用方显式选择保留 exact raw bytes，工作区外 content-addressed store、Snapshot/Batch、identity epoch、coverage gap、不可变 continuity checkpoint 和单写 append-only journal 已实现。定向测试覆盖 anti-backfill、部分分母、持续缺席非 delist、symbol reuse、证据篡改和并发陈旧写入。两轮本机实采均真实落盘为 FAILED，三家 complete snapshot=0、captureStartedAt=null；这证明失败语义，不证明 live 覆盖。
...
M0.0-M0.3、M1.1-M1.5-A、M1.5-B0、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1、B0.2-A 和 B0.2-C 已通过各自本地工程出口，生产零变更；M2.2 真实能力 Gate、外部来源解决、bulk historical acquisition、forward capture start、live、生产存储和持续 SLO 尚未通过。当前运行入口是：

```text
V2-M2.2-B0.2-C1-EGRESS-CAPABLE-FORWARD-CAPTURE-START
```

B0.2-C 的代码与本地证据出口已通过，但本机 egress 没有形成任何完整 Snapshot。C1 只允许在可信可达网络使用同一 no-authority runner 取得至少两轮、跨度达到冻结 cadence、raw 可复核、active gap=0 且无 identity conflict 的三 Venue 完整证据；失败 journal 不能冒充 capture start。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 B1 的硬前置，C1 不能替代或解锁它。
