...
**当前工程入口**：`V2-M1.6-PARTITIONED-FACT-STORAGE Retention and Capacity Foundation`。建立高频事实时间分区、受控保留、容量水位、purge 审计和恢复合同。外部通道恢复后并行执行 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE`，只运行固定 31 个一分钟周期；两者都通过后进入 M1.7 24 小时持续 Shadow。不得接入 Candidate、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.5-A 和 M1.5-B0 已通过各自本地出口，生产零变更；live、长期存储和持续 SLO 尚未通过。当前唯一工程下一包是：
...
- 只重建高频 `PointInTimeMarketFact` 的可扩展存储与 retention 边界，不改变 Fact 业务含义、Provider、Feature、Context 或交易层。
- 保持 append-only 语义，以时间分区和受控 partition drop 实现物理保留；每次 drop 必须有 cutoff、权限、审计和 replay 安全证明。
- 使用 fixture 与 PG16 验证分区路由、跨分区读取、幂等、污染检测、备份/恢复和旧 artifact replay parity。
- 不修改旧 migration/checksum；使用独立 additive migration 与回滚兼容合同。
- 可与外部 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE` 并行，但 M1 authority、M2 runtime 读取和 24 小时 Shadow 必须等待两条路径汇合。

该包的本地阶段仍不允许修改生产；任何 no-authority Shadow、migration 或身份创建必须另过精确发布 Gate。全程禁止接入现有页面、读取 Legacy 运行模块、生成 Candidate/Signal/Plan、删除 Legacy 代码或宣称 live 全市场能力完成。
