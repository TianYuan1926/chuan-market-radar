# Market Radar V2 受控替换工程与运行蓝图 v1.6

状态：`ACTIVE_DESIGN_AUTHORITY / M0_ENGINEERING_EXIT_LOCAL_PASS / M1.1-M1.6_LOCAL_PASS / M2.2-B0.2-C1_OPERATIONAL_CAPTURE_START_PASS / FORWARD_ONLY_READY / B0.2_EXTERNAL_RESOLUTION_BLOCKED / COLLECTOR_DOCKER_AND_SHADOW_UNPROVEN / PRODUCTION_UNCHANGED`
...
**当前本地工程入口**：M2.2-B0、B0.1、B0.2-A、B0.2-C 与 C1 已通过对应出口。C1 以 exact clean release/config 连续取得两轮三 Venue COMPLETE，完整分母、raw digest、identity evidence、journal chain、冻结 cadence、gap 和 conflict 均已机器复核，三个 Continuity 都是 `FORWARD_ONLY_READY`。下一入口回到关键路径的 `V2-M1.5-B1-A-REACHABLE-DOCKER-RUNNER-PREFLIGHT`，只证明 exact source image、Collector 四分母、身份与脱敏证据；通过前不得启动 31 周期 Shadow。五个来源候选仍全部为 `RESEARCH_ONLY`，B0.2-B 继续受外部权利结论和 exact historical source 阻断，bulk acquisition 与 cohort freeze 均为 false。M2.2 Gate=`INSUFFICIENT`，Detector=DRAFT、Candidate emission=false；M1.7 前不得开放 M2 runtime、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.5-A、M1.5-B0、M1.6、M2.0、M2.1、M2.2-A、M2.2-B0、B0.1、B0.2-A、B0.2-C 和 C1 已通过对应本地/运行起点出口，生产零变更；M2.2 真实能力 Gate、外部来源解决、bulk historical acquisition、Collector Docker、生产存储和持续 SLO 尚未通过。当前本地入口是：

```text
V2-M1.5-B1-A-REACHABLE-DOCKER-RUNNER-PREFLIGHT
```

C1 已用同一 no-authority release/config 取得两轮、跨度达到冻结 cadence、raw 可复核、active gap=0 且无 unresolved/identity conflict 的三 Venue 完整证据。B1-A 只允许在 branch-scoped 可达 Docker runner 构建 exact source image、执行 M1 Collector 预检并上传脱敏证据，不接生产、不写 Candidate；通过后才进入固定 31 周期 B1-B。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 historical B1 acquisition 的硬前置，C1 不能替代或解锁它。
