...
**当前执行入口**：`V2-M1.6-P0R-B8-FIXED-DISPATCH-TRANSPORT-STAGING-ROOT-REMEDIATION`。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 fresh recovery evidence 通过前不变。香港单 AZ 私有 COS 的 `COMPLIANCE` 31 天和 macOS Keychain age X25519 身份保持有效。B7 clean source `be87cf...` 的 GitHub 四门、fresh read-only rebind 和全新 16-member transport v3 已通过；OrcaTerm 文件管理器却在 exact 9,176,819-byte 包上三次失败，目标文件与 run staging 均未产生，生产零变更。当前 B8 只把同一 exact、无 secret 内层包通过签名 fixed dispatch 原子落到目标 staging，不能请求 STS、读取数据库或启动恢复；本地运输 `10/10`、P0R `110/110` 与完整 CI 已 PASS，clean source、远端四门和 real-target staging acceptance 仍待完成。正确顺序是 `clean exact commit -> GitHub 四门 -> signed fixed-dispatch transport staging -> exact 16-member/mode/owner/hash/outer-cleanup/zero-drift 验收 -> 临时 8022 bootstrap -> bridge READY -> 动作时用户 MFA/native Copy -> backup/exact retrieval/isolated PG16 restore -> evidence/secret/runtime/route cleanup -> zero drift -> fresh topology/calibration/P0`。当前不请求 STS、不开放 8022、不使用 OrcaTerm 文件管理器、不复用任何旧 run；只有 P0R 与 fresh P0 均 PASS 才能请求 P1，M2 runtime、页面和交易计划仍关闭。
