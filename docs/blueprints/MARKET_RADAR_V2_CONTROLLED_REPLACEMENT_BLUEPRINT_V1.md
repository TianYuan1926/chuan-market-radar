...
**当前本地工程入口**：M2.2-A 历史数据接纳合同、target-blind replay harness、固定 Detector 分母、三业务分母加完整背景窗口、统计指标和 lifecycle proposal Gate 已本地完成。独立 custody 模式下 research Bundle 物理禁止 inline holdout，单次 Gate 必须绑定 sealed artifact；提前量按数据真正可知的 knowledge cutoff 计算。但仓库没有可接纳真实 cohort，Top20 ranking、sensitivity 和真实 holdout 工件也未形成，因此 Gate 真实状态为 `INSUFFICIENT`，五个 Detector 仍为 DRAFT 且 Candidate emission=false。下一包为 `V2-M2.2-B-REAL-HISTORICAL-COHORT-ACQUISITION-AND-FREEZE`。外部通道恢复后并行执行 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE`，只运行固定 31 个一分钟周期；B1 与 M1.6 都通过后进入 M1.7 24 小时持续 Shadow。M1.7 前不得开放 M2 runtime、页面或交易计划。
...
M0.0-M0.3、M1.1-M1.5-A、M1.5-B0、M1.6、M2.0、M2.1 和 M2.2-A 已通过各自本地出口，生产零变更；M2.2 真实能力 Gate、live、生产存储和持续 SLO 尚未通过。等待外部 B1 时，当前唯一允许并行的本地下一包是：

```text
V2-M2.2-B-REAL-HISTORICAL-COHORT-ACQUISITION-AND-FREEZE
```

M2.2-A 已完成合同与 harness。M2.2-B 只允许：

- 建立来源许可明确、event/knowledge cutoff 完整的冻结历史 replay cohort；合成 fixture 与真实历史证据物理分开。
- 同时保存 candidate、event、matched non-event 三分母和 Candidate Universe 完整背景窗口；病例对照采样不得冒充 Candidate precision。
- 生成 train/validation/holdout 的真实时间窗、purge/embargo、symbol/regime assignment 和独立 holdout custody 工件；本包不打开 holdout。
- 冻结 Candidate ranking policy、试验 registry 和全部 sensitivity 参数身份；本包不挑最佳结果、不运行最终 Gate。
- 数据不足、许可不明、lineage 不完整或完整背景窗口缺失时，必须停止在 `INSUFFICIENT`。
- 可与外部 `V2-M1.5-B1-EGRESS-EARLY-SHADOW-GATE` 并行，但任何 Detector runtime、M1 authority 读取和 24 小时 Shadow 必须等待 M1.7。

该包的本地阶段仍不允许修改生产。全程禁止接入现有页面、读取 Legacy 运行模块或 M1 runtime、写 Candidate Store、自动调权/晋级、生成等级/Signal/Plan、删除 Legacy 代码或宣称 live 全市场能力完成。
