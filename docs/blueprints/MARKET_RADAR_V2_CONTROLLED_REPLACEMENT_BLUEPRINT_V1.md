# Market Radar V2 受控替换工程与运行蓝图 v1.47
...
- `A0`：`ENGINEERING_MATERIALS_AND_SUPPLY_CHAIN_LOCAL_PASS / EXACT_RUNTIME_AND_SIGNED_DISPATCH_REMOTE_LINUX_PASS / TOTAL_GATE_INCOMPLETE / PRODUCTION_UNCHANGED`。精确版本、零高危依赖、许可证、SBOM、Actions/runner/Docker pin、双重 lint、Next 构建、原生材料烟测以及 exact Node 22.23.1/npm 10.9.8 本地与 Ubuntu 远程完整 CI 已通过；host tar 方言与浅 Git 历史问题均完成根因治理和防复发门禁。独立 secret/SAST/镜像扫描、性能/资源基线、完整制品 provenance/回滚以及 P0R 真实恢复仍未形成完整证据。
...
当前第一批子门禁已本地与远程通过并固化为 `v2:a0:materials:verify`、`test:v2-a0-materials`、运行烟测和 `v2-full-quality.yml`。host tar 修复提交 `29ab47dec0b9fbbbe66e1cfe7ce90aa2e1e4c25d` 已取得 Signed Dispatch Ubuntu PASS；浅 Git 历史修复提交 `1d5638d0fb538bacec09086ec7b719d9e7a85ce9` 已由 Full Quality `30200077285` 在 exact Node `22.23.1` / npm `10.9.8` 和完整祖先历史下复验 PASS。P0R 的真实离机备份与独立 PostgreSQL 16 恢复仍是独立生产第一关键路径。A0 总 PASS 前只允许继续无 authority 的本地合同与扫描工程，不允许启动 M1.5C/M1.5D 或宣称工程地基完整。
