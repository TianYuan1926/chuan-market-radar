...
**当前执行入口**：`V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY` 的 exact-source 重新资格化。P0R-D0 六小时无扩容本地机器证明和 fresh P0 组合准入工具均已本地 PASS，但原 P0 在真实 recovery evidence 通过前不变。前三个 gateway dispatch 均已 claim、`FAIL_DISPATCH_NOT_REUSABLE` 且批准已消费并过期；第三次 source `ad9aeb9842f6b57d934e7729e5e8a5cf2680a3b7` 的同源四门全部 PASS，并真实完成 target Caddy recreate、健康、mount 和 route probe，随后因 request v3 未绑定 signing key path 在 evidence seal 失败。自动 baseline rollback 已恢复原 Caddy config/image；十个非 Caddy full IDs、production HEAD `cec0b657...`、clean worktree、ready/fresh health 与全部非目标面不变，exact staging/gateway/outbound/diagnostic state 均 absent。request v4 signer 根治已由 clean commit `4d104261...` 推送；其 Signed Dispatch `30755615525`、A0 `30755615545` 与 Independent Security `30755615533` PASS，但 Full Quality `30755615546` 真实 FAIL，故当前只为 `3/4`。bridge v6 已根除 session-classification 调度竞态，并通过 focused `13/13`、完整 P0R `142/142`、Go helper 与最终字节完整 CI，V2 Ops `283/283`。固定后续顺序调整为 `clean commit/push -> 同源四门从零 PASS -> fresh production baseline -> 双构建 deterministic package -> 新 exact approval -> signer-identity-bound execution/自动回滚 -> 自动取回解密验签 -> fresh rebind -> P0R recovery`；只有 gateway real-target acceptance、P0R 与 fresh P0 均 PASS 才能请求 P1，M2 runtime、页面和交易计划继续关闭。
...
P0R 不允许以 Object Lock、Keychain 身份、transport bundle、target staging、canary、临时清理、阈值下调或 backup exit 0 冒充生产可恢复；临时凭证、对象上传、exact version retrieval、隔离 restore 和 cleanup 均是独立、可审计动作。当前顺序收敛为 `GitHub exact-source 四门 -> 构建确定性 Caddy-only gateway package -> 精确生产批准 -> 当前生产 Caddy image 隔离 validate -> 只重建 Caddy或自动 baseline rollback -> 本机自动下载 ciphertext、解密和验证 host/dispatch/schema/time/hash -> fresh read-only rebind 与生产零漂移 -> 新 route target/lease/run/object key/v4 plan/transport-v4 -> fixed-dispatch target acceptance -> 仅一次不可替代 MFA/native Copy -> read-only encrypted backup / exact retrieval / isolated PG16 restore -> lease-bound 全清理 -> fresh health/topology -> calibration -> fresh P0`。route authority 和 descriptor-first 修复保持上游资格；旧响应页关闭前仍禁止 browser state/screenshot/AX/OCR/computer-use，新 evidence return 路线不得读取该页。除精确 production mutation 批准及平台强制本人 MFA/native Secret Copy 外，调查、构建、派发、下载、验证、清理和回滚均由代理自主完成。不得复用任何旧 token、失败 run/package、旧 dispatch、旧计划、route evidence v1、transport v3/16-member package、手填 final hash、生产 node_modules host-path 假设、port 22 fallback、裸 `tee`、断线 receiver、单 TTY prearm、签发后第二次 SSH、无 route authority producer 的 execute、直接转发 child READY、超长复合命令或空 `/dev/shm` 占位。付费扩容已退出活跃路线，六小时零付费本地机器模型与组合准入实现已通过，但这不取消原 P0 blocker：fresh 证据必须同时满足所有旧非容量门禁、稳态 60% / 峰值 70%、完整恢复和健康门禁，否则 P1 继续 BLOCKED。P0R 完成后只允许重新执行 fresh P0，不直接授权 migration、身份创建、分区预建、Worker 启动或生产写入。B0.2-B 继续并行等待账户所有者或合格审查者给出 exact source 权利结论和合格历史身份；它仍是 historical B1 acquisition 的硬前置，C1 不能替代或解锁它。
