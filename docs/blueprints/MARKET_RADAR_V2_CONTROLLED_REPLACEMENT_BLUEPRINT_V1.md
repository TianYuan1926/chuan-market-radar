...
v1.87 根据固定生产派发“任务可送达、脱敏结果只滞留服务器”的重复阻断，停止把 Edge/API Explorer response、AX/browser-state、人工复制或 OrcaTerm 文件回传当作验收通道。新增 `V2-PRODUCTION-EVIDENCE-GATEWAY-CADDY-ONLY`：生产结果先由生产 SSH Ed25519 host key 对 canonical statement 作 namespace 签名，再以本机固定 X25519 recipient 经 HKDF-SHA256 + AES-256-GCM 加密；Caddy 只在既有 HTTP 入口的高熵精确 `.mre` 路径提供 ciphertext，禁止目录、重定向、明文、secret 和任意文件读取。本机固定 poller 经既有 SOCKS 下载，校验 pinned host key/fingerprint、dispatch/schema/time/hash 后以 mode-600、no-clobber 方式保存 payload、sealed object 和 receipt；每个对象由 exact transient systemd timer 在两小时租约后删除。生产包只允许复用当前 Caddy image、只重建 Caddy、挂载只读 outbox；数据库、Redis、Worker、env、migration、Feature Flag、生产仓库和其他容器均禁止改变，任何验证失败都必须恢复 baseline Caddy 并证明非目标容器 identity 不变。审批与发布控制文件统一采用 descriptor-first `O_NOFOLLOW`、mode-600、大小上限和读取前后 inode/mtime 稳定校验。当前 dispatch `37/37`、gateway `7/7`、P0R rebind `14/14`、V2 Ops `274/274` 与故障注入通过；本机没有 Docker/Caddy CLI，因此真实 Caddy syntax/runtime validation 被放入生产变更前、当前生产 Caddy image 的隔离 network-none preflight，尚未执行。最新完整 CI、clean commit、GitHub 四门、精确生产批准、Caddy-only bootstrap、自动取回和 fresh rebind 均待完成；生产没有发生本轮有意 mutation，P0R backup、exact retrieval 和独立 restore 仍未执行。
