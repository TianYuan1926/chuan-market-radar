...
当前 route authority/producer/transaction directed `23/23`、rebind/transport/bundle `30/30`、完整 P0R `138/138`、exact-toolchain launcher `3/3`、全 V2 Ops `263/263` 与 Go helper、ESLint、Biome 均 PASS。一次 host 默认 Node 24/npm 11 运行按 exact runtime gate 失败且不计 PASS；固定入口现在自动选择 Node `22.23.1` / npm `10.9.8`，不要求用户切版本。M0 exact CI binding `6/6` 锁死外层 launcher、内层 14 项有序生产门禁和 self-building verifier。隔离候选完整 `ci:production` 已在 M0 强制的唯一 V2 实施分支身份门 fail closed，Foundation=`643 total / 636 pass / 6 explicit skip / 1 expected blocker`；该门不得绕过，也不能把候选运行称为 full CI PASS。候选审计与 clean commit 后必须精确合入 `codex/market-radar-v2-implementation`，再从头运行最终字节完整 CI。clean exact source、远端四门、fresh rebind、external cleanup 和真实恢复仍待完成，不得复用旧 `125/125` 与 transport v3 资格冒充新 source 资格。
