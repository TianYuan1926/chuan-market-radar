...
- 去杠杆：优先防守，等待结构恢复。
- 未知：不参与加权，只显示数据边界。
- BTC.D 上行：山寨多头环境偏逆风，只能提高谨慎度和确认要求，不能直接做空或删除独立强势候选。
- BTC.D 下行 + TOTAL2/TOTAL3 扩张：山寨资金环境偏顺风，只能解释背景，不能降低 `3:1` 最低盈亏比。
- TOTAL2/TOTAL3 走弱：优先解释为山寨整体资金不足或风险偏好下降，不单独生成空头交易计划。
- BTC.D / TOTAL2 / TOTAL3 缺失：Macro Weather 继续使用 BTC/ETH 锚点；缺失项必须显示为 unavailable 或 null，不得伪造。
